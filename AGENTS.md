...
- **Liquibase / MySQL 5.7 — campos temporais (obrigatório)**: nunca criar coluna `TIMESTAMP NOT NULL` sem `DEFAULT` explícito em changelog Liquibase. Para campos preenchidos pela aplicação, use preferencialmente `DATETIME NOT NULL`; para campos preenchidos pelo banco, declare `DEFAULT CURRENT_TIMESTAMP` e `ON UPDATE CURRENT_TIMESTAMP` quando aplicável. Antes de finalizar changelog, revisar no diff campos como `created_at`, `updated_at`, `used_at`, `started_at`, `finished_at`, `completed_at` e seguir `docs/canonical/liquibase-mysql57-temporal-fields-canon.v1.md`.
...
- **Acesso a logs e banco via MCP (obrigatório)**: o ambiente Codex possui acesso operacional a banco de dados e logs pelo MCP Server. Não afirmar ao usuário que falta "acesso direto aos logs/banco" antes de tentar consultar o MCP com as tools disponíveis (`db_query`, `db_health`, `java_module_logs` ou equivalentes), usando timeout maior e novas tentativas quando houver instabilidade. Se a consulta via MCP falhar mesmo após tentativas razoáveis, informe a falha concreta do MCP e não trate como ausência permanente de acesso.
...
- **Logs**: os logs dos modulos Java Spring Boot podem ser acessados pelo MCP Server.  Chame o endpoint MCP https://mcpserverdigi.shop/mcp via JSON-RPC. Antes de concluir que não há acesso a logs, tente a tool `java_module_logs` pelo MCP Server e registre objetivamente qualquer erro real de consulta.
...
- **Controle de versões/deploy de PDE pelo painel (obrigatório)**: antes de considerar qualquer PDE publicado, homologado ou pronto para tráfego, consultar no painel pós-deploy o bloco de deploys por ambiente. Esse bloco deve confirmar ambiente, compose executado, commit/tag de imagem, versão PDE publicada, containers declarados, URLs e portas expostas. GitHub Actions verde não basta como evidência de publicação; a decisão comercial deve considerar somente ambiente com backend e frontend públicos respondendo e versão correta visível no painel.
- **Homologação obrigatória de PDE antes de produção**: toda nova versão, ajuste relevante ou ativo comercial de PDE deve ser publicado primeiro em ambiente de homologação antes de ir para produção. Quando aplicável, usar o mesmo host com portas separadas para frontend e backend de homologação, mantendo isolamento claro de URLs, variáveis, banco/estado de teste e métricas para não misturar tráfego real com validação. Antes de liberar produção, validar visualmente a URL de homologação, confirmar versão/experiência publicada, testar jornada principal em desktop e mobile, verificar eventos no painel unificado do PDE, confirmar funcionamento de CTA/login/paywall/checkout quando existirem e registrar a conclusão nos documentos de registro do tema. Para PDE com vídeo, a homologação deve confirmar carregamento do vídeo, fallback visual, performance percebida da primeira dobra e continuidade do funil antes de qualquer deploy produtivo ou campanha paga.
...
- **MCP Server — filtros disponíveis para pesquisa de logs (referência operacional)**:
  ...
  - Módulos aceitos em `module`: `backend`, `ai-worker`, `lead-portal`, `facebook-ads`, `email-service`, `lead-portal-payment`, `mds`, `mois`, `mois-sales-library-worker`, `mois-hotmart`, `clickbank-coletor-mois`, `oprm-coletor-receita`, `ops-monitor-worker`, `pde-platform-backend`.
...
- **Pepper / publicação de produto** : quando uma correção envolver capa, imagem, descrição, oferta ou asset publicado de produto na Pepper, usar o acesso operacional/API da Pepper disponível no ambiente ou informado pelo usuário para atualizar diretamente o cadastro publicado e validar visualmente o resultado no marketplace. Não limitar a solução a gerar o asset local quando o problema percebido estiver no cadastro externo já publicado. Nunca imprimir tokens, credenciais ou dados sensíveis da Pepper; validar apenas por chamadas seguras, status de atualização e inspeção do produto publicado.
