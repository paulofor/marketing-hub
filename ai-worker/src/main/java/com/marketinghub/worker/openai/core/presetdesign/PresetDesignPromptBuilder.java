...
/** Responsabilidade: montar o prompt, schema e request OpenAI da etapa presetdesign. */
public class PresetDesignPromptBuilder implements StagePromptBuilder<PresetDesignInput> {
    ...
    @Override
    public OpenAiRequest build(StageExecution<PresetDesignInput> execution) {
        Map<String, Object> data = execution.input().promptData();

        String promptResource = presetdesignProperties.promptResource();
        ...
        String schemaJson = loadResource(presetdesignProperties.schemaResource());
    ...
    private String buildResponsesApiRequest(String prompt, String schemaJson) {
        try {
            ...
            format.put("name", presetdesignProperties.schemaName());
