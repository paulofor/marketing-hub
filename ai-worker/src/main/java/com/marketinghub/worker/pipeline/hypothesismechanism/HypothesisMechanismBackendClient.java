...
public class HypothesisMechanismBackendClient implements StageBackendPort<HypothesisMechanismInput, HypothesisMechanismOutput> {
    ...
    /** Inicializa o cliente com WebClient, propriedades da etapa Mecanismo e ObjectMapper para normalizar contexto. */
    public HypothesisMechanismBackendClient(WebClient.Builder builder, HypothesisMechanismWorkerProperties properties, ObjectMapper objectMapper) {
        this.webClient = builder.build();
        this.properties = properties;
        this.objectMapper = objectMapper;
    }

    /** Busca no backend os jobs da etapa Mecanismo iniciados e aptos para processamento. */
    @Override
    public List<StageExecution<HypothesisMechanismInput>> listPending(int limit) {
        int effectiveLimit = Math.max(1, limit);
        String uri = joinPath(properties.backendBaseUrl(), properties.apiPrefix(), "/internal/hypothesis-pipeline/mechanism/stage-executions/pending");
    ...
    @Override
    public void markRunning(StageExecution<HypothesisMechanismInput> execution) {
        webClient.post()
    ...
    public void saveOpenAiRequest(StageExecution<HypothesisMechanismInput> execution, OpenAiRequest request) {
    ...
    /** Envia ao backend a resposta validada da OpenAI para concluir a etapa Mecanismo. */
    @Override
    public void markCompleted(StageExecution<HypothesisMechanismInput> execution, StageResult<HypothesisMechanismOutput> result) {
    ...
    /** Envia ao backend os dados de falha quando a etapa Mecanismo não é concluída. */
    @Override
    public void markFailed(StageExecution<HypothesisMechanismInput> execution, Throwable error) {
        Map<String, Object> body = new LinkedHashMap<>();
        body.put("marketNicheId", execution.aggregateId());
        body.put("stageCode", execution.stageCode());
    ...
    private StageExecution<HypothesisMechanismInput> toStageExecution(Map<String, Object> item) {
    ...
    /** Monta o bloco textual de contexto estratégico exigido pelo prompt da etapa Mecanismo. */
    ...
    /** Extrai o mecanismo OpenAI bruto armazenado nas métricas do mecanismo da etapa. */
    private OpenAiResult<String> openAiResult(StageResult<HypothesisMechanismOutput> result) {
        ...
        throw new IllegalStateException("Resultado da etapa Mecanismo sem métrica openAiResult");
    }
    ...
    private String toJsonOrText(Object value) {
        ...
        try {
            ...
            org.slf4j.LoggerFactory.getLogger(HypothesisMechanismBackendClient.class).warn(
                    "Falha ao serializar valor do prompt da etapa Mecanismo; usando fallback textual. valueType={}",
    ...
    /** Monta a URL base dos endpoints internos de execução da etapa Mecanismo. */
    private String stageExecutionBaseUrl() {
        return joinPath(properties.backendBaseUrl(), properties.apiPrefix(), "/internal/hypothesis-pipeline/mechanism/stage-executions");
    }
    ...
    private void logBackendPayload(String idJob, Map<String, Object> body) {
        try {
            org.slf4j.LoggerFactory.getLogger(HypothesisMechanismBackendClient.class).info(
                    "Enviando payload para backend na etapa Mecanismo. jobId={} payload={}",
                    idJob,
                    objectMapper.writeValueAsString(body));
        } catch (Exception error) {
            org.slf4j.LoggerFactory.getLogger(HypothesisMechanismBackendClient.class).warn(
                    "Falha ao serializar payload de log da etapa Mecanismo. jobId={}",
                    idJob,
                    error);
        }
    }
