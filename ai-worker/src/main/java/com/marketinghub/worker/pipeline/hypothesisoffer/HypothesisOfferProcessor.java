...
/** Responsabilidade: construir a oferta do nicho usando uma etapa OpenAI isolada no pipeline genérico. */
public class HypothesisOfferProcessor implements StageProcessor<HypothesisOfferInput, HypothesisOfferOutput> {
    private static final Logger log = LoggerFactory.getLogger(HypothesisOfferProcessor.class);
    private final ObjectMapper objectMapper;
    private final HypothesisOfferWorkerProperties properties;
    private final OpenAiClientPort openAiClient;
    private final HypothesisOfferResponseValidator responseValidator;
    private final HypothesisOfferBackendClient backendClient;
    private final PromptTemplateResolver promptTemplateResolver;

    /** Inicializa o processor com dependências específicas da etapa Oferta. */
    public HypothesisOfferProcessor(
    ...
    @Override
    public StageResult<HypothesisOfferOutput> process(StageContext<HypothesisOfferInput> context) {
        OpenAiRequest request = buildOpenAiRequest(context);
        backendClient.saveOpenAiRequest(context.execution(), request);
        log.info("Enviando request cru para OpenAI na etapa Oferta. jobId={} requestBodyJson={}", context.execution().idJob(), request.requestBodyJson());
        OpenAiDispatch dispatch = openAiClient.dispatch(request);
        OpenAiResult<String> rawResult = openAiClient.awaitResult(dispatch);
        log.info("Resposta crua da OpenAI recebida na etapa Oferta. jobId={} rawResponse={}", context.execution().idJob(), rawResult.rawResponse());
        HypothesisOfferOutput output = responseValidator.validateAndParse(rawResult.modelResponse());
        StageArtifact requestArtifact = context.artifactStore().save(
                "OPENAI_REQUEST",
                "hypothesis-offer-request.json",
                "application/json",
    ...
    private String toJsonOrText(Object value) {
        ...
        try {
            ...
            log.warn("Could not serialize hypothesis offer prompt value; using toString fallback. valueType={}", value.getClass().getName(), ex);
    ...
    private String loadResource(String path) {
        try {
            ...
            log.error("Could not load resource for hypothesis offer. path={}", path, ex);
