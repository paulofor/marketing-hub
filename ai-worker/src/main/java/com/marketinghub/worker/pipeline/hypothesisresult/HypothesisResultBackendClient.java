...
/** Responsabilidade: integrar a etapa Resultado do pipeline de hipótese aos endpoints internos do backend. */
public class HypothesisResultBackendClient implements StageBackendPort<HypothesisResultInput, HypothesisResultOutput> {
    private final WebClient webClient;
    private final HypothesisResultWorkerProperties properties;
    private final ObjectMapper objectMapper;

    /** Inicializa o cliente com WebClient, propriedades da etapa Resultado e ObjectMapper para normalizar contexto. */
    public HypothesisResultBackendClient(WebClient.Builder builder, HypothesisResultWorkerProperties properties, ObjectMapper objectMapper) {
        this.webClient = builder.build();
        this.properties = properties;
        this.objectMapper = objectMapper;
    }

    /** Busca no backend os jobs da etapa Resultado iniciados e aptos para processamento. */
    @Override
    public List<StageExecution<HypothesisResultInput>> listPending(int limit) {
        int effectiveLimit = Math.max(1, limit);
        String uri = joinPath(properties.backendBaseUrl(), properties.apiPrefix(), "/internal/hypothesis-pipeline/result/stage-executions/pending");
    ...
    @Override
    public void markRunning(StageExecution<HypothesisResultInput> execution) {
        webClient.post()
    ...
    public void saveOpenAiRequest(StageExecution<HypothesisResultInput> execution, OpenAiRequest request) {
    ...
    /** Envia ao backend a resposta validada da OpenAI para concluir a etapa Resultado. */
    @Override
    public void markCompleted(StageExecution<HypothesisResultInput> execution, StageResult<HypothesisResultOutput> result) {
    ...
    /** Envia ao backend os dados de falha quando a etapa Resultado não é concluída. */
    @Override
    public void markFailed(StageExecution<HypothesisResultInput> execution, Throwable error) {
        Map<String, Object> body = new LinkedHashMap<>();
        body.put("marketNicheId", execution.aggregateId());
        body.put("stageCode", execution.stageCode());
    ...
    private StageExecution<HypothesisResultInput> toStageExecution(Map<String, Object> item) {
        ...
        HypothesisResultInput input = new HypothesisResultInput(marketNicheId, stageCode, idJob, buildPromptDataFromPending(item));
    ...
    /** Monta o bloco textual de contexto estratégico exigido pelo prompt da etapa Resultado. */
    ...
    private OpenAiResult<String> openAiResult(StageResult<HypothesisResultOutput> result) {
        ...
        throw new IllegalStateException("Resultado da etapa Resultado sem métrica openAiResult");
    }
    ...
    /** Monta a URL base dos endpoints internos de execução da etapa Resultado. */
    private String stageExecutionBaseUrl() {
        return joinPath(properties.backendBaseUrl(), properties.apiPrefix(), "/internal/hypothesis-pipeline/result/stage-executions");
    }
    ...
    private void logBackendPayload(String idJob, Map<String, Object> body) {
        try {
            org.slf4j.LoggerFactory.getLogger(HypothesisResultBackendClient.class).info(
                    "Enviando payload para backend na etapa Resultado. jobId={} payload={}",
                    idJob,
                    objectMapper.writeValueAsString(body));
        } catch (Exception error) {
            org.slf4j.LoggerFactory.getLogger(HypothesisResultBackendClient.class).warn(
                    "Falha ao serializar payload de log da etapa Resultado. jobId={}",
                    idJob,
                    error);
        }
    }
