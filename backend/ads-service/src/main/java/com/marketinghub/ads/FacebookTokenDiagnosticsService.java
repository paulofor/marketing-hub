...
public class FacebookTokenDiagnosticsService {
    ...
    private static final List<String> RECOMMENDED_PERMISSIONS = List.of(
        "business_management",
        "pages_show_list",
        "pages_read_engagement"
    );
    private static final byte[] TEST_VIDEO_BYTES = Base64.getDecoder().decode(
    ...
    public FacebookTokenDiagnosticResponse diagnose(FacebookAccount account) {
        ...
        String tokenSource = StringUtils.hasText(account.getAccessToken()) ? "ACCESS_TOKEN" : "SYSTEM_USER_ACCESS_TOKEN";
    ...
    public FacebookVideoUploadTestResponse testVideoUpload(FacebookAccount account) {
        ...
        body.add("source", new ByteArrayResource(TEST_VIDEO_BYTES) {
            @Override
        ...
        try {
            ...
            return new FacebookVideoUploadTestResponse(
                ...
                true,
                videoId,
                GraphCallResult.success("advideos_upload", response.getStatusCode().value(), "Upload de video aceito pela Meta")
            );
        } catch (RestClientResponseException ex) {
            log.error(
                "Falha no teste de upload de video Meta: accountId={}, endpoint={}, status={}, response={}",
                account.getId(),
                endpoint,
                ex.getRawStatusCode(),
            ...
            log.error("Falha de integracao no teste de upload de video Meta: accountId={}, endpoint={}", account.getId(), endpoint, ex);
            return new FacebookVideoUploadTestResponse(
                account.getId(),
                account.getName(),
                checkedAt,
            ...
            log.error("Falha inesperada no teste de upload de video Meta: accountId={}, endpoint={}", account.getId(), endpoint, ex);
            return new FacebookVideoUploadTestResponse(
                account.getId(),
                account.getName(),
                checkedAt,
    ...
    private GraphCallResult debugToken(FacebookAccount account, String token) {
        ...
        String url = graphUrl("/debug_token")
        ...
        try {
            ...
            String message = Boolean.TRUE.equals(valid) ? "Token valido na Meta" : "Token invalido na Meta";
            return GraphCallResult.success("debug_token", response.getStatusCode().value(), message)
                .withTokenDebug(new TokenDebugDetails(
                    valid,
                    text(data, "type"),
                    text(data, "app_id"),
            ...
            log.error("Falha de integracao ao consultar debug_token da Meta: accountId={}", account.getId(), ex);
    ...
    private List<PermissionDiagnostic> fetchPermissions(String token) {
        String url = graphUrl("/me/permissions")
        ...
        try {
            ...
            if (data.isArray()) {
                for (JsonNode node : data) {
                    ...
                    if (StringUtils.hasText(permission)) {
                        permissions.add(new PermissionDiagnostic(
                            permission,
                            text(node, "status"),
            ...
            log.error("Falha ao listar permissoes Meta: status={}, response={}", ex.getRawStatusCode(), sanitizeBody(ex.getResponseBodyAsString()), ex);
            return List.of(new PermissionDiagnostic("permissions_lookup", "FAILED: " + graphMessage(ex), false, false));
        } catch (RestClientException ex) {
            log.error("Falha de integracao ao listar permissoes Meta", ex);
            return List.of(new PermissionDiagnostic("permissions_lookup", "FAILED: " + ex.getMessage(), false, false));
        }
    }
    ...
    private GraphCallResult checkAdAccountAccess(FacebookAccount account, String token) {
        ...
        String url = graphUrl(endpoint)
            .queryParam("fields", "id,name,currency,account_status")
            .queryParam("access_token", token)
            .toUriString();
        try {
            ResponseEntity<String> response = restTemplate.getForEntity(url, String.class);
            return GraphCallResult.success("ad_account", response.getStatusCode().value(), "Conta de anuncios acessivel");
        } catch (RestClientResponseException ex) {
            log.error(
                "Falha ao validar conta de anuncios Meta: accountId={}, endpoint={}, status={}, response={}",
                account.getId(),
                endpoint,
                ex.getRawStatusCode(),
                sanitizeBody(ex.getResponseBodyAsString()),
            ...
            log.error("Falha de integracao ao validar conta de anuncios Meta: accountId={}, endpoint={}", account.getId(), endpoint, ex);
    ...
    private GraphCallResult checkVideoLibraryReadiness(FacebookAccount account, String token) {
        ...
        try {
            ResponseEntity<String> response = restTemplate.getForEntity(url, String.class);
            return GraphCallResult.success("advideos_read", response.getStatusCode().value(), "Biblioteca de videos acessivel");
        } catch (RestClientResponseException ex) {
            log.error(
                "Falha ao validar biblioteca de videos Meta: accountId={}, endpoint={}, status={}, response={}",
                account.getId(),
                endpoint,
                ex.getRawStatusCode(),
                sanitizeBody(ex.getResponseBodyAsString()),
            ...
            log.error("Falha de integracao ao validar biblioteca de videos Meta: accountId={}, endpoint={}", account.getId(), endpoint, ex);
    ...
    private String resolveOperationalToken(FacebookAccount account) {
        ...
        return StringUtils.hasText(account.getSystemUserAccessToken()) ? account.getSystemUserAccessToken().trim() : null;
    }
    ...
    public record GraphCallResult(
        ...
        public static GraphCallResult failed(
            ...
            return new GraphCallResult(operation, "FAILED", httpStatus, message, code, subcode, fbtraceId, null);
        }
        ...
        public GraphCallResult withTokenDebug(TokenDebugDetails tokenDebug) {
            return new GraphCallResult(operation, status, httpStatus, message, code, subcode, fbtraceId, tokenDebug);
        }
    }
}
