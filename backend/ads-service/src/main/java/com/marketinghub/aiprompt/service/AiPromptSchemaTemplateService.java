...
public class AiPromptSchemaTemplateService {
  ...
  public AiPromptSchemaTemplateService(
      AiPromptSchemaTemplateRepository repository, ObjectMapper objectMapper) {
  ...
  public List<AiPromptSchemaTemplateResponse> list(String pipelineCode, String stageCode) {
    if (StringUtils.hasText(pipelineCode) && StringUtils.hasText(stageCode)) {
      return repository
          .findByPipelineCodeAndStageCodeOrderByVersionDesc(
    ...
    if (StringUtils.hasText(pipelineCode)) {
      return repository
          .findByPipelineCodeOrderByStageCodeAscVersionDesc(normalize(pipelineCode))
          .stream()
          .map(this::toResponse)
          .toList();
    }
  ...
  public AiPromptSchemaTemplateResponse update(
      String templateKey, UpdateAiPromptSchemaTemplateRequest request) {
  ...
  private AiPromptSchemaTemplate load(String templateKey) {
    return repository
        ...
        .orElseThrow(
            () ->
                new EntityNotFoundException("AI prompt schema template not found: " + templateKey));
  }
  ...
  private void validate(UpdateAiPromptSchemaTemplateRequest request) {
    ...
    if (!StringUtils.hasText(request.promptMarkdownContent())) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "promptMarkdownContent is required");
    }
    ...
    try {
      ...
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "schemaJson must be valid JSON", ex);
    }
  }
