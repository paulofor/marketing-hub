...
public class EpmService {
  ...
  public FinancialPlanListResponse listFinancialPlans() {
    return new FinancialPlanListResponse(
        financialPlanRepository.findAll().stream().map(this::toPlan).toList());
  }
  ...
  @Transactional
  public FinancialPlanResponse updateFinancialPlan(
      Long planId, UpdateFinancialPlanRequest request) {
  ...
  public FinancialPlanNicheResponse createPlanNiche(Long planId, CreatePlanNicheRequest request) {
    FinancialPlanNiche niche =
        FinancialPlanNiche.builder()
            ...
            .spendLimitCents(
                request.spendLimitCents() == null
                    ? value(request.plannedBudgetCents())
                    : value(request.spendLimitCents()))
  ...
  public FinancialPlanNicheListResponse listPlanNiches(Long planId) {
    findPlan(planId);
    return new FinancialPlanNicheListResponse(
        financialPlanNicheRepository.findByFinancialPlanIdOrderByIdAsc(planId).stream()
            .map(this::toNiche)
            .toList());
  }
  ...
  @Transactional
  public FinancialPlanHypothesisResponse createPlanHypothesis(
      Long planNicheId, CreatePlanHypothesisRequest request) {
    FinancialPlanNiche niche = findNiche(planNicheId);
    long costPerExperiment =
        request.plannedCostPerExperimentCents() == null
            ? 0L
            : request.plannedCostPerExperimentCents();
    long lossLimit =
        request.lossLimitCents() == null
            ? costPerExperiment * request.plannedExperiments()
            : request.lossLimitCents();
    FinancialPlanHypothesis hypothesis =
        FinancialPlanHypothesis.builder()
            .financialPlanNiche(niche)
            ...
            .status(
                request.status() == null ? FinancialPlanHypothesisStatus.PLANNED : request.status())
            .notes(request.notes())
            .build();
  ...
  public FinancialPlanHypothesisListResponse listPlanHypotheses(Long planNicheId) {
    ...
    return new FinancialPlanHypothesisListResponse(
        financialPlanHypothesisRepository
            .findByFinancialPlanNicheIdOrderByIdAsc(planNicheId)
            .stream()
  ...
  @Transactional
  public ExperimentBudgetResponse createExperimentBudget(
      Long planHypothesisId, CreateExperimentBudgetRequest request) {
    long plannedTotal = value(request.plannedDailyBudgetCents()) * request.plannedDurationDays();
    long spendLimit =
        request.spendLimitCents() == null ? plannedTotal : value(request.spendLimitCents());
    ExperimentBudget budget =
        ExperimentBudget.builder()
  ...
  public ExperimentBudgetListResponse listExperimentBudgets(Long planHypothesisId) {
    ...
    return new ExperimentBudgetListResponse(
        experimentBudgetRepository
            .findByFinancialPlanHypothesisIdOrderByIdAsc(planHypothesisId)
            .stream()
            .map(this::toBudget)
            .toList());
  }
  ...
  @Transactional
  public ExperimentBudgetResponse updateExperimentBudget(
      Long experimentBudgetId, UpdateExperimentBudgetRequest request) {
  ...
  @Transactional
  public ExperimentMetricResponse createExperimentMetric(
      Long experimentBudgetId, CreateExperimentMetricRequest request) {
    ExperimentBudget budget = findBudget(experimentBudgetId);
    ...
    long netProfit =
        grossProfit
            - value(request.paymentFeeCents())
            - value(request.platformFeeCents())
            - value(request.aiCostCents())
            - value(request.taxEstimateCents());
    ExperimentFinancialMetric metric =
        ExperimentFinancialMetric.builder()
            .experimentBudget(budget)
            ...
            .landingConversionDecimal(
                ratio(
                    request.leads() == null ? 0 : request.leads(),
                    request.visitors() == null ? 0 : request.visitors(),
                    6))
            .purchaseConversionDecimal(
                ratio(
                    request.purchases() == null ? 0 : request.purchases(),
                    request.visitors() == null ? 0 : request.visitors(),
                    6))
            .notes(request.notes())
            .build();
  ...
  public ExperimentMetricResponse getLatestExperimentMetric(Long experimentBudgetId) {
    ...
    return experimentFinancialMetricRepository
        .findFirstByExperimentBudgetIdOrderByMeasuredAtDesc(experimentBudgetId)
        .map(this::toMetric)
        .orElseThrow(() -> notFound("Métrica financeira não encontrada"));
  }

  /** Registra uma decisão financeira para um experimento. */
  @Transactional
  public ExperimentDecisionResponse createExperimentDecision(
      Long experimentBudgetId, CreateExperimentDecisionRequest request) {
    ExperimentFinancialDecision decision =
        ExperimentFinancialDecision.builder()
  ...
  public ExperimentDecisionListResponse listExperimentDecisions(Long experimentBudgetId) {
    findBudget(experimentBudgetId);
    return new ExperimentDecisionListResponse(
        experimentFinancialDecisionRepository
            .findByExperimentBudgetIdOrderByDecidedAtDesc(experimentBudgetId)
            .stream()
            .map(this::toDecision)
            .toList());
  }
  ...
  @Transactional
  public ProductPriceScenarioResponse createProductPriceScenario(
      Long planId, CreateProductPriceScenarioRequest request) {
    FinancialPlan plan = findPlan(planId);
    long totalBudget =
        request.totalBudgetCents() == null
            ? value(plan.getTotalBudgetCents())
            : value(request.totalBudgetCents());
    long netRevenue =
        value(request.priceCents())
            - value(request.expectedPaymentFeeCents())
            - value(request.expectedPlatformFeeCents())
            - value(request.expectedTaxCents());
    ProductPriceScenario scenario =
        ProductPriceScenario.builder()
            .financialPlan(plan)
            .name(request.name())
  ...
  public ProductPriceScenarioListResponse listProductPriceScenarios(Long planId) {
    findPlan(planId);
    return new ProductPriceScenarioListResponse(
        productPriceScenarioRepository.findByFinancialPlanIdOrderByIdAsc(planId).stream()
            .map(this::toScenario)
            .toList());
  }
  ...
  public FinancialPlanSummaryResponse getFinancialPlanSummary(Long planId) {
    FinancialPlan plan = findPlan(planId);
    List<FinancialPlanNiche> niches =
        financialPlanNicheRepository.findByFinancialPlanIdOrderByIdAsc(planId);
    List<FinancialPlanHypothesis> hypotheses =
        niches.stream()
            .flatMap(
                n ->
                    financialPlanHypothesisRepository
                        .findByFinancialPlanNicheIdOrderByIdAsc(n.getId())
                        .stream())
            .toList();
    List<ExperimentBudget> budgets =
        hypotheses.stream()
            .flatMap(
                h ->
                    experimentBudgetRepository
                        .findByFinancialPlanHypothesisIdOrderByIdAsc(h.getId())
                        .stream())
            .toList();
    List<ExperimentFinancialMetric> latestMetrics =
        budgets.stream()
            .map(
                b ->
                    experimentFinancialMetricRepository
                        .findFirstByExperimentBudgetIdOrderByMeasuredAtDesc(b.getId())
                        .orElse(null))
            .filter(Objects::nonNull)
            .toList();
    long actualSpend =
        latestMetrics.stream().mapToLong(ExperimentFinancialMetric::getAdSpendCents).sum();
    long revenue =
        latestMetrics.stream().mapToLong(ExperimentFinancialMetric::getRevenueCents).sum();
    long grossProfit =
        latestMetrics.stream().mapToLong(ExperimentFinancialMetric::getGrossProfitCents).sum();
    long netProfit =
        latestMetrics.stream()
            .mapToLong(ExperimentFinancialMetric::getEstimatedNetProfitCents)
            .sum();
    ...
    int withoutSignal =
        (int)
            latestMetrics.stream()
                .filter(
                    m -> m.getPurchases() == 0 && m.getLeads() == 0 && m.getCheckoutClicks() == 0)
                .count();
    long plannedExperimentBudget =
        budgets.stream().mapToLong(ExperimentBudget::getPlannedTotalBudgetCents).sum();
    return new FinancialPlanSummaryResponse(
        planId,
        plan.getTotalBudgetCents(),
        plannedExperimentBudget,
        actualSpend,
        revenue,
        grossProfit,
        netProfit,
  ...
  private FinancialPlanResponse toPlan(FinancialPlan plan) {
    return new FinancialPlanResponse(
        plan.getId(),
        plan.getName(),
        plan.getCycleStartDate(),
        plan.getCycleEndDate(),
        plan.getTotalBudgetCents(),
        plan.getDefaultDailyBudgetCents(),
        plan.getDefaultExperimentDurationDays(),
        plan.getDefaultExperimentsPerHypothesis(),
        plan.getStatus(),
        plan.getNotes(),
        plan.getCreatedAt(),
        plan.getUpdatedAt());
  ...
  private FinancialPlanNicheResponse toNiche(FinancialPlanNiche niche) {
    return new FinancialPlanNicheResponse(
        niche.getId(),
        niche.getFinancialPlan().getId(),
        niche.getExternalNicheId(),
        niche.getNicheName(),
        niche.getPlannedBudgetCents(),
        niche.getSpendLimitCents(),
        niche.getStatus(),
        niche.getNotes(),
        niche.getCreatedAt(),
        niche.getUpdatedAt());
  ...
  private FinancialPlanHypothesisResponse toHypothesis(FinancialPlanHypothesis hypothesis) {
    return new FinancialPlanHypothesisResponse(
        hypothesis.getId(),
        hypothesis.getFinancialPlanNiche().getId(),
        hypothesis.getExternalHypothesisId(),
        hypothesis.getTitle(),
        hypothesis.getPlannedExperiments(),
        hypothesis.getPlannedCostPerExperimentCents(),
        hypothesis.getLossLimitCents(),
  ...
  private ExperimentBudgetResponse toBudget(ExperimentBudget budget) {
    return new ExperimentBudgetResponse(
        budget.getId(),
        budget.getFinancialPlanHypothesis().getId(),
        budget.getExternalExperimentId(),
        budget.getName(),
        budget.getPlannedDailyBudgetCents(),
        budget.getPlannedDurationDays(),
        budget.getPlannedTotalBudgetCents(),
        budget.getSpendLimitCents(),
        budget.getActualSpendCents(),
        budget.getRemainingBudgetCents(),
        budget.getStartDate(),
        budget.getEndDate(),
        budget.getStatus(),
        budget.getNotes(),
  ...
  private ExperimentMetricResponse toMetric(ExperimentFinancialMetric metric) {
    return new ExperimentMetricResponse(
        metric.getId(),
        metric.getExperimentBudget().getId(),
        metric.getMeasuredAt(),
        metric.getImpressions(),
        metric.getClicks(),
        metric.getVisitors(),
        metric.getLeads(),
        metric.getSampleRequests(),
        metric.getCheckoutClicks(),
        metric.getPurchases(),
        metric.getAdSpendCents(),
        metric.getRevenueCents(),
        metric.getPaymentFeeCents(),
        metric.getPlatformFeeCents(),
        metric.getAiCostCents(),
        metric.getTaxEstimateCents(),
        metric.getGrossProfitCents(),
        metric.getEstimatedNetProfitCents(),
        metric.getCtrDecimal(),
        metric.getCpcCents(),
        metric.getCplCents(),
        metric.getCpaCents(),
        metric.getRoasDecimal(),
        metric.getLandingConversionDecimal(),
        metric.getPurchaseConversionDecimal(),
        metric.getNotes(),
        metric.getCreatedAt());
  }
  ...
  private ExperimentDecisionResponse toDecision(ExperimentFinancialDecision decision) {
    return new ExperimentDecisionResponse(
        decision.getId(),
        decision.getExperimentBudget().getId(),
  ...
  private ProductPriceScenarioResponse toScenario(ProductPriceScenario scenario) {
    return new ProductPriceScenarioResponse(
        scenario.getId(),
        scenario.getFinancialPlan().getId(),
        scenario.getName(),
        scenario.getPriceCents(),
        scenario.getExpectedPaymentFeeCents(),
        scenario.getExpectedPlatformFeeCents(),
        scenario.getExpectedTaxCents(),
        scenario.getExpectedNetRevenuePerSaleCents(),
        scenario.getTotalBudgetCents(),
        scenario.getBreakEvenSales(),
        scenario.getNotes(),
  ...
  private FinancialPlan findPlan(Long id) {
    return financialPlanRepository
        .findById(id)
        .orElseThrow(() -> notFound("Plano financeiro não encontrado"));
  }

  /** Busca nicho financeiro ou retorna erro HTTP 404. */
  private FinancialPlanNiche findNiche(Long id) {
    return financialPlanNicheRepository
        .findById(id)
        .orElseThrow(() -> notFound("Nicho financeiro não encontrado"));
  }

  /** Busca hipótese financeira ou retorna erro HTTP 404. */
  private FinancialPlanHypothesis findHypothesis(Long id) {
    return financialPlanHypothesisRepository
        .findById(id)
        .orElseThrow(() -> notFound("Hipótese financeira não encontrada"));
  }

  /** Busca orçamento de experimento ou retorna erro HTTP 404. */
  private ExperimentBudget findBudget(Long id) {
    return experimentBudgetRepository
        .findById(id)
        .orElseThrow(() -> notFound("Orçamento de experimento não encontrado"));
  }
  ...
  private BigDecimal ratio(long numerator, long denominator, int scale) {
  ...
  private Long centsPer(long cents, long quantity) {
    return quantity == 0
        ? null
  ...
  private int ceilSales(long totalBudget, long netRevenuePerSale) {
    return netRevenuePerSale <= 0
        ? 0
        : BigDecimal.valueOf(totalBudget)
            .divide(BigDecimal.valueOf(netRevenuePerSale), 0, RoundingMode.CEILING)
            .intValue();
  }
}
