...
public class FrameworkImageStaleJobScheduler {
  ...
  public FrameworkImageStaleJobScheduler(
      FrameworkImageGenerationService service,
  ...
  public void run() {
    ...
    int failedCount =
        service.failStaleProcessingJobs(
            staleBefore, batchSize, "Job expirado por timeout de processamento");
    if (failedCount > 0) {
      log.warn(
          "framework-image-stale-guard failedCount={} staleBefore={}", failedCount, staleBefore);
    }
  }
}
