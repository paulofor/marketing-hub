...
public class ExperimentFunnelAttributionService {
  ...
  public Map<String, EnumMap<ExperimentFunnelStage, Long>> aggregateByCampaignCode(
      Long experimentId) {
  ...
  private void collectRenderEvents(
      Long experimentId,
      Instant baseline,
  ...
  private void collectSampleEmailOpens(
      Long experimentId,
      Instant baseline,
      Map<String, EnumMap<ExperimentFunnelStage, Long>> target) {
    String sql =
  ...
  private void collectCheckoutAccess(
      Long experimentId,
      Instant baseline,
      Map<String, EnumMap<ExperimentFunnelStage, Long>> target) {
    String sql =
  ...
  private void collectPurchases(
    ...
    String sql =
        """
            ...
            .formatted(ExperimentFunnelService.FLOW_SCOPE_CONDITION);
    queryByCampaignCode(
        sql, ExperimentFunnelStage.COMPRA, target, experimentId, experimentId, baseline, baseline);
  }

  private void collectDeliveryEmailOpens(
      Long experimentId,
      Instant baseline,
      Map<String, EnumMap<ExperimentFunnelStage, Long>> target) {
    String sql =
  ...
  private void queryByCampaignCode(
      String sql,
    ...
    jdbcTemplate.query(
        sql,
        (RowCallbackHandler)
            rs -> record(target, rs.getString("campaign_code"), stage, rs.getLong("total")),
        params);
  }
  ...
  private void record(
    ...
    EnumMap<ExperimentFunnelStage, Long> stageMap =
        target.computeIfAbsent(normalized, key -> new EnumMap<>(ExperimentFunnelStage.class));
