...
/**
 * Configuração inicial dos diagnósticos estatísticos do funil. Mantém thresholds em um ponto único
 * para facilitar evolução sem espalhar regra na UI.
 */
@Component
public class ExperimentFunnelDiagnosticConfig {
  ...
  private static final List<ConversionRuleSpec> PRIORITIZED_RULES =
      List.of(
          ...
          new ConversionRuleSpec(
              ExperimentFunnelStage.ENVIO_FORM, ExperimentFunnelStage.ABERTURA_EMAIL_AMOSTRA, 0.05),
          new ConversionRuleSpec(
              ExperimentFunnelStage.ACESSO_CHECKOUT, ExperimentFunnelStage.COMPRA, 0.03));

  private static final List<ConversionRuleSpec> LOW_TICKET_PRIORITIZED_RULES =
      List.of(
          new ConversionRuleSpec(
              ExperimentFunnelStage.VISUALIZACAO_ANUNCIO,
              ExperimentFunnelStage.ACESSO_FORM_LEAD,
  ...
  private static final List<ConversionRuleSpec> PDE_MEMBERSHIP_SUBSCRIPTION_PRIORITIZED_RULES =
      List.of(
          new ConversionRuleSpec(
              ExperimentFunnelStage.VISUALIZACAO_ANUNCIO,
              ExperimentFunnelStage.ACESSO_FORM_LEAD,
          ...
          new ConversionRuleSpec(
              ExperimentFunnelStage.ACESSO_CHECKOUT, ExperimentFunnelStage.COMPRA, 0.03),
          new ConversionRuleSpec(
              ExperimentFunnelStage.COMPRA, ExperimentFunnelStage.DOWNLOAD_MATERIAL_PAGO, 0.60));
  ...
  /**
   * Retorna o volume mínimo de evento principal para considerar o aprendizado operacional
   * suficiente.
   */
  ...
  public record ConversionRuleSpec(
    ...
    public ConversionRuleSpec(
        ExperimentFunnelStage from, ExperimentFunnelStage to, double minAcceptableRate) {
    ...
    public List<Double> allThresholdRates() {
      return java.util.stream.Stream.concat(
              java.util.stream.Stream.of(minAcceptableRate), additionalThresholdRates.stream())
          .distinct()
