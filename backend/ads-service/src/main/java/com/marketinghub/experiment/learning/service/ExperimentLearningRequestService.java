...
import com.marketinghub.repository.jpa.experiment.learning.ExperimentLearningRequestRepository;
...
public class ExperimentLearningRequestService {
  ...
  private static final Set<ExperimentLearningStatus> ACTIVE_STATUSES =
      EnumSet.of(ExperimentLearningStatus.PENDING, ExperimentLearningStatus.PROCESSING);
  ...
  public ExperimentLearningRequestService(
      ExperimentLearningRequestRepository repository,
  ...
  @Transactional
  public ExperimentLearningRequest create(
      Long experimentId, CreateExperimentLearningRequest payload) {
    var experiment =
        experimentRepository
            .findById(experimentId)
            .orElseThrow(
                () -> new EntityNotFoundException("Experimento não encontrado: " + experimentId));
    ...
    ExperimentLearningRequest request =
        ExperimentLearningRequest.builder()
  ...
  @Transactional(readOnly = true)
  public List<ExperimentLearningRequest> listByStatus(
      Collection<ExperimentLearningStatus> statuses) {
  ...
  @Transactional
  public ExperimentLearningRequest updateStatus(
      Long requestId,
    ...
    ExperimentLearningRequest request =
        repository
            .findById(requestId)
            .orElseThrow(
                () -> new EntityNotFoundException("Solicitação não encontrada: " + requestId));
    switch (status) {
  ...
  private void applyReady(ExperimentLearningRequest request, ExperimentLearningPayloadDto payload) {
    if (payload == null) {
      throw new IllegalArgumentException(
          "Payload do aprendizado é obrigatório quando o status é READY");
    }
  ...
  private void applyFailed(ExperimentLearningRequest request, String failureReason) {
    ...
    request.setFailureReason(
        StringUtils.hasText(failureReason) ? failureReason : "Falha desconhecida");
  }
