...
public class PipelineOperationalMetricsService {
  ...
  public PipelineOperationalMetricsService(
      ExperimentPipelineGenerationJobRepository jobRepository, ObjectMapper objectMapper) {
  ...
  public PipelineOperationalMetricsDto collect(int limit) {
    ...
    List<ExperimentPipelineGenerationJob> jobs =
        jobRepository.findAll(PageRequest.of(0, safeLimit)).getContent();

    Map<ExperimentPipelineSection, List<ExperimentPipelineGenerationJob>> bySection =
        new EnumMap<>(ExperimentPipelineSection.class);
    jobs.forEach(
        job -> bySection.computeIfAbsent(job.getSection(), ignored -> new ArrayList<>()).add(job));

    List<PipelineSectionMetricDto> sectionMetrics =
        bySection.entrySet().stream()
            .map(entry -> toSectionMetrics(entry.getKey(), entry.getValue()))
            .toList();

    double avgDuration =
        sectionMetrics.stream()
            .mapToDouble(PipelineSectionMetricDto::averageDurationSeconds)
            .average()
            .orElse(0D);
    double failRate =
        sectionMetrics.stream()
            .mapToDouble(PipelineSectionMetricDto::failureRate)
            .average()
            .orElse(0D);
    double reworkRate =
        sectionMetrics.stream()
            .mapToDouble(PipelineSectionMetricDto::reworkRate)
            .average()
            .orElse(0D);
    double placeholderRate =
        sectionMetrics.stream()
            .mapToDouble(PipelineSectionMetricDto::placeholderRate)
            .average()
            .orElse(0D);
    double qualityScore =
        sectionMetrics.stream()
            .mapToDouble(PipelineSectionMetricDto::averageQualityScore)
            .average()
            .orElse(0D);
  ...
  private PipelineSectionMetricDto toSectionMetrics(
      ExperimentPipelineSection section, List<ExperimentPipelineGenerationJob> jobs) {
    long total = jobs.size();
    long failed =
        jobs.stream()
            .filter(j -> j.getStatus() == ExperimentPipelineGenerationJobStatus.FAILED)
            .count();
  ...
  private double extractQualityScore(ExperimentPipelineGenerationJob job) {
    ...
    try {
      Map<String, Object> payload =
          objectMapper.readValue(job.getRequestBodyJson(), new TypeReference<>() {});
