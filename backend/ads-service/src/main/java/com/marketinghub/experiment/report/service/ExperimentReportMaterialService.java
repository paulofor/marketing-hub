...
/**
 * Responsável por consolidar todas as informações necessárias para montar o relatório objetivo de
 * um experimento.
 */
@Service
public class ExperimentReportMaterialService {
  ...
  public ExperimentReportMaterialService(
  ...
  public ExperimentReportMaterialDto build(Long experimentId) {
    ...
    List<LeadPortalFlow> flows =
        leadPortalFlowRepository.findAllByExperimentIdOrderByCreatedAtDesc(experimentId);
    List<ExperimentFunnelStageDto> funnelStages = experimentFunnelService.summarize(experimentId);

    ExperimentReportMaterialDto dto =
        ExperimentReportMaterialDto.builder()
  ...
  private ExperimentReportMaterialDto.ExperimentSnapshot toExperimentSnapshot(
      Experiment experiment) {
    if (experiment == null) {
      return null;
    }
  ...
  private ExperimentReportMaterialDto.HypothesisSnapshot toHypothesisSnapshot(
  ...
  private ExperimentReportMaterialDto.InstantFormSnapshot toInstantFormSnapshot(
      com.marketinghub.ads.FacebookInstantForm instantForm) {
    if (instantForm == null) {
      return null;
    }
  ...
  private List<ExperimentReportMaterialDto.CreativeSnapshot> mapCreatives(
    ...
    return creatives.stream()
        ...
        .map(
            creative ->
                ExperimentReportMaterialDto.CreativeSnapshot.builder()
                    .id(creative.getId())
                    ...
                    .emotionalTriggers(
                        toNameList(creative.getEmotionalTriggers(), trigger -> trigger.getName()))
                    .visualProofs(toNameList(creative.getVisualProofs(), proof -> proof.getName()))
                    .build())
        .toList();
  }

  private <T> List<String> toNameList(
      java.util.Collection<T> source, java.util.function.Function<T, String> extractor) {
    if (source == null || source.isEmpty()) {
      return Collections.emptyList();
    }
    return source.stream().map(extractor).filter(Objects::nonNull).sorted().toList();
  }

  private List<ExperimentReportMaterialDto.CreativeVariantSnapshot> mapCreativeVariants(
    ...
    return variants.stream()
        ...
        .map(
            variant ->
                ExperimentReportMaterialDto.CreativeVariantSnapshot.builder()
                    .id(variant.getId())
  ...
  private List<ExperimentReportMaterialDto.LandingPageSnapshot> mapLandingPages(
    ...
    return landingPages.stream()
        .sorted(
            Comparator.comparing(
                LandingPage::getCreatedAt, Comparator.nullsLast(Comparator.naturalOrder())))
        .map(
            page ->
                ExperimentReportMaterialDto.LandingPageSnapshot.builder()
                    .id(page.getId())
                    .url(page.getUrl())
  ...
  private List<ExperimentReportMaterialDto.LeadPortalFlowSnapshot> mapLeadPortalFlows(
      List<LeadPortalFlow> flows) {
    ...
    return flows.stream()
        .map(
            flow ->
                ExperimentReportMaterialDto.LeadPortalFlowSnapshot.builder()
                    .id(flow.getId())
                    .name(flow.getName())
                    .slug(flow.getSlug())
                    ...
                    .previewImageUrl(
                        flow.getSimpleFormStyle() != null
                            ? flow.getSimpleFormStyle().getPreviewImageUrl()
                            : null)
  ...
  private List<ExperimentReportMaterialDto.LeadPortalQuestionSnapshot> mapLeadPortalQuestions(
    ...
    return questions.stream()
        ...
        .map(
            question ->
                ExperimentReportMaterialDto.LeadPortalQuestionSnapshot.builder()
                    .id(question.getId())
                    .title(question.getTitle())
                    .type(question.getType() != null ? question.getType().name() : null)
                    .required(question.isRequired())
                    .options(
                        Optional.ofNullable(question.getOptions()).orElse(Collections.emptyList()))
                    .build())
