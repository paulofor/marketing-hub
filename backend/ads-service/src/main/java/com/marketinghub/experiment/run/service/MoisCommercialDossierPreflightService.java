...
public class MoisCommercialDossierPreflightService {
  private static final List<String> COMMERCIAL_DOSSIER_PIPELINES =
      List.of("salespagepatterns.v1", "warmupecosystem.v1");
  ...
  public MoisCommercialDossierPreflightService(
  ...
  public CommercialDossierPreflightResult evaluate(Experiment experiment) {
    ...
    List<PipelineDossieProduto> dossiers =
        pipelineDossieProdutoRepository.findCompletedCommercialDossiers(
            COMMERCIAL_DOSSIER_PIPELINES, PageRequest.of(0, 50));
    List<DossierMatch> matches =
        dossiers.stream()
  ...
  private DossierMatch match(PipelineDossieProduto dossier, Set<String> experimentTerms) {
    String text =
        (safe(dossier.getRespostaFinal())
                + " "
                + safe(dossier.getResponse())
                + " "
                + safe(dossier.getRequest()))
  ...
  public record CommercialDossierPreflightResult(
      boolean approved, String summary, String evidenceReference) {
    /** Cria resultado aprovado com resumo e referência de evidência. */
    public static CommercialDossierPreflightResult approved(
        String summary, String evidenceReference) {
    ...
    public static CommercialDossierPreflightResult blocked(
        String summary, String evidenceReference) {
  ...
  private record DossierMatch(
      Long dossierId, String productKey, String pipelineCode, List<String> matchedTerms) {
    ...
    private String summary() {
      return "%s #%d com termos %s"
          .formatted(
              pipelineCode, dossierId, String.join(", ", matchedTerms.stream().limit(5).toList()));
    }
  }
}
