...
public class SalesPageTypeService {
  ...
  public List<ExperimentSalesPageTypeSelectionResponse> replaceExperimentSelections(
      Long experimentId, UpdateExperimentSalesPageTypeSelectionRequest request) {
    ...
    if (requestedSelections.isEmpty()) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "at least one sales page type is required");
    }
    if (requestedSelections.size() > MAX_AB_TEST_SELECTIONS) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "sales page A/B test accepts at most 2 variants");
    }
    ...
    for (UpdateExperimentSalesPageTypeSelectionItem item : requestedSelections) {
      ...
      if (!typeCodes.add(typeCode)) {
        throw new ResponseStatusException(
            HttpStatus.BAD_REQUEST, "duplicated typeCode: " + typeCode);
      }
      SalesPageType type =
          typeRepository
              .findById(typeCode)
      ...
      if (!variantKeys.add(variantKey)) {
        throw new ResponseStatusException(
            HttpStatus.BAD_REQUEST, "duplicated variantKey: " + variantKey);
      }
      entities.add(
          ExperimentSalesPageTypeSelection.builder()
              .experiment(experiment)
              .salesPageType(type)
    ...
    return selectionRepository.saveAll(entities).stream().map(this::toSelectionResponse).toList();
  }
  ...
  private String normalizeVariantKey(String value, int index) {
    ...
    if (normalized.length() > 16) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "variantKey must have at most 16 characters");
    }
    return normalized;
  }
  ...
  private BigDecimal normalizeTrafficWeight(BigDecimal value) {
    ...
    if (normalized.compareTo(BigDecimal.ZERO) <= 0
      ...
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "trafficWeight must be between 0 and 100");
    }
  ...
  private ExperimentSalesPageTypeSelectionResponse toSelectionResponse(
      ExperimentSalesPageTypeSelection selection) {
