...
public class ExperimentAiPromptSchemaUsageService {
  ...
  public void linkHypothesisTemplates(Long experimentId) {
    ...
    executions.stream()
        ...
        .forEach(
            execution ->
                templateRepository
                    .findById(execution.getPromptTemplateId())
                    .ifPresent(
                        template ->
                            upsertUsage(
                                experiment,
  ...
  @Transactional
  public void linkSalesPageTemplate(
      Long experimentId, AiPromptSchemaTemplate template, String stageCode, String sourceJobId) {
    Experiment experiment = findExperiment(experimentId);
    upsertUsage(
        experiment, template, SALES_PAGE_USAGE_CONTEXT, stageCode, sourceJobId, Instant.now());
  }
  ...
  public long countHypothesisTemplates(Long experimentId) {
    return usageRepository.countByExperimentIdAndPipelineCode(
        experimentId, HYPOTHESIS_PIPELINE_CODE);
  }
  ...
  private void upsertUsage(
    ...
    Optional<ExperimentAiPromptSchemaUsage> existing =
        usageRepository.findByExperimentIdAndTemplateKeyAndUsageContextAndStageCode(
            experiment.getId(), template.getTemplateKey(), usageContext, stageCode);
    ExperimentAiPromptSchemaUsage usage =
        existing.orElseGet(
            () ->
                ExperimentAiPromptSchemaUsage.builder()
                    .experiment(experiment)
