...
/**
 * Coordena o planejamento de variações de público sem alterar campanhas em execução
 * automaticamente.
 */
@Service
public class ExperimentAudienceTestService {
  ...
  public ExperimentAudienceTestDto create(
      Long experimentId, CreateExperimentAudienceTestRequest request) {
    Experiment experiment = experimentRepository.findById(experimentId).orElseThrow();
    ExperimentAudienceTest test =
        ExperimentAudienceTest.builder()
            ...
            .build();
    List<ExperimentAudienceTestItem> items =
        request.getItems().stream().map(item -> toItem(experiment, item)).toList();
  ...
  public void delete(Long experimentId, Long audienceTestId) {
    ExperimentAudienceTest test =
        audienceTestRepository
            .findById(audienceTestId)
            .orElseThrow(() -> new IllegalArgumentException("Teste de público não encontrado"));
    if (test.getExperiment() == null
        || !Objects.equals(test.getExperiment().getId(), experimentId)) {
  ...
  private ExperimentAudienceTestItem toItem(
      Experiment experiment, CreateExperimentAudienceTestRequest.Item item) {
    TargetingElement element =
        targetingElementRepository
            .findById(item.getTargetingElementId())
            .orElseThrow(
                () -> new IllegalArgumentException("Elemento de segmentação não encontrado"));
  ...
  private void validateElement(
      Experiment experiment, TargetingElement element, TargetingCandidateType candidateType) {
    if (element.getNiche() == null
        || element.getNiche().getId() == null
        || experiment.getNiche() == null
        || experiment.getNiche().getId() == null
        || !Objects.equals(element.getNiche().getId(), experiment.getNiche().getId())) {
      throw new IllegalArgumentException(
          "Elemento de segmentação não pertence ao mesmo nicho do experimento");
    }
  ...
  private ExperimentAudienceTestDto toDto(ExperimentAudienceTest test) {
    Long lower = 0L;
    Long upper = 0L;
    List<ExperimentAudienceTestDto.Item> items =
            ...
            .map(
                item -> {
                  ...
                  return ExperimentAudienceTestDto.Item.builder()
                      ...
                      .metaAudienceSizeLowerBound(
                          element != null ? element.getMetaAudienceSizeLowerBound() : null)
                      .metaAudienceSizeUpperBound(
                          element != null ? element.getMetaAudienceSizeUpperBound() : null)
                      .build();
                })
            .toList();
    for (ExperimentAudienceTestDto.Item item : items) {
      long itemLower =
          item.metaAudienceSizeLowerBound() != null ? item.metaAudienceSizeLowerBound() : 0L;
      long itemUpper =
          item.metaAudienceSizeUpperBound() != null ? item.metaAudienceSizeUpperBound() : itemLower;
      lower += itemLower;
      upper += itemUpper;
    }
