...
public class ExperimentCampaignMetricService {
    ...
    public ExperimentCampaignMetricService(ExperimentCampaignMetricRepository repository,
        ...
        this.pdeResetHttpClient = HttpClient.newBuilder()
    ...
    private void resetPdeAnalyticsIfNeeded(Experiment experiment) {
        ...
        if (resetUrl == null || resetUrl.isBlank()) {
            throw new IllegalStateException("URL de reset de analytics PDE não configurada para Clube MUSA");
        }
        try {
            HttpRequest request = HttpRequest.newBuilder(URI.create(resetUrl))
                    .timeout(Duration.ofSeconds(10))
                    .POST(HttpRequest.BodyPublishers.noBody())
                    .build();
            HttpResponse<String> response = pdeResetHttpClient.send(request, HttpResponse.BodyHandlers.ofString());
            ...
            log.error(
                    ...
                    resetUrl,
                    ex);
            throw new IllegalStateException("Não foi possível limpar analytics PDE no início da campanha", ex);
    ...
    private String resolvePdeResetUrl(Experiment experiment) {
        ...
        return baseUrl + "/api/pde/access/analytics/" + CLUBEMUSA_PRODUCT_SLUG + "/reset-campaign-start";
    }
