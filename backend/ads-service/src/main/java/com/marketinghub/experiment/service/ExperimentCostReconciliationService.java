...
public class ExperimentCostReconciliationService {
    ...
    private final GeraSalesPagePublicationStageAuditRepository geraSalesPagePublicationStageAuditRepository;
    ...
    public ExperimentCostReconciliationService(
        ...
        this.geraSalesPagePublicationStageAuditRepository = geraSalesPagePublicationStageAuditRepository;
    ...
    public ExperimentDto enrich(Experiment experiment, ExperimentDto dto) {
        ...
        BigDecimal baseBrl = money(experiment.getCost())
                .add(money(experiment.getExpense()))
                .add(money(experiment.getCampaignMetric() != null ? experiment.getCampaignMetric().getSpend() : null));
        BigDecimal technicalCostBrl = currencyConversionService.usdToBrl(totalTechnicalCostUsd(experiment.getId()));
        ...
        BigDecimal visibleTotal = auditableTotal.compareTo(BigDecimal.ZERO) > 0 ? auditableTotal : legacyTotal;
    ...
    private BigDecimal totalTechnicalCostUsd(Long experimentId) {
        return money(pipelineJobRepository.sumCostUsdByExperimentId(experimentId))
                .add(money(geraLandingStageExecutionRepository.sumCompletedCostUsdByExperimentId(experimentId)))
                .add(money(geraSalesPagePublicationStageAuditRepository.sumCostUsdByExperimentId(experimentId)))
