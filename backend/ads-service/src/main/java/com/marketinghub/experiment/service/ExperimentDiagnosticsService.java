...
public class ExperimentDiagnosticsService {
   ...
   */
  public ExperimentDiagnosticsService(
  ...
  /**
   * Monta o alerta exibido na tela do experimento a partir do status atual e dos artefatos
   * pendentes.
   */
  ...
  public ExperimentDiagnosticsDto diagnose(Long experimentId) {
    ...
    long stuckCampaigns =
        campaigns.stream().filter(c -> !hasMetaId(c.getExternalId(), c.getId())).count();
    long stuckAdSets =
        adSets.stream().filter(s -> !hasMetaId(s.getExternalId(), s.getId())).count();
    ...
    if (statusFailed && hasPendingArtifacts) {
      ...
      resolution =
          "Mantenha o status em Planejado e verifique os workers de Facebook para concluir a publicação.";
    } else {
    ...
    return new ExperimentDiagnosticsDto(
        severity, headline, description, resolution, artifacts, failureDetails);
  }

  /** Retorna detalhes da falha operacional mais recente registrada pelo job ou pela Meta. */
  private ExperimentFailureDetailsDto resolveFailureDetails(
      Long experimentId, boolean statusFailed) {
    if (!statusFailed) {
      return null;
    }
    return publicationJobStepRepository
        .findTopByExperimentIdAndErrorMessageIsNotNullOrderByOccurredAtDescIdDesc(experimentId)
    ...
    return facebookApiLogService.findLogs(experimentId, 200).stream()
        .max(
            Comparator.comparing(
                this::resolveOccurrence, Comparator.nullsLast(Comparator.naturalOrder())))
        .filter(this::isFailureLog)
        .map(
            log ->
                new ExperimentFailureDetailsDto(
                    log.errorMessage(),
                    log.endpoint(),
                    log.statusCode(),
  ...
  private static String buildHealthyDescription(
      Experiment experiment, List<FacebookAdsCampaign> campaigns) {
  ...
  private static String buildFailedResolution(
      Collection<FacebookAdsCampaign> campaigns, boolean suggestAccountReview) {
    String accountInfo =
        campaigns.stream()
            .findFirst()
            .map(
                c -> {
                  String accountName =
                      c.getFacebookAccount() != null ? c.getFacebookAccount().getName() : null;
    ...
    if (suggestAccountReview) {
      return "Revise o acesso e as permissões da conta %s, corrija os bloqueios identificados nos logs e depois volte o status do experimento para Planejado para liberar uma nova tentativa."
          .formatted(accountInfo);
    }
  ...
  private static List<ExperimentPublishingArtifactDto> collectArtifacts(
      List<FacebookAdsCampaign> campaigns, List<FacebookAdsAdSet> adSets, List<FacebookAdsAd> ads) {
    ...
    campaigns.stream()
        ...
        .forEach(
            c ->
                artifacts.add(
                    new ExperimentPublishingArtifactDto(
                        "CAMPAIGN",
                        c.getId(),
                        c.getName(),
                        c.getStatus().name(),
