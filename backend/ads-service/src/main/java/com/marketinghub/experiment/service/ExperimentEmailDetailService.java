...
public class ExperimentEmailDetailService {
  ...
  public ExperimentEmailDetailService(
  ...
  public ExperimentEmailDetailDto updateApproval(Long experimentId, Long stepId, boolean approved) {
    ...
    Map<String, String> metadata =
        new LinkedHashMap<>(
            context.journey().getMetadata() != null ? context.journey().getMetadata() : Map.of());
  ...
  private Context loadContext(Long experimentId, Long stepId) {
    if (experimentId == null || stepId == null) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "experimentId and stepId are required");
    }
    ...
    if (experiment.getJourneyTemplate() == null) {
      throw new ResponseStatusException(
          HttpStatus.NOT_FOUND, "Experiment does not have a journey template");
    }
    JourneyStep step =
    ...
    if (!step.getTemplate().getId().equals(experiment.getJourneyTemplate().getId())) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "Step does not belong to experiment template");
    }
            ...
            .orElseThrow(
                () ->
                    new ResponseStatusException(
                        HttpStatus.NOT_FOUND, "Experiment does not have a journey"));
