...
public class ExperimentJourneyService {
  ...
  public ExperimentJourneyService(
        ...
        .map(
            journey -> {
              List<JourneyAssignment> assignments =
                  assignmentRepository.findAllByJourneyId(journey.getId());
  ...
  public Result rebuild(Long experimentId) {
    ...
    if (template == null) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "Experiment is not linked to a journey template");
    }
    ...
    List<JourneyAssignment> persisted =
        assignments.isEmpty() ? List.of() : assignmentRepository.saveAll(assignments);
  ...
  private Comparator<JourneyAssignment> byStepPosition() {
    return Comparator.comparing(
            (JourneyAssignment assignment) ->
                assignment.getNextStep() != null
                    ? assignment.getNextStep().getPosition()
                    : Integer.MAX_VALUE,
