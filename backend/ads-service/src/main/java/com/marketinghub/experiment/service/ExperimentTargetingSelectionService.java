...
/**
 * Gerencia as escolhas de público do experimento e evita seleções sem identificador oficial da
 * Meta.
 */
@Service
public class ExperimentTargetingSelectionService {
  ...
  private TargetingElement resolveTargetingElement(
      Long targetingElementId, Experiment experiment, TargetingCandidateType candidateType) {
    ...
    if (experiment.getHypothesisRef() != null
        && element.getHypothesis() != null
        && !Objects.equals(
            experiment.getHypothesisRef().getId(), element.getHypothesis().getId())) {
  ...
  public List<ExperimentTargetingSelectionDto> list(Long experimentId) {
    return repository.findByExperimentIdOrderByCandidateTypeAscTermAsc(experimentId).stream()
        .map(
            item ->
                ExperimentTargetingSelectionDto.builder()
                    .id(item.getId())
                    .experimentId(item.getExperiment().getId())
                    .candidateType(item.getCandidateType())
                    .term(item.getTerm())
                    .targetingElementId(
                        item.getTargetingElement() != null
                            ? item.getTargetingElement().getId()
                            : null)
                    .build())
        .toList();
  }
  ...
  public List<ExperimentTargetingSelectionDto> save(
      Long experimentId, SaveExperimentTargetingSelectionsRequest request) {
    ...
    if (request != null && request.getItems() != null) {
      for (SaveExperimentTargetingSelectionsRequest.Item item : request.getItems()) {
        ...
        TargetingElement linkedElement =
            resolveTargetingElement(
                item.getTargetingElementId(), experiment, item.getCandidateType());
        items.add(
  ...
  public TargetingRequestDto runSimpleFlow(Long experimentId) {
    ...
    List<ExperimentTargetingSelection> selections =
        repository.findByExperimentIdOrderByCandidateTypeAscTermAsc(experimentId);
  ...
  public ExperimentSimpleFlowStatusDto getSimpleFlowStatus(Long experimentId) {
    ...
    return targetingRequestRepository
        .findFirstByExperimentIdOrderByCreatedAtDesc(experimentId)
        .map(
            request -> {
              TargetingRequestDto requestDto =
                  targetingRequestMapper.toDetailedDto(request, SIMPLE_FLOW_RESOLUTION_ETA_SECONDS);
              var summary =
                  targetingResolutionJobService
