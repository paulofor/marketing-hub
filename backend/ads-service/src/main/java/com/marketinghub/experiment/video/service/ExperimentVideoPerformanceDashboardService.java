...
public class ExperimentVideoPerformanceDashboardService {
    ...
    public ExperimentVideoPerformanceDashboardDto summarize(Long experimentId) {
        ...
        List<ExperimentVideoAsset> assets = videoAssetRepository.findByExperimentIdOrderByCreatedAtDesc(experimentId);
        ...
        List<ExperimentVideoPerformanceAssetDto> assetDtos = buildAssetDtos(assets, metaCreatives, funnel);
        List<ExperimentVideoPerformanceCampaignDto> campaignDtos = buildCampaignDtos(campaigns, metricsByCampaignId);
        ExperimentVideoPerformanceSummaryDto summary = buildSummary(assets, metaCreatives, campaignDtos, funnel);
    ...
    private List<FacebookAdsCampaign> fetchCampaigns(Long experimentId) {
        ...
        for (FacebookAdsAdSet adSet : adSets) {
            ...
            adSetsByCampaign.computeIfAbsent(adSet.getCampaign().getId(), ignored -> new ArrayList<>()).add(adSet);
        }
        campaigns.forEach(campaign -> campaign.setAdSets(
    ...
    private Map<String, ExperimentCampaignMetric> fetchMetricsByCampaignId(List<FacebookAdsCampaign> campaigns) {
        List<String> campaignIds = campaigns.stream()
                .map(FacebookAdsCampaign::getId)
                .filter(Objects::nonNull)
                .toList();
        ...
        for (ExperimentCampaignMetric metric : campaignMetricRepository.findByCampaignIdIn(campaignIds)) {
        ...
        for (FacebookAdsCampaign campaign : campaigns) {
            for (FacebookAdsAdSet adSet : safeList(campaign.getAdSets())) {
                for (FacebookAdsAd ad : safeList(adSet.getAds())) {
                    ...
                    snapshots.add(new MetaCreativeSnapshot(
                            campaign.getId(),
                            creative.getId(),
                            creative.getKind(),
    ...
    private List<ExperimentVideoPerformanceAssetDto> buildAssetDtos(
            List<ExperimentVideoAsset> assets, List<MetaCreativeSnapshot> metaCreatives, FunnelSnapshot funnel) {
        return assets.stream()
                .filter(this::shouldExposeAsset)
                .sorted(Comparator.comparing(ExperimentVideoAsset::getCreatedAt, Comparator.nullsLast(Comparator.reverseOrder())))
    ...
    private ExperimentVideoPerformanceAssetDto toAssetDto(
        ...
        List<MetaCreativeSnapshot> matches = metaCreatives.stream()
                .filter(creative -> matchesAsset(asset, creative))
                .toList();
        String attributionLevel = matches.isEmpty() ? "EXPERIMENT" : "AD";
        List<ExperimentVideoPerformanceCreativeDto> creativeDtos = matches.stream()
                .map(creative -> toCreativeDto(creative, funnel))
                .toList();
    ...
    private ExperimentVideoPerformanceCreativeDto toCreativeDto(MetaCreativeSnapshot creative, FunnelSnapshot funnel) {
    ...
    private List<ExperimentVideoPerformanceCampaignDto> buildCampaignDtos(
            List<FacebookAdsCampaign> campaigns, Map<String, ExperimentCampaignMetric> metricsByCampaignId) {
    ...
    private ExperimentVideoPerformanceSummaryDto buildSummary(
        ...
        long impressions = campaigns.stream().mapToLong(ExperimentVideoPerformanceCampaignDto::impressions).sum();
        long clicks = campaigns.stream().mapToLong(ExperimentVideoPerformanceCampaignDto::clicks).sum();
        BigDecimal spend = campaigns.stream()
                .map(ExperimentVideoPerformanceCampaignDto::spend)
                .filter(Objects::nonNull)
                .reduce(BigDecimal.ZERO, BigDecimal::add);
        Instant lastMetricAt = campaigns.stream()
                .map(ExperimentVideoPerformanceCampaignDto::metricsLastSyncedAt)
                .filter(Objects::nonNull)
                .max(Comparator.naturalOrder())
                .orElse(null);
        long approvedAssets = assets.stream()
                .filter(asset -> asset.getReviewStatus() == ExperimentVideoReviewStatus.APPROVED)
                .count();
        long metaVideoCreatives = metaCreatives.stream()
                .filter(creative -> creative.kind() == AdCreativeKind.VIDEO)
                .count();
    ...
    private boolean matchesAsset(ExperimentVideoAsset asset, MetaCreativeSnapshot creative) {
        ...
        addToken(assetTokens, asset.getSalesVideoJob() != null ? asset.getSalesVideoJob().getId() : null);
        addToken(assetTokens, asset.getSalesVideoProfile() != null ? asset.getSalesVideoProfile().getId() : null);
    ...
    private String extractMetaVideoId(FacebookAdsAdCreative creative) {
        if (creative.getKind() != AdCreativeKind.VIDEO || !StringUtils.hasText(creative.getVideoDataJson())) {
            return null;
        }
        try {
