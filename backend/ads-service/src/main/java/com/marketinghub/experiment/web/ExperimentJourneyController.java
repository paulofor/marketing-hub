...
public class ExperimentJourneyController {
  ...
  public ExperimentJourneyController(
      ExperimentJourneyService journeyService, JourneyMapper journeyMapper) {
  ...
  public ExperimentJourneyAssignmentsResponse getAssignments(@PathVariable Long experimentId) {
    return journeyService
        .findCurrent(experimentId)
        .map(
            result ->
                new ExperimentJourneyAssignmentsResponse(
                    result.journey().getId(),
                    result.templateId(),
                    result.assignments().stream()
                        .map(journeyMapper::toAssignmentResponse)
                        .toList()))
