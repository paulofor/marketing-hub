...
public class ExperimentAdSetJobController {
  ...
  public ExperimentAdSetJobController(
      ExperimentAdSetWorkflowJobCoordinator coordinator, ObjectMapper objectMapper) {
  ...
  public List<ExperimentAdSetJobPayloadDto> claimJobs(
      @Valid @RequestBody ExperimentAdSetJobClaimRequest request) {
    List<ExperimentAdSetJob> jobs =
        coordinator.claimJobs(request.worker(), request.workerId(), request.limit());
    return jobs.stream().map(this::toPayload).collect(Collectors.toList());
  }

  @PostMapping("/{jobId}/complete")
  @ResponseStatus(HttpStatus.ACCEPTED)
  public void completeJob(
      @PathVariable Long jobId, @Valid @RequestBody ExperimentAdSetJobResultRequest request) {
  ...
  @ResponseStatus(HttpStatus.ACCEPTED)
  public void failJob(
      @PathVariable Long jobId, @Valid @RequestBody ExperimentAdSetJobFailureRequest request) {
  ...
  private JsonNode readPayload(String raw) {
    ...
    try {
      ...
      LOGGER.error(
          "Falha ao desserializar payload do job da integração Facebook Ads. rawPayloadLength={}",
          raw.length(),
          ex);
      return null;
    }
