...
public class FacebookCampaignStopController {
  ...
  public List<FacebookCampaignStopRequestDto> listStopRequests() {
    return stopService.listPendingStopRequests().stream().map(this::toDto).toList();
  }

  @PostMapping("/{campaignId}/stop-results")
  @ResponseStatus(HttpStatus.NO_CONTENT)
  public void registerStopResult(
      @PathVariable String campaignId, @RequestBody FacebookCampaignStopResultRequest request) {
