...
public class FacebookConfigurationController {
  ...
  public FacebookConfigurationController(
      FacebookPageRepository pageRepository, FacebookAccountRepository accountRepository) {
  ...
  public FacebookConfigurationStatus configurationStatus() {
    ...
    FacebookConfigurationStatus.TokenRenewalDiagnostics tokenRenewal =
        buildTokenRenewalDiagnostics(accounts);
  ...
  private FacebookConfigurationStatus.WorkerDiagnostics buildWorkerDiagnostics(
      List<FacebookAccount> accounts) {
    FacebookAccount workerAccount =
        accounts.stream().filter(FacebookAccount::isWorkerEnabled).findFirst().orElse(null);
    ...
    addMessageIfMissing(
        messages,
        workerAccount.getAccessToken(),
        FacebookWorkerValidationError.ACCESS_TOKEN_MISSING);
    addMessageIfMissing(
        messages,
        workerAccount.getAdAccountId(),
    ...
    return new FacebookConfigurationStatus.WorkerDiagnostics(
        true, ready, workerAccount.getId(), workerAccount.getName(), messages);
  }

  private void appendRecordedWorkerValidation(
      List<FacebookConfigurationStatus.DiagnosticMessage> messages, FacebookAccount workerAccount) {
    ...
    if (formattedTimestamp != null) {
      messageBuilder
          .append("Última validação do worker em ")
    ...
    messages.add(
        0,
        new FacebookConfigurationStatus.DiagnosticMessage(
            code + "_RECORDED", messageBuilder.toString()));
  }
  ...
  private void addMessageIfMissing(
    ...
    messages.add(
        new FacebookConfigurationStatus.DiagnosticMessage(error.code(), error.userMessage()));
  }

  // Executa a operação buildTokenRenewalDiagnostics da integração Facebook Ads.
  private FacebookConfigurationStatus.TokenRenewalDiagnostics buildTokenRenewalDiagnostics(
      List<FacebookAccount> accounts) {
    List<FacebookAccount> enabledAccounts =
        accounts.stream().filter(FacebookAccount::isTokenRenewalEnabled).toList();

    List<FacebookConfigurationStatus.TokenRenewalAccountStatus> statuses =
        enabledAccounts.stream().map(this::buildTokenRenewalAccountStatus).toList();
    ...
    return new FacebookConfigurationStatus.TokenRenewalDiagnostics(
        enabledAccounts.size(), (int) eligibleCount, statuses);
  }
  ...
  private FacebookConfigurationStatus.TokenRenewalAccountStatus buildTokenRenewalAccountStatus(
    ...
    if (!hasAppId) {
      messages.add(
          new FacebookConfigurationStatus.DiagnosticMessage(
              "APP_ID_MISSING", "Cadastre o App ID do Facebook utilizado para gerar o token."));
    }
    ...
    if (eligible) {
      ...
      if (expiresInDays == null) {
        detail =
            "O token atual não possui data de expiração informada. O worker aguardará até que haja necessidade de renovação.";
        ...
        detail =
            "Token válido por mais 1 dia. A renovação automática será tentada na próxima execução.";
    ...
    return new FacebookConfigurationStatus.TokenRenewalAccountStatus(
        account.getId(), account.getName(), eligible, messages);
  }

  public record FacebookConfigurationStatus(
      boolean hasConfiguredPages, WorkerDiagnostics worker, TokenRenewalDiagnostics tokenRenewal) {
    ...
    public record TokenRenewalDiagnostics(
        int enabledAccounts, int eligibleAccounts, List<TokenRenewalAccountStatus> accounts) {}

    public record TokenRenewalAccountStatus(
        Long accountId, String accountName, boolean eligible, List<DiagnosticMessage> messages) {}
