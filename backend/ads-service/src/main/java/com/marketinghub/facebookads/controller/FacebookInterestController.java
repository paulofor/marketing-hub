...
public class FacebookInterestController {
  ...
  public FacebookInterest update(
      @PathVariable Long id, @RequestBody FacebookInterestUpdateRequest request) {
    ...
    if (request.status() == FacebookInterestStatus.VALID
        && !StringUtils.hasText(request.facebookInterestId())) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "facebookInterestId is required when status is VALID");
    }
  ...
  public record FacebookInterestUpdateRequest(
      FacebookInterestStatus status, String facebookInterestId, String name) {}
