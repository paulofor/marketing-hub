...
public class ExperimentAdSetWorkflowService {
  private static final Set<ExperimentStatus> VALIDATION_ELIGIBLE_STATUSES =
  ...
  public ExperimentAdSetWorkflowService(
  ...
  @Transactional
  public ExperimentAdSetWorkflowDto start(
      Long experimentId, StartExperimentAdSetWorkflowRequest request) {
    ...
    if (workflow.getStatus() == ExperimentAdSetWorkflowStatus.NOT_STARTED
        || restart
  ...
  private ExperimentAdSetWorkflow createWorkflow(Long experimentId) {
    ...
    ExperimentAdSetWorkflow workflow =
        ExperimentAdSetWorkflow.builder()
            .experiment(experiment)
  ...
  public ExperimentAdSetJobDetailDto getJobDetail(Long experimentId, Long jobId) {
    ...
    if (job.getWorkflow() == null || !job.getWorkflow().getId().equals(workflow.getId())) {
      throw new EntityNotFoundException(
          "Job %d não pertence ao experimento %d".formatted(jobId, experimentId));
    }
    List<ExperimentAdSetJobApiLogDto> apiLogs =
        jobApiLogRepository.findByJobIdOrderByCreatedAtAsc(jobId).stream()
            .map(this::toApiLogDto)
            .collect(Collectors.toList());
    return new ExperimentAdSetJobDetailDto(
        toJobDto(job), job.getPayload(), job.getResultPayload(), apiLogs);
  }
  ...
  private ExperimentAdSetWorkflowDto buildDto(ExperimentAdSetWorkflow workflow) {
    ...
    List<ExperimentAdSetJobDto> jobDtos =
        jobs.stream()
            .sorted(Comparator.comparing(ExperimentAdSetJob::getId))
            .map(this::toJobDto)
            .toList();
    List<ExperimentAdSetSpecDto> specDtos =
        specs.stream()
            .sorted(
                Comparator.comparing(
                    spec -> spec.getSlot() != null ? spec.getSlot().ordinal() : Integer.MAX_VALUE))
            .map(
                spec ->
                    new ExperimentAdSetSpecDto(
                        spec.getId(),
                        spec.getSlot(),
                        spec.getLabel(),
                        spec.getAgeMin(),
