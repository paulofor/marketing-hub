...
public class CampaignStrategyService {
  ...
  public CampaignStrategyService(
      CampaignStrategyRepository strategyRepository,
  ...
  public CampaignStrategyEvaluation evaluateAfterMetrics(ExperimentCampaignMetric metric) {
    if (metric == null || metric.getCampaign() == null) {
      throw new IllegalArgumentException(
          "Metricas de campanha sao obrigatorias para avaliar estrategia");
    }
    ...
    if (!strategy.isEnabled()) {
      return recordEvaluation(
          strategy,
          metric,
          CampaignStrategyDecision.KEEP_LEARNING,
          "Estrategia desabilitada; mantendo campanha sem decisao automatica.",
          null,
          null);
    }
    ...
    if (campaign.getStopRequestedAt() != null) {
      return recordEvaluation(
          strategy,
          metric,
          CampaignStrategyDecision.STOP_REQUESTED,
  ...
  public CampaignStrategy findLatestByExperimentId(Long experimentId) {
    ...
    return strategyRepository
        .findTopByCampaign_Experiment_IdOrderByCreatedAtDesc(experimentId)
        .orElse(null);
  }
  ...
  private FunnelSnapshot loadFunnelSnapshot(FacebookAdsCampaign campaign) {
    ...
    try {
      ...
      ExperimentFunnelStageDiagnosticDto checkout =
          findStage(diagnostics, ExperimentFunnelStage.ACESSO_CHECKOUT);
      ExperimentFunnelStageDiagnosticDto purchase =
          findStage(diagnostics, ExperimentFunnelStage.COMPRA);
  ...
  private CampaignStrategyEvaluation recordEvaluation(
  ...
  private ExperimentFunnelStageDiagnosticDto findStage(
  ...
  private record FunnelSnapshot(
      Long checkoutAttempts, Long checkoutClicks, Long purchases, BigDecimal checkoutRate) {}
}
