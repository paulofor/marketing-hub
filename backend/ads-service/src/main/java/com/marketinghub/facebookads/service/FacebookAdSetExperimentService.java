...
public class FacebookAdSetExperimentService {
  private static final List<ExperimentStatus> STATUSES =
  ...
  public FacebookAdSetExperimentService(
  ...
  /**
   * Lista experimentos prontos para conjuntos de anúncios, priorizando o público selecionado no
   * experimento.
   */
  public List<ExperimentReadyForAdSetDto> listExperimentsReadyForAdSets(Long experimentId) {
    if (experimentId != null) {
      return experimentRepository
          .findForAdSetTargetingById(experimentId, ExperimentPlatform.FACEBOOK)
          .map(this::toReadyForAdSetDto)
          .map(List::of)
          .orElseGet(List::of);
    }
    List<Experiment> experiments =
        experimentRepository.findAllReadyForAdSets(ExperimentPlatform.FACEBOOK, STATUSES);
  ...
  /**
   * Retorna somente o pacote de segmentação necessário para o worker publicar a campanha do
   * experimento.
   */
  public Optional<FacebookAdSetTargetingPackageDto> getTargetingPackageForCampaign(
      Long experimentId) {
    return experimentRepository
        .findForAdSetTargetingById(experimentId, ExperimentPlatform.FACEBOOK)
        .flatMap(
            experiment -> {
              ...
              return Optional.of(
                  new FacebookAdSetTargetingPackageDto(experiment.getId(), targeting));
            });
  }
  ...
  private ExperimentReadyForAdSetDto toReadyForAdSetDto(Experiment experiment) {
    ...
    return new ExperimentReadyForAdSetDto(
        ...
        experiment.getHypothesisRef() != null
            ? hypothesisMapper.toDto(experiment.getHypothesisRef())
            : null,
        targeting);
  }

  /**
   * Monta o pacote de público usando as seleções do experimento e, se não existirem, o pacote
   * aprovado do nicho.
   */
  ...
  /**
   * Monta o público salvo na tela do experimento com elementos aprovados e identificáveis pela
   * Meta.
   */
  private TargetingPackageDto buildSelectedTargetingPackage(Experiment experiment) {
    List<ExperimentTargetingSelection> selections =
        targetingSelectionRepository.findByExperimentIdWithTargetingElement(experiment.getId());
  ...
  private TargetingPackageDto buildApprovedNicheTargetingPackage(Experiment experiment) {
    ...
    UUID hypothesisId =
        experiment.getHypothesisRef() != null ? experiment.getHypothesisRef().getId() : null;
    Map<TargetingElementType, List<TargetingElementDto>> mapped = emptyTargetingMap();
  ...
  private boolean hasAnyPublishableTargeting(
      Map<TargetingElementType, List<TargetingElementDto>> mapped) {
  ...
  private Map<TargetingElementType, List<TargetingElementDto>> emptyTargetingMap() {
    Map<TargetingElementType, List<TargetingElementDto>> mapped =
        new EnumMap<>(TargetingElementType.class);
  ...
  private TargetingPackageDto toTargetingPackage(
      Map<TargetingElementType, List<TargetingElementDto>> mapped) {
  ...
  private List<TargetingElementDto> mapElements(
      Long nicheId, UUID hypothesisId, TargetingElementType type) {
    List<TargetingElement> elements =
        targetingElementRepository.findApprovedForExperiment(nicheId, type, hypothesisId);
    if (elements.isEmpty()) {
      return List.of();
    }
