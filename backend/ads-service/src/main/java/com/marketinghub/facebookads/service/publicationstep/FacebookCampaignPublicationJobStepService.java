...
public class FacebookCampaignPublicationJobStepService {
  ...
  public FacebookCampaignPublicationJobStepService(
      FacebookCampaignPublicationJobStepRepository repository,
  ...
  public void register(FacebookCampaignPublicationJobStepRequest request) {
    if (request == null
        ...
        || !StringUtils.hasText(request.stepName())) {
              ...
              .orElseThrow(
                  () ->
                      new EntityNotFoundException(
                          "Experimento %d não encontrado".formatted(request.experimentId())));
    }
    ...
    step.setProvider(
        StringUtils.hasText(request.provider()) ? request.provider().trim() : "FACEBOOK");
