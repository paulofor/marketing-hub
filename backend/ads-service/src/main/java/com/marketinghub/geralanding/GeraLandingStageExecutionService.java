...
public class GeraLandingStageExecutionService {
  ...
  public GeraLandingPublishResponse approveAndPublishLanding(Long experimentId) {
    ...
    log.info(
        "GeraLanding publish approval loaded experiment (experimentId={}, experimentName={})",
        experimentId,
        experiment.getName());
    ...
    if (!StringUtils.hasText(landingPageHtml)) {
      log.warn(
          "GeraLanding publish approval blocked because landing HTML is missing (experimentId={})",
          experimentId);
      throw new ResponseStatusException(
          HttpStatus.CONFLICT, "Landing HTML ainda não foi gerado para este experimento");
    }
    log.info(
        "GeraLanding publish approval found landing HTML (experimentId={}, htmlLength={})",
        experimentId,
        landingPageHtml.length());

    String slug = "exp-" + experimentId + "-landing-geralanding";
    log.info(
        "GeraLanding publish approval resolved slug (experimentId={}, slug={})",
        experimentId,
        slug);
    ...
    log.info(
        "GeraLanding publish approval injected funnel controls (experimentId={}, htmlLengthBefore={}, htmlLengthAfter={})",
        experimentId,
        landingPageHtml.length(),
        htmlWithFunnelControls.length());

    String facebookPixelId = resolveFacebookPixelId(experiment);
    log.info(
        "GeraLanding publish approval resolved Facebook Pixel (experimentId={}, hasPixelId={})",
        experimentId,
        StringUtils.hasText(facebookPixelId));
    String htmlWithFacebookPixel = injectFacebookPixel(htmlWithFunnelControls, facebookPixelId);
    log.info(
        "GeraLanding publish approval injected Facebook Pixel (experimentId={}, htmlLengthBefore={}, htmlLengthAfter={})",
        experimentId,
        htmlWithFunnelControls.length(),
        htmlWithFacebookPixel.length());

    log.info(
        "GeraLanding publish approval sending flow to Lead Portal (experimentId={}, slug={}, htmlLength={})",
        experimentId,
        slug,
        htmlWithFacebookPixel.trim().length());
    publishToLeadPortal(
        slug, "Landing GeraLanding - Experimento " + experimentId, htmlWithFacebookPixel.trim());
    log.info(
        "GeraLanding publish approval sent flow to Lead Portal successfully (experimentId={}, slug={})",
        experimentId,
        slug);
    log.info(
        "GeraLanding publish approval resolving publication URLs (experimentId={}, slug={})",
        experimentId,
        slug);
    String iframeUrl = resolveIframeUrl(slug);
    String standaloneUrl = resolveStandaloneLandingUrl(iframeUrl);
    log.info(
        "GeraLanding publish approval resolved publication URLs (experimentId={}, iframeUrl={}, standaloneUrl={})",
        experimentId,
        iframeUrl,
        standaloneUrl);
    ...
    log.info(
        "GeraLanding publish approval saved follow-up URL (experimentId={}, followUpActionUrl={})",
        experimentId,
        standaloneUrl);
    return new GeraLandingPublishResponse(
        experimentId,
        null,
        iframeUrl,
        standaloneUrl,
  ...
  private void publishToLeadPortal(String slug, String name, String html) {
    if (!StringUtils.hasText(leadPortalBaseUrl)) {
      log.warn(
          "GeraLanding Lead Portal publication blocked because base URL is not configured (slug={})",
          slug);
      throw new ResponseStatusException(
          HttpStatus.BAD_GATEWAY, "Lead Portal base URL não configurada");
    }
    URI uri =
        UriComponentsBuilder.fromHttpUrl(leadPortalBaseUrl)
            .path("/api/flows/{slug}")
            .buildAndExpand(slug)
            .toUri();
    log.info(
        "GeraLanding Lead Portal publication request prepared (slug={}, uri={}, name={}, htmlLength={})",
        slug,
        uri,
        name,
    ...
    GeraLandingLeadPortalPublishRequest payload =
        new GeraLandingLeadPortalPublishRequest(
            slug, name, "Fluxo publicado pelo módulo GeraLanding", html);
    log.info(
        "GeraLanding Lead Portal publication request parameters (slug={}, uri={}, headers={}, payload={})",
        slug,
        uri,
        headers,
        payload);
    try {
      ...
      log.info(
          "GeraLanding Lead Portal publication request completed (slug={}, uri={})", slug, uri);
    } catch (RestClientException ex) {
      String rootCauseMessage = NestedExceptionUtils.getMostSpecificCause(ex).getMessage();
      log.error(
          "GeraLanding Lead Portal publication request failed (slug={}, uri={}, rootCause={})",
          slug,
          uri,
          rootCauseMessage,
          ex);
  ...
  private String injectFacebookPixel(String html, String facebookPixelId) {
    ...
    String pixelSnippet =
        """
                ...
                /></noscript>
                """
            .formatted(facebookPixelId, facebookPixelId)
            .trim();
    if (html.toLowerCase(Locale.ROOT).contains("</head>")) {
  ...
  public String generateAndPersistProvisionalHtmlFromExperiment(Long experimentId, String jobId) {
    ...
    if (!StringUtils.hasText(experiment.getLandingPageWireframe())) {
      throw new IllegalStateException(
          "Não foi possível montar HTML provisório: experiment.landingPageWireframe ausente");
    }
    if (!StringUtils.hasText(experiment.getLandingPageCopy())) {
      throw new IllegalStateException(
          "Não foi possível montar HTML provisório: experiment.landingPageCopy ausente");
    }

    String copyStageHtml =
        imagePlanningProvisionalHtmlAssembler.assemble(
            experimentId,
            experiment.getLandingPageCopy(),
            experiment.getLandingPageWireframe(),
            jobId);
    String htmlWithGeneratedImages = copyStageHtml;
    String enrichedImagePlanning =
        landingPageImageInjector.injectImageUrlsIntoPlanning(
            experimentId, experiment.getLandingPageImagePlanning());
    ...
    GeraLandingStageExecution execution =
        executionRepository
            .findTopByIdJobOrderByExecutionRequestedAtDesc(toDatabaseIdJob(jobId))
            .orElseThrow(
                () ->
                    new EntityNotFoundException(
                        "GeraLanding execution not found for idJob: " + jobId));
  ...
  public void registerWorkerPromptExecution(GeraLandingWorkerPromptRequest request) {
    ...
    Experiment experiment =
        experimentRepository
            ...
            .orElseThrow(
                () ->
                    new EntityNotFoundException("Experiment not found: " + request.experimentId()));
                ...
                () ->
                    executionRepository
                        .findTopByExperimentIdAndStageCodeOrderByExecutionRequestedAtDesc(
                            request.experimentId(), request.stageCode()))
            .orElseThrow(
                () ->
                    new EntityNotFoundException(
                        "GeraLanding execution not found for idJob: " + idJob));
  ...
  public void receiveResult(String idJob, GeraLandingResultReceiveRequest request) {
    ...
    try {
      ...
      execution.setStatus(
          StringUtils.hasText(request.errorMessage()) ? STATUS_FAILED : STATUS_COMPLETED);
      executionRepository.save(execution);
  ...
  private String resolveProvisionalHtml(
      String idJob, GeraLandingResultReceiveRequest request, GeraLandingStageExecution execution) {
    ...
    if (STAGE_COPY.equalsIgnoreCase(request.stageCode())) {
      ...
      if (!StringUtils.hasText(wireframe)) {
        throw new IllegalStateException(
            "Não foi possível montar HTML provisório da copy: experiment.landingPageWireframe ausente");
      }
  ...
  private String resolveImagePlanningProvisionalHtml(
      String idJob, GeraLandingResultReceiveRequest request, GeraLandingStageExecution execution) {
    Experiment experiment = execution.getExperiment();
    if (experiment == null && request.experimentId() != null) {
      experiment = experimentRepository.findById(request.experimentId()).orElse(null);
    ...
    if (experiment == null
        ...
        || !StringUtils.hasText(experiment.getLandingPageCopy())) {
      return null;
    }
    String baseHtml =
        imagePlanningProvisionalHtmlAssembler.assemble(
  ...
  private void persistStageArtifactOnExperiment(
      GeraLandingResultReceiveRequest request, GeraLandingStageExecution execution) {
    ...
    if (!StringUtils.hasText(request.modelResponse())
    ...
    if (STAGE_WIREFRAME.equalsIgnoreCase(stageCode)) {
      ...
      if (!StringUtils.hasText(htmlFromDesignPreset)) {
        htmlFromDesignPreset =
            designPresetProvisionalHtmlAssembler.assemble(
                experiment.getLandingPageWireframe(),
                experiment.getLandingPageCopy(),
  ...
  private String injectBehaviorTrackingAttributesAndScript(String html) {
    ...
    for (Element section :
        document.select("section[data-section-id], section[id], [data-section-id]")) {
      String sectionId =
          section.hasAttr("data-section-id") ? section.attr("data-section-id") : section.id();
  ...
  public List<GeraLandingExecutionSummaryResponse> listExperimentStageExecutions(
    ...
    List<GeraLandingStageExecution> executions =
        includeCompleted
            ? executionRepository
            ...
            : executionRepository
                .findTop20ByExperimentIdAndStageCodeAndStatusNotOrderByExecutionRequestedAtDesc(
                    experimentId, stageCode, STATUS_COMPLETED);
    return executions.stream()
        .map(
            execution ->
                new GeraLandingExecutionSummaryResponse(
                    fromDatabaseIdJob(execution.getIdJob()),
                    execution.getStatus(),
  ...
  @Transactional(readOnly = true)
  public GeraLandingStageExecutionDetailResponse getStageExecutionDetail(
      Long experimentId, String idJob) {
    GeraLandingStageExecution execution =
        executionRepository
            .findTopByExperimentIdAndIdJobOrderByExecutionRequestedAtDesc(
                experimentId, toDatabaseIdJob(idJob))
            .orElseThrow(
                () ->
                    new EntityNotFoundException(
                        "GeraLanding execution not found for idJob: " + idJob));
  ...
  public List<GeraLandingPendingExecutionResponse> listPendingExecutions() {
    return executionRepository
        .findTop20ByStatusInOrderByExecutionRequestedAtAsc(
            List.of(STATUS_STARTED, STATUS_WAITING_OPENAI_DISPATCH))
        .stream()
        .map(
            execution ->
                new GeraLandingPendingExecutionResponse(
                    execution.getExperimentId(),
                    fromDatabaseIdJob(execution.getIdJob()),
                    execution.getStageCode()))
        .toList();
