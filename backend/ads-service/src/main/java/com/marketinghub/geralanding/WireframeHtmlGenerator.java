...
public class WireframeHtmlGenerator {
    ...
    private static String toCss(String uiSizesJson){String n=uiSizesJson.trim();StringBuilder css=new StringBuilder();int i=0;while(i<n.length()){if(n.charAt(i)=='\"'){int se=findStringEnd(n,i+1);String sel=n.substring(i+1,se);i=n.indexOf('{',se);if(i<0)break;int oe=findMatchingBrace(n,i);String ro=n.substring(i+1,oe);if(sel.startsWith("@media"))css.append(sel).append("{\n").append(parseNestedRules(ro)).append("}\n");else css.append(sel).append(" {\n").append(parseDeclarations(ro)).append("}\n");i=oe+1;}else i++;}return css.toString();}
