package com.marketinghub.geralanding.copy.provisorio;
...
/**
 * Conjunto exclusivo da etapa LANDING_PAGE_COPY: monta o HTML provisório a partir de wireframe +
 * copy, sem executar responsabilidades de image planning/design preset.
 */
@Component
public class CopyProvisionalHtmlProcessor {
  ...
  private final CopyWireframeHtmlGenerator wireframeHtmlGenerator =
      new CopyWireframeHtmlGenerator();
  ...
  public String process(String wireframeJson, String copyJson) {
    log.info(
        "[GeraLanding][CopyProvisionalHtmlProcessor] Entrada wireframeLength={}, copyLength={}",
        lengthOf(wireframeJson),
        lengthOf(copyJson));
    log.debug(
        "[GeraLanding][CopyProvisionalHtmlProcessor] Entrada wireframeJson: {}", wireframeJson);
    ...
    Map<String, Object> wireframe =
        unwrapPayload(parseJsonObject(wireframeJson, "wireframeJson"), "landingPageWireframe");
    Map<String, Object> copy =
        unwrapPayload(parseJsonObject(copyJson, "copyJson"), "landingPageCopy");
    log.info(
        "[GeraLanding][CopyProvisionalHtmlProcessor] Payload parseado wireframeKeys={}, copyKeys={}",
        wireframe.size(),
        copy.size());

    String baseHtml = buildHtmlFromWireframe(wireframe);
    log.info(
        "[GeraLanding][CopyProvisionalHtmlProcessor] HTML base gerado com length={}",
        lengthOf(baseHtml));
  ...
  public String process(String html, Map<String, Object> copyJson) {
    ...
    log.info(
        "[GeraLanding][CopyProvisionalHtmlProcessor] Aplicando copy em {} itens mapeados",
        copyByItemId.size());
  ...
  private String applyCopyByItemId(String baseHtml, Map<String, String> copyByItemId) {
    ...
    String title =
        firstNonBlank(
            copyByItemId.get("title"), copyByItemId.get("s1-title"), copyByItemId.get("s2-title"));
    if (StringUtils.hasText(title)) {
      document.title(title);
    }
    String normalizedHtml = normalizeSerializedHtml(document.outerHtml());
    log.info(
        "[GeraLanding][CopyProvisionalHtmlProcessor] Aplicação de copy concluída. htmlLength={}, titleApplied={}",
        lengthOf(normalizedHtml),
  ...
  private Element resolveElementByItemId(
      Document document, Map<String, Element> elementByNormalizedId, String itemId) {
    ...
    try {
      ...
      throw new IllegalArgumentException(
          "Falha ao interpretar "
              + payloadLabel
              + ": esperado JSON objeto ({...}) com elementos compatíveis com o contrato.",
          e);
    }
  }
  ...
  private String buildHtmlFromWireframePaginaModel(Map<String, Object> wireframe) {
    ...
    try {
      ...
      throw new IllegalArgumentException(
          "Falha ao montar HTML base para o novo formato de wireframe", e);
    }
  }
  ...
  private Map<String, String> collectCopyByItemId(Map<String, Object> copyJson) {
    ...
    for (Object section : sections) {
      ...
      Object rawItems =
          firstNonNull(sectionMap.get("items"), sectionMap.get("values"), sectionMap.get("fields"));
