...
public class DesignPresetProvisionalHtmlAssembler {
  ...
  private static final Logger log =
      LoggerFactory.getLogger(DesignPresetProvisionalHtmlAssembler.class);

  private final DesignPresetProvisionalHtmlProcessor processor;
  private final ObjectMapper objectMapper;

  /**
   * Inicializa o montador com o processador tokenizado e o mapper JSON usados na consolidação do
   * HTML.
   */
  public DesignPresetProvisionalHtmlAssembler(
      DesignPresetProvisionalHtmlProcessor processor, ObjectMapper objectMapper) {
  ...
  /**
   * Consolida wireframe/copy com o resultado do design preset para produzir o HTML provisório da
   * etapa.
   */
  public String assemble(
      ...
      String jobId) {
    return assemble(
        wireframeJson, copyJson, imagePlanningJson, null, designPresetOutputJson, jobId);
  }

  /**
   * Consolida os artefatos da landing aplicando o manifesto de imagens finais antes de montar o
   * HTML provisório.
   */
  ...
  public String assemble(
    ...
    try {
      Map<String, Object> wireframePayload =
          normalizePayload(wireframeJson, "landingPageWireframe");
      Map<String, Object> copyPayload = normalizePayload(copyJson, "landingPageCopy");
      String enrichedImagePlanningJson =
          enrichImagePlanningWithAssets(imagePlanningJson, imageAssetsJson);
      String html =
          processor.process(
  ...
  private String buildErrorDetails(Exception ex) {
    ...
    String rootMessage =
        StringUtils.hasText(rootCause.getMessage()) ? rootCause.getMessage() : "<sem-mensagem>";
  ...
  /**
   * Normaliza o payload aceitando tanto raiz direta quanto raiz aninhada no nome canônico do
   * artefato.
   */
  @SuppressWarnings("unchecked")
  private Map<String, Object> normalizePayload(String sourceJson, String preferredRoot)
      throws Exception {
  ...
  /**
   * Enriquecer o planejamento de imagens com URLs definitivas vindas do manifesto consolidado do
   * experimento.
   */
  private String enrichImagePlanningWithAssets(String imagePlanningJson, String imageAssetsJson)
      throws Exception {
    ...
    for (JsonNode imageNode : planningImages) {
      ...
      String resolvedUrl =
          firstText(asset, "resolvedUrl", "webUrl", "sourceUrl", "imageUrl", "url", "src");
  ...
  /**
   * Localiza o objeto canônico do planejamento de imagens dentro dos formatos aceitos pelo
   * pipeline.
   */
  ...
  /**
   * Resolve o asset correspondente a uma imagem planejada por elementId, sectionId ou
   * planningItemKey.
   */
  private JsonNode resolveAssetForPlanningImage(
      ObjectNode imageObject, Map<String, JsonNode> assetsByKey) {
    String[] candidateFields = {
      "elementId", "sectionId", "planningItemKey", "imageBindingKey", "slotId"
    };
  ...
  /**
   * Retorna o HTML sem anexar metadados técnicos para preservar aderência ao contrato canônico do
   * artefato final.
   */
