...
public class BackendPublicLandingService {
  ...
  public PublicLandingPublicationResponse approveEndPublish(Long experimentId) {
    ...
    try {
      ...
      log.info(
          "GeraLanding public landing publish approval loaded experiment (experimentId={}, experimentName={})",
          experimentId,
          experiment.getName());
      ...
      publishToLeadPortal(
          slug, "Landing GeraLanding - Experimento " + experimentId, finalHtml.trim());
      log.info(
          "GeraLanding public landing publish approval sent flow to Lead Portal successfully (experimentId={}, slug={})",
          experimentId,
          slug);
      ...
      log.info(
          "GeraLanding public landing publish approval resolved publication URLs (experimentId={}, iframeUrl={}, standaloneUrl={})",
          experimentId,
          iframeUrl,
          standaloneUrl);
      ...
      log.info(
          "GeraLanding public landing publish approval saved follow-up URL (experimentId={}, followUpActionUrl={})",
          experimentId,
          standaloneUrl);
  ...
  /**
   * Método canônico de etapa reutilizado para publicar a landing pública pelo endpoint
   * approve-end-publish.
   */
  ...
  public Object listStageExecutions(Long experimentId) {
    throw new ResponseStatusException(
        HttpStatus.GONE, "Landing pública não possui stage-executions próprias");
  }

  /**
   * Retorna lista vazia porque a landing pública publica de forma síncrona e não possui pendências
   * internas.
   */
  ...
  public Object detailStageExecution(Long experimentId, String idJob) {
    throw new ResponseStatusException(
        HttpStatus.GONE, "Landing pública não possui detalhe de execução próprio");
  }

  /**
   * Seleciona o HTML canônico da landing, priorizando html_geralanding e usando landing_page_html
   * como fallback.
   */
  private String resolveLandingPageHtml(Experiment experiment, Long experimentId) {
    ...
    if (!StringUtils.hasText(landingPageHtml)) {
      log.warn(
          "GeraLanding public landing publish approval blocked because landing HTML is missing (experimentId={})",
          experimentId);
      throw new ResponseStatusException(
          HttpStatus.CONFLICT, "Landing HTML ainda não foi gerado para este experimento");
    }
    log.info(
        "GeraLanding public landing publish approval found landing HTML (experimentId={}, htmlLength={})",
        experimentId,
  ...
  /**
   * Monta o HTML final publicável com envio de lead, tracking, controles de funil e pixel do
   * Facebook.
   */
  private String buildFinalPublicHtml(
      Experiment experiment, Long experimentId, String slug, String landingPageHtml) {
    ...
    log.info(
        "GeraLanding public landing publish approval injected funnel controls (experimentId={}, htmlLengthBefore={}, htmlLengthAfter={})",
        experimentId,
        landingPageHtml.length(),
    ...
    log.info(
        "GeraLanding public landing publish approval resolved Facebook Pixel (experimentId={}, hasPixelId={})",
        experimentId,
    ...
    log.info(
        "GeraLanding public landing publish approval injected Facebook Pixel (experimentId={}, htmlLengthBefore={}, htmlLengthAfter={})",
        experimentId,
        htmlWithFunnelControls.length(),
  ...
  private void publishToLeadPortal(String slug, String name, String html) {
    if (!StringUtils.hasText(leadPortalBaseUrl)) {
      log.warn(
          "GeraLanding public landing Lead Portal publication blocked because base URL is not configured (slug={})",
          slug);
      throw new ResponseStatusException(
    ...
    URI uri =
        UriComponentsBuilder.fromHttpUrl(leadPortalBaseUrl)
            ...
            .toUri();
    log.info(
        "GeraLanding public landing Lead Portal publication request prepared (slug={}, uri={}, name={}, htmlLength={})",
        slug,
        uri,
        name,
    ...
    PublicLandingLeadPortalPublishRequest payload =
        new PublicLandingLeadPortalPublishRequest(
            slug, name, "Fluxo publicado pelo módulo GeraLanding", html);
    log.info(
        "GeraLanding public landing Lead Portal publication request parameters (slug={}, uri={}, headers={}, payload={})",
        slug,
        uri,
        headers,
        payload);
    try {
      ...
      log.info(
          "GeraLanding public landing Lead Portal publication request completed (slug={}, uri={})",
          slug,
          uri);
    } catch (RestClientException ex) {
      log.error(
          "GeraLanding public landing Lead Portal publication request failed (slug={}, uri={}, endpoint={}, errorClass={}, message={})",
          slug,
          uri,
          uri,
          ex.getClass().getName(),
          ...
          ex);
      throw new ResponseStatusException(
          HttpStatus.BAD_GATEWAY, "Falha ao publicar landing no Lead Portal", ex);
    }
  }
  ...
  /**
   * Injeta envio canônico do formulário quando a landing possui campos de lead sem contrato de
   * submissão.
   */
  private String injectLeadSubmissionScript(String html, String slug) {
    if (!StringUtils.hasText(html)
        || !StringUtils.hasText(slug)
        || hasLeadSubmissionContract(html)) {
      return html;
    }
  ...
  private boolean hasLeadCaptureControls(Document document) {
    ...
    boolean hasEmail =
        !document
            .select("#input-email, [name=email], input[type=email], [autocomplete=email]")
            .isEmpty();
  ...
  private String resolveFacebookPixelId(Experiment experiment) {
    ...
    try {
      ...
      log.warn(
          "GeraLanding public landing Facebook Pixel resolution failed (experimentId={})",
          experiment.getId(),
          ex);
      return null;
    }
  ...
  private String resolveStandaloneLandingUrl(String iframeUrl) {
    ...
    try {
      ...
      log.warn(
          "GeraLanding public landing standalone URL resolution failed (iframeUrl={})",
          iframeUrl,
          ex);
      return null;
    }
