...
public class HypothesisFrameworkMapperSupport {
  ...
  public void storeSnapshot(
      ...
      HypothesisFrameworkDto appliedPartial) {
  ...
  private void fillFallbacks(HypothesisFrameworkDto dto, Hypothesis hypothesis) {
    ...
    dto.getResult()
        .setDesiredResult(
            firstNonBlank(dto.getResult().getDesiredResult(), hypothesis.getPromise()));
    dto.getMechanism()
        .setCore(firstNonBlank(dto.getMechanism().getCore(), hypothesis.getMechanism()));
    dto.getMechanism()
        .setUnique(firstNonBlank(dto.getMechanism().getUnique(), hypothesis.getUniqueMechanism()));
    ...
    dto.getOffer()
        .setCorePromise(firstNonBlank(dto.getOffer().getCorePromise(), hypothesis.getPromise()));
  ...
  private HypothesisFrameworkDto.Pain mergePain(
      HypothesisFrameworkDto.Pain base, HypothesisFrameworkDto.Pain partial) {
  ...
  private HypothesisFrameworkDto.Result mergeResult(
      HypothesisFrameworkDto.Result base, HypothesisFrameworkDto.Result partial) {
    if (partial == null) return base;
  ...
  private HypothesisFrameworkDto.Mechanism mergeMechanism(
      HypothesisFrameworkDto.Mechanism base, HypothesisFrameworkDto.Mechanism partial) {
    if (partial == null) return base;
  ...
  private HypothesisFrameworkDto.Proof mergeProof(
      HypothesisFrameworkDto.Proof base, HypothesisFrameworkDto.Proof partial) {
    if (partial == null) return base;
  ...
  private HypothesisFrameworkDto.Offer mergeOffer(
      HypothesisFrameworkDto.Offer base, HypothesisFrameworkDto.Offer partial) {
    if (partial == null) return base;
  ...
  private HypothesisFrameworkDto.Checklist mergeChecklist(
      HypothesisFrameworkDto.Checklist base, HypothesisFrameworkDto.Checklist partial) {
    if (partial == null) return base;
    ...
    base.setApprovedForExperiment(
        resolveBoolean(partial.getApprovedForExperiment(), base.getApprovedForExperiment()));
  ...
  private void syncLegacyFields(
      Hypothesis hypothesis, HypothesisFrameworkDto dto, HypothesisFrameworkDto appliedPartial) {
    ...
    if (shouldUpdateResult(appliedPartial)) {
      hypothesis.setPromise(
          firstNonBlank(dto.getResult().getDesiredResult(), hypothesis.getPromise()));
    }
    if (shouldUpdateMechanism(appliedPartial)) {
      hypothesis.setMechanism(
          firstNonBlank(dto.getMechanism().getCore(), hypothesis.getMechanism()));
      hypothesis.setUniqueMechanism(
          firstNonBlank(dto.getMechanism().getUnique(), hypothesis.getUniqueMechanism()));
    }
    ...
    if (shouldUpdateOffer(appliedPartial)) {
      ...
      if (StringUtils.hasText(dto.getOffer().getOfferType())) {
        try {
          hypothesis.setOfferType(
              OfferType.valueOf(dto.getOffer().getOfferType().trim().toUpperCase()));
        } catch (IllegalArgumentException ex) {
          log.warn(
              "Unknown offer type '{}' for hypothesis {}",
              dto.getOffer().getOfferType(),
              hypothesis.getId());
        }
      }
    }
  ...
  private boolean hasPainContent(HypothesisFrameworkDto.Pain pain) {
    return pain != null
        && (hasText(pain.getSurface())
            || hasText(pain.getRoot())
            || hasText(pain.getEmotional())
            || hasText(pain.getSocial())
            || hasText(pain.getCost())
            || hasText(pain.getSummary()));
  }

  private boolean hasResultContent(HypothesisFrameworkDto.Result result) {
    return result != null
        && (hasText(result.getDesiredResult())
            || hasText(result.getDesiredIdentity())
            || hasText(result.getBusinessOutcome())
            || hasText(result.getSuccessSignal())
            || hasText(result.getSummary()));
  }

  private boolean hasMechanismContent(HypothesisFrameworkDto.Mechanism mechanism) {
    return mechanism != null
        && (hasText(mechanism.getCore())
            || hasText(mechanism.getUnique())
            || hasText(mechanism.getVisible())
            || hasText(mechanism.getBelievability())
            || hasText(mechanism.getSummary()));
  }

  private boolean hasProofContent(HypothesisFrameworkDto.Proof proof) {
    return proof != null
        && (hasText(proof.getType())
            || hasText(proof.getAsset())
            || hasText(proof.getMessage())
            || hasText(proof.getDeliveryStage())
            || hasText(proof.getSummary()));
  }

  private boolean hasOfferContent(HypothesisFrameworkDto.Offer offer) {
    return offer != null
        && (hasText(offer.getName())
            || hasText(offer.getCorePromise())
            || hasText(offer.getDeliverables())
            || hasText(offer.getRiskReversal())
            || hasText(offer.getPriceLogic())
            || hasText(offer.getCta())
            || offer.getPriceAmount() != null
            || hasText(offer.getOfferType())
  ...
  private HypothesisFrameworkDto clone(HypothesisFrameworkDto dto) {
    ...
    try {
      HypothesisFrameworkDto cloned =
          mapper.readValue(mapper.writeValueAsString(dto), HypothesisFrameworkDto.class);
