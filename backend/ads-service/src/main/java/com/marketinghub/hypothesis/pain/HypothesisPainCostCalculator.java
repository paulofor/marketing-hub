...
public class HypothesisPainCostCalculator {
  ...
  public BigDecimal calculateFlexCostUsd(
      String openAiModelCode, Integer inputTokens, Integer outputTokens) {
    ...
    OpenAiModel model =
        openAiModelRepository
            .findByCode(openAiModelCode.trim())
            .orElseThrow(
                () ->
                    new EntityNotFoundException(
                        "Modelo OpenAI não encontrado para cálculo de custo da etapa Dor: "
                            + openAiModelCode));
  ...
  private BigDecimal calculateTokenCost(Integer tokens, BigDecimal pricePerMillionTokens) {
    BigDecimal normalizedTokens =
        tokens != null ? BigDecimal.valueOf(tokens.longValue()) : BigDecimal.ZERO;
    BigDecimal normalizedPrice =
        pricePerMillionTokens != null ? pricePerMillionTokens : BigDecimal.ZERO;
    return normalizedTokens
        .multiply(normalizedPrice)
