...
public class HypothesisKanbanFacade {
  ...
  public Map<HypothesisStatus, List<HypothesisDto>> board(Long marketNicheId) {
    Iterable<com.marketinghub.hypothesis.Hypothesis> it =
        repository.findByMarketNicheId(marketNicheId);
    return StreamSupport.stream(it.spliterator(), false)
        .map(mapper::toDto)
        .collect(
            Collectors.groupingBy(
                HypothesisDto::getStatus,
                () -> new EnumMap<>(HypothesisStatus.class),
                Collectors.toList()));
  }
}
