...
/**
 * Responsabilidade: executar a etapa de fechamento que transforma o framework concluído em hipótese
 * de backlog.
 */
@Service
public class HypothesisPipelineFinalizationService {
  ...
  public Hypothesis finalizeHypothesis(Long marketNicheId, FinalizeHypothesisRequest request) {
    ...
    frameworkMapperSupport.storeSnapshot(
        hypothesis, finalizedFramework(title, pain, result, mechanism, proof, offer), null);
  ...
  private String requireCompletedStageResponse(
      Long marketNicheId, String stageCode, String stageLabel) {
  ...
  private String requireCompletedStageText(
      Long marketNicheId, String stageCode, String stageLabel) {
    ...
    try {
      ...
      throw new IllegalStateException(
          "A etapa "
              + stageLabel
              + " possui resposta inválida para fechar a hipótese: "
              + ex.getMessage(),
          ex);
    }
  }
  ...
  /**
   * Vincula as execuções concluídas usadas no fechamento à hipótese gerada para auditoria
   * posterior.
   */
  ...
  /**
   * Monta o snapshot canônico Dor → Resultado → Mecanismo → Prova → Oferta aprovado para
   * experimento.
   */
  private HypothesisFrameworkDto finalizedFramework(
      String title, String pain, String result, String mechanism, String proof, String offer) {
