...
import com.marketinghub.repository.jpa.imagedeliverable.ImageDeliverablePackageRepository;
...
public class ImageDeliverablePackageService {
  ...
  public ImageDeliverablePackageService(
      ImageDeliverablePackageRepository repository,
        ...
        .orElseThrow(
            () ->
                new ResponseStatusException(
                    HttpStatus.NOT_FOUND, "Image deliverable package not found: " + id));
  }
  ...
  public ImageDeliverablePackage create(CreateImageDeliverablePackageRequest request) {
    ...
    ImageDeliverableStatus status =
        Optional.ofNullable(request.getStatus()).orElse(ImageDeliverableStatus.RECEIVED);
    ...
    ImageDeliverablePackage pack =
        ImageDeliverablePackage.builder()
            .lead(lead)
  ...
  public ImageDeliverablePackage update(Long id, UpdateImageDeliverablePackageRequest request) {
    ...
    if (request.getLeadId() != null
        && !Objects.equals(pack.getLead().getId(), request.getLeadId())) {
  ...
  private Lead resolveLead(UUID leadId) {
    ...
    return leadRepository
        .findById(leadId)
        .orElseThrow(
            () -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Lead not found: " + leadId));
  }
  ...
  private Asset resolveAsset(Long assetId) {
    ...
    return assetRepository
        .findById(assetId)
        .orElseThrow(
            () -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Asset not found: " + assetId));
  }
  ...
  private Integer normalizePlannedOutputs(Integer plannedOutputs) {
    ...
    if (plannedOutputs <= 0) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "plannedOutputs must be greater than zero");
    }
  ...
  private List<ImageDeliverableItem> buildItems(
      List<ImageDeliverableItemRequest> itemRequests, ImageDeliverablePackage pack) {
    ...
    for (int i = 0; i < itemRequests.size(); i++) {
      ...
      if (item.getAssetId() == null) {
        throw new ResponseStatusException(
            HttpStatus.BAD_REQUEST, "assetId is required for each item");
      }
      order.put(item.getAssetId(), i);
      requestedTypes.putIfAbsent(
          item.getAssetId(),
