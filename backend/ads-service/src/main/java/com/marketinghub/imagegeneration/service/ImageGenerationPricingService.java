...
import com.marketinghub.repository.jpa.imagegeneration.ImageGenerationPriceRepository;
...
public class ImageGenerationPricingService {
  ...
  public Optional<ImageGenerationPrice> resolvePrice(Long qualityId, ImageOrientation orientation) {
    ...
    if (orientation != null) {
      directMatch =
          priceRepository.findFirstByQualityIdAndOrientationOrderByPreferredDescWidthDesc(
  ...
  public Optional<BigDecimal> estimateTotalCost(
    ...
    return resolvePrice(qualityId, orientation)
        .map(
            price ->
                price
                    .getUnitPriceUsd()
                    .multiply(BigDecimal.valueOf(imageCount))
