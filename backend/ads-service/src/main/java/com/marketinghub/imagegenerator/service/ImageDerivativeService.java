...
public class ImageDerivativeService {
    ...
    public List<ImageGeneratorVariant> createVariants(String originalFormat, String imageBase64) {
        try {
            ...
            if (original == null) {
                throw new IllegalArgumentException("Não foi possível ler a imagem original retornada pela IA.");
            }

            List<ImageGeneratorVariant> variants = new ArrayList<>();
            variants.add(new ImageGeneratorVariant(
                    "original",
                    originalFormat,
                    imageBase64,
                    original.getWidth(),
            ...
            log.error("Falha ao gerar derivados web da imagem. modulo=image-generator operacao=createVariants", ex);
            throw new UncheckedIOException("Não foi possível gerar versões leves da imagem.", ex);
        } catch (IllegalArgumentException ex) {
            log.error("Falha ao validar imagem base64 para derivados web. modulo=image-generator operacao=createVariants", ex);
            throw ex;
        }
    }

    /** Cria uma variante JPEG redimensionada para a largura máxima informada. */
    private ImageGeneratorVariant createJpegVariant(String role, BufferedImage original, int maxWidth, float quality)
            throws IOException {
    ...
    private BufferedImage resize(BufferedImage original, int maxWidth) {
        ...
        int targetHeight = Math.max(1, Math.round((float) original.getHeight() * maxWidth / original.getWidth()));
