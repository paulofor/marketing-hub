...
public class JourneyExecutionService {
  ...
  public JourneyExecutionService(
    ...
    this.handlerByType =
        handlers.stream().collect(Collectors.toMap(JourneyChannelHandler::supportedType, h -> h));
  }

  public void processDueAssignments() {
    Instant now = Instant.now();
    List<JourneyAssignment> candidates =
        assignmentRepository
            .findEligibleAssignments(
                List.of(JourneyAssignmentStatus.PENDING, JourneyAssignmentStatus.IN_PROGRESS),
                now,
  ...
  private void processSingle(Long assignmentId, Instant referenceTime) {
    ...
    if (handler == null) {
      ...
      handlePermanentFailure(
          ...
          ChannelDispatchResult.permanentFailure(
              "No handler for stimulus type",
              Map.of("stimulusType", step.getStimulusType().name())));
      return;
    }
    ...
    try {
      ...
      dispatchResult =
          ChannelDispatchResult.transientFailure(
              "Unexpected error: " + ex.getMessage(), null, Map.of());
    }
  ...
  private void skipStep(
    ...
    assignment.setStatus(
        next == null ? JourneyAssignmentStatus.COMPLETED : JourneyAssignmentStatus.IN_PROGRESS);
    assignmentRepository.save(assignment);
  ...
  private void handleFrequencyCap(
      JourneyAssignment assignment, JourneyStep step, Instant now, FrequencyCapResult capResult) {
    assignment.setNextAttemptAt(capResult.nextAttemptAt());
    assignment.setRetryCount(0);
    assignment.setStatus(JourneyAssignmentStatus.IN_PROGRESS);
    assignmentRepository.save(assignment);
    recordEvent(
        assignment, step, now, JourneyEventType.STIMULUS_FREQUENCY_CAPPED, capResult.metadata());
  }
  ...
  private void handleTransientFailure(
      JourneyAssignment assignment, JourneyStep step, Instant now, ChannelDispatchResult result) {
  ...
  private JourneyStep resolveNextStep(JourneyStep current) {
    return stepRepository
        .findFirstByTemplateAndPositionGreaterThanOrderByPositionAsc(
            current.getTemplate(), current.getPosition())
        .orElse(null);
  }
