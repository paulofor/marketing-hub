...
public class MetaAdsChannelHandler implements JourneyChannelHandler {
  ...
  @Autowired
  public MetaAdsChannelHandler(
      RestTemplateBuilder restTemplateBuilder, MetaMarketingProperties properties) {
  ...
  public ChannelDispatchResult dispatch(
      JourneyAssignment assignment, JourneyStep step, Map<String, Object> context) {
    if (!properties.isEnabled()) {
      return ChannelDispatchResult.permanentFailure(
          "Meta Marketing API integration disabled", Map.of());
    }
    if (properties.getAccessToken() == null || properties.getAdAccountId() == null) {
      return ChannelDispatchResult.permanentFailure(
          "Meta access token or ad account not configured", Map.of());
    }
    ...
    try {
      ...
      ResponseEntity<MetaAdResponse> response =
          restTemplate.exchange(url, HttpMethod.POST, request, MetaAdResponse.class);
      ...
      if (response.getStatusCode().is5xxServerError()
        ...
        return ChannelDispatchResult.transientFailure(
            "Meta API transient response: " + response.getStatusCode(), retryAt, Map.of());
      }
      return ChannelDispatchResult.permanentFailure(
          "Meta API returned status " + response.getStatusCode(), Map.of());
