...
public class WhatsAppChannelHandler implements JourneyChannelHandler {
  ...
  private WhatsAppMessage dispatchMessage(
    ...
    if (step.getMetadata().containsKey("templateName")) {
      ...
      String templateLanguage =
          String.valueOf(step.getMetadata().getOrDefault("templateLanguage", "en_US"));
      Map<String, Object> templateData = extractTemplateData(step.getMetadata());
      return messagingService.sendTemplateMessage(
          account, to, templateName, templateLanguage, templateData, metadata);
    }
  ...
  private Map<String, Object> extractTemplateData(Map<String, String> metadata) {
    ...
    if (components instanceof List<?> list && !list.isEmpty()) {
      ...
      throw new IllegalArgumentException(
          "WhatsApp templateComponents must be a JSON array or object");
    }
