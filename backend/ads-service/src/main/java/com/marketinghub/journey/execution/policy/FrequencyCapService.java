...
public class FrequencyCapService {
  ...
  public FrequencyCapService(
      EventLogRepository eventLogRepository, JourneyExecutionProperties properties) {
  ...
  public FrequencyCapResult evaluate(JourneyAssignment assignment, Instant now) {
    ...
    if (cap.getPerDay() > 0) {
      ...
      long dailyCount =
          eventLogRepository.countByActorIdAndEventTypeAndOccurredAtAfter(
              actorId, eventType, dayStart);
  ...
  private FrequencyCapResult block(
      UUID actorId,
      String window,
      int limit,
      long current,
