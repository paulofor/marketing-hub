...
public class JourneyAssignmentService {
  ...
  public JourneyAssignmentService(
  ...
  public Page<JourneyAssignment> list(
      Long journeyId, JourneyAssignmentStatus status, Pageable pageable) {
  ...
  public List<JourneyAssignment> assign(Long journeyId, JourneyAssignmentRequest request) {
    ...
    JourneyAssignmentStatus status =
        request.status() != null ? request.status() : JourneyAssignmentStatus.PENDING;
    ...
    if (request.leadIds() != null && !request.leadIds().isEmpty()) {
      assignments.addAll(
          createLeadAssignments(
              journey, request.leadIds(), status, currentStep, nextStep, contextPayload));
    }
    ...
    if (assignments.isEmpty()) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "At least one leadId or segmentIdentifier must be provided");
    }
            ...
            .orElseThrow(
                () ->
                    new ResponseStatusException(
                        HttpStatus.NOT_FOUND, "Journey assignment not found"));
    assignmentRepository.delete(assignment);
  }

  private List<JourneyAssignment> createLeadAssignments(
      Journey journey,
      List<UUID> leadIds,
      JourneyAssignmentStatus status,
