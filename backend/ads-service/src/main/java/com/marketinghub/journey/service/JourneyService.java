...
public class JourneyService {
  ...
  public JourneyMetricsResponse metrics() {
    ...
    Map<JourneyStatus, Long> statusBreakdown =
        Arrays.stream(JourneyStatus.values())
            .collect(
                Collectors.toMap(
                    status -> status,
  ...
  public Journey update(Long id, JourneyUpdateRequest request) {
    ...
    if (request.templateId() != null
        && !Objects.equals(request.templateId(), journey.getTemplate().getId())) {
      JourneyTemplate template =
          templateRepository
              .findById(request.templateId())
              .orElseThrow(
  ...
  private MarketNiche resolveNiche(Long nicheId) {
    ...
    return marketNicheRepository
        .findById(nicheId)
        .orElseThrow(
            () -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Market niche not found"));
  }
