...
public class JourneyStepService {
            ...
            .orElseThrow(
                () ->
                    new ResponseStatusException(
                        HttpStatus.NOT_FOUND, "Journey template not found"));
  ...
  public JourneyStep update(Long stepId, JourneyStepUpdateRequest request) {
    ...
    boolean positionUpdated =
        request.position() != null && !Objects.equals(request.position(), step.getPosition());
    if (positionUpdated) {
  ...
  private void reorderSteps(
      JourneyTemplate template, JourneyStep targetStep, Integer desiredPosition) {
    List<JourneyStep> steps = stepRepository.findByTemplateOrderByPositionAsc(template);
    steps.removeIf(
        existing -> existing.getId() != null && existing.getId().equals(targetStep.getId()));
  ...
  private VisualProof resolveVisualProof(Long visualProofId) {
    ...
    return visualProofRepository
        .findById(visualProofId)
        .orElseThrow(
            () -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Visual proof not found"));
  }
  ...
  private EmotionalTrigger resolveEmotionalTrigger(Long emotionalTriggerId) {
    ...
    return emotionalTriggerRepository
        .findById(emotionalTriggerId)
        .orElseThrow(
            () -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Emotional trigger not found"));
  }
}
