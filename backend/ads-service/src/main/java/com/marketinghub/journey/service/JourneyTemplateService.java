...
public class JourneyTemplateService {
  private static final List<JourneyPhase> DEFAULT_PHASES =
      List.of(
          JourneyPhase.ATTENTION, JourneyPhase.INTEREST, JourneyPhase.DESIRE, JourneyPhase.ACTION);
  ...
  public Page<JourneyTemplate> list(Pageable pageable) {
    ...
    List<JourneyTemplate> templates =
        templateRepository.findAllWithStepsByIdIn(pageIds.getContent());
    ...
    List<JourneyTemplate> ordered =
        pageIds.getContent().stream().map(byId::get).filter(Objects::nonNull).toList();
  ...
  public JourneyTemplate get(Long id) {
    return templateRepository
        ...
        .orElseThrow(
            () -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Journey template not found"));
  }
