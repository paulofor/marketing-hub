...
public class MdsAdminService {
  ...
  public MdsAdminRequestListResponse listRequests(
      String status, Instant from, Instant to, String tenantOrProduct, int page, int size) {
    ...
    if (tenantOrProduct != null && !tenantOrProduct.isBlank()) {
      ...
      spec =
          spec.and((root, query, cb) -> cb.like(cb.lower(root.get("correlationId")), searchTerm));
    }
  ...
  public MdsAdminRequestDetailResponse getRequestDetail(Long id) {
    ...
    List<MdsAdminProcessingEventResponse> timeline =
        processingEventRepository.findByRequestIdOrderByCreatedAtAscIdAsc(id).stream()
  ...
  public MdsAdminArtifactsResponse listArtifactsWithLineage(Long requestId) {
    ...
    List<MdsArtifactLineageEdge> edges =
        lineageEdgeRepository.findByParentArtifact_Request_IdOrChildArtifact_Request_IdOrderByIdAsc(
            requestId, requestId);

    Map<Long, List<Long>> parentByChild =
        edges.stream()
            .collect(
    ...
    List<MdsAdminArtifactItemResponse> artifacts =
        artifactRecordRepository.findByRequestIdOrderByCreatedAtAscIdAsc(requestId).stream()
            .map(
                record ->
    ...
    List<MdsAdminArtifactLineageEdgeResponse> lineage =
        edges.stream()
            .map(
                edge ->
                    new MdsAdminArtifactLineageEdgeResponse(
                        edge.getId(),
  ...
  public MdsAdminRetryResponse retryRequest(Long id) {
    ...
    MdsProcessingEvent retryEvent =
        MdsProcessingEvent.builder()
            .request(request)
  ...
  private MdsAdminRequestListItemResponse toListItem(MdsRequest request) {
    MdsProcessingEvent latestEvent =
        processingEventRepository
            .findTopByRequestIdOrderByCreatedAtDescIdDesc(request.getId())
            .orElse(null);
    MdsProcessingEvent heartbeatEvent =
        processingEventRepository
            .findTopByRequestIdAndEventTypeOrderByCreatedAtDescIdDesc(
                request.getId(), MdsEventType.HEARTBEAT)
            .orElse(null);

    long attempts =
        processingEventRepository.countByRequestIdAndEventType(request.getId(), MdsEventType.INFO);
    int normalizedAttempts =
        (int) Math.max(attempts, request.getStatus() == MdsRequestStatus.PENDING ? 0 : 1);
