...
public class MdsArtifactService {
  ...
  public MdsArtifactService(
      MdsRequestRepository requestRepository,
  ...
  public MdsArtifactPublishBatchResponse publishBatch(MdsArtifactPublishBatchRequest request) {
    MdsRequest mdsRequest =
        requestRepository
            .findById(request.requestId())
            .orElseThrow(
                () -> new ResponseStatusException(HttpStatus.NOT_FOUND, "mds request not found"));
    ...
    for (MdsArtifactPublishBatchRequest.MdsArtifactPayload artifactPayload : request.artifacts()) {
      ...
      MdsArtifactRecord record =
          MdsArtifactRecord.builder()
  ...
  public MdsLineageResponse createLineage(MdsLineageCreateRequest request) {
    return createLineage(
        request.parentArtifactId(), request.childArtifactId(), request.relationType());
  }
  ...
  public MdsRecommendedMechanismResponse getRecommendedMechanismByRequest(Long requestId) {
    MdsArtifactRecord record =
        artifactRecordRepository
            .findFirstByRequestIdAndArtifactTypeOrderByCreatedAtDescIdDesc(
                requestId, "mechanismSpec")
            .orElseThrow(
                () ->
                    new ResponseStatusException(
                        HttpStatus.NOT_FOUND, "recommended mechanism not found"));
  ...
  public MdsReportResponse getReportByRequest(Long requestId) {
    MdsArtifactRecord record =
        artifactRecordRepository
            .findFirstByRequestIdAndArtifactTypeOrderByCreatedAtDescIdDesc(
                requestId, "mechanismDiscoveryReport")
            .orElseThrow(
                () ->
                    new ResponseStatusException(
                        HttpStatus.NOT_FOUND, "mechanism discovery report not found"));
  ...
  public List<MdsArtifactSummaryResponse> listArtifactsByRequest(Long requestId) {
    ...
    return artifactRecordRepository.findByRequestIdOrderByCreatedAtAscIdAsc(requestId).stream()
        .map(
            record ->
                new MdsArtifactSummaryResponse(
                    record.getId(),
                    record.getArtifactType(),
                    record.getSchemaVersion(),
                    record.getVersion(),
  ...
  private MdsLineageResponse createLineage(
      Long parentArtifactId, Long childArtifactId, String relationType) {
    MdsArtifactRecord parent =
        artifactRecordRepository
            .findById(parentArtifactId)
            .orElseThrow(
                () ->
                    new ResponseStatusException(HttpStatus.NOT_FOUND, "parent artifact not found"));
    MdsArtifactRecord child =
        artifactRecordRepository
            .findById(childArtifactId)
            .orElseThrow(
                () ->
                    new ResponseStatusException(HttpStatus.NOT_FOUND, "child artifact not found"));

    MdsArtifactLineageEdge edge =
        MdsArtifactLineageEdge.builder()
  ...
  private String toJson(Object payload) {
    try {
      ...
      throw new ResponseStatusException(
          HttpStatus.UNPROCESSABLE_ENTITY, "invalid artifact content json");
    }
  }
  ...
  private Map<String, Object> readContent(String contentJson) {
    try {
      return objectMapper.readValue(
          contentJson, new com.fasterxml.jackson.core.type.TypeReference<>() {});
    } catch (JsonProcessingException e) {
      throw new ResponseStatusException(
          HttpStatus.UNPROCESSABLE_ENTITY, "invalid artifact content json");
    }
