...
public class MdsSourceAccessService {
  ...
  @Transactional
  public MdsSourceAccessPublishBatchResponse publishBatch(
      MdsSourceAccessPublishBatchRequest request) {
    List<Long> ids = new ArrayList<>();

    for (MdsSourceAccessPublishBatchRequest.SourceAccessPayload payload : request.records()) {
      MdsSourceAccessRecord record =
          MdsSourceAccessRecord.builder()
