...
public class BackendMetaAudienceService {
  ...
  /**
   * Persiste uma audiência recebida do OPRM sem calcular elegibilidade, nome, recorte ou volume de
   * negócio.
   */
  @Transactional
  public MetaAudienceResponse requestAudience(MetaAudienceRequest request) {
    ...
    MarketNiche niche =
        nicheRepository
            .findById(request.marketNicheId())
            .orElseThrow(
  ...
  /**
   * Lista registros em READY e apenas anexa os emails brutos persistidos para execução técnica do
   * worker.
   */
  ...
  public List<MetaAudiencePendingResponse> listPending(int limit) {
    ...
    return audienceRepository
        .findByEligibilityStatusOrderByUpdatedAtAsc("READY", PageRequest.of(0, safeLimit))
        .stream()
        .map(
            a ->
                new MetaAudiencePendingResponse(
                    a.getId(),
                    a.getMarketNiche().getId(),
                    a.getSourceCnaeCode(),
                    a.getAudienceName(),
                    a.getFacebookAdAccountId(),
                    loadEmails(a.getSourceCnaeCode())))
        .toList();
  }
  ...
  public ExperimentMetaAudienceResponse linkExperiment(LinkMetaAudienceExperimentRequest request) {
    ...
    MetaAudience audience =
        audienceRepository
            .findById(request.metaAudienceId())
            .orElseThrow(
                () ->
                    new ResponseStatusException(HttpStatus.NOT_FOUND, "Audiência não encontrada."));
    ...
    Instant now = Instant.now();
    ExperimentMetaAudience link =
        experimentAudienceRepository
  ...
  public List<ExperimentMetaAudienceResponse> listByExperiment(Long experimentId) {
    ...
    return experimentAudienceRepository
        .findByExperimentIdOrderByUpdatedAtDesc(experimentId)
        .stream()
  ...
  /**
   * Valida somente campos necessários para persistência e relacionamento, sem regra de negócio de
   * público.
   */
  private void validateRequiredPersistenceFields(MetaAudienceRequest request) {
    if (request == null
        || request.marketNicheId() == null
        || !StringUtils.hasText(request.cnaeCode())
        || !StringUtils.hasText(request.audienceName())
        || !StringUtils.hasText(request.segmentName())
        || !StringUtils.hasText(request.facebookAdAccountId())
        || !StringUtils.hasText(request.audienceType())
        || !StringUtils.hasText(request.sourceType())
        || !StringUtils.hasText(request.eligibilityStatus())) {
  ...
  private void validateExperimentLinkRequest(LinkMetaAudienceExperimentRequest request) {
    if (request == null
        || request.metaAudienceId() == null
        || request.metaAudienceSegmentId() == null
  ...
  private MetaAudienceSegment loadSegment(Long segmentId, Long audienceId) {
    MetaAudienceSegment segment =
        segmentRepository
            .findById(segmentId)
            .orElseThrow(
                () ->
                    new ResponseStatusException(
                        HttpStatus.NOT_FOUND, "Parcela da audiência não encontrada."));
    if (!segment.getMetaAudience().getId().equals(audienceId)) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "Parcela funcional não pertence à audiência informada.");
    }
    return segment;
  }
  ...
  /**
   * Lê os emails persistidos pelo backend para entrega técnica ao worker, sem calcular seleção de
   * público.
   */
  ...
  private MetaAudienceResponse toResponse(MetaAudience audience) {
    return new MetaAudienceResponse(
        audience.getId(),
        audience.getMarketNiche().getId(),
        audience.getAudienceName(),
        audience.getEligibilityStatus(),
        audience.getTotalContacts(),
        audience.getUniqueEmails());
  }
