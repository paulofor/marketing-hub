...
import com.marketinghub.repository.jpa.microservice.exception.MicroserviceExceptionRepository;
...
public class MicroserviceExceptionService {
  ...
  public MicroserviceExceptionService(
      MicroserviceExceptionRepository repository,
  ...
  @Transactional
  public MicroserviceExceptionLog logException(
      Long microserviceId, MicroserviceExceptionRequest request) {
    Microservice microservice =
        microserviceRepository
            .findById(microserviceId)
            .orElseThrow(
                () ->
                    new ResponseStatusException(
                        HttpStatus.NOT_FOUND, "Microserviço não encontrado"));
    ...
    String exceptionType =
        StringUtils.hasText(request.exceptionType()) ? request.exceptionType() : "Exception";
    String context = serializeContext(request.context());

    MicroserviceExceptionLog log =
        MicroserviceExceptionLog.builder()
            .microservice(microservice)
  ...
  public Page<MicroserviceExceptionLog> list(
    ...
    if (microserviceId != null && hasSeverity) {
      return repository.findAllByMicroserviceIdAndSeverityIgnoreCase(
          microserviceId, severity, pageable);
    }
  ...
  @Transactional(readOnly = true)
  public Map<Long, MicroserviceExceptionSummary> summarizeByMicroservices(
      List<Microservice> microservices) {
    if (microservices == null || microservices.isEmpty()) {
      return Collections.emptyMap();
    }
    List<Long> ids =
        microservices.stream().map(Microservice::getId).filter(Objects::nonNull).toList();

    Map<Long, MicroserviceExceptionLog> latestByMicroservice =
        repository.findLatestByMicroserviceIds(ids).stream()
            .collect(
                Collectors.toMap(log -> log.getMicroservice().getId(), log -> log, (a, b) -> a));

    Map<Long, Long> countsByMicroservice =
        repository.countByMicroserviceIds(ids).stream()
            .collect(
                Collectors.toMap(
                    MicroserviceExceptionRepository.MicroserviceExceptionCountView
                        ::getMicroserviceId,
    ...
    for (Long id : ids) {
      ...
      summaries.put(
          id,
          MicroserviceExceptionSummary.builder()
