...
public class MicroserviceExceptionController {
  ...
  public MicroserviceExceptionController(
      MicroserviceExceptionService service, MicroserviceExceptionMapper mapper) {
    this.service = service;
    this.mapper = mapper;
  }

  @PostMapping("/microservices/{microserviceId}/exceptions")
  public MicroserviceExceptionDto logException(
      @PathVariable Long microserviceId, @Valid @RequestBody MicroserviceExceptionRequest request) {
    return mapper.toDto(service.logException(microserviceId, request));
  }

  @GetMapping("/microservices/{microserviceId}/exceptions")
  public Page<MicroserviceExceptionDto> listForMicroservice(
  ...
  public Page<MicroserviceExceptionDto> listAll(
      @RequestParam(value = "microserviceId", required = false) Long microserviceId,
      @RequestParam(value = "severity", required = false) String severity,
      @PageableDefault(size = 20, sort = "occurredAt", direction = Sort.Direction.DESC)
          Pageable pageable) {
    return service.list(microserviceId, severity, pageable).map(mapper::toDto);
