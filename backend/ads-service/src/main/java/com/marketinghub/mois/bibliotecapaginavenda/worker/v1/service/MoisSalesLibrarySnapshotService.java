...
/**
 * Gerencia capturas HTML das páginas de venda canônicas da biblioteca MOIS usando o modelo
...
public class MoisSalesLibrarySnapshotService {
  ...
  /**
   * Captura páginas diretamente pelo backend para acionamentos manuais sobre o modelo operacional
   * novo.
   */
  public MoisSalesLibraryDtos.SalesLibrarySnapshotCaptureResponse captureSnapshots(
    ...
    List<MoisSalesLibraryDtos.SalesLibrarySnapshotCaptureItem> items =
        pages.stream().map(this::captureOneSafely).toList();
    int captured =
        (int)
            items.stream()
                .filter(
                    item -> "CAPTURED".equals(item.status()) || "DUPLICATE".equals(item.status()))
                .count();
  ...
  public List<MoisSalesLibraryDtos.SalesLibraryPageSnapshotResponse> listSnapshots(long pageId) {
    return jdbcTemplate.query(
        """
                ...
                """,
        (rs, rowNum) ->
            new MoisSalesLibraryDtos.SalesLibraryPageSnapshotResponse(
                rs.getLong("id"),
  ...
  public MoisSalesLibraryDtos.HtmlCaptureClaimResponse claimHtmlCapture(
    ...
    log.info(
        "MOIS sales-library captura reservada no modelo operacional novo. modulo=MOIS, operacao=claimHtmlCapture, workspaceId={}, pageId={}, executionId={}",
        request.workspaceId(),
        page.pageId(),
        executionId);
    return new MoisSalesLibraryDtos.HtmlCaptureClaimResponse(
        true,
        new MoisSalesLibraryDtos.HtmlCaptureJobResponse(
            executionId, page.pageId(), page.urlCanonical(), page.title()));
  }
  ...
  public MoisSalesLibraryDtos.HtmlCapturePersistResponse completeHtmlCapture(
      long executionId, MoisSalesLibraryDtos.HtmlCaptureCompleteRequest request) {
    ...
    String hash =
        request.sha256() == null || request.sha256().isBlank() ? sha256(rawHtml) : request.sha256();
    Instant capturedAt = request.capturedAt() == null ? Instant.now() : request.capturedAt();
    Long duplicateExecutionId =
        findDuplicateCaptureExecution(executionId, execution.pageId(), hash);
    if (duplicateExecutionId != null) {
      ...
      jdbcTemplate.update(
          ...
          truncate(resolveFinalUrl(request), 1024),
          truncate(request.redirectRootUrl(), 1024),
          hash,
          rawHtmlBytes.length,
          duplicateMessage,
          Timestamp.from(capturedAt),
          executionId);
      updatePageForCaptureCompletion(
          execution.pageId(),
          executionId,
          "DUPLICATE",
          request,
    ...
    updatePageForCaptureCompletion(
        ...
        capturedAt);
    log.info(
        "MOIS sales-library captura concluída no modelo operacional novo. modulo=MOIS, operacao=completeHtmlCapture, pageId={}, executionId={}, status=CAPTURED, bytes={}",
        execution.pageId(),
        executionId,
        rawHtmlBytes.length);
  ...
  public MoisSalesLibraryDtos.HtmlCapturePersistResponse failHtmlCapture(
      long executionId, MoisSalesLibraryDtos.HtmlCaptureFailRequest request) {
    CaptureExecution execution = findCaptureExecution(executionId);
    String message =
    ...
    jdbcTemplate.update(
        """
                ...
                """,
        request.httpStatus(),
        truncate(request.redirectDestinationUrl(), 1024),
        truncate(request.redirectRootUrl(), 1024),
    ...
    jdbcTemplate.update(
        ...
        execution.pageId());
    log.info(
        "MOIS sales-library captura falhou no modelo operacional novo. modulo=MOIS, operacao=failHtmlCapture, pageId={}, executionId={}, errorCategory={}",
        execution.pageId(),
        executionId,
        request.errorCategory());
  ...
  /**
   * Seleciona páginas sem HTML útil, usando html_bytes = 0 como critério para processamento da
   * etapa 1.
   */
  ...
  private PageToCapture mapPageToCapture(java.sql.ResultSet rs, int rowNum)
      throws java.sql.SQLException {
    return new PageToCapture(
        rs.getLong("id"), rs.getString("url_canonical"), rs.getString("title"));
  }

  /** Executa uma captura isolada convertendo qualquer exceção em execução FAILED auditável. */
  private MoisSalesLibraryDtos.SalesLibrarySnapshotCaptureItem captureOneSafely(
      PageToCapture page) {
    ...
    try {
      ...
      log.warn(
          "Falha ao capturar HTML bruto da sales page MOIS. pageId={}, executionId={}, url={}, categoria={}",
          page.pageId(),
          executionId,
          page.urlCanonical(),
          errorMessage,
  ...
  /**
   * Busca a URL canônica, tenta fallback pela raiz do redirecionamento e persiste a execução quando
   * a captura é útil.
   */
  private MoisSalesLibraryDtos.SalesLibrarySnapshotCaptureItem captureOne(
      PageToCapture page, long executionId) throws Exception {
    ...
    if (!primary.isCapturable()
        && shouldTryRedirectRoot(page.urlCanonical(), redirectDestinationUrl, redirectRootUrl)) {
      log.info(
          "MOIS sales-library captura tentando raiz do redirecionamento. pageId={}, executionId={}, urlCanonical={}, redirectDestinationUrl={}, redirectRootUrl={}",
          page.pageId(),
          executionId,
          page.urlCanonical(),
    ...
    log.info(
        "MOIS sales-library captura payload bruto recebido. pageId={}, executionId={}, url={}, redirectDestinationUrl={}, redirectRootUrl={}, effectiveUrl={}, httpStatus={}, contentType={}, htmlPreview={}",
        page.pageId(),
        executionId,
        page.urlCanonical(),
        redirectDestinationUrl,
    ...
    jdbcTemplate.update(
        """
                ...
                WHERE id = ?
                """,
        truncate(redirectDestinationUrl, 1024),
        truncate(redirectRootUrl, 1024),
        effective.statusCode(),
        truncate(effective.contentType(), 255),
        rawHtml,
        hash,
        rawHtmlBytes.length,
        screenshot,
  ...
  private void failCaptureExecution(
      PageToCapture page,
      long executionId,
      String errorMessage,
      Integer httpStatus,
  ...
  private void markCaptureDuplicate(
      PageToCapture page,
      long executionId,
  ...
  private void updatePageForCaptureCompletion(
      long pageId,
      long executionId,
      String status,
    ...
    updatePageCaptureState(
        ...
        request.httpStatus(),
        request.contentType(),
        resolveFinalUrl(request),
        request.redirectRootUrl(),
        hash,
        rawHtmlBytes,
        null,
        errorMessage,
  ...
  private void updatePageCaptureState(
      long pageId,
      long executionId,
      String status,
      Integer httpStatus,
  ...
  private byte[] renderBasicScreenshot(String rawHtml, String baseUrl) {
    try {
      ...
      log.warn(
          "Falha ao renderizar screenshot básico da sales page MOIS. baseUrl={}, erroClasse={}, erro={}",
          baseUrl,
          ex.getClass().getName(),
          ex.getMessage(),
          ex);
  ...
  private boolean shouldTryRedirectRoot(
      String originalUrl, String redirectDestinationUrl, String redirectRootUrl) {
  ...
  private String rootUrl(String url) {
    ...
    try {
      ...
      log.warn(
          "Falha ao extrair raiz da URL redirecionada MOIS. url={}, erroClasse={}, erro={}",
          url,
          ex.getClass().getName(),
          ex.getMessage(),
          ex);
  ...
  /**
   * Classifica falhas HTTP para impedir que URLs indisponíveis sejam tratadas como pendência
   * normal.
   */
  ...
  private String sha256(String value) {
    try {
      ...
      log.warn(
          "Falha ao calcular hash de captura HTML MOIS. operacao=sha256, erroClasse={}, erro={}",
          ex.getClass().getName(),
          ex.getMessage(),
          ex);
  ...
  private record CaptureResponse(
      String rawHtml, String finalUrl, int statusCode, String contentType) {
