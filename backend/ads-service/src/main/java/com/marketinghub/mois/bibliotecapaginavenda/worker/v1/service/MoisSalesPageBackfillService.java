...
public class MoisSalesPageBackfillService implements ApplicationRunner {
  ...
  /**
   * Executa o backfill na inicialização para consolidar o estado legado nas duas novas tabelas
   * operacionais.
   */
  @Override
  @Transactional
  public void run(ApplicationArguments args) {
    if (!enabled) {
      log.info(
          "Backfill MOIS sales page ignorado por configuração. modulo=MOIS, operacao=salesPageBackfill, enabled={}",
          enabled);
      return;
    }
    try {
      if (!gateway.hasBackfillTables()) {
        log.info(
            "Backfill MOIS sales page ignorado porque o schema necessário ainda não existe. modulo=MOIS, operacao=salesPageBackfill");
        return;
      }
      ...
      log.error(
          "Falha no backfill MOIS sales page. modulo=MOIS, operacao=salesPageBackfill, enabled={}, erroClasse={}, erro={}",
          enabled,
          ex.getClass().getName(),
          ex.getMessage(),
          ex);
