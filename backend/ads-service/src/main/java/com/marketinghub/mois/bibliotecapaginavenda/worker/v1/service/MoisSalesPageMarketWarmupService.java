...
/**
 * Orquestra a solicitação, reserva e persistência da pesquisa de aquecimento de mercado da
 * Biblioteca MOIS.
 */
@Service
...
public class MoisSalesPageMarketWarmupService {
  ...
  private final MoisSalesPageMarketWarmupScoreEngine scoreEngine =
      new MoisSalesPageMarketWarmupScoreEngine();
  ...
  public MoisSalesLibraryDtos.MarketWarmupRequestResponse requestResearch(long pageId) {
    try {
      SalesPageWarmupData page =
          gateway
              .findSalesPage(pageId)
              .orElseThrow(
                  () ->
                      new IllegalArgumentException(
                          "Página MOIS não encontrada para aquecimento: " + pageId));
      validatePageAnalyzedForWarmup(page);
      MarketWarmupJobData job =
          gateway.findActiveJobByPage(pageId).orElseGet(() -> gateway.createPendingJob(page));
      log.info(
          "Pesquisa de aquecimento MOIS solicitada. modulo=MOIS, operacao=requestMarketWarmup, pageId={}, jobId={}, status={}",
          pageId,
          job.jobId(),
          mapJobStatus(job.status()));
      return new MoisSalesLibraryDtos.MarketWarmupRequestResponse(
          pageId, job.jobId(), mapJobStatus(job.status()), job.createdAt());
    } catch (RuntimeException ex) {
      log.error(
          "Falha ao solicitar pesquisa de aquecimento MOIS. modulo=MOIS, operacao=requestMarketWarmup, pageId={}, erroClasse={}, erro={}",
          pageId,
          ex.getClass().getName(),
          ex.getMessage(),
          ex);
  ...
  public MoisSalesLibraryDtos.MarketWarmupSummaryResponse getSummary(long pageId) {
    try {
      MarketWarmupSummaryData summary =
          gateway
              .findLatestSummaryByPage(pageId)
              .orElseThrow(
                  () ->
                      new IllegalArgumentException(
                          "Pesquisa de aquecimento não encontrada para página: " + pageId));
      return mapSummary(summary);
    } catch (RuntimeException ex) {
      log.error(
          "Falha ao consultar resumo de aquecimento MOIS. modulo=MOIS, operacao=getMarketWarmupSummary, pageId={}, erroClasse={}, erro={}",
          pageId,
          ex.getClass().getName(),
          ex.getMessage(),
          ex);
  ...
  public MoisSalesLibraryDtos.MarketWarmupSearchAttemptListResponse listSearchAttempts(
      long pageId) {
    try {
      MarketWarmupJobData job =
          gateway
              .findLatestJobByPage(pageId)
              .orElseThrow(
                  () ->
                      new IllegalArgumentException(
                          "Pesquisa de aquecimento não encontrada para página: " + pageId));
      return new MoisSalesLibraryDtos.MarketWarmupSearchAttemptListResponse(
          pageId,
          job.jobId(),
          gateway.listSearchAttempts(job.jobId()).stream().map(this::mapSearchAttempt).toList());
    } catch (RuntimeException ex) {
      log.error(
          "Falha ao listar tentativas de busca do aquecimento MOIS. modulo=MOIS, operacao=listMarketWarmupSearchAttempts, pageId={}, erroClasse={}, erro={}",
          pageId,
          ex.getClass().getName(),
          ex.getMessage(),
          ex);
          ...
          gateway
              ...
              .orElseThrow(
                  () ->
                      new IllegalArgumentException(
                          "Pesquisa de aquecimento não encontrada para página: " + pageId));
      return new MoisSalesLibraryDtos.MarketWarmupSourceListResponse(
          pageId,
          job.jobId(),
          gateway.listSources(job.jobId()).stream().map(this::mapSource).toList());
    } catch (RuntimeException ex) {
      log.error(
          "Falha ao listar fontes de aquecimento MOIS. modulo=MOIS, operacao=listMarketWarmupSources, pageId={}, erroClasse={}, erro={}",
          pageId,
          ex.getClass().getName(),
          ex.getMessage(),
          ex);
          ...
          gateway
              ...
              .orElseThrow(
                  () ->
                      new IllegalArgumentException(
                          "Pesquisa de aquecimento não encontrada para página: " + pageId));
      return new MoisSalesLibraryDtos.MarketWarmupSignalListResponse(
          pageId,
          job.jobId(),
          gateway.listSignals(job.jobId()).stream().map(this::mapSignal).toList());
    } catch (RuntimeException ex) {
      log.error(
          "Falha ao listar sinais de aquecimento MOIS. modulo=MOIS, operacao=listMarketWarmupSignals, pageId={}, erroClasse={}, erro={}",
          pageId,
          ex.getClass().getName(),
          ex.getMessage(),
          ex);
  ...
  @Transactional
  public MoisSalesLibraryDtos.MarketWarmupClaimResponse claimJob(
      MoisSalesLibraryDtos.MarketWarmupClaimRequest request) {
    try {
      MarketWarmupClaimData pending =
          gateway.findNextPendingJob(request.workspaceId()).orElse(null);
      if (pending == null) {
        log.info(
            "Nenhum job de aquecimento MOIS pendente para reserva. modulo=MOIS, operacao=claimMarketWarmupJob, workspaceId={}, workerId={}",
            request.workspaceId(),
            request.workerId());
      ...
      if (!claimed) {
        log.warn(
            "Job de aquecimento MOIS não foi reservado por concorrência operacional. modulo=MOIS, operacao=claimMarketWarmupJob, workspaceId={}, workerId={}, jobId={}",
            request.workspaceId(),
            request.workerId(),
            pending.job().jobId());
        return new MoisSalesLibraryDtos.MarketWarmupClaimResponse(false, null);
      ...
      log.info(
          "Job de aquecimento MOIS reservado. modulo=MOIS, operacao=claimMarketWarmupJob, workspaceId={}, workerId={}, pageId={}, jobId={}",
          request.workspaceId(),
          request.workerId(),
          page.pageId(),
          pending.job().jobId());
      return new MoisSalesLibraryDtos.MarketWarmupClaimResponse(
          true,
          new MoisSalesLibraryDtos.MarketWarmupClaimedJob(
              ...
              page.producerName(),
              page.offerSummary(),
              page.mechanismSummary(),
              page.promiseSummary(),
              page.proofSummary(),
              page.salesPageText()));
    } catch (RuntimeException ex) {
      log.error(
          "Falha ao reservar job de aquecimento MOIS. modulo=MOIS, operacao=claimMarketWarmupJob, workspaceId={}, workerId={}, erroClasse={}, erro={}",
          request.workspaceId(),
          request.workerId(),
          ex.getClass().getName(),
          ex.getMessage(),
    ...
    try {
      MarketWarmupJobData job =
          gateway
              ...
              .orElseThrow(
                  () ->
                      new IllegalArgumentException("Job de aquecimento não encontrado: " + jobId));
      ...
      log.info(
          "Iniciando conclusão do job de aquecimento MOIS. modulo=MOIS, operacao=completeMarketWarmupJob, workspaceId={}, pageId={}, jobId={}, fontesRecebidas={}, sinaisRecebidos={}",
          job.workspaceId(),
          job.pageId(),
          jobId,
      ...
      log.info(
          "Job de aquecimento MOIS concluído. modulo=MOIS, operacao=completeMarketWarmupJob, workspaceId={}, pageId={}, jobId={}, fontes={}, sinais={}, score={}",
          job.workspaceId(),
          job.pageId(),
          jobId,
          request.sources().size(),
      ...
      log.error(
          "Falha ao concluir job de aquecimento MOIS. modulo=MOIS, operacao=completeMarketWarmupJob, jobId={}, erroClasse={}, erro={}",
          jobId,
          ex.getClass().getName(),
          ex.getMessage(),
          ex);
  ...
  public void failJob(long jobId, MoisSalesLibraryDtos.MarketWarmupFailRequest request) {
    try {
              ...
              .orElseThrow(
                  () ->
                      new IllegalStateException(
                          "Job de aquecimento não encontrado ou não pode falhar: " + jobId));
      boolean failed =
          gateway.markJobFailed(jobId, request.errorCategory(), request.errorMessage());
      if (!failed) {
      ...
      persistSearchAttempts(job, request.searchAttempts());
      log.info(
          "Job de aquecimento MOIS marcado como falho. modulo=MOIS, operacao=failMarketWarmupJob, jobId={}, errorCategory={}, errorMessage={}",
          jobId,
          request.errorCategory(),
          request.errorMessage());
    } catch (RuntimeException ex) {
      log.error(
          "Falha ao registrar erro de job de aquecimento MOIS. modulo=MOIS, operacao=failMarketWarmupJob, jobId={}, erroClasse={}, erro={}",
          jobId,
          ex.getClass().getName(),
          ex.getMessage(),
          ex);
  ...
  /**
   * Refileira jobs antigos presos em FETCHING para permitir nova execução sem intervenção direta no
   * banco.
   */
  @Transactional
  public MoisSalesLibraryDtos.MarketWarmupReprocessStaleResponse reprocessStaleJobs(
    ...
    int staleMinutes =
        request.staleMinutes() == null ? DEFAULT_STALE_MINUTES : request.staleMinutes();
    try {
      long requeued = gateway.requeueStaleFetchingJobs(request.workspaceId(), staleMinutes);
      log.info(
          "Jobs de aquecimento MOIS presos refileirados. modulo=MOIS, operacao=reprocessStaleMarketWarmupJobs, workspaceId={}, staleMinutes={}, requeuedJobs={}",
          request.workspaceId(),
          staleMinutes,
          requeued);
      return new MoisSalesLibraryDtos.MarketWarmupReprocessStaleResponse(
          request.workspaceId(), staleMinutes, requeued, Instant.now());
    } catch (RuntimeException ex) {
      log.error(
          "Falha ao reprocessar jobs de aquecimento MOIS presos. modulo=MOIS, operacao=reprocessStaleMarketWarmupJobs, workspaceId={}, staleMinutes={}, erroClasse={}, erro={}",
          request.workspaceId(),
          staleMinutes,
          ex.getClass().getName(),
          ex.getMessage(),
  ...
  /**
   * Persiste as tentativas de busca pública reportadas pelo worker para explicar falhas e
   * descartes.
   */
  private void persistSearchAttempts(
      MarketWarmupJobData job,
      List<MoisSalesLibraryDtos.MarketWarmupSearchAttemptCompleteItem> attempts) {
    if (attempts == null || attempts.isEmpty()) {
      return;
    }
    for (MoisSalesLibraryDtos.MarketWarmupSearchAttemptCompleteItem attempt : attempts) {
      gateway.insertSearchAttempt(
          job.jobId(), job.pageId(), job.workspaceId(), mapSearchAttemptWrite(attempt));
    }
  }

  /** Persiste os termos de pesquisa extraídos por OpenAI para auditoria do planejamento. */
  private void persistSearchTerms(
      MarketWarmupJobData job,
      List<MoisSalesLibraryDtos.MarketWarmupSearchTermCompleteItem> terms) {
    if (terms == null || terms.isEmpty()) {
      return;
    }
    for (MoisSalesLibraryDtos.MarketWarmupSearchTermCompleteItem term : terms) {
      gateway.insertSearchTerm(
          job.jobId(),
          job.pageId(),
          job.workspaceId(),
          new MarketWarmupSearchTermData(term.termText(), term.reason(), term.source()));
    }
  }

  /** Persiste os resultados brutos de pesquisa enviados pelo worker para rastrear evidências. */
  private void persistSearchResults(
      MarketWarmupJobData job,
      List<MoisSalesLibraryDtos.MarketWarmupSearchResultCompleteItem> results) {
    if (results == null || results.isEmpty()) {
      return;
    }
    for (MoisSalesLibraryDtos.MarketWarmupSearchResultCompleteItem result : results) {
      gateway.insertSearchResult(
          job.jobId(),
          job.pageId(),
          job.workspaceId(),
          new MarketWarmupSearchResultData(
              result.termText(),
              result.resultUrl(),
              result.resultTitle(),
              result.resultSnippet(),
              result.rawPayload(),
              result.relatedToProduct()));
    }
  }

  /** Persiste as conclusões finais do dossiê para exibição direta pela tela. */
  private void persistFinalDossier(
      MarketWarmupJobData job, MoisSalesLibraryDtos.MarketWarmupFinalDossierCompleteItem dossier) {
    if (dossier == null) {
      return;
    }
    gateway.insertFinalDossier(
        job.jobId(),
        job.pageId(),
        job.workspaceId(),
        new MarketWarmupFinalDossierData(
            dossier.prestigeSummary(),
            dossier.externalWarmupResources(),
            dossier.finalConclusion(),
            dossier.rawModelResponse()));
  }

  /** Persiste as fontes enviadas pelo worker mantendo a posição de entrada para vincular sinais. */
  private List<Long> persistSources(
      MarketWarmupJobData job, List<MoisSalesLibraryDtos.MarketWarmupSourceCompleteItem> sources) {
    List<Long> sourceIds = new ArrayList<>();
    for (int index = 0; index < sources.size(); index++) {
      MoisSalesLibraryDtos.MarketWarmupSourceCompleteItem source = sources.get(index);
      long sourceId =
          gateway.insertSource(
              job.jobId(), job.pageId(), job.workspaceId(), mapSourceWrite(source));
      sourceIds.add(sourceId);
      log.info(
          "Fonte pública de aquecimento MOIS persistida. modulo=MOIS, operacao=completeMarketWarmupJob, workspaceId={}, pageId={}, jobId={}, sourceIndex={}, sourceId={}, platform={}, sourceType={}, sourceUrl={}",
          job.workspaceId(),
          job.pageId(),
          job.jobId(),
          index,
  ...
  private void validatePageAnalyzedForWarmup(SalesPageWarmupData page) {
    String effectiveStatus =
        page.analysisStatus() == null || page.analysisStatus().isBlank()
            ? page.currentStatus()
            : page.analysisStatus();
    String normalizedStatus =
        effectiveStatus == null ? "" : effectiveStatus.trim().toUpperCase(Locale.ROOT);
    if (!ANALYZED_STATUSES.contains(normalizedStatus)) {
      throw new IllegalStateException(
          "Dossiê MOIS só pode ser iniciado depois da análise comercial da página. pageId="
              + page.pageId()
              + ", statusAtual="
              + nullSafe(page.currentStatus())
              + ", statusAnalise="
              + nullSafe(page.analysisStatus()));
    }
  }
  ...
  private void persistSignals(
      MarketWarmupJobData job,
      List<Long> sourceIds,
      List<MoisSalesLibraryDtos.MarketWarmupSignalCompleteItem> signals) {
    for (MoisSalesLibraryDtos.MarketWarmupSignalCompleteItem signal : signals) {
      if (signal.sourceIndex() < 0 || signal.sourceIndex() >= sourceIds.size()) {
        throw new IllegalArgumentException(
            "Índice de fonte inválido para sinal de aquecimento: " + signal.sourceIndex());
      }
      gateway.insertSignal(
          job.jobId(),
          job.pageId(),
          job.workspaceId(),
          sourceIds.get(signal.sourceIndex()),
          mapSignalWrite(signal));
    }
  }

  /** Converte o resumo persistido em contrato de leitura da API. */
  private MoisSalesLibraryDtos.MarketWarmupSummaryResponse mapSummary(
      MarketWarmupSummaryData summary) {
    return new MoisSalesLibraryDtos.MarketWarmupSummaryResponse(
        summary.jobId(),
        summary.pageId(),
        summary.scoreTotal(),
        mapTemperature(summary.marketTemperature()),
        mapEcosystem(summary.ecosystemType()),
        mapRecommendation(summary.recommendation()),
        splitLines(summary.mainPains()),
        splitLines(summary.mainObjections()),
        splitLines(summary.mainPromises()),
        splitLines(summary.mainChannels()),
        splitLines(summary.mainCompetitors()),
        summary.saturationRisk(),
        summary.opportunityRecommendation(),
        summary.nextExperimentSuggestion(),
        mapJobStatus(summary.status()),
  ...
  private MoisSalesLibraryDtos.MarketWarmupSearchAttemptResponse mapSearchAttempt(
      MarketWarmupSearchAttemptData attempt) {
    return new MoisSalesLibraryDtos.MarketWarmupSearchAttemptResponse(
        ...
        attempt.queryText(),
        attempt.resultCount(),
        attempt.qualifiedCount(),
        attempt.rejectedCount(),
        attempt.status(),
        attempt.rejectionReason(),
        attempt.sampleResultTitle(),
  ...
  private MoisSalesLibraryDtos.MarketWarmupSignalResponse mapSignal(
      MarketWarmupSignalReadData signal) {
    return new MoisSalesLibraryDtos.MarketWarmupSignalResponse(
        signal.signalId(),
        signal.jobId(),
        signal.sourceId(),
        signal.pageId(),
        mapSignalType(signal.signalType()),
  ...
  private MarketWarmupSearchAttemptData mapSearchAttemptWrite(
      MoisSalesLibraryDtos.MarketWarmupSearchAttemptCompleteItem attempt) {
    return new MarketWarmupSearchAttemptData(
        null,
        null,
        null,
        attempt.queryText(),
  ...
  private MarketWarmupSourceData mapSourceWrite(
      MoisSalesLibraryDtos.MarketWarmupSourceCompleteItem source) {
  ...
  private MarketWarmupSignalData mapSignalWrite(
      MoisSalesLibraryDtos.MarketWarmupSignalCompleteItem signal) {
    return new MarketWarmupSignalData(
  ...
  private MarketWarmupSummaryWriteData mapSummaryWrite(
      MoisSalesLibraryDtos.MarketWarmupSummaryCompleteItem summary) {
    return new MarketWarmupSummaryWriteData(
        summary.scoreTotal(),
        summary.marketTemperature().name(),
        summary.ecosystemType().name(),
        summary.recommendation().name(),
        summary.mainPains(),
        summary.mainObjections(),
        summary.mainPromises(),
        summary.mainChannels(),
  ...
  private MoisSalesLibraryDtos.MarketWarmupTemperature mapTemperature(String temperature) {
    return temperature == null
        ? null
        : MoisSalesLibraryDtos.MarketWarmupTemperature.valueOf(temperature);
  }
  ...
  private MoisSalesLibraryDtos.MarketWarmupEcosystemType mapEcosystem(String ecosystem) {
    return ecosystem == null
        ? null
        : MoisSalesLibraryDtos.MarketWarmupEcosystemType.valueOf(ecosystem);
  }
  ...
  private MoisSalesLibraryDtos.MarketWarmupRecommendation mapRecommendation(String recommendation) {
    return recommendation == null
        ? null
        : MoisSalesLibraryDtos.MarketWarmupRecommendation.valueOf(recommendation);
  }
  ...
  private MoisSalesLibraryDtos.MarketWarmupSourceType mapSourceType(String sourceType) {
    return sourceType == null
        ? null
        : MoisSalesLibraryDtos.MarketWarmupSourceType.valueOf(sourceType);
  }
  ...
  private MoisSalesLibraryDtos.MarketWarmupSignalType mapSignalType(String signalType) {
    return signalType == null
        ? null
        : MoisSalesLibraryDtos.MarketWarmupSignalType.valueOf(signalType);
  }
