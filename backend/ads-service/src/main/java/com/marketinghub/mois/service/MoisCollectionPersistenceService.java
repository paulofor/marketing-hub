...
public class MoisCollectionPersistenceService {
  ...
  public MoisCollectionPersistenceDtos.CollectionJobStateResponse upsertJobState(
      String jobId, MoisCollectionPersistenceDtos.CollectionJobStateResponse state) {
  ...
  public Optional<MoisCollectionPersistenceDtos.CollectionJobStateResponse> getJobState(
      String jobId) {
    List<MoisCollectionPersistenceDtos.CollectionJobStateResponse> items =
        jdbcTemplate.query(
  ...
  public MoisCollectionPersistenceDtos.CollectionJobStateListResponse listJobStates(
      String workspaceId, String status) {
    StringBuilder sql =
        new StringBuilder("SELECT payload_json FROM mois_collection_job_state WHERE 1=1");
  ...
  private MoisCollectionPersistenceDtos.CollectionJobStateResponse readState(String payload) {
    try {
      return objectMapper.readValue(
          payload, MoisCollectionPersistenceDtos.CollectionJobStateResponse.class);
    } catch (JsonProcessingException ex) {
  ...
  /**
   * Recria as referências relacionais de um job preservando dados comerciais Hotmart quando
   * enviados no metadado.
   */
    ...
    jdbcTemplate.batchUpdate(
        ...
        (ps, item) -> {
          ...
          ps.setString(
              18,
              coalesceNotBlank(
                  metadataValue(item, "hotmartDescription"),
                  metadataValue(item, "description"),
                  metadataValue(item, "productDescription")));
          ...
          ps.setString(
              ...
              coalesceNotBlank(
                  item.title(),
                  metadataValue(item, "productName"),
                  metadataValue(item, "hotmartProductName")));
          ps.setString(23, coalesceNotBlank(item.url(), metadataValue(item, "productUrl")));
          ps.setString(
              24,
              coalesceNotBlank(
                  metadataValue(item, "hotmartProducer"),
                  metadataValue(item, "producerName"),
                  metadataValue(item, "producer")));
          ps.setString(
              25,
              coalesceNotBlank(
  ...
  private String metadataValue(
      com.marketinghub.mois.dto.MoisWorkspaceDtos.CollectedReferenceResponse item, String key) {
  ...
  public MoisCollectionPersistenceDtos.SourceHighlightListResponse summarizeBySource(
      String workspaceId, String status) {
    List<MoisCollectionPersistenceDtos.CollectionJobStateResponse> states =
        listJobStates(workspaceId, status).items();
    Map<String, List<com.marketinghub.mois.dto.MoisWorkspaceDtos.CollectedReferenceResponse>>
        bySource = new HashMap<>();
    for (MoisCollectionPersistenceDtos.CollectionJobStateResponse state : states) {
      if (state.references() == null) {
        continue;
      }
      for (com.marketinghub.mois.dto.MoisWorkspaceDtos.CollectedReferenceResponse reference :
          state.references()) {
        bySource
            .computeIfAbsent(
                reference.source() == null ? "UNKNOWN" : reference.source(),
                key -> new ArrayList<>())
            .add(reference);
      }
    }
    List<MoisCollectionPersistenceDtos.SourceHighlightResponse> items =
        bySource.entrySet().stream()
            .map(entry -> toSourceHighlight(entry.getKey(), entry.getValue()))
            .sorted(
                Comparator.comparing(
                        MoisCollectionPersistenceDtos.SourceHighlightResponse::averageSuccessScore)
                    .reversed())
            .toList();
  ...
  private MoisCollectionPersistenceDtos.SourceHighlightResponse toSourceHighlight(
    ...
    int favorites =
        (int)
            references.stream()
                .filter(
                    com.marketinghub.mois.dto.MoisWorkspaceDtos.CollectedReferenceResponse
                        ::favorite)
                .count();
    double avgSuccess =
        references.stream()
