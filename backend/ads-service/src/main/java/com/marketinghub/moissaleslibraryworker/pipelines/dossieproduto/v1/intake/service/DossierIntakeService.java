...
public class DossierIntakeService {
  ...
  public void start(String productKey) {
    ...
    if (!productContextGateway.hasCollectedProductContext(pageId)) {
      throw new IllegalStateException(
          "Dossiê de produto exige HTML, descrição ou resumo coletado antes do início: "
              + productKey);
    }
  ...
  public DossierIntakeRecebeRequestResponse recebeRequest(
      String productKey, String jobId, DossierIntakeRecebeRequestRequest request) {
    long pageId = Long.parseLong(productKey);
    Instant now = Instant.now();
    var page =
        salesPageRepository
  ...
  public DossierIntakeRecebeResponseResponse recebeResponse(
      String productKey, String jobId, DossierIntakeRecebeResponseRequest request) {
    long pageId = Long.parseLong(productKey);
    Instant now = Instant.now();
    ...
    return new DossierIntakeRecebeResponseResponse(
  ...
  public DossierIntakePendingResponse pending(DossierIntakePendingRequest request) {
    List<DossierIntakePendingJob> jobs =
        salesPageRepository
