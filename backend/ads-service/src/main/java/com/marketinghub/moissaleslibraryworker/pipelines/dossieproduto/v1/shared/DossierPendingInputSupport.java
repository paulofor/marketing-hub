...
public final class DossierPendingInputSupport {
  ...
  private static List<PipelineDossieProduto> currentFlowAudits(
      Long pageId, PipelineDossieProdutoRepository repository) {
    String productKey = String.valueOf(pageId);
    Instant currentFlowStartedAt =
        repository
    ...
    if (currentFlowStartedAt == null) {
      return repository.findByIdExternoAndVersaoPipelineOrderByDataHoraAscIdAsc(
          productKey, PIPELINE_VERSION);
    }
    return repository
        .findByIdExternoAndVersaoPipelineAndDataHoraGreaterThanEqualOrderByDataHoraAscIdAsc(
  ...
  private static Map<String, Object> previousStages(
      List<PipelineDossieProduto> currentAudits, String currentStageCode) {
    Map<String, Object> previousStages = new LinkedHashMap<>();
    currentAudits.forEach(
        audit -> {
          if (!currentStageCode.equals(audit.getCodigoEtapa()) && hasText(audit.getResponse())) {
            previousStages.put(
                audit.getCodigoEtapa(),
                Map.of(
  ...
  /**
   * Recupera respostas funcionais anteriores em ordem de auditoria para consumo simples pelo
   * worker.
   */
  private static List<String> previousStageResponses(
      List<PipelineDossieProduto> currentAudits, String currentStageCode) {
