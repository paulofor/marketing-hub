...
public class DossierSituacaoService {
  ...
  /**
   * Pesquisa registros da auditoria do fluxo atual pelo identificador externo, etapa e lista de
   * status.
   */
  public DossierSituacaoResponse consultar(
      String codigoEtapa, String idExterno, DossierSituacaoRequest request) {
    List<String> status =
        request.status().stream()
            .filter(valor -> valor != null && !valor.isBlank())
            .map(String::trim)
            .toList();
    List<PipelineDossieProduto> auditorias =
        buscarRegistrosDoFluxoAtual(codigoEtapa, idExterno, status);
    Map<String, String> requestsPorJob = mapearRequestsPorJob(auditorias);
    List<DossierSituacaoItem> registros =
        auditorias.stream()
  ...
  /**
   * Limita a consulta à última execução iniciada pela etapa intake, preservando histórico fora da
   * tela atual.
   */
  ...
  /**
   * Prioriza o request do próprio registro e usa o request correlacionado quando a linha atual é de
   * response.
   */
  private String requestVisivel(String requestDoRegistro, String requestCorrelacionado) {
    return requestDoRegistro == null || requestDoRegistro.isBlank()
        ? requestCorrelacionado
        : requestDoRegistro;
  }
}
