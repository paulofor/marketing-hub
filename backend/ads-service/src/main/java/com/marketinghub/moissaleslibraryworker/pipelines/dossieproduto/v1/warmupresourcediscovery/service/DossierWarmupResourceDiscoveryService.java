...
/**
 * Publica pendências e contratos da etapa descoberta de recursos de aquecimento do pipeline de
 * dossiê MOIS v1.
 */
@Service
public class DossierWarmupResourceDiscoveryService {
  ...
  public DossierWarmupResourceDiscoveryRecebeRequestResponse recebeRequest(
      String productKey, String jobId, DossierWarmupResourceDiscoveryRecebeRequestRequest request) {
    long pageId = Long.parseLong(productKey);
    Instant now = Instant.now();
    var page =
        salesPageRepository
    ...
    return new DossierWarmupResourceDiscoveryRecebeRequestResponse(
  ...
  public DossierWarmupResourceDiscoveryRecebeResponseResponse recebeResponse(
      String productKey,
      String jobId,
      DossierWarmupResourceDiscoveryRecebeResponseRequest request) {
    long pageId = Long.parseLong(productKey);
    Instant now = Instant.now();
    ...
    return new DossierWarmupResourceDiscoveryRecebeResponseResponse(
   ...
   * mais antiga.
   */
  public DossierWarmupResourceDiscoveryPendingResponse pending(
      DossierWarmupResourceDiscoveryPendingRequest request) {
    List<DossierWarmupResourceDiscoveryPendingJob> jobs =
        salesPageRepository
