...
/**
 * Publica pendências e contratos da etapa extração de sinais de aquecimento do pipeline de dossiê
 * MOIS v1.
 */
@Service
public class DossierWarmupSignalExtractionService {
  ...
  /**
   * Cria o service da etapa com acesso aos repositórios canônicos da página/produto e da auditoria
   * do pipeline.
   */
            ...
            .orElseThrow(
                () ->
                    new IllegalArgumentException(
                        "Página/produto MOIS não encontrada: " + productKey));
  ...
  /**
   * Recebe o request operacional da etapa, coloca a página em espera do módulo e audita a entrada
   * do pipeline.
   */
  public DossierWarmupSignalExtractionRecebeRequestResponse recebeRequest(
      String productKey, String jobId, DossierWarmupSignalExtractionRecebeRequestRequest request) {
    long pageId = Long.parseLong(productKey);
    Instant now = Instant.now();
    var page =
        salesPageRepository
    ...
    return new DossierWarmupSignalExtractionRecebeRequestResponse(
        jobId, productKey, STAGE_CODE, STATUS_WAITING);
  }

  /**
   * Recebe a resposta operacional da etapa, avança sucesso para a próxima etapa e audita a saída do
   * pipeline.
   */
  public DossierWarmupSignalExtractionRecebeResponseResponse recebeResponse(
      String productKey, String jobId, DossierWarmupSignalExtractionRecebeResponseRequest request) {
    long pageId = Long.parseLong(productKey);
    Instant now = Instant.now();
    ...
    return new DossierWarmupSignalExtractionRecebeResponseResponse(
        jobId, productKey, STAGE_CODE, status, nextStageCode);
  }
  ...
  /**
   * Entrega até dez trabalhos iniciados da etapa atual ao executor, ordenados pela data operacional
   * mais antiga.
   */
  public DossierWarmupSignalExtractionPendingResponse pending(
      DossierWarmupSignalExtractionPendingRequest request) {
    List<DossierWarmupSignalExtractionPendingJob> jobs =
        salesPageRepository
