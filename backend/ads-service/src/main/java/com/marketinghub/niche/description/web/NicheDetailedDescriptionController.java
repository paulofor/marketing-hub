...
public class NicheDetailedDescriptionController {
  ...
  public NicheDetailedDescriptionController(
      NicheDetailedDescriptionService service, NicheDetailedDescriptionMapper mapper) {
  ...
  public List<NicheDetailedDescriptionDto> list(@PathVariable Long nicheId) {
    return service.listByNiche(nicheId).stream().map(mapper::toDto).collect(Collectors.toList());
  }

  @PostMapping
  public NicheDetailedDescriptionDto create(
      @PathVariable Long nicheId, @RequestBody CreateNicheDetailedDescriptionRequest request) {
    request.setMarketNicheId(nicheId);
    return mapper.toDto(service.create(request));
  }

  @PatchMapping("/{descriptionId}/active")
  public NicheDetailedDescriptionDto updateActive(
