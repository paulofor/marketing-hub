...
public class MarketNicheService {
  ...
  /**
   * Define o custo total inicial do nicho usando o total explícito ou, na falta dele, o custo de
   * origem informado.
   */
  ...
  private DifferentiatedTechnology resolveDifferentiatedTechnology(Long id) {
    ...
    return differentiatedTechnologyRepository
        .findById(id)
        .orElseThrow(
            () ->
                new ResponseStatusException(
                    HttpStatus.NOT_FOUND, "Differentiated technology not found: " + id));
  }
  ...
  private NicheDetailedDescription resolveDetailedDescription(Long nicheId, Long descriptionId) {
    ...
    return detailedDescriptionRepository
        .findByIdAndMarketNicheIdAndActiveTrue(descriptionId, nicheId)
        .orElseThrow(
            () ->
                new ResponseStatusException(
  ...
  @Transactional
  public MarketNiche requestHypotheses(
      Long id,
      int quantity,
      String model,
    ...
    if (differentiatedTechnologyId != null) {
      niche.setDifferentiatedTechnology(
          resolveDifferentiatedTechnology(differentiatedTechnologyId));
    }
   ...
   */
  public Map<Long, MarketNicheEnrichmentProfile> latestEnrichmentProfilesByNicheIds(
      List<Long> nicheIds) {
    if (nicheIds == null) {
      return Map.of();
    }
    List<Long> safeIds = nicheIds.stream().filter(id -> id != null).toList();
    if (safeIds.isEmpty()) {
      return Map.of();
    }
    return enrichmentProfileRepository.findLatestByMarketNicheIds(safeIds).stream()
        .collect(
            Collectors.toMap(profile -> profile.getMarketNiche().getId(), Function.identity()));
  }

  /**
   * Lista o perfil enriquecido mais recente por nicho para habilitar navegação contextual no
   * frontend.
   */
  public Map<Long, MarketNicheEnrichmentProfile> latestEnrichmentProfilesByNiche(
      List<MarketNiche> niches) {
    List<Long> nicheIds = niches.stream().map(MarketNiche::getId).filter(id -> id != null).toList();
    if (nicheIds.isEmpty()) {
      return Map.of();
    }
  ...
  /**
   * Lista nichos com solicitação pendente e experimento comercialmente pronto para criação de pixel
   * do Facebook.
   */
    ...
    MarketNiche niche =
        repository
            ...
            .orElseThrow(
                () ->
                    new ResponseStatusException(
                        HttpStatus.NOT_FOUND, "Market niche not found: " + nicheId));
  ...
  @Transactional
  public MarketNiche attachFacebookPixel(
