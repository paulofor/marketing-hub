...
public class NicheLearningService {
  ...
  public NicheLearningService(
      ExperimentLearningRepository learningRepository, ExperimentLearningMapper learningMapper) {
  ...
  public NicheLearningDictionaryDto summarize(Long nicheId) {
    ...
    Map<LearningInsightType, LinkedHashMap<String, LearningStatementDto>> grouped =
        new EnumMap<>(LearningInsightType.class);
    Instant updatedAt = null;
    for (ExperimentLearning learning : learnings) {
      if (learning.getCompletedAt() != null
          && (updatedAt == null || learning.getCompletedAt().isAfter(updatedAt))) {
        updatedAt = learning.getCompletedAt();
      }
      for (LearningInsightDto insight : learningMapper.safeInsights(learning)) {
        if (insight == null
            || insight.getType() == null
            || !StringUtils.hasText(insight.getStatement())) {
          continue;
        }
  ...
  public List<BacklogRecommendationDto> backlog(Long nicheId) {
    ...
    for (ExperimentLearning learning : learnings) {
      ...
      for (ExperimentLearningSuggestionDto suggestion : suggestions) {
        ...
        dto.setPrimaryMetric(
            suggestion.getPrimaryMetric() != null
                ? suggestion.getPrimaryMetric()
                : learning.getPrimaryMetric());
  ...
  private List<ExperimentLearning> fetchLearnings(Long nicheId) {
    List<ExperimentLearning> learnings =
        learningRepository.findTop50ByNicheIdOrderByCompletedAtDesc(nicheId);
