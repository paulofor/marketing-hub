...
public class MarketNicheController {
  ...
  @PutMapping("/{id}")
  public MarketNicheDto update(
      @PathVariable Long id, @RequestBody CreateMarketNicheRequest request) {
    return mapper.toDto(service.update(id, request));
  }

  /** Agenda a geração de interesses para segmentação do nicho. */
  @PatchMapping("/{id}/interests-to-generate")
  public MarketNicheDto requestInterests(
  ...
  public MarketNicheDto requestJobTitles(
      @PathVariable Long id,
      @RequestParam("quantity") int quantity,
      @RequestParam(value = "model", required = false) String model) {
  ...
  public MarketNicheDto requestBehaviors(
      @PathVariable Long id,
      @RequestParam("quantity") int quantity,
      @RequestParam(value = "model", required = false) String model) {
  ...
  /**
   * Agenda a geração de hipóteses e opcionalmente define contexto de tecnologia ou descrição
   * detalhada.
   */
  @PatchMapping("/{id}/hypotheses-to-generate")
  public MarketNicheDto requestHypotheses(
      @PathVariable Long id,
      @RequestParam("quantity") int quantity,
      @RequestParam(value = "model", required = false) String model,
      @RequestParam(value = "differentiatedTechnologyId", required = false)
          Long differentiatedTechnologyId,
      @RequestParam(value = "detailedDescriptionId", required = false) Long detailedDescriptionId) {
    return mapper.toDto(
        service.requestHypotheses(
            id, quantity, model, differentiatedTechnologyId, detailedDescriptionId));
  }

  /** Agenda a geração de descrições detalhadas para aprofundar o nicho. */
  @PatchMapping("/{id}/detailed-descriptions-to-generate")
  public MarketNicheDto requestDetailedDescriptions(
      @PathVariable Long id,
      @RequestParam("quantity") int quantity,
      @RequestParam(value = "model", required = false) String model) {
  ...
  public MarketNicheListPageDto listSummary(
    ...
    List<Long> nicheIds =
        nichePage.getContent().stream().map(MarketNicheListItemProjection::getId).toList();
    Map<Long, MarketNicheEnrichmentProfile> profilesByNiche =
        service.latestEnrichmentProfilesByNicheIds(nicheIds);
    List<MarketNicheListItemDto> items =
        nichePage.getContent().stream()
            .map(
                item ->
                    new MarketNicheListItemDto(
                        item.getId(),
                        item.getName(),
                        profilesByNiche.containsKey(item.getId())
                            ? profilesByNiche.get(item.getId()).getId()
                            : null,
                        item.getCreatedAt(),
                        item.getTotalCost(),
  ...
  public List<MarketNicheDto> list() {
    ...
    Map<Long, MarketNicheEnrichmentProfile> profilesByNiche =
        service.latestEnrichmentProfilesByNiche(niches);
  ...
  private MarketNicheDto toDtoWithEnrichment(
      MarketNiche niche, MarketNicheEnrichmentProfile profile) {
