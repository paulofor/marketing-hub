  ...
  public CompletionDisposition apply(AgentTask task, CompleteAgentTaskRequest request) {
    ...
    if (PREPARATION.contains(activity)) {
      ...
      if ("economics".equals(activity)) {
        ...
        if (!OpalaCommercialAssetComparator.sameReviewableAssets(
            identity, context.snapshot(task.getSourceReference()))) {
        ...
        .map(
            t -> {
              ...
              if ("economics".equals(step))
                return !OpalaCommercialAssetComparator.sameDecimal(
                        snapshot.path("priceBrl"), current.path("priceBrl"))
                    || !OpalaCommercialAssetComparator.sameDecimal(
                        snapshot.path("budgetLimitBrl"), current.path("budgetLimitBrl"))
                    || !snapshot.path("windowEnd").equals(current.path("windowEnd"))
                    || !OpalaCommercialAssetComparator.sameFinancialPlanRevision(
              ...
              return !OpalaCommercialAssetComparator.sameReviewableAssets(snapshot, current);
            })
        .orElse(false);
  }
