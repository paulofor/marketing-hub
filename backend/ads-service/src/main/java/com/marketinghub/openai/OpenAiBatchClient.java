...
public class OpenAiBatchClient {
  ...
  private static final Set<String> TERMINAL_STATUSES =
      Set.of("completed", "failed", "cancelled", "expired");
      ...
      try {
        ...
        if (output.response() != null && output.response().isSuccessful()) {
          OpenAiResponse response =
              objectMapper.convertValue(output.response().body(), OpenAiResponse.class);
          responses.put(output.customId(), response);
        } else if (output.response() != null) {
          log.error(
              "OpenAI batch request {} failed with status {}",
              output.customId(),
  ...
  private record OpenAiBatch(
      String id, String status, @JsonProperty("output_file_id") String outputFileId) {}

  private record OpenAiFile(String id) {}

  private record BatchOutput(
      @JsonProperty("custom_id") String customId,
      BatchOutputResponse response,
      BatchOutputError error) {}

  private record BatchOutputResponse(
      @JsonProperty("status_code") Integer statusCode, Map<String, Object> body) {
