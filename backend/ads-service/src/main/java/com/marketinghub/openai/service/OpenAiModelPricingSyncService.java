...
/**
 * Responsabilidade: sincronizar no banco os preços oficiais dos modelos OpenAI usados no cálculo
 * financeiro.
 */
@Service
public class OpenAiModelPricingSyncService {
   ...
   */
  public OpenAiModelPricingSyncService(
      OpenAiPricingPageClient pricingPageClient, OpenAiModelRepository repository) {
    this.pricingPageClient = pricingPageClient;
    this.repository = repository;
  }

  /**
   * Consulta a fonte oficial e persiste preços atuais, inclusive para variantes datadas já
   * cadastradas.
   */
  @Transactional
  ...
  private void applyPrice(
      OpenAiModel model, OpenAiModelPricing price, Instant syncedAt, boolean updateIdentity) {
    if (updateIdentity) {
