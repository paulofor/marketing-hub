...
/**
 * Serviço responsável apenas por ler e persistir dados do fluxo CNAE de oportunidade solicitado
 * pelo módulo OPRM.
 */
@Service
public class OprmCnaeOpportunityPersistenceService {
  ...
  public OprmCnaeOpportunityScoreResponseDto saveScore(
      String cnaeCode, OprmCnaeOpportunityScoreRequestDto request) {
    OprmCnaeOpportunityScore score =
        scoreRepository.findById(cnaeCode).orElseGet(OprmCnaeOpportunityScore::new);
        ...
        .orElseThrow(
            () ->
                new ResponseStatusException(
                    HttpStatus.NOT_FOUND, "Score OPRM não encontrado para CNAE: " + cnaeCode));
  }

  /**
   * Lista scores já calculados pelo OPRM, com filtro opcional para retornar somente itens ainda não
   * enriquecidos.
   */
  @Transactional(readOnly = true)
  public List<OprmCnaeOpportunityScoreResponseDto> listTopScores(int limit, boolean notEnriched) {
    List<OprmCnaeOpportunityScore> scores =
        notEnriched
            ? scoreRepository.findByEnrichedAtIsNullOrderByOpportunityScoreDescCnaeCodeAsc(
                PageRequest.of(0, normalizeLimit(limit)))
            : scoreRepository.findAllByOrderByOpportunityScoreDescCnaeCodeAsc(
                PageRequest.of(0, normalizeLimit(limit)));
    return scores.stream().map(this::toScoreResponse).toList();
  }
  ...
  public OprmCnaeCycleResponseDto upsertCycle(OprmCnaeCycleUpsertRequestDto request) {
    OprmCnaeProcessingCycle cycle =
        cycleRepository.findById(request.cycleId()).orElseGet(OprmCnaeProcessingCycle::new);
  ...
  public List<OprmCnaeCycleResponseDto> listCycles(int limit) {
    return cycleRepository
        .findAllByOrderByStartedAtDesc(PageRequest.of(0, normalizeLimit(limit)))
        .stream()
  ...
  /**
   * Retorna o próximo número de ciclo para o tipo informado sem executar regra de negócio de
   * oportunidade.
   */
  @Transactional(readOnly = true)
  ...
  public List<OprmNicheCandidateResponseDto> listCandidates(String cnaeCode) {
    return candidateRepository
        .findByCnaeCodeOrderByOpportunityScoreDescCreatedAtDesc(cnaeCode)
        .stream()
        .map(this::toCandidateResponse)
        .toList();
  }
  ...
  /**
   * Lista os nichos já enriquecidos pelo OPRM priorizando maiores scores para consulta direta no
   * frontend.
   */
  @Transactional(readOnly = true)
  public List<OprmNicheCandidateResponseDto> listEnrichedCandidates(int limit) {
    return candidateRepository
        .findAllByOrderByOpportunityScoreDescCreatedAtDesc(PageRequest.of(0, normalizeLimit(limit)))
        .stream()
        .map(this::toCandidateResponse)
        .toList();
  }
  ...
  public OprmNicheCandidateResponseDto approveCandidate(
      Long id, OprmNicheCandidateApprovalRequestDto request) {
  ...
  private OprmNicheCandidate loadCandidate(Long id) {
    return candidateRepository
        ...
        .orElseThrow(
            () ->
                new ResponseStatusException(
                    HttpStatus.NOT_FOUND, "Candidato de nicho não encontrado: " + id));
  }
