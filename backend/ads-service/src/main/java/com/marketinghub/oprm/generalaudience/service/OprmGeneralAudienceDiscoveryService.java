...
/**
 * Responsável pelo pipeline de descoberta de subnichos, dores, evidências e quality gate de
 * públicos gerais.
 */
@Service
public class OprmGeneralAudienceDiscoveryService {
  ...
  private static final Set<String> GENERIC_TERMS =
      Set.of("marketing digital", "renda extra", "beleza", "relacionamento");
  private static final Set<String> RISKY_PROMISE_TERMS =
      Set.of(
          "garantido",
          "garantida",
          "certeza",
  ...
  private final OprmGeneralAudienceHypothesisMaterializationRepository
      hypothesisMaterializationRepository;
  private final OprmGeneralAudienceLeadExperimentMaterializationRepository
      leadExperimentMaterializationRepository;
  ...
  public OprmGeneralAudienceDiscoveryService(
      ...
      OprmGeneralAudienceLeadExperimentMaterializationRepository
          leadExperimentMaterializationRepository,
  ...
  public GeneralAudiencePainAngleResponse createPainAngle(
      Long subnicheId, CreateGeneralAudiencePainAngleRequest request) {
    ...
    angle.setLandingConfirmationQuestion(
        normalizeOptionalText(request.landingConfirmationQuestion()));
    angle.setComplianceNotes(normalizeOptionalText(request.complianceNotes()));
    angle.setStatus(
        request.status() == null
            ? OprmGeneralAudiencePainAngleStatus.DISCOVERED
            : request.status());
  ...
  public GeneralAudiencePainAngleResponse updatePainAngle(
      Long angleId, UpdateGeneralAudiencePainAngleRequest request) {
  ...
  public GeneralAudienceSourceEvidenceResponse createSourceEvidence(
      Long seedId, CreateGeneralAudienceSourceEvidenceRequest request) {
    ...
    if (request.subnicheId() != null) {
      ...
      if (!seed.getId().equals(subniche.getSeed().getId())) {
        throw new ResponseStatusException(
            HttpStatus.BAD_REQUEST, "subnicheId não pertence à semente informada");
      }
    }
  ...
  public GeneralAudienceQualityReadingResponse createQualityReading(
      Long subnicheId, CreateGeneralAudienceQualityReadingRequest request) {
    if (request == null) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "Payload de leitura de qualidade é obrigatório");
    }
    OprmGeneralAudienceSubniche subniche = findSubniche(subnicheId);
    ...
    reading.setCorrectProfessionLeads(
        nonNegativeOrZero(request.correctProfessionLeads(), "correctProfessionLeads"));
    reading.setRealPainResponses(
        nonNegativeOrZero(request.realPainResponses(), "realPainResponses"));
    ...
    reading.setOutOfProfileLeads(
        nonNegativeOrZero(request.outOfProfileLeads(), "outOfProfileLeads"));
    reading.setCuriousWithoutProfession(
        nonNegativeOrZero(request.curiousWithoutProfession(), "curiousWithoutProfession"));
    reading.setLowCompletionEvents(
        nonNegativeOrZero(request.lowCompletionEvents(), "lowCompletionEvents"));
    reading.setConfusingPromiseReports(
        nonNegativeOrZero(request.confusingPromiseReports(), "confusingPromiseReports"));
    reading.setLeadMagnetNoResponse(
        nonNegativeOrZero(request.leadMagnetNoResponse(), "leadMagnetNoResponse"));
    ...
    reading.setApproved(
        blockers.isEmpty() && reading.getQualityScore().compareTo(new BigDecimal("60.00")) >= 0);
  ...
  public GeneralAudienceQualityGateResponse evaluateQualityGate(Long subnicheId) {
    ...
    requireText(
        ...
        "Pergunta qualificadora obrigatória ausente.");
    ...
    long approvedAngles =
        painAngleRepository.countBySubnicheIdAndStatusIn(
            subnicheId, Set.of(OprmGeneralAudiencePainAngleStatus.APPROVED));
    if (approvedAngles == 0) {
    ...
    qualityReadingRepository
        .findTopBySubnicheIdOrderByCapturedAtDesc(subnicheId)
        .ifPresentOrElse(
            reading -> {
            ...
            () ->
                recommendations.add(
                    "Registrar leitura de qualidade real após a primeira captação de leads."));
    return new GeneralAudienceQualityGateResponse(
        subnicheId, blockers.isEmpty(), blockers, recommendations);
  }
  ...
  public GeneralAudienceHypothesisResponse createHypothesis(
      Long angleId, CreateGeneralAudienceHypothesisRequest request) {
    CreateGeneralAudienceHypothesisRequest safeRequest =
        request == null ? new CreateGeneralAudienceHypothesisRequest(null, null, null) : request;
    OprmGeneralAudiencePainAngle angle = findPainAngle(angleId);
    ...
    String title =
        resolveDefaultText(safeRequest.title(), "Hipótese Público Geral - " + subniche.getName());
    String successRule =
        resolveDefaultText(
    ...
    var hypothesis =
        hypothesisMaterializationRepository.createHypothesis(
  ...
  public GeneralAudienceLeadExperimentResponse createLeadExperiment(
      Long angleId, CreateGeneralAudienceLeadExperimentRequest request) {
    OprmGeneralAudiencePainAngle angle = findPainAngle(angleId);
    ...
    var experiment =
        leadExperimentMaterializationRepository.createLeadExperiment(
            subniche.getMarketNicheId(),
            ...
            request.sampleSize(),
            buildLeadExperimentCampaignAngle(
                subniche, angle, leadMagnet, safePromise, qualificationQuestion),
            buildLeadExperimentAdCopy(angle, safePromise, qualificationQuestion),
            buildLeadExperimentLandingCopy(
                subniche, angle, leadMagnet, safePromise, qualificationQuestion));
  ...
  /**
   * Registra dados de público para o Facebook Ads buscar depois pelo backend, sem acessar o módulo
   * de targeting.
   */
  @Transactional
  public GeneralAudienceTargetingPreparationResponse prepareInitialTargeting(
      Long angleId, GeneralAudienceTargetingPreparationRequest request) {
    if (request == null) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "Payload de dados para Facebook Ads é obrigatório");
    }
    OprmGeneralAudiencePainAngle angle = findPainAngle(angleId);
    validateAngleCanCreateHypothesis(angle);
    OprmGeneralAudienceSubniche subniche = angle.getSubniche();
    String qualificationQuestion =
        requiredText(resolveQualificationQuestion(subniche, angle), "qualificationQuestion");
    ...
    saved.addAll(
        saveFacebookAdsData(
            ...
            angle,
            request,
            OprmGeneralAudienceAdSignalType.JOB_TITLE,
            request.jobTitles(),
            true));
    saved.addAll(
        saveFacebookAdsData(
            ...
            angle,
            request,
            OprmGeneralAudienceAdSignalType.INTEREST,
            request.interests(),
            false));
    saved.addAll(
        saveFacebookAdsData(
            ...
            angle,
            request,
            OprmGeneralAudienceAdSignalType.BEHAVIOR,
            request.behaviors(),
            false));
    long readyJobTitles =
        saved.stream()
    ...
    List<GeneralAudienceTargetingElementResponse> elements =
        saved.stream().map(this::toFacebookAdsDataResponse).toList();
    List<String> blockers = conservativeTargetingBlockers(saved, request, readyJobTitles);
    List<String> recommendations =
        conservativeTargetingRecommendations(request, qualificationQuestion);
        ...
        .orElseThrow(
            () ->
                new ResponseStatusException(
                    HttpStatus.NOT_FOUND, "Semente de público geral não encontrada: " + seedId));
  }
  ...
  private OprmGeneralAudiencePainAngle findPainAngle(Long angleId) {
    return painAngleRepository
        .findById(angleId)
        .orElseThrow(
            () ->
                new ResponseStatusException(
                    HttpStatus.NOT_FOUND, "Ângulo de público geral não encontrado: " + angleId));
  }
  ...
  private List<OprmGeneralAudienceFacebookAdsData> saveFacebookAdsData(
    ...
    for (int index = 0; index < terms.size(); index++) {
      ...
      if (!StringUtils.hasText(term)
          || items.stream().anyMatch(element -> element.getTerm().equals(term))) {
        continue;
      }
      items.add(
          buildFacebookAdsData(
              subniche,
              angle,
              request,
              type,
  ...
  private OprmGeneralAudienceFacebookAdsData buildFacebookAdsData(
    ...
    data.setLandingConfirmationInstruction(
        normalizeOptionalText(request.landingConfirmationInstruction()));
  ...
  private OprmGeneralAudienceAdSignalStatus resolveFacebookAdsDataStatus(
      OprmGeneralAudienceFacebookAdsData data) {
  ...
  private String buildFacebookAdsDataNotes(
      GeneralAudienceTargetingPreparationRequest request, boolean requiredForPublication) {
    return String.join(
        "\n",
        requiredForPublication
  ...
  private GeneralAudienceTargetingElementResponse toFacebookAdsDataResponse(
      OprmGeneralAudienceFacebookAdsData data) {
  ...
  private List<String> conservativeTargetingBlockers(
    ...
    boolean hasJobTitle =
        saved.stream()
            .anyMatch(
                element -> element.getSignalType() == OprmGeneralAudienceAdSignalType.JOB_TITLE);
    if (!hasJobTitle) {
      blockers.add("Informe ao menos um cargo/termo de trabalho para revisão antes de publicação.");
    }
    if (readyJobTitles == 0) {
      blockers.add(
          "Nenhum JOB_TITLE pronto para Facebook Ads; o publicador atual não deve criar ad set amplo puro.");
    }
  ...
  private List<String> conservativeTargetingRecommendations(
      GeneralAudienceTargetingPreparationRequest request, String qualificationQuestion) {
    List<String> recommendations = new ArrayList<>();
    if (request.interests() == null || request.interests().isEmpty()) {
      recommendations.add(
          "Adicionar interesses específicos como enriquecimento, não como substituto do JOB_TITLE.");
    }
  ...
  private void validateAngleCanCreateLeadExperiment(
      OprmGeneralAudiencePainAngle angle, CreateGeneralAudienceLeadExperimentRequest request) {
    if (request == null) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "Payload do experimento de lead é obrigatório");
    }
    validateAngleCanCreateHypothesis(angle);
    ...
    if (request.stopLossCpl() == null || request.stopLossCpl().compareTo(BigDecimal.ZERO) <= 0) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "stopLossCpl deve ser maior que zero");
    }
    if (request.dailyBudget() == null || request.dailyBudget().compareTo(BigDecimal.ZERO) <= 0) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "dailyBudget deve ser maior que zero");
    }
    if (request.dailyBudget().compareTo(MAX_GENERAL_AUDIENCE_DAILY_BUDGET) > 0) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "dailyBudget deve manter orçamento pequeno até 100.00");
    }
    if (request.durationDays() == null
        || request.durationDays() < 1
        || request.durationDays() > 14) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "durationDays deve representar duração curta entre 1 e 14 dias");
    }
  }

  /** Resolve a pergunta qualificadora priorizando a pergunta específica da landing. */
  private String resolveQualificationQuestion(
      OprmGeneralAudienceSubniche subniche, OprmGeneralAudiencePainAngle angle) {
  ...
  private String buildLeadExperimentAdCopy(
      OprmGeneralAudiencePainAngle angle, String safePromise, String qualificationQuestion) {
    return String.join(
        "\n",
  ...
  private List<String> qualityReadingBlockers(OprmGeneralAudienceQualityReading reading) {
    ...
    if (reading.getTotalLeads() > 0
        && reading.getCorrectProfessionLeads() * 2 < reading.getTotalLeads()) {
  ...
  private List<String> qualityReadingRecommendations(OprmGeneralAudienceQualityReading reading) {
    ...
    if (reading.getPriceOrNextStepQuestions() == 0) {
      recommendations.add(
          "Validar próximo passo comercial, pois ainda não houve pergunta de preço ou avanço.");
    }
    if (reading.getLowCompletionEvents() > 0) {
      recommendations.add(
          "Simplificar formulário ou pergunta qualificadora para reduzir baixo preenchimento.");
    }
    if (reading.getLeadMagnetNoResponse() > 0) {
      recommendations.add(
          "Revisar entrega e follow-up: há lead que baixa a isca, mas não responde.");
    }
    return recommendations;
  }
  ...
  private int nonNegativeOrZero(Integer value, String fieldName) {
    ...
    if (value < 0) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, fieldName + " não pode ser negativo");
    }
    return value;
  }
  ...
  private void validateAngleCanCreateHypothesis(OprmGeneralAudiencePainAngle angle) {
    if (angle.getStatus() != OprmGeneralAudiencePainAngleStatus.APPROVED) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "Ângulo precisa estar aprovado antes de criar hipótese");
    }
    OprmGeneralAudienceSubniche subniche = angle.getSubniche();
  ...
  private void validateSafeAngle(OprmGeneralAudiencePainAngle angle) {
    if (isGeneric(angle.getPain())) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "pain genérico demais para público geral");
    }
    if (containsRiskyPromise(angle.getSafePromise())
        || containsRiskyPromise(angle.getFirstAdHook())) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "Ângulo contém promessa arriscada ou absoluta");
    }
  }
  ...
  private String requiredText(String value, String fieldName) {
    if (!StringUtils.hasText(value)) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, fieldName + " não pode ficar vazio");
    }
    return value.trim();
  }
  ...
  private GeneralAudienceQualityReadingResponse toQualityReadingResponse(
      OprmGeneralAudienceQualityReading reading) {
  ...
  private GeneralAudienceSourceEvidenceResponse toSourceEvidenceResponse(
      OprmGeneralAudienceSourceEvidence evidence) {
