...
/**
 * Responsável por registrar no banco a situação de confirmação preparada pelo OPRM sem acionar
 * outros módulos.
 */
@Service
public class OprmGeneralAudienceLandingConfirmationService {
  ...
  public GeneralAudienceLandingConfirmationResponse createConfirmationFlow(
      Long angleId, CreateGeneralAudienceLandingConfirmationRequest request) {
    if (request == null) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "Payload da landing/formulário é obrigatório");
    }
    ...
    OprmGeneralAudienceLandingConfirmation confirmation =
        landingConfirmationRepository.save(
            buildConfirmation(
                angle,
                subniche,
                request,
  ...
  private void validateAngleCanCreateConfirmationRecord(
      OprmGeneralAudiencePainAngle angle, CreateGeneralAudienceLandingConfirmationRequest request) {
    ...
    if (angle.getStatus() != OprmGeneralAudiencePainAngleStatus.APPROVED) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "Ângulo precisa estar aprovado antes da landing/formulário");
    }
    OprmGeneralAudienceSubniche subniche = angle.getSubniche();
  ...
  private OprmGeneralAudienceLandingConfirmation buildConfirmation(
      ...
      String nextStep) {
    OprmGeneralAudienceLandingConfirmation confirmation =
        new OprmGeneralAudienceLandingConfirmation();
  ...
  private GeneralAudienceLandingConfirmationResponse toResponse(
      OprmGeneralAudienceLandingConfirmation confirmation, List<String> qualificationOptions) {
  ...
  private List<String> normalizeQualificationOptions(List<String> options) {
    if (options == null) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "qualificationOptions é obrigatório");
    }
    ...
    if (normalized.size() < 2) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "qualificationOptions precisa ter pelo menos duas opções úteis");
    }
  ...
  private void requiredText(String value, String field) {
    if (!StringUtils.hasText(value)) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, field + " é obrigatório para registrar confirmação");
    }
  }

  /** Monta slug determinístico para o registro OPRM sem publicar rota externa. */
  private String buildSlug(
      OprmGeneralAudienceSubniche subniche, OprmGeneralAudiencePainAngle angle, Long experimentId) {
    String base =
        "oprm-publico-geral-" + subniche.getId() + "-" + angle.getId() + "-" + experimentId;
  ...
  private String normalizeSlug(String value) {
    String normalized =
        Normalizer.normalize(value.toLowerCase(Locale.ROOT), Normalizer.Form.NFD)
            .replaceAll("\\p{M}", "")
            .replaceAll("[^a-z0-9]+", "-")
            .replaceAll("(^-|-$)", "");
