...
public class OprmGeneralAudienceService {
  ...
  private final OprmGeneralAudienceMarketNicheMaterializationRepository
      marketNicheMaterializationRepository;
  ...
  public OprmGeneralAudienceService(
      ...
      OprmGeneralAudienceMarketNicheMaterializationRepository
          marketNicheMaterializationRepository) {
  ...
  public GeneralAudienceSeedResponse createSeed(CreateGeneralAudienceSeedRequest request) {
    ...
    seed.setStatus(
        request.status() == null ? OprmGeneralAudienceSeedStatus.DRAFT : request.status());
   ...
   */
  @Transactional
  public GeneralAudienceSeedResponse updateSeed(
      Long seedId, UpdateGeneralAudienceSeedRequest request) {
  ...
  public GeneralAudienceSubnicheResponse createSubniche(
      Long seedId, CreateGeneralAudienceSubnicheRequest request) {
    OprmGeneralAudienceSeed seed = findSeed(seedId);
    ...
    subniche.setStatus(
        request.status() == null ? OprmGeneralAudienceSubnicheStatus.DISCOVERED : request.status());
  ...
  public GeneralAudienceSubnicheResponse updateSubniche(
      Long subnicheId, UpdateGeneralAudienceSubnicheRequest request) {
  ...
  public GeneralAudienceMarketNicheConversionResponse convertSubnicheToMarketNiche(
      Long subnicheId, ConvertGeneralAudienceSubnicheToMarketNicheRequest request) {
    ConvertGeneralAudienceSubnicheToMarketNicheRequest safeRequest =
        request == null
    ...
    var marketNiche =
        marketNicheMaterializationRepository.saveMarketNiche(
  ...
  private GeneralAudienceSubnicheSummaryResponse toSubnicheSummaryResponse(
  ...
  private String materializedDescription(
      OprmGeneralAudienceSubniche subniche, String overrideDescription) {
  ...
  private String materializedBaseSegmentation(
      OprmGeneralAudienceSubniche subniche, String overrideBaseSegmentation) {
    if (StringUtils.hasText(overrideBaseSegmentation)) {
      return overrideBaseSegmentation.trim();
    }
    return "Público Geral OPRM: "
        + subniche.getSeed().getName()
        + " > "
        + subniche.getName()
        + ". País: "
        + subniche.getSeed().getCountry()
        + ". Idioma: "
        + subniche.getSeed().getLanguage()
        + ". Triagem obrigatória: "
        + requiredText(subniche.getQualificationQuestion(), "qualificationQuestion");
  }
  ...
  private String materializedInterests(
      OprmGeneralAudienceSubniche subniche, String overrideInterests) {
  ...
  private String materializedDemographicFilters(
      OprmGeneralAudienceSubniche subniche, String overrideDemographicFilters) {
    if (StringUtils.hasText(overrideDemographicFilters)) {
      return overrideDemographicFilters.trim();
    }
    return "Usar demografia apenas quando fizer sentido para "
        + subniche.getName()
