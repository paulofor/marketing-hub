...
/**
 * Serviço responsável por persistir emails associados a CNAEs dos estabelecimentos da Receita no
 * OPRM.
 */
@Service
...
public class OprmEstabelecimentoCnaeRaizService {
  ...
  public OprmEstabelecimentoCnaeRaizBatchResponseDto upsertBatch(
      OprmEstabelecimentoCnaeRaizBatchRequestDto request) {
    if (request == null
        || request.estabelecimentos() == null
        || request.estabelecimentos().isEmpty()) {
      return new OprmEstabelecimentoCnaeRaizBatchResponseDto(0, 0);
    }
    List<OprmEstabelecimentoCnaeRaizUpsertDto> rows =
        normalizeAndValidate(request.estabelecimentos());
    if (rows.isEmpty()) {
    ...
    try {
      ...
      log.error(
          "Falha ao persistir lote de estabelecimentos OPRM. received={} valid={} sqlTentada='{}' exceptionClass={} exceptionMessage={} errorLine={}",
    ...
    log.info(
        "Lote de estabelecimentos OPRM persistido com sucesso. received={} persisted={}",
        request.estabelecimentos().size(),
        rows.size());
    return new OprmEstabelecimentoCnaeRaizBatchResponseDto(
  ...
  private List<OprmEstabelecimentoCnaeRaizUpsertDto> normalizeAndValidate(
      List<OprmEstabelecimentoCnaeRaizUpsertDto> inputRows) {
