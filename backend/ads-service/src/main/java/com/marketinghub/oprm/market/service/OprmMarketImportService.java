...
import com.marketinghub.repository.jpa.oprm.market.OprmCnpjImportFileRepository;
import com.marketinghub.repository.jpa.oprm.market.OprmCnpjImportRunRepository;
...
/**
 * Serviço responsável por orquestrar o ciclo de ingestão OPRM CNPJ/CNAE, incluindo abertura de run,
 * registro de eventos de arquivos, consolidação e finalização.
 */
public class OprmMarketImportService {

  private static final String UPSERT_MARKET_SIZE_SQL =
      "insert into oprm_market_size_by_cnae (avg_socios_por_empresa,total_empresas,total_empresas_mei,total_empresas_simples,total_estabelecimentos,total_estabelecimentos_ativos,updated_at,cnae_code,snapshot_date) values (?,?,?,?,?,?,?,?,?)";
        ...
        request.files().stream()
            .map(
                seed -> {
                  ...
                  file.setStartedAt(
                      seed.startedAt() != null ? seed.startedAt() : request.startedAt());
  ...
  public void registerFileEvent(Long runId, Long fileId, OprmImportFileEventRequestDto request) {
    OprmCnpjImportRun run =
        runRepository
            .findById(runId)
            .orElseThrow(
                () -> new ResponseStatusException(NOT_FOUND, "Import run not found: " + runId));
    OprmCnpjImportFile file =
        fileRepository
            .findById(fileId)
            .orElseThrow(
                () -> new ResponseStatusException(NOT_FOUND, "Import file not found: " + fileId));
    ...
    log.info(
        "[runId={} fileId={}] registerFileEvent status={} rowsRead={} rowsValid={} rowsRejected={} cnaesReceived={} marketSizesReceived={} finishedAt={}",
        runId,
        fileId,
        file.getStatus(),
        file.getRowsRead(),
    ...
    if (request.cnaes() != null && !request.cnaes().isEmpty()) {
      for (OprmCnaeUpsertDto row : request.cnaes()) {
        OprmCnpjCnaeDim item =
            cnaeDimRepository.findById(row.cnaeCode()).orElseGet(OprmCnpjCnaeDim::new);
    ...
    if (request.marketSizes() != null && !request.marketSizes().isEmpty()) {
      log.info(
          "[runId={} fileId={}] Consolidando marketSizes para snapshotDate={} totalRegistros={}",
          runId,
          fileId,
          run.getSnapshotDate(),
      ...
      for (OprmMarketSizeUpsertDto row : request.marketSizes()) {
        ...
        log.info(
            "[runId={} fileId={}] Preparando upsert marketSize: snapshotDate={} cnaeCodeRaw='{}' cnaeCodeNormalized='{}' cnaeCodeLength={} totalEstabelecimentos={} totalEstabelecimentosAtivos={} totalEmpresas={} totalEmpresasMei={} totalEmpresasSimples={} avgSociosPorEmpresa={}",
            runId,
            fileId,
            run.getSnapshotDate(),
            row.cnaeCode(),
        ...
        OprmMarketSizeByCnae item =
            marketSizeRepository.findById(id).orElseGet(OprmMarketSizeByCnae::new);
        item.setId(id);
        item.setTotalEstabelecimentos(
            row.totalEstabelecimentos() != null ? row.totalEstabelecimentos() : 0L);
        item.setTotalEstabelecimentosAtivos(
            row.totalEstabelecimentosAtivos() != null ? row.totalEstabelecimentosAtivos() : 0L);
        ...
        item.setTotalEmpresasSimples(
            row.totalEmpresasSimples() != null ? row.totalEmpresasSimples() : 0L);
        ...
        try {
          ...
          log.error(
              "[runId={} fileId={}] Falha ao persistir marketSize: snapshotDate={} cnaeCodeRaw='{}' cnaeCodeNormalized='{}' cnaeCodeLength={} payload={} sqlTentada='{}'.",
              runId,
              fileId,
              run.getSnapshotDate(),
              row.cnaeCode(),
      ...
      log.info(
          "[runId={} fileId={}] Consolidacao marketSizes persistida com sucesso. snapshotDate={} totalRegistros={}",
          runId,
          fileId,
          run.getSnapshotDate(),
          request.marketSizes().size());
  ...
  public void completeRun(Long runId, OprmCompleteImportRunRequestDto request) {
    log.info(
        "[OPRM-TOTALIZACAO] completeRun recebido. runId={}, requestedStatus={}, requestedFinishedAt={}, requestedFilesProcessed={}, requestedRowsRead={}, requestedRowsValid={}, requestedRowsRejected={}, requestedErrorMessage={}",
        runId,
        request.status(),
        request.finishedAt(),
    ...
    if (files.isEmpty()) {
      log.warn(
          "[OPRM-TOTALIZACAO] completeRun abortado: runId={} não possui arquivos vinculados.",
          runId);
      throw new ResponseStatusException(
          BAD_REQUEST, "Import run has no files to finalize: " + runId);
    }

    long startedCount =
        files.stream().filter(file -> "STARTED".equalsIgnoreCase(file.getStatus())).count();
    long startedEstablishmentsCount =
        files.stream()
            .filter(file -> "STARTED".equalsIgnoreCase(file.getStatus()))
            .filter(file -> "ESTABELECIMENTOS".equalsIgnoreCase(file.getDatasetType()))
            .count();
    long failedCount =
        files.stream().filter(file -> "FAILED".equalsIgnoreCase(file.getStatus())).count();
    long completedCount =
        files.stream().filter(file -> "COMPLETED".equalsIgnoreCase(file.getStatus())).count();
    long partialCount =
        files.stream().filter(file -> "PARTIAL".equalsIgnoreCase(file.getStatus())).count();

    log.info(
        "[OPRM-TOTALIZACAO] Estado pré-finalização runId={}: filesTotal={}, started={}, completed={}, partial={}, failed={}",
        runId,
        files.size(),
        startedCount,
        completedCount,
    ...
    if (startedEstablishmentsCount > 0) {
      String message =
          "Finalização bloqueada: existem "
              + startedEstablishmentsCount
      ...
      if ("STARTED".equalsIgnoreCase(file.getStatus())) {
        ...
        if (file.getErrorMessage() == null || file.getErrorMessage().isBlank()) {
          file.setErrorMessage(
              "File closed during run finalization without explicit completion event.");
        }
      }
    ...
    run.setFilesProcessed(
        request.filesProcessed() != null ? request.filesProcessed() : files.size());
    ...
    log.info(
        "[OPRM-TOTALIZACAO] completeRun persistido. runId={}, finalStatus={}, finishedAt={}, filesProcessed={}, rowsRead={}, rowsValid={}, rowsRejected={}, errorMessage={}",
        runId,
        run.getStatus(),
        run.getFinishedAt(),
  ...
  public void finalizeLatestStartedRun(String triggerLabel) {
    ...
    OprmCnpjImportRun run =
        runRepository.findFirstByStatusOrderByStartedAtDesc("STARTED").orElse(null);
    if (run == null) {
      log.info(
          "[OPRM-TOTALIZACAO] Nenhum run STARTED encontrado para finalização automática. triggerLabel={} executionAtUtc={}",
          triggerLabel,
          executionAt);
      return;
    }
    ...
    log.info(
        "[OPRM-TOTALIZACAO] Finalização automática iniciada. triggerLabel={} runId={} snapshotDate={} startedAt={} status={}",
        triggerLabel,
        run.getId(),
        run.getSnapshotDate(),
        run.getStartedAt(),
    ...
    completeRun(
        run.getId(),
        new OprmCompleteImportRunRequestDto(
            null,
            executionAt,
            null,
            null,
    ...
    log.info(
        "[OPRM-TOTALIZACAO] Finalização automática concluída com sucesso. triggerLabel={} runId={}",
        triggerLabel,
        run.getId());
  }
  ...
  private void validateFileBelongsToRun(Long runId, OprmCnpjImportFile file) {
    if (file.getRun() == null
        || file.getRun().getId() == null
        || !runId.equals(file.getRun().getId())) {
      throw new ResponseStatusException(BAD_REQUEST, "File does not belong to informed run.");
    }
  }

  /**
   * Resolve o status final da run priorizando o status solicitado e fallback por presença de
   * falhas.
   */
  ...
  public OprmTopCnaeMarketVolumeDto getLatestCnaeMarketVolume(String cnaeCode) {
    return marketSizeRepository
        .findLatestSnapshotByCnaeCode(cnaeCode)
        .orElseThrow(
            () ->
                new ResponseStatusException(
                    NOT_FOUND, "CNAE não encontrado no snapshot mais recente: " + cnaeCode));
  }
}
