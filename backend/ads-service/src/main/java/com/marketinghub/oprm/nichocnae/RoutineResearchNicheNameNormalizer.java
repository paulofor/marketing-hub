...
/**
 * Normaliza nomes de nicho CNAE para impedir que linguagem de solução direcione a pesquisa de
 * rotina.
 */
public class RoutineResearchNicheNameNormalizer {
  ...
  private static final Pattern SEPARATOR_PREFIX =
      Pattern.compile("^[\\s:;,.\\-–—]+|[\\s:;,.\\-–—]+$");
  private static final List<Pattern> SOLUTION_PREFIXES =
      List.of(
  ...
  /**
   * Cria um padrão de prefixo tolerante a maiúsculas/minúsculas e acentuação nos termos
   * cadastrados.
   */
  private static Pattern caseInsensitivePrefix(String expression) {
    return Pattern.compile(
        "^\\s*(?:" + expression + ")\\b\\s*", Pattern.CASE_INSENSITIVE | Pattern.UNICODE_CASE);
  }

  /** Resultado estruturado da normalização do nome usado para abrir o ciclo de pesquisa. */
  public record NormalizedNicheName(
      String originalNicheName, String neutralNicheName, BigDecimal solutionLanguageRiskScore) {}
}
