...
/**
 * Serviço responsável por transformar nicho enriquecido OPRM em sinais iniciais pesquisáveis pela
 * Meta Ads.
 */
@Service
public class OprmEnrichedNicheMetaSignalService {
  ...
  /**
   * Monta o pacote de sinais Meta Ads a partir do CNAE, nome neutro e evidências do cartão de
   * rotina.
   */
  public MetaSignalPackage buildSignalPackage(
      OprmRoutineResearchCycle cycle, OprmNicheRoutineCard card) {
    ...
    return new MetaSignalPackage(
        limit(interests, MAX_INTERESTS), limit(roles, MAX_ROLES), limit(behaviors, MAX_BEHAVIORS));
  }

  /** Adiciona sinais de dicionários por CNAE para os melhores nichos já ranqueados pelo OPRM. */
  private void addDictionarySignals(
      String cnaeCode, Set<String> interests, Set<String> roles, Set<String> behaviors) {
  ...
  private void addBeautySignals(Set<String> interests, Set<String> roles, Set<String> behaviors) {
    ...
    addAll(
        ...
        "Proprietário de salão de beleza");
  ...
  private void addFashionRetailSignals(
      Set<String> interests, Set<String> roles, Set<String> behaviors) {
    addAll(
        interests,
        "Loja de roupas",
        "Moda feminina",
  ...
  private void addSalesPromotionSignals(
      Set<String> interests, Set<String> roles, Set<String> behaviors) {
    addAll(
        interests,
        "Vendas",
  ...
  private void addAdministrativeServicesSignals(
      Set<String> interests, Set<String> roles, Set<String> behaviors) {
    addAll(
        interests,
        "Administração",
  ...
  private void addPassengerTransportSignals(
      Set<String> interests, Set<String> roles, Set<String> behaviors) {
    addAll(
        interests,
  ...
  private void addAestheticServicesSignals(
      Set<String> interests, Set<String> roles, Set<String> behaviors) {
    addAll(
        interests,
        "Estética",
        ...
        "Beleza");
    addAll(
        roles, "Esteticista", "Designer de sobrancelhas", "Depiladora", "Profissional de estética");
    addAll(behaviors, "Small business owners", "Engaged Shoppers", "Facebook Page admins");
  }

  /** Adiciona sinais específicos de serviços domésticos. */
  private void addDomesticServicesSignals(
      Set<String> interests, Set<String> roles, Set<String> behaviors) {
    addAll(
        interests,
        "Serviços domésticos",
  ...
  private void addOperationalTextSignals(
      OprmNicheRoutineCard card, Set<String> interests, Set<String> roles, Set<String> behaviors) {
    if (card == null) {
      return;
    }
    String text =
  ...
  /**
   * Adiciona sinais comerciais de baixa contaminação úteis para profissionais autônomos e pequenos
   * negócios.
   */
  ...
  /**
   * Monta resumo textual dos sinais para exibição em campos do backend consumidos pelo Facebook
   * Ads.
   */
   ...
   */
  public record MetaSignalPackage(
      List<String> interests, List<String> roles, List<String> behaviors) {}
}
