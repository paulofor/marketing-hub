...
/**
 * Responsável por selecionar o próximo nicho CNAE com score alto e abrir o ciclo de pesquisa de
 * rotina.
 */
@Service
public class BackendRoutineResearchOrchestratorService {
  private static final Logger LOGGER =
      LoggerFactory.getLogger(BackendRoutineResearchOrchestratorService.class);
  ...
  private static final String CYCLE_STATUS_NEEDS_EXECUTOR_ROUTINE_EVIDENCE =
  ...
  private static final String CYCLE_STATUS_CANCELLED_BY_MANUAL_RESTART =
  ...
  private final RoutineResearchNicheNameNormalizer nicheNameNormalizer =
      new RoutineResearchNicheNameNormalizer();
  ...
  public List<RecordRoutineResearchOrchestratorPending> listPending() {
    ...
    List<RecordRoutineResearchOrchestratorPending> pendingCandidates =
        nicheCandidateRepository
            ...
            .toList();
    LOGGER.info(
        "Prévia de pendência da etapa zero OPRM nichocnae retornada (count={})",
  ...
  public List<RecordRoutineResearchOrchestratorRecent> listRecentProcessed(int limit) {
    ...
    LOGGER.info(
        "Listando ciclos recentes da etapa zero OPRM nichocnae (requestedLimit={}, safeLimit={})",
        limit,
        safeLimit);
    List<RecordRoutineResearchOrchestratorRecent> recentCycles =
        routineResearchCycleRepository
            ...
            .toList();
    LOGGER.info(
        "Ciclos recentes da etapa zero OPRM nichocnae retornados (count={})", recentCycles.size());
    return recentCycles;
  }

  /**
   * Lista jobs por status para o executor decidir reprocessamentos com aprendizado da tentativa
   * anterior.
   */
  @Transactional(readOnly = true)
  public List<RecordRoutineResearchOrchestratorRecent> listJobsByStatus(
    ...
    LOGGER.info(
        ...
        limit,
        safeLimit);
    List<RecordRoutineResearchOrchestratorRecent> jobs =
        routineResearchCycleRepository
  ...
  /**
   * Reabre o mesmo job e limpa os artefatos das etapas que serão executadas novamente com novo
   * input.
   */
  @Transactional
  public RecordRoutineResearchOrchestratorReprocessResult reprocessCycle(
        ...
        nicheCandidateRepository
            ...
            .orElseThrow(
                () ->
                    new ResponseStatusException(
                        HttpStatus.NOT_FOUND,
                        "Niche candidate not found: " + cycle.getSourceNicheId()));
    ...
    OprmNicheRoutineCard previousQualityCard =
        routineCardRepository.findFirstByResearchCycleIdOrderByIdDesc(cycle.getId()).orElse(null);
    String triggerSource = resolveReprocessTriggerSource(request);
    Instant now = Instant.now();
    preserveCurrentAiCostBeforeRerun(cycle);
    RecordRoutineResearchKnowledgeSnapshot knowledgeSnapshot =
        buildKnowledgeSnapshot(cycle.getId(), previousCycleStatus, previousQualityCard);
    RecordRoutineResearchOrchestratorReprocessPlan reprocessPlan =
        planReprocess(cycle.getId(), previousCycleStatus, knowledgeSnapshot);
    clearArtifactsForStageRerun(cycle.getId(), reprocessPlan);
    reopenCycleForStageRerun(
        cycle, previousCycleStatus, previousQualityCard, triggerSource, now, reprocessPlan);
  ...
  private void preserveCurrentAiCostBeforeRerun(OprmRoutineResearchCycle cycle) {
    ...
    BigDecimal preservedCost =
        cycle.getReprocessPreservedCostUsd() == null
            ? BigDecimal.ZERO
  ...
  /**
   * Remove artefatos derivados para que as etapas do próprio job sejam executadas novamente com
   * novo input.
   */
  ...
  /**
   * Atualiza o ciclo existente para que a fila volte à primeira etapa reexecutável sem criar outro
   * job.
   */
  private void reopenCycleForStageRerun(
    ...
    cycle.setErrorMessage(
        buildStageRerunLearningNote(
            previousCycleStatus, previousQualityCard, triggerSource, reprocessPlan));
    cycle.setUpdatedAt(now);
  }

  /**
   * Monta a nota auditável que mantém o aprendizado do gate mesmo após limpar artefatos
   * reexecutáveis.
   */
  ...
  private RecordRoutineResearchKnowledgeSnapshot buildKnowledgeSnapshot(
    ...
    List<Long> acceptedSignalIds =
        signals.stream().filter(this::isAcceptedSignal).map(OprmExtractedSignal::getId).toList();
  ...
  private RecordRoutineResearchOrchestratorReprocessPlan planReprocess(
    ...
    boolean preserveAcceptedEvidence =
        shouldPreserveAcceptedEvidence(previousCycleStatus, knowledgeSnapshot);
  ...
  /**
   * Identifica sinais negativos que apontam fonte/claim contaminado e não devem guiar a próxima
   * síntese.
   */
  ...
  /**
   * Define se a reprovação deve preservar evidência aceita e voltar apenas ao planejador de
   * queries.
   */
  ...
  private List<String> resolveEvidenceGaps(
      String previousCycleStatus, OprmNicheRoutineCard previousQualityCard) {
  ...
  /**
   * Normaliza a origem solicitada pelo executor externo sem permitir valores arbitrários no
   * histórico.
   */
  private String resolveReprocessTriggerSource(
      RecordRoutineResearchOrchestratorReprocessRequest request) {
  ...
  /**
   * Informa se o ciclo terminal permite nova pesquisa manual para sair de falha, material fraco ou
  ...
  private String buildReprocessMessage(String previousStatus) {
    ...
    if (CYCLE_STATUS_NEEDS_MORE_RESEARCH.equals(previousStatus)
        || CYCLE_STATUS_NEEDS_MORE_MEI_RESEARCH.equals(previousStatus)) {
  ...
  /**
   * Executa a etapa zero para o CNAE escolhido, encerrando ciclos abertos antes de criar uma
   * execução nova.
   */
  @Transactional
  public RecordRoutineResearchOrchestratorResult runForCnae(String cnaeCode) {
    LOGGER.info(
        "Iniciando etapa zero OPRM nichocnae por CNAE manual com reinício forçado (cnaeCode={})",
        cnaeCode);
    List<OprmNicheCandidate> candidates =
        nicheCandidateRepository.findManualRoutineResearchCandidateByCnaeCode(
            cnaeCode, PageRequest.of(0, 1));
    if (candidates.isEmpty()) {
      LOGGER.info(
          "Nenhum candidato com score encontrado para acionamento manual do CNAE (cnaeCode={})",
          cnaeCode);
      throw new ResponseStatusException(
          HttpStatus.NOT_FOUND,
          "Não existe candidato de nicho com score para iniciar um novo pipeline deste CNAE: "
              + cnaeCode);
    }
  ...
  /**
   * Encerra ciclos ainda abertos do CNAE para impedir que execuções antigas continuem concorrendo
   * com o novo ciclo manual.
   */
  private void finishOpenCyclesForManualRestart(String cnaeCode) {
    ...
    openCycles.forEach(
        cycle -> {
          ...
          cycle.setUpdatedAt(now);
          cycle.setErrorMessage(
              "Ciclo encerrado automaticamente para reinício manual completo do CNAE "
                  + cnaeCode
                  + ".");
        });
  ...
  public RecordRoutineResearchOrchestratorResult runNext() {
    LOGGER.info(
        "Iniciando etapa zero OPRM nichocnae no backend: buscando próximo candidato pendente com bloqueio pessimista (limit=1)");
    List<OprmNicheCandidate> candidates =
        nicheCandidateRepository.findNextPendingRoutineResearchCandidate(PageRequest.of(0, 1));
    LOGGER.info(
        "Busca de candidato pendente da etapa zero OPRM nichocnae concluída (count={})",
        candidates.size());
    if (candidates.isEmpty()) {
      LOGGER.info(
          "Nenhum candidato pendente encontrado para etapa zero OPRM nichocnae; ciclo não será criado.");
  ...
  /**
   * Cria ciclo de pesquisa para um candidato já selecionado e atualiza o status operacional do
   * nicho.
   */
  private RecordRoutineResearchOrchestratorResult startCycleForCandidate(
      OprmNicheCandidate candidate, String triggerSource) {
    LOGGER.info(
  ...
  private OprmRoutineResearchCycle createCycle(OprmNicheCandidate candidate, Instant now) {
    NormalizedNicheName normalizedName =
        nicheNameNormalizer.normalize(
  ...
  private RecordRoutineResearchOrchestratorRecent toRecentProcessed(
      OprmRoutineResearchCycle cycle) {
    OprmMeiAudienceProfile audienceProfile =
        meiAudienceProfileRepository
            .findFirstByResearchCycleIdOrderByIdDesc(cycle.getId())
            .orElse(null);
    OprmNicheRoutineCard routineCard =
        routineCardRepository.findFirstByResearchCycleIdOrderByIdDesc(cycle.getId()).orElse(null);
  ...
  /**
   * Identifica o nicho de mercado já associado ao ciclo para evitar linguagem de criação duplicada
   * na UI.
   */
  private Long resolveExistingMarketNicheId(OprmRoutineResearchCycle cycle) {
    Long candidateMarketNicheId =
        nicheCandidateRepository
            .findById(cycle.getSourceNicheId())
