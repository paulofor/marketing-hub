...
/**
 * Expõe contratos de leitura/escrita do backend para a etapa enriched-niche-materializer do
...
public class BackendEnrichedNicheMaterializerService {
  ...
  /**
   * Registra conclusão do materializador de nicho enriquecido usando a decisão informada pelo
   * executor externo.
   */
  @Transactional
  ...
  public EnrichedNicheMaterializerFailureResponse fail(
      Long stageExecutionId, EnrichedNicheMaterializerFailureRequest request) {
    OprmNichoCnaeV2StageExecution execution = findExecution(stageExecutionId);
  ...
  /**
   * Grava uma pendência da etapa de materializador de nicho enriquecido solicitada pelo executor
   * externo.
   */
  @Transactional
  public EnrichedNicheMaterializerCreateResponse create(
      EnrichedNicheMaterializerCreateRequest request) {
    OprmNichoCnaeV2StageExecution saved =
        stageExecutionRepository.save(toPendingExecution(request));
    return new EnrichedNicheMaterializerCreateResponse(
        ...
        .orElseThrow(
            () ->
                new ResponseStatusException(
                    ...
                    "NichoCNAE v2 enriched-niche-materializer stage execution not found: "
                        + stageExecutionId));
  }
  ...
  private EnrichedNicheMaterializerPendingResponse toPendingResponse(
