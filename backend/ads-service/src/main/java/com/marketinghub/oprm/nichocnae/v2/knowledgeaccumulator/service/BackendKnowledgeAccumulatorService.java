...
/**
 * Expõe contratos de leitura/escrita do backend para a etapa knowledge-accumulator do pipeline
 * NichoCNAE v2.
 */
@Service
public class BackendKnowledgeAccumulatorService {
  ...
  /**
   * Registra conclusão da acumulação de conhecimento usando a próxima etapa informada pelo executor
   * externo.
   */
  @Transactional
  ...
  public KnowledgeAccumulatorFailureResponse fail(
      Long stageExecutionId, KnowledgeAccumulatorFailureRequest request) {
    OprmNichoCnaeV2StageExecution execution = findExecution(stageExecutionId);
  ...
  private OprmNichoCnaeV2StageExecution toPendingExecution(
      KnowledgeAccumulatorCreateRequest request) {
  ...
  private OprmNichoCnaeV2StageExecution createTechnicalRetry(
    ...
    OprmNichoCnaeV2StageExecution retry =
        toPendingExecution(
            new KnowledgeAccumulatorCreateRequest(
        ...
        .orElseThrow(
            () ->
                new ResponseStatusException(
                    ...
                    "NichoCNAE v2 knowledge-accumulator stage execution not found: "
                        + stageExecutionId));
  }
  ...
  private KnowledgeAccumulatorPendingResponse toPendingResponse(
