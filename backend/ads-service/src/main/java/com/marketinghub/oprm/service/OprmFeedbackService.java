...
import com.marketinghub.repository.jpa.oprm.OprmFeedbackHistoryRepository;
import com.marketinghub.repository.jpa.oprm.OprmFeedbackSnapshotRepository;
...
public class OprmFeedbackService {
  ...
  public void publishFeedback(OprmFeedbackPublishRequestDto request) {
    ...
    OprmFeedbackSnapshot snapshot =
        OprmFeedbackSnapshot.builder()
            .job(job)
    ...
    OprmFeedbackHistory history =
        OprmFeedbackHistory.builder()
            .occupationName(request.occupationName())
            .personaLabel(request.personaLabel())
            .correlationId(request.correlationId())
            .generatedAt(generatedAt)
            .previousRoutineConfidence(
                decimalFromScore(request.scoreReweighting(), "previous_routine_confidence"))
            .recalibratedRoutineConfidence(
                decimalFromScore(request.scoreReweighting(), "recalibrated_routine_confidence"))
            .previousFrameworkConfidence(
                decimalFromScore(request.scoreReweighting(), "previous_framework_confidence"))
            .recalibratedFrameworkConfidence(
                decimalFromScore(request.scoreReweighting(), "recalibrated_framework_confidence"))
            .averageHypothesisImpact(
                decimalFromScore(request.scoreReweighting(), "average_hypothesis_impact"))
            .notes("feedback snapshot persisted from OPRM worker")
            .build();
    feedbackHistoryRepository.save(history);
    log.info(
        "oprm-feedback-published jobId={} correlationId={} occupationName={} personaLabel={} generatedAt={}",
        job.getId(),
        request.correlationId(),
        request.occupationName(),
        request.personaLabel(),
        generatedAt);
  }
  ...
  public List<OprmFeedbackHistoryEntryDto> listHistory(String occupationName, String personaLabel) {
    return feedbackHistoryRepository
        .findByOccupationNameIgnoreCaseAndPersonaLabelIgnoreCaseOrderByGeneratedAtAsc(
            occupationName, personaLabel)
        .stream()
        .map(
            entry ->
                new OprmFeedbackHistoryEntryDto(
  ...
  private Instant parseGeneratedAt(String generatedAt) {
    try {
      ...
      throw new ResponseStatusException(
          HttpStatus.UNPROCESSABLE_ENTITY, "generatedAt must use ISO-8601 format");
    }
  }
  ...
  private String toJson(Object value) {
    try {
      ...
      throw new ResponseStatusException(
          HttpStatus.UNPROCESSABLE_ENTITY, "unable to serialize OPRM feedback payload");
    }
  }
