...
public class OprmOccupationService {
  ...
  public OprmOccupationResponseDto createOccupation(OprmOccupationUpsertRequestDto request) {
    ...
    repository
        .findByOccupationSeedRefIgnoreCase(normalizedSeedRef)
        .ifPresent(
            existing -> {
              throw new ResponseStatusException(
                  HttpStatus.CONFLICT, "occupationSeedRef already exists");
            });

    OprmOccupation saved =
        repository.save(
            OprmOccupation.builder()
  ...
  @Transactional
  public OprmOccupationResponseDto updateOccupation(
      UUID id, OprmOccupationUpsertRequestDto request) {
    OprmOccupation occupation =
        repository
            .findById(id)
            .orElseThrow(
                () ->
                    new ResponseStatusException(HttpStatus.NOT_FOUND, "OPRM occupation not found"));
  ...
  private String normalizeSeedRef(String value) {
    ...
    if (normalized.isBlank()) {
      throw new ResponseStatusException(
          HttpStatus.UNPROCESSABLE_ENTITY, "occupationSeedRef is required");
    }
    return normalized;
  }
  ...
  private String writeAliasesJson(List<String> aliases) {
    ...
    try {
      ...
      throw new ResponseStatusException(
          HttpStatus.UNPROCESSABLE_ENTITY, "aliases payload is invalid");
    }
  }
