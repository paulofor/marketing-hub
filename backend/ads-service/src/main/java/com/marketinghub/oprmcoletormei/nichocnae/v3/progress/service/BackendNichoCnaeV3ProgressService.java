...
public class BackendNichoCnaeV3ProgressService {
  private static final Logger log =
      LoggerFactory.getLogger(BackendNichoCnaeV3ProgressService.class);
  ...
  public NichoCnaeV3JobProgressResponse latestByCnae(String cnaeCode) {
    return repository
        .findTop1ByCnaeCodeAndStageCodeOrderByCreatedAtDesc(cnaeCode, "cnae-intake")
        .map(
            latest -> {
              List<OprmNichoCnaeV3StageExecution> executions =
                  repository.findByJobIdOrderByCreatedAtAsc(latest.getJobId());
  ...
  public void confirmFinalization(String cnaeCode) {
    OprmNichoCnaeV3StageExecution latest =
        repository
            .findTop1ByCnaeCodeAndStageCodeOrderByCreatedAtDesc(cnaeCode, "cnae-intake")
            .orElseThrow(
                () ->
                    new ResponseStatusException(
                        HttpStatus.NOT_FOUND, "Job v3 não encontrado para este CNAE."));
    OprmNichoCnaeV3StageExecution qualityGate =
        repository
            .findByJobIdAndStageCode(latest.getJobId(), QUALITY_GATE_STAGE)
            .orElseThrow(
                () ->
                    new ResponseStatusException(
                        HttpStatus.CONFLICT, "Quality gate ainda não foi concluído."));
    ...
    if (!qualityGateApproved(qualityGate)) {
      throw new ResponseStatusException(
          HttpStatus.CONFLICT,
          "Quality gate bloqueou a materialização; reprocessar busca/coleta antes de finalizar.");
    }
    if (!repository.existsByJobIdAndStageCode(latest.getJobId(), FINAL_STAGE)) {
      materializerService.create(
          latest.getJobId(),
          latest.getCnaeCode(),
          qualityGate.getOutputPayload(),
          qualityGate.getAttemptNumber(),
          qualityGate.getKnowledgeVersion());
    }
  }

  /** Monta a revisão final quando a etapa 9 terminou e a etapa 10 ainda aguarda confirmação. */
  private NichoCnaeV3FinalizationReviewResponse buildFinalizationReview(
      String jobId, List<OprmNichoCnaeV3StageExecution> executions) {
    OprmNichoCnaeV3StageExecution qualityGate =
        executions.stream()
            .filter(e -> QUALITY_GATE_STAGE.equals(e.getStageCode()))
            .findFirst()
            .orElse(null);
    boolean hasFinalStage = executions.stream().anyMatch(e -> FINAL_STAGE.equals(e.getStageCode()));
    if (qualityGate == null
        || qualityGate.getStatus() != OprmNichoCnaeV3StageExecutionStatus.COMPLETED
        || hasFinalStage) {
      return null;
    }
    if (!qualityGateApproved(qualityGate)) {
  ...
  private JsonNode parseOutput(String outputPayload) {
    ...
    try {
      ...
      log.warn(
          "Falha ao ler payload de progresso NichoCNAE v3. modulo=OPRM etapa=progress tamanhoPayload={}",
  ...
  private String buildNicheInformation(JsonNode output, String cnaeDescription) {
    return "Descrição CNAE: "
        + cnaeDescription
        + "\n\nRotina observada: "
        + textOrDefault(firstText(output, "routineSummary", "routine", "rotina"), "Não informada.")
        + "\n\nTarefas diárias: "
        + textOrDefault(
  ...
  private String buildEnrichedInformation(JsonNode output, String fallback) {
    return "Persona: "
        + textOrDefault(firstText(output, "personaSummary", "persona"), "Não informada.")
        + "\n\nEvidências: "
        + textOrDefault(firstText(output, "evidenceSummary", "evidences"), fallback)
        + "\n\nGatilhos comerciais: "
        + textOrDefault(firstText(output, "commercialTriggers", "triggers"), "Não informados.")
        + "\n\nObjeções: "
        + textOrDefault(firstText(output, "objections"), "Não informadas.");
  }
}
