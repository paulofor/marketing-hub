...
/**
 * Base compartilhada para services canônicos de etapas NichoCNAE v3 sem assumir execução
 * operacional.
 */
public abstract class OprmNichoCnaeV3StageServiceSupport {
  private static final Logger LOGGER =
      LoggerFactory.getLogger(OprmNichoCnaeV3StageServiceSupport.class);
  ...
  private final OpenAiTextResponseExtractor responseExtractor =
      new OpenAiTextResponseExtractor(objectMapper);
  private final OpenAiRequestInputExtractor requestInputExtractor =
      new OpenAiRequestInputExtractor(objectMapper);
  ...
  private void markCnaePipelineStarted(
      String cnaeCode, String currentStageCode, String statusStarted) {
    OprmCnpjCnaeDim cnae =
        cnaeRepository
            .findById(cnaeCode)
            .orElseThrow(
                () -> new ResponseStatusException(HttpStatus.NOT_FOUND, "CNAE não encontrado."));
  ...
  /**
   * Recebe o request bruto da etapa, atualiza o CNAE para aguardar o módulo e audita o payload no
   * pipeline NichoCNAE.
   */
  protected PipelineNichoCnae doRecebeRequest(
      String cnaeCode, String jobId, OprmNichoCnaeV3RecebeRequestRequest request) {
    if (!StringUtils.hasText(jobId)) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "jobId é obrigatório no recebimento do request.");
    }
    OprmCnpjCnaeDim cnae =
        cnaeRepository
            .findById(cnaeCode)
  ...
  /**
   * Recebe o response bruto da etapa, conclui ou falha o CNAE e audita o retorno no pipeline
   * NichoCNAE.
   */
  protected OprmNichoCnaeV3RecebeResponseResponse doRecebeResponse(
      String cnaeCode,
      String jobId,
      OprmNichoCnaeV3RecebeResponseRequest request,
      String nextStageCode) {
    if (!StringUtils.hasText(jobId)) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "jobId é obrigatório no recebimento do response.");
    }
    OprmCnpjCnaeDim cnae =
        cnaeRepository
            .findById(cnaeCode)
  ...
  /**
   * Calcula o custo autoritativo no backend e usa custo legado apenas quando não há dados
   * suficientes.
   */
  ...
  /**
   * Converte contagem de tokens recebida do executor para inteiro aceito pelo serviço canônico de
   * preços.
   */
  ...
  protected OprmNichoCnaeV3StageExecution doCreate(
      String jobId,
      String cnaeCode,
      String inputPayload,
  ...
  protected List<OprmCnpjCnaeDim> pendingCnaes() {
    return cnaeRepository
        .findByNichocnaeCurrentStageCodeAndNichocnaePipelineStatusOrderByNichocnaePipelineUpdatedAtAsc(
            stageCode, STATUS_STARTED, PageRequest.of(0, PENDING_LIMIT));
  }

  /**
   * Recupera a execução mais recente do CNAE na etapa corrente para preservar o contrato de
   * callback.
   */
  protected Optional<OprmNichoCnaeV3StageExecution> pendingExecution(OprmCnpjCnaeDim cnae) {
    return repository.findTop1ByCnaeCodeAndStageCodeOrderByCreatedAtDesc(
        cnae.getCnaeCode(), stageCode);
  }
  ...
  /**
   * Registra conclusão reportada pelo executor externo e publica a próxima etapa no cadastro do
   * CNAE.
   */
  protected OprmNichoCnaeV3StageExecution doComplete(
  ...
  private boolean createNextStageWhenAllowed(
      OprmNichoCnaeV3StageExecution current, String outputPayload, String nextStageCode) {
    String normalizedNextStage = normalizedStage(nextStageCode);
  ...
  private String inputPayloadFallback(String cnaeCode, String cnaeDescription) {
    String safeDescription =
        cnaeDescription == null ? "" : cnaeDescription.replace("\\", "\\\\").replace("\"", "\\\"");
  ...
  private boolean requiresUserConfirmation(
      OprmNichoCnaeV3StageExecution current, String nextStageCode) {
    return "quality-gate".equals(current.getStageCode())
        && "persona-routine-materializer".equals(nextStageCode);
  }
  ...
  private boolean isAllowedNextStage(String nextStageCode) {
    if ("persona-tournament".equals(stageCode)
        && "persona-routine-materializer".equals(nextStageCode)) {
      return true;
    }
  ...
  private OprmNichoCnaeV3StageExecution find(Long stageExecutionId) {
    OprmNichoCnaeV3StageExecution execution =
        repository
            .findById(stageExecutionId)
            .orElseThrow(
                () ->
                    new ResponseStatusException(
                        HttpStatus.NOT_FOUND, "Execução v3 não encontrada."));
