...
public class BackendNichoCnaeV3PipelineSituacaoService {
  ...
  /**
   * Retorna os registros de auditoria que coincidem com etapa, id externo e qualquer status
   * informado.
   */
  public List<NichoCnaeV3PipelineSituacaoResponse> search(
      String etapa, String idExterno, List<String> status) {
    ...
    if (normalizedStatus.isEmpty()) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "status deve conter ao menos uma situação.");
    }
    return repository
        .findByCodigoEtapaAndIdExternoAndStatusInOrderByDataHoraDesc(
            etapa, idExterno, normalizedStatus)
        .stream()
        .map(this::toResponse)
        .toList();
  }
  ...
  private List<String> normalizeStatus(List<String> status) {
    ...
    return status.stream().filter(StringUtils::hasText).map(String::trim).distinct().toList();
  }
