...
/**
 * Sincronizador idempotente que repara divergências simples entre contrato oficial e banco
 * operacional.
 */
@Component
public class PipelineDefinitionSynchronizer {
  ...
  /**
   * Inicializa o sincronizador com persistência centralizada, registry canônico e diagnóstico do
   * serviço.
   */
  @Autowired
  ...
  /**
   * Sincroniza com segurança um pipeline já existente, preservando campos operacionais
   * configuráveis.
   */
  @Transactional
  ...
  public PipelineSyncResultDto rebuildOfficialStages(Long pipelineId) {
    ...
    definition.stages().stream()
        ...
        .forEach(
            stageDefinition -> {
              PipelineStage stage =
                  createStageFromSnapshot(pipeline, definition, stageDefinition, existingStages);
  ...
  /**
   * Localiza pipeline oficial já salvo pelo código canônico ou alias recebido antes de criar novo
   * registro.
   */
  private Pipeline findExistingOfficialPipeline(
      String requestedCode, PipelineDefinition definition) {
  ...
  private void synchronizePipelineFields(
      Pipeline pipeline, PipelineDefinition definition, List<String> appliedActions) {
    if (definition.pipelineFieldPolicy().nameStructural()
        && !definition.name().equals(pipeline.getName())) {
  ...
  /**
   * Cria ou corrige uma etapa oficial sem alterar modelo OpenAI, descrição operacional ou ativo
   * configurado.
   */
  private void synchronizeStage(
    ...
    if (definition.stageFieldPolicy().nameStructural()
        && !stageDefinition.name().equals(stage.getName())) {
      stage.setName(stageDefinition.name());
      appliedActions.add("Nome da etapa corrigido: " + stageDefinition.operationalCode());
    }
    if (definition.stageFieldPolicy().positionStructural()
        && !stage.getPosition().equals(stageDefinition.position())) {
      stage.setPosition(stageDefinition.position());
      appliedActions.add(
          "Posição canônica da etapa corrigida: " + stageDefinition.operationalCode());
    }
    if (definition.stageFieldPolicy().requiredStructural()
        && stage.isRequired() != stageDefinition.required()) {
      stage.setRequired(stageDefinition.required());
      appliedActions.add(
          "Obrigatoriedade estrutural da etapa corrigida: " + stageDefinition.operationalCode());
    }
    if (!sameText(stageDefinition.executionModule(), stage.getExecutionModule())) {
      ...
      appliedActions.add(
          "Módulo executor da etapa corrigido: " + stageDefinition.operationalCode());
    }
  ...
  /**
   * Cria uma etapa oficial mínima, deixando campos operacionais sem configuração específica do
   * usuário.
   */
  ...
  /**
   * Cria uma etapa oficial reutilizando descrição e modelo OpenAI de uma etapa compatível anterior
   * quando existir.
   */
  ...
  private Optional<PipelineStage> findConfiguredStageSnapshot(
      ...
      PipelineStageDefinition stageDefinition,
      List<PipelineStage> existingStages) {
    Optional<PipelineStage> officialMatch =
    ...
    Map<String, List<String>> aliases =
        Map.of(
            ...
            "LANDING_PAGE_IMAGE_GENERATION",
                List.of("image-generation", "framework-image-generation"),
            "LANDING_PAGE_DESIGN_PRESET", List.of("preset-design"),
            "LANDING_PAGE_DELIVERABLES",
                List.of(
                    "landing-html", "geralanding-html", "landing-page-html", "LANDING_PAGE_HTML"));
  ...
  private Pipeline createPipeline(PipelineDefinition definition) {
    Pipeline pipeline =
        Pipeline.builder()
            ...
            .description(
                "Pipeline oficial sincronizado a partir de " + definition.canonicalVersion())
            .active(true)
  ...
  /**
   * Detecta divergências que poderiam causar perda de histórico ou sobrescrita operacional
   * perigosa.
   */
  private List<PipelineDiagnosticsIssueDto> destructiveIssues(
  ...
  private PipelineSyncResultDto blockedResult(
      PipelineDiagnosticsDto diagnostics, List<String> appliedActions) {
    return PipelineSyncResultDto.builder()
        .status("BLOQUEADO")
        .synchronizedSafely(false)
        .pipelineId(diagnostics.pipelineId())
