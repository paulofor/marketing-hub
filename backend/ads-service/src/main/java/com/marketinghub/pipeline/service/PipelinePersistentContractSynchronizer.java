...
/**
 * Sincroniza a separação persistente entre definição canônica de pipeline e configuração
...
public class PipelinePersistentContractSynchronizer {
  ...
  /**
   * Inicializa o sincronizador persistente com repositórios centralizados de definição e
   * configuração.
   */
  ...
  /**
   * Sincroniza uma definição oficial persistente e cria configurações operacionais sem sobrescrever
   * escolhas do usuário.
   */
  ...
  private PipelineDefinitionEntity createPipelineDefinition(PipelineDefinition definition) {
    PipelineDefinitionEntity entity =
  ...
  /**
   * Sincroniza definição persistente de etapa e garante uma configuração operacional editável para
   * ela.
   */
  private void syncStageDefinition(
      ...
      Pipeline pipeline) {
    PipelineStageDefinitionEntity entity =
        stageDefinitionRepository
            .findByPipelineDefinitionIdAndCanonicalCode(
                pipelineDefinition.getId(), stageDefinition.canonicalCode())
  ...
  /**
   * Atualiza campos estruturais persistentes da etapa, preservando configuração operacional em
   * tabela separada.
   */
  ...
  /**
   * Cria ou mantém configuração operacional, copiando valores legados apenas quando ainda não
   * existe configuração.
   */
  private void syncStageConfig(
      PipelineStageDefinitionEntity stageDefinition, PipelineStage configuredStage) {
    configRepository
        .findByPipelineStageDefinitionId(stageDefinition.getId())
        .orElseGet(
            () -> {
              PipelineStageConfig config =
                  PipelineStageConfig.builder()
                      ...
                      .openAiModel(
                          configuredStage == null ? null : configuredStage.getOpenAiModel())
                      .descriptionOverride(
                          configuredStage == null ? null : configuredStage.getDescription())
  ...
  /**
   * Localiza a etapa operacional legada equivalente para migrar configuração sem tocar nos campos
   * estruturais.
   */
  private PipelineStage findConfiguredStage(
      Pipeline pipeline, PipelineStageDefinition stageDefinition) {
    return pipeline.getStages().stream()
        .filter(
            stage ->
                stageDefinition.operationalCode().equals(stage.getCode())
