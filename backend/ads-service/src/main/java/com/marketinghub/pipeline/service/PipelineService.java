...
public class PipelineService {
  private static final Map<String, List<String>> GERA_LANDING_STAGE_ALIASES =
  ...
  public void deleteStage(Long pipelineId, Long stageId) {
    ...
    PipelineDefinition definition =
        definitionRegistry.findByPipelineCode(stage.getPipeline().getCode()).orElse(null);
    if (definition != null
        && definitionRegistry
            .findStage(definition, stage.getCode())
            .filter(PipelineStageDefinition::required)
            .isPresent()) {
  ...
  public PipelineMetadataDto metadata() {
    return PipelineMetadataDto.builder()
        ...
        .officialPipelines(
            definitionRegistry.officialPipelines().stream().map(this::toOfficialDto).toList())
        .build();
  }
  ...
  public PipelineDiagnosticsDto diagnostics(Long id) {
    ...
    PipelineDefinition definition =
        definitionRegistry.findByPipelineCode(pipeline.getCode()).orElse(null);
    if (definition == null) {
          ...
          .issues(
              List.of(
                  issue(
                      ...
                      null,
                      null,
                      "Pipeline não possui definição oficial no backend.",
    ...
    for (PipelineStage stage : pipeline.getStages()) {
      validateStageAgainstDefinition(
          definition, stage, configuredCodes, configuredPositions, issues);
    }
    for (PipelineStageDefinition expected : definition.stages()) {
      ...
      if (!present) {
        issues.add(
            issue(
                "ERROR",
                expected.operationalCode(),
                expected.canonicalCode(),
  ...
  private PipelineStage findStageInPipeline(Long pipelineId, Long stageId) {
    PipelineStage stage =
        stageRepository
            .findById(stageId)
            .orElseThrow(
                () -> new EntityNotFoundException("Etapa de pipeline não encontrada: " + stageId));
  ...
  private void validateOfficialPipelineCreate(PipelineRequest request) {
    PipelineDefinition definition =
        definitionRegistry.findByPipelineCode(request.getCode()).orElse(null);
    if (definition != null && !definition.module().equals(request.getModule())) {
      throw badRequest(
          "Pipeline oficial deve usar o módulo canônico "
              + definition.module()
              + ": "
              + request.getCode());
    }
  ...
  private void validateOfficialPipelineUpdate(Pipeline pipeline, PipelineRequest request) {
    ...
    PipelineDefinition definition =
        definitionRegistry.findByPipelineCode(pipeline.getCode()).orElseThrow();
    if (!definitionRegistry
        .normalize(pipeline.getCode())
        .equals(definitionRegistry.normalize(request.getCode()))) {
  ...
  private void validateStageRequest(
      Pipeline pipeline, PipelineStage currentStage, PipelineStageRequest request) {
    ...
    PipelineStageDefinition requestedDefinition =
        definitionRegistry
            .findStage(definition, request.getCode())
            .orElseThrow(
                () ->
                    badRequest(
                        "Etapa não mapeia para definição canônica oficial: " + request.getCode()));
    if (currentStage != null) {
      PipelineStageDefinition currentDefinition =
          definitionRegistry
              .findStage(definition, currentStage.getCode())
              .orElseThrow(
                  () ->
                      badRequest(
                          "Etapa atual não mapeia para definição canônica oficial: "
                              + currentStage.getCode()));
      if (!currentDefinition.equals(requestedDefinition)) {
        throw badRequest(
            "Código estrutural de etapa oficial não pode ser alterado: " + currentStage.getCode());
      }
    }
    if (requestedDefinition.required() && !request.isRequired()) {
      throw badRequest(
          "Etapa obrigatória oficial não pode deixar de ser obrigatória: " + request.getCode());
    }
    if (requestedDefinition.required() && !request.isActive()) {
      throw badRequest(
          "Etapa obrigatória oficial não pode ser desativada sem regra explícita: "
              + request.getCode());
    }
    if (!java.util.Objects.equals(
        requestedDefinition.executionModule(),
        normalizeOptionalText(request.getExecutionModule()))) {
      throw badRequest(
          "Módulo executor de etapa oficial não pode divergir do contrato: " + request.getCode());
    }
    if (!java.util.Objects.equals(
        requestedDefinition.rootPackage(), normalizeOptionalText(request.getRootPackage()))) {
      throw badRequest(
          "Pacote raiz de etapa oficial não pode divergir do contrato: " + request.getCode());
    }
  }

  /** Impede duplicidade de posição ou código operacional antes da constraint do banco. */
  private void ensureUniqueStagePositionAndCode(
      Pipeline pipeline, PipelineStage currentStage, PipelineStageRequest request) {
  ...
  private void validateStageAgainstDefinition(
    ...
    PipelineStageDefinition stageDefinition =
        definitionRegistry.findStage(definition, stage.getCode()).orElse(null);
    ...
    if (!configuredCodes.add(normalizedCode)) {
      issues.add(
          issue(
              "ERROR",
              stage.getCode(),
              stageDefinition == null ? null : stageDefinition.canonicalCode(),
    ...
    return OfficialPipelineDto.builder()
        ...
        .stages(
            definition.stages().stream()
                .map(stage -> toOfficialStageDto(definition, stage))
                .toList())
        .build();
  }
  ...
  private GeraLandingStageModelDto toGeraLandingStageModelDto(
      String stageCode, List<PipelineStage> activeStages) {
    return activeStages.stream()
        .filter(stage -> matchesGeraLandingStage(stage, stageCode))
        .filter(stage -> stage.getOpenAiModel() != null)
        .findFirst()
        .map(
            stage -> buildGeraLandingStageModelDto(stageCode, stage, stage.getOpenAiModel(), false))
  ...
  private GeraLandingStageModelDto buildGeraLandingStageModelDto(
    ...
    GeraLandingStageModelDto.GeraLandingStageModelDtoBuilder builder =
        GeraLandingStageModelDto.builder()
            .stageCode(stageCode)
  ...
  private String resolveDefaultModelCode(String stageCode) {
    return "landing-page-image-generation".equals(stageCode)
        ? IMAGE_DEFAULT_MODEL_CODE
        : TEXT_DEFAULT_MODEL_CODE;
  }
  ...
  private String resolveDefaultModelName(String stageCode) {
    return "landing-page-image-generation".equals(stageCode)
        ? IMAGE_DEFAULT_MODEL_NAME
        : TEXT_DEFAULT_MODEL_NAME;
  }
  ...
  private static Map<String, List<String>> buildGeraLandingStageAliases() {
    ...
    aliases.put(
        ...
        List.of("landing-page-wireframe", "LANDING_PAGE_WIREFRAME", "landing-wireframe"));
    aliases.put(
        "landing-page-copy", List.of("landing-page-copy", "LANDING_PAGE_COPY", "landing-copy"));
    aliases.put(
  ...
  private OpenAiModel resolveOpenAiModel(Long openAiModelId) {
    ...
    return openAiModelRepository
        .findById(openAiModelId)
  ...
  private void sortStages(Pipeline pipeline) {
    pipeline
        .getStages()
        .sort(Comparator.comparing(PipelineStage::getPosition).thenComparing(PipelineStage::getId));
  }
}
