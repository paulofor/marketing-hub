...
/**
 * Responsabilidade: expor contratos de leitura, escrita e auditoria da etapa Imagem do pipeline
 * GeraAnuncio v2.
 */
@Service
public class GeraAnuncioImagemService {
  ...
  /**
   * Inicia uma solicitação da etapa Imagem para o candidato CNAE informado e cria o jobId UUID da
   * execução.
   */
  @Transactional
  ...
  public void recebeRequest(
      String experimentKey, String jobId, GeraAnuncioImagemRecebeRequestRequest request) {
    if (request == null) {
  ...
  public String recebeResponse(
      String experimentKey, String jobId, GeraAnuncioImagemRespostaRequest request) {
    if (request == null) {
      throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "payload required");
    }
    if (!StringUtils.hasText(jobId)) {
  ...
  public List<GeraAnuncioImagemSituacaoResponse> consultaSituacao(
      String idExterno, GeraAnuncioImagemSituacaoRequest request) {
  ...
  public GeraAnuncioImagemDetailResponse detailStageExecution(String stageExecutionId) {
    ...
    if (experimentId == null) {
      return new GeraAnuncioImagemDetailResponse(
  ...
  private GeraAnuncioImagemPendingResponse toPending(Experiment experiment) {
    return new GeraAnuncioImagemPendingResponse(
        stageExecutionId(experiment),
        experiment.getId(),
        stageJobId(experiment),
        experiment.getCreativeGenerationRequestedAt(),
