...
public class CommercialPlanService {
  ...
  public CommercialPlanMilestone updateMilestone(
      Long planId, Long milestoneId, UpdateCommercialPlanMilestoneRequest request) {
    CommercialPlanMilestone milestone =
        milestoneRepository
    ...
    if (!milestone.getPlan().getId().equals(planId)) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "Marco nao pertence ao plano informado.");
    }
  ...
  @Transactional
  public CommercialPlanSimulation simulate(
      Long planId, CreateCommercialPlanSimulationRequest request) {
    CommercialPlan plan = getPlan(planId);
    List<CommercialPlanMilestone> milestones =
        milestoneRepository.findByPlanIdOrderBySequenceOrderAsc(planId);
    CommercialPlanSimulation simulation =
        buildSimulation(plan, milestones, request.decisionNotes());
  ...
  private void createDefaultMilestones(CommercialPlan plan) {
    for (int index = 0; index < DEFAULT_MILESTONES.size(); index++) {
      ...
      milestoneRepository.save(
          CommercialPlanMilestone.builder()
              .plan(plan)
  ...
  private CommercialPlan syncExecution(CommercialPlan plan) {
    List<CommercialPlanMilestone> milestones =
        milestoneRepository.findByPlanIdOrderBySequenceOrderAsc(plan.getId());
    if (milestones == null) {
      milestones = List.of();
    }
  ...
  private CommercialPlanSimulation buildSimulation(
      CommercialPlan plan, List<CommercialPlanMilestone> milestones, String decisionNotes) {
    boolean missingCommercialGate =
        hasCriticalPlanningGap(
            ...
            plan.getDeadline());
    boolean hasBlockedMilestone =
        milestones.stream()
            .anyMatch(milestone -> milestone.getStatus() == CommercialPlanMilestoneStatus.BLOCKED);
    CommercialPlanRecommendation recommendation =
        missingCommercialGate || hasBlockedMilestone
            ? CommercialPlanRecommendation.CORRECT
            : CommercialPlanRecommendation.CONTINUE;
    String blocker =
        firstText(
            plan.getCurrentBlocker(), "criterio comercial incompleto ou gargalo ainda nao isolado");
    return CommercialPlanSimulation.builder()
        .plan(plan)
        .recommendation(recommendation)
        .mostLikelyScenario(
            missingCommercialGate
        ...
        .bestRealisticScenario(
            "A proxima acao valida uma dor, oferta ou friccao do funil sem aumentar complexidade.")
        .worstLikelyScenario("Mais artefatos ou midia sao criados sem corrigir o gargalo real.")
        .mainRisk(
            hasBlockedMilestone
                ? blocker
                : "Interpretar baixo resultado como problema de mercado antes de validar oferta, formulario e mensuracao.")
        .bestNextAction(bestNextAction(plan, missingCommercialGate, blocker))
        .actionToAvoid(
            "Criar nova campanha, nova landing ou novo criativo antes de fechar o criterio de decisao e o gargalo atual.")
        .continueCondition(
            firstText(
                plan.getSuccessCriteria(),
                "existir sinal comercial forte dentro do prazo definido"))
        .stopCondition(
            firstText(
                plan.getStopCriteria(),
                "atingir limite de acessos, gasto ou prazo sem nova evidencia comercial"))
        ...
        .evidence30Days(
            "Decidir continuidade, correcao, pausa ou encerramento com base em evidencia persistida.")
  ...
  private String bestNextAction(
      CommercialPlan plan, boolean missingCommercialGate, String blocker) {
    ...
    return firstText(
        plan.getNextAction(),
        "executar o menor teste capaz de gerar evidencia de compra ou intencao clara");
  }
  ...
  private MarketNiche resolveNiche(Long nicheId) {
    ...
    return nicheRepository
        ...
        .orElseThrow(
            () ->
                new ResponseStatusException(
                    HttpStatus.NOT_FOUND, "Nicho nao encontrado: " + nicheId));
  }
  ...
  private Hypothesis resolveHypothesis(UUID hypothesisId) {
    ...
    return hypothesisRepository
        ...
        .orElseThrow(
            () ->
                new ResponseStatusException(
                    HttpStatus.NOT_FOUND, "Hipotese nao encontrada: " + hypothesisId));
  }
  ...
  private Experiment resolveExperiment(Long experimentId) {
    ...
    return experimentRepository
        ...
        .orElseThrow(
            () ->
                new ResponseStatusException(
                    HttpStatus.NOT_FOUND, "Experimento nao encontrado: " + experimentId));
  }
  ...
  private void ensurePlanExists(Long planId) {
    if (!planRepository.existsById(planId)) {
      throw new ResponseStatusException(
          HttpStatus.NOT_FOUND, "Plano comercial nao encontrado: " + planId);
    }
  }
