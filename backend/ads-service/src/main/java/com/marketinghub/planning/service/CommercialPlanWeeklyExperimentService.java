...
public class CommercialPlanWeeklyExperimentService {
  ...
  private static final List<String> JULY_FIRST_WEEK_DEFAULT_OBJECTIVES =
      List.of(
  ...
  /**
   * Lista as semanas comerciais do mes do plano com experimentos do periodo e objetivos da proxima
   * semana.
   */
  ...
  public List<CommercialPlanWeekDto> listWeeks(Long planId, String referenceMonth) {
    ...
    for (int index = 0; index < periods.size(); index++) {
      ...
      boolean objectivesEditable =
          planReferenceMonth && hasNextWeek && isObjectiveEditWindowOpen(period.endDate());
      Integer objectiveWeekNumber = weekNumber + 1;
      weeks.add(
          new CommercialPlanWeekDto(
              weekNumber,
              period.startDate(),
              period.endDate(),
              experiments.size(),
              ...
              planReferenceMonth && hasNextWeek
                  ? listObjectives(plan, objectiveWeekNumber)
                  : List.of(),
              experiments));
    }
    return weeks;
  ...
  public List<CommercialPlanWeekObjectiveDto> updateObjectives(
      Long planId, Integer weekNumber, UpdateCommercialPlanWeekObjectivesRequest request) {
    CommercialPlan plan = planService.getPlan(planId);
    WeekPeriod period = resolveWeekPeriod(plan, weekNumber);
    if (!isObjectiveEditWindowOpen(period.endDate())) {
      throw new ResponseStatusException(
          HttpStatus.FORBIDDEN, objectiveEditWindowMessage(period.endDate(), false, true));
    }
    Integer objectiveWeekNumber = weekNumber + 1;
  ...
  private WeekPeriod resolveWeekPeriod(CommercialPlan plan, Integer weekNumber) {
    ...
    throw new ResponseStatusException(
        HttpStatus.BAD_REQUEST, "Semana do planejamento fora do mes do plano.");
  }
  ...
  private String objectiveEditWindowMessage(
      LocalDate weekEndDate, boolean editable, boolean planReferenceMonth) {
  ...
  private YearMonth resolveReferenceMonth(String referenceMonth, YearMonth planMonth) {
    ...
    try {
      ...
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "Mes de referencia invalido. Use o formato yyyy-MM.", ex);
    }
  }

  /**
   * Monta semanas comerciais pela primeira, segunda, terceira, quarta e quinta segunda-feira do
   * mes.
   */
  ...
  private List<CommercialPlanWeekObjectiveDto> listObjectives(
      CommercialPlan plan, Integer weekNumber) {
  ...
  private List<CommercialPlanWeekExperimentDto> listExperiments(
  ...
  private CommercialPlanWeekExperimentDto mapExperiment(ResultSet rs, int rowNum)
      throws SQLException {
    ...
    BigDecimal videoCost =
        money(currencyConversionService.usdToBrl(rs.getBigDecimal("video_cost_usd")));
  ...
  private List<CommercialPlanFunnelStageDto> buildFunnelStages(
      List<CommercialPlanWeekExperimentDto> experiments) {
    ...
    Instant lastEventAt =
        experiments.stream()
            ...
            .orElse(null);
    List<FunnelStageMetric> metrics =
        List.of(
    ...
    for (FunnelStageMetric metric : metrics) {
      BigDecimal conversion =
          metric.applicable() ? conversionFromPrevious(metric.actualTotal(), previousActual) : null;
      BigDecimal costPerConversion =
          metric.applicable() ? costPerConversion(totalCost, metric.actualTotal()) : null;
      result.add(
          new CommercialPlanFunnelStageDto(
              metric.code(),
              metric.name(),
              null,
              metric.actualTotal(),
  ...
  private record FunnelStageMetric(
      String code, String name, Long actualTotal, boolean applicable, String evidenceSource) {}
}
