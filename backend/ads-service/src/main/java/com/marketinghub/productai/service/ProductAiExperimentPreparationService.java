...
public class ProductAiExperimentPreparationService {
  ...
  public ProductAiExperimentPreparationDto prepare(UUID hypothesisId) {
    Hypothesis hypothesis =
        hypothesisRepository
            ...
            .orElseThrow(
                () -> new ResponseStatusException(HttpStatus.NOT_FOUND, "hypothesis not found"));
    return buildPreparation(hypothesis);
  }

  /**
   * Bloqueia criação de experimento Produto IA quando a hipótese ainda não tem rastreabilidade
   * mínima.
   */
  @Transactional(readOnly = true)
  ...
  /**
   * Bloqueia criação de experimento quando a hipótese preparada tem subtipo diferente do
   * experimento.
   */
  @Transactional(readOnly = true)
  public void assertReadyForExperiment(UUID hypothesisId, ProductAiSubtype expectedSubtype) {
    ...
    if (expectedSubtype != null && preparation.productAiSubtype() != expectedSubtype) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST,
          "Hipótese preparada para "
              + preparation.productAiSubtype()
              + ", mas o experimento solicitou "
              + expectedSubtype);
    }
  }

  /**
   * Completa uma hipótese rastreada do sistema para o MVP de amostra personalizada sem criar
   * experimento manual.
   */
  @Transactional
  ...
  public PersonalizedSamplePreparationDto prepareProductAiHypothesis(
      UUID hypothesisId, ProductAiSubtype requestedSubtype) {
    Hypothesis hypothesis =
        hypothesisRepository
            .findById(hypothesisId)
            .orElseThrow(
    ...
    ProductAiSubtype subtype =
        requestedSubtype != null ? requestedSubtype : ProductAiSubtype.AI_PERSONALIZED_SAMPLE;
  ...
  private ProductAiExperimentPreparationDto buildPreparation(Hypothesis hypothesis) {
    ...
    ProductAiExperimentPreparationDto.ProductAiExperimentDraftDto draft =
        ready
  ...
  /**
   * Bloqueia preparo quando a hipótese ainda não foi gerada com base comercial suficiente pelo
   * sistema.
   */
  ...
  /**
   * Resolve se o preparo atualiza a hipótese original ou cria uma variante paralela para preservar
   * o funil atual.
   */
  ...
  /**
   * Cria uma hipótese variante com a mesma base comercial e subtipo diferente para comparação de
   * funis.
   */
  ...
  private String buildVariantTitle(Hypothesis source, ProductAiSubtype subtype) {
    String suffix =
        subtype == ProductAiSubtype.AI_VISUAL_PREVIEW ? " - Prévia paga" : " - " + subtype.name();
  ...
  private DeliverablePackage ensureOfferPackage(Hypothesis hypothesis, ProductAiSubtype subtype) {
    ...
    DeliverablePackage offerPackage =
