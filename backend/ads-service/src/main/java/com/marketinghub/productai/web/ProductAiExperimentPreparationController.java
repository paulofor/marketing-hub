...
/**
 * Responsabilidade: expor o preparo sistêmico de hipóteses Produto IA antes da criação de
 * experimento.
 */
@RestController
...
public class ProductAiExperimentPreparationController {
  ...
  /**
   * Completa a hipótese existente para o MVP de amostra personalizada antes da criação do
   * experimento.
   */
  @PostMapping("/hypotheses/{hypothesisId}/personalized-sample-preparation")
  public PersonalizedSamplePreparationDto preparePersonalizedSample(
      @PathVariable UUID hypothesisId) {
    return service.preparePersonalizedSampleHypothesis(hypothesisId);
  }

  /**
   * Completa ou deriva a hipótese existente para o subtipo Produto IA informado antes do
   * experimento.
   */
  ...
  public PersonalizedSamplePreparationDto prepareProductAiHypothesis(
    ...
    return service.prepareProductAiHypothesis(
        hypothesisId, request != null ? request.productAiSubtype() : null);
  }
}
