...
public class PromptDomainService {
  ...
  public PromptDomainService(
      PromptDomainRepository repository,
  ...
  private void validateCreate(CreatePromptDomainRequest request) {
    if (request == null) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "Dados obrigatórios não informados");
    }
    if (!StringUtils.hasText(request.getCode())) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "O código do domínio é obrigatório");
    }
    if (!StringUtils.hasText(request.getName())) {
    ...
    if (request.getObjects() == null || request.getObjects().isEmpty()) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "Selecione ao menos um objeto para o domínio");
    }
    String normalizedCode = normalizeCode(request.getCode());
    if (repository.existsByCodeIgnoreCase(normalizedCode)) {
      throw new ResponseStatusException(
          HttpStatus.CONFLICT, "Já existe um domínio com este código");
    }
  }
