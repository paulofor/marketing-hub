...
public class ProofArtifactService {
  ...
  public ProofArtifactService(
      ProofArtifactRepository repository,
    ...
    ProofArtifact artifact =
        repository
            ...
            .orElseThrow(
                () ->
                    new ResponseStatusException(
                        HttpStatus.NOT_FOUND, "Proof artifact not found: " + id));
  ...
  private void applyPayload(ProofArtifact artifact, CreateProofArtifactRequest request) {
    ...
    artifact.setCustomType(
        StringUtils.hasText(request.getCustomType()) ? request.getCustomType().trim() : null);
    artifact.setAssetPlan(
        StringUtils.hasText(request.getAssetPlan()) ? request.getAssetPlan() : null);
    artifact.setAssetUrl(
        StringUtils.hasText(request.getAssetUrl()) ? request.getAssetUrl().trim() : null);
    artifact.setMessage(StringUtils.hasText(request.getMessage()) ? request.getMessage() : null);
    artifact.setDeliveryNotes(
        StringUtils.hasText(request.getDeliveryNotes()) ? request.getDeliveryNotes() : null);
  ...
  private VisualProof resolveVisualProof(Long id) {
    ...
    return visualProofRepository
        .findById(id)
        .orElseThrow(
            () ->
                new ResponseStatusException(HttpStatus.NOT_FOUND, "Visual proof not found: " + id));
  }
}
