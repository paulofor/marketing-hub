...
public interface ExperimentFinancialMetricRepository
    extends JpaRepository<ExperimentFinancialMetric, Long> {
  /** Lista as métricas financeiras vinculadas ao orçamento de experimento informado. */
  List<ExperimentFinancialMetric> findByExperimentBudgetIdOrderByMeasuredAtDesc(
      Long experimentBudgetId);

  /** Busca a métrica financeira manual mais recente do orçamento de experimento informado. */
  Optional<ExperimentFinancialMetric> findFirstByExperimentBudgetIdOrderByMeasuredAtDesc(
      Long experimentBudgetId);
}
