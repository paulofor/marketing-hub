...
public interface ExperimentPromiseGenerationRequestRepository
    extends JpaRepository<ExperimentPromiseGenerationRequest, Long> {
  ...
  List<ExperimentPromiseGenerationRequest> findByStatusOrderByCreatedAtAsc(
      ExperimentPromiseGenerationRequestStatus status, Pageable pageable);
  ...
  /**
   * Descarta em lote solicitações já usadas na criação de um experimento registrando o instante
   * informado.
   */
  @Modifying
  @Query(
  ...
  /**
   * Soma o custo em dólar de solicitações concluídas para compor o custo inicial do experimento
   * criado.
   */
  @Query(
      """
