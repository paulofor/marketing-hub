...
public interface FacebookAdsRecommendationRepository
    extends JpaRepository<FacebookAdsRecommendation, Long> {
  ...
  @Query(
      "select r from FacebookAdsRecommendation r where r.campaign.id = :campaignId order by r.collectedAt desc, r.id asc")
  List<FacebookAdsRecommendation> findByCampaignIdOrderByCollectedAtDescIdAsc(
