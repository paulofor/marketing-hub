...
/** Repositório JPA responsável pela persistência de ExperimentAdSetJob. */
public interface ExperimentAdSetJobRepository extends JpaRepository<ExperimentAdSetJob, Long> {

  List<ExperimentAdSetJob> findTop50ByWorkerAndStatusOrderByCreatedAtAsc(
      ExperimentAdSetWorker worker, ExperimentAdSetJobStatus status);

  Optional<ExperimentAdSetJob> findFirstByWorkflowIdAndTypeOrderByFinishedAtDesc(
      Long workflowId, ExperimentAdSetJobType type);
  ...
  long countByWorkflowIdAndTypeAndResourceId(
      Long workflowId, ExperimentAdSetJobType type, Long resourceId);

  boolean existsByWorkflowIdAndTypeAndStatusIn(
      Long workflowId, ExperimentAdSetJobType type, Collection<ExperimentAdSetJobStatus> statuses);

  boolean existsByWorkflowIdAndTypeAndResourceIdAndStatusIn(
      Long workflowId,
  ...
  @Modifying
  @Query(
      "update ExperimentAdSetJob j set j.status = :running, j.lockedBy = :workerId, j.lockedAt = :now, "
  ...
  @Modifying
  @Query(
      "update ExperimentAdSetJob j set j.status = :pending, j.lockedBy = null, j.lockedAt = null, j.startedAt = null "
