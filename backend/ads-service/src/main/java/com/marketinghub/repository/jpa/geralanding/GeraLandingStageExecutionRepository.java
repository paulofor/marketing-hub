...
public interface GeraLandingStageExecutionRepository
    extends JpaRepository<GeraLandingStageExecution, byte[]> {
  ...
  Optional<GeraLandingStageExecution> findTopByExperimentIdAndIdJobOrderByExecutionRequestedAtDesc(
      Long experimentId, byte[] idJob);
  ...
  List<GeraLandingStageExecution> findTop20ByStatusInOrderByExecutionRequestedAtAsc(
      List<String> statuses);

  /** Busca todas as execuções de um experimento em ordem cronológica para auditoria completa. */
  List<GeraLandingStageExecution> findByExperimentIdOrderByExecutionRequestedAtAsc(
      Long experimentId);
  ...
  List<GeraLandingStageExecution>
      findTop20ByStageCodeAndStatusAndExecutionRequestedAtBeforeOrderByExecutionRequestedAtAsc(
          String stageCode, String status, Instant executionRequestedAtBefore);
