...
public interface GeraSalesPageStageExecutionRepository
    extends JpaRepository<GeraSalesPageStageExecution, String> {
  ...
  List<GeraSalesPageStageExecution> findByExperimentIdOrderByExecutionRequestedAtAsc(
      Long experimentId);

  /** Lista execuções concluídas de uma etapa para backfill de snapshots de publicação. */
  List<GeraSalesPageStageExecution>
      findByExperimentIdAndStageCodeAndStatusOrderByExecutionRequestedAtAsc(
          Long experimentId, String stageCode, String status);

  /**
   * Lista pendências de uma etapa com experimento, nicho e hipótese carregados para montar o
   * contrato do worker.
   */
