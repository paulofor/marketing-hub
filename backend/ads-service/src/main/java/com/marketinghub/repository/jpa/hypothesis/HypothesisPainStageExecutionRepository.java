 ...
 */
public interface HypothesisPainStageExecutionRepository
    extends JpaRepository<HypothesisPainStageExecution, byte[]> {
  /** Busca a execução mais recente pelo identificador técnico do job. */
  Optional<HypothesisPainStageExecution> findTopByIdJobOrderByExecutionRequestedAtDesc(
      byte[] idJob);

  /** Busca a execução mais recente de uma etapa dentro de um nicho. */
  Optional<HypothesisPainStageExecution>
      findTopByMarketNicheIdAndStageCodeOrderByExecutionRequestedAtDesc(
          Long marketNicheId, String stageCode);
  ...
  Optional<HypothesisPainStageExecution>
      findTopByMarketNicheIdAndStageCodeAndStatusOrderByExecutionRequestedAtDesc(
          Long marketNicheId, String stageCode, String status);
  ...
  List<HypothesisPainStageExecution> findTop20ByStageCodeAndStatusOrderByExecutionRequestedAtAsc(
      String stageCode, String status);

  /** Busca execuções antigas sem conclusão para aplicação da política de lease operacional. */
  List<HypothesisPainStageExecution>
      findTop50ByStageCodeAndStatusInAndCompletedAtIsNullAndProcessingStartedAtBeforeOrderByProcessingStartedAtAsc(
          String stageCode, List<String> statuses, Instant threshold);

  /**
   * Lista todas as execuções de uma etapa dentro de um nicho com hipótese carregada para
   * totalização e contexto.
   */
  @EntityGraph(attributePaths = {"hypothesis"})
  List<HypothesisPainStageExecution> findByMarketNicheIdAndStageCodeOrderByExecutionRequestedAtDesc(
      Long marketNicheId, String stageCode);

  /**
   * Lista execuções vinculadas a uma hipótese para rastrear prompts/schemas usados no experimento.
   */
  List<HypothesisPainStageExecution> findByHypothesisIdOrderByExecutionRequestedAtAsc(
      UUID hypothesisId);
  ...
  List<HypothesisPainStageExecution>
      findByMarketNicheIdAndStageCodeAndHypothesisIdOrderByExecutionRequestedAtDesc(
          Long marketNicheId, String stageCode, UUID hypothesisId);

  /**
   * Lista as últimas vinte execuções de uma etapa dentro de um nicho excluindo um status
   * operacional.
   */
  List<HypothesisPainStageExecution>
