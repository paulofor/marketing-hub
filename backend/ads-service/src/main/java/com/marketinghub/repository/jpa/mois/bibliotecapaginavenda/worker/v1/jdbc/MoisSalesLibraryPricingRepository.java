...
public class MoisSalesLibraryPricingRepository implements MoisSalesLibraryPricingGateway {
  ...
  /**
   * Busca o preço batch por código normalizado e pelo código original para preservar
  ...
  public Optional<ModelPricing> findPricingByModelCode(String modelCode) {
    ...
    List<ModelPricing> rows =
        jdbcTemplate.query(
            ...
            (rs, rowNum) ->
                new ModelPricing(
                    rs.getBigDecimal("price_input_batch"), rs.getBigDecimal("price_output_batch")),
            normalizedCode,
            originalCode,
            normalizedCode);
