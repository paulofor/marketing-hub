...
public class DossierProductContextGateway {
        ...
        rs -> {
          ...
          return Optional.of(
              new DossierProductContext(
                  ...
                  rs.getString("hotmart_temperature"),
  ...
  public record DossierProductContext(
    ...
    /**
     * Verifica se há HTML ou descrições/resumos suficientes para o dossiê trabalhar com evidência
     * real.
     */
    public boolean hasCollectedPayload() {
      return htmlBytes > 0
          || hasText(rawHtml)
          || hasText(hotmartDescription)
          || hasText(offerSummary)
          || hasText(mechanismSummary)
          || hasText(promiseSummary)
          || hasText(proofSummary);
    }
