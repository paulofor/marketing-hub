...
public interface OprmCnaeProcessingCycleRepository
    extends JpaRepository<OprmCnaeProcessingCycle, String> {
  ...
  @Query(
      "select coalesce(max(c.cycleNumber), 0) + 1 from OprmCnaeProcessingCycle c where c.cycleType = :cycleType")
