...
public interface OprmNicheCandidateRepository extends JpaRepository<OprmNicheCandidate, Long> {
  ...
  @Query(
      "select c from OprmNicheCandidate c "
          ...
          + "and c.opportunityScore is not null "
          + "order by c.opportunityScore desc, c.createdAt asc")
  List<OprmNicheCandidate> findManualRoutineResearchCandidateByCnaeCode(
      @Param("cnaeCode") String cnaeCode, Pageable pageable);

  /**
   * Lista sem bloqueio pessimista os melhores candidatos ainda pendentes de pesquisa de rotina para
   * visualização.
   */
  @Query(
      "select c from OprmNicheCandidate c "
  ...
  /**
   * Lista candidatos de nicho enriquecidos priorizando maiores scores para acompanhamento no
   * frontend.
   */
  List<OprmNicheCandidate> findAllByOrderByOpportunityScoreDescCreatedAtDesc(Pageable pageable);

  /**
   * Lista candidatos iniciados na etapa atual da geração de anúncios, em ordem de solicitação mais
   * antiga.
   */
  List<OprmNicheCandidate>
