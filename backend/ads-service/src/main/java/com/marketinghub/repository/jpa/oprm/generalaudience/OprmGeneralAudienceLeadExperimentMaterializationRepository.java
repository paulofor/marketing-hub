...
/**
 * Repositório responsável por materializar experimentos de lead/isca do fluxo OPRM de público
 * geral.
 */
@Repository
  ...
  public OprmGeneralAudienceMaterializedLeadExperiment createLeadExperiment(
    ...
    if (hypothesis == null) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "hypothesisId não encontrado: " + hypothesisId);
    }
    if (hypothesis.getMarketNiche() == null
        || !marketNicheId.equals(hypothesis.getMarketNiche().getId())) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "Hipótese não pertence ao MarketNiche do subnicho geral");
    }
