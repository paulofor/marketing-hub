...
public interface ProductAiPaidDeliveryStageExecutionRepository
  ...
  Optional<ProductAiPaidDeliveryStageExecution> findTopByIdJobOrderByExecutionRequestedAtDesc(
      String idJob);

  /** Busca execução ativa mais recente de uma compra. */
  Optional<ProductAiPaidDeliveryStageExecution> findTopByPurchaseIdOrderByExecutionRequestedAtDesc(
      Long purchaseId);

  /** Lista pendências de etapa para consumo do worker. */
  List<ProductAiPaidDeliveryStageExecution>
      findTop20ByPipelineCodeAndStageCodeAndStatusOrderByExecutionRequestedAtAsc(
          String pipelineCode, String stageCode, String status);
}
