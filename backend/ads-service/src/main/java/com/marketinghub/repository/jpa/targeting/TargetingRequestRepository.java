...
/** Repositório JPA responsável pela persistência de TargetingRequest. */
public interface TargetingRequestRepository extends JpaRepository<TargetingRequest, UUID> {
  ...
  @Query(
      """
            ...
            """)
  List<TargetingRequest> findByFilters(
      @Param("status") TargetingRequestStatus status,
  ...
  @EntityGraph(attributePaths = {"candidates", "candidates.options"})
  Optional<TargetingRequest> findFirstByExperimentIdOrderByCreatedAtDesc(Long experimentId);
}
