...
public class LandingVideoSlotService {
  ...
  public LandingVideoSlotService(
  ...
  public LandingVideoSlotDto create(Long landingId, CreateLandingVideoSlotRequest request) {
            ...
            .orElseThrow(
                () ->
                    VideoModuleException.notFound(
                        VideoModuleErrorCode.LANDING_NOT_FOUND,
    ...
    slotRepository
        .findByLandingPageIdAndSlotName(landingId, request.getSlotName())
        .ifPresent(
            existing -> {
              throw VideoModuleException.conflict(
                  VideoModuleErrorCode.SLOT_CONFLICT, "Já existe um slot com esse nome na landing");
            });
    ...
    recordHistory(
        ...
        StringUtils.hasText(publishedBy)
            ? LandingVideoSlotChangeType.PUBLISHED
            : LandingVideoSlotChangeType.CREATED,
        null,
        publishedBy);
  ...
  public List<LandingVideoSlotDto> list(Long landingId) {
    ...
    return slotRepository.findByLandingPageIdAndTenantId(landingPage.getId(), tenantId).stream()
        .map(SalesVideoMapper::toDto)
        .toList();
  }

  @Transactional
  public LandingVideoSlotDto update(
    ...
    if (StringUtils.hasText(request.getSlotName())
        && !request.getSlotName().equals(slot.getSlotName())) {
      slotRepository
          .findByLandingPageIdAndSlotName(landingId, request.getSlotName())
          ...
          .ifPresent(
              existing -> {
                throw VideoModuleException.conflict(
                    VideoModuleErrorCode.SLOT_CONFLICT, "Já existe um slot com esse nome");
              });
  ...
  public List<LandingVideoSlotHistoryDto> history(Long landingId, Long slotId) {
    ...
    ensureSameLanding(slot, landingId);
    TenantContextHolder.assertTenant(slot.getTenantId());
    return historyRepository.findBySlotIdOrderByChangedAtDesc(slotId).stream()
        .map(SalesVideoMapper::toDto)
        .toList();
  }
  ...
  private SalesVideoProfile loadProfile(Long profileId) {
    return profileRepository
        ...
        .orElseThrow(
            () ->
                VideoModuleException.notFound(
                    VideoModuleErrorCode.PROFILE_NOT_FOUND, "Perfil não encontrado: " + profileId));
  }
        ...
        .orElseThrow(
            () ->
                VideoModuleException.badRequest(
                    VideoModuleErrorCode.ASSET_NOT_FOUND, "Asset não encontrado: " + id));
  }
  ...
  private void ensureSameLanding(LandingVideoSlot slot, Long landingId) {
    if (!slot.getLandingPage().getId().equals(landingId)) {
      throw VideoModuleException.badRequest(
          VideoModuleErrorCode.SLOT_TENANT_MISMATCH, "Slot não pertence à landing solicitada");
    }
  }

  private void recordHistory(
      LandingVideoSlot slot, LandingVideoSlotChangeType changeType, String notes, String actor) {
    LandingVideoSlotHistory history =
        LandingVideoSlotHistory.builder()
            .slot(slot)
  ...
  private void ensurePublicationCompliance(SalesVideoProfile profile, Asset asset) {
    ...
    if (!StringUtils.hasText(profile.getHumanReviewApprovedBy())
        || profile.getHumanReviewApprovedAt() == null) {
      throw VideoModuleException.conflict(
          VideoModuleErrorCode.COMPLIANCE_HUMAN_REVIEW_REQUIRED,
    ...
    if (profile.isRequiresConsent()
      ...
      throw VideoModuleException.conflict(
          VideoModuleErrorCode.COMPLIANCE_CONSENT_REQUIRED,
