...
public class SalesVideoAssetCleanupService {
  ...
  public SalesVideoAssetCleanupService(
  ...
  public int cleanup(Duration retention, int batchSize) {
    ...
    for (Map<String, Object> row : candidates) {
      ...
      if (jobRepository.existsByAnyAssetReference(assetId)
          || slotRepository.existsByAnyAssetReference(assetId)) {
        continue;
      }
  ...
  private void deleteStoredObject(Asset asset) {
    try {
      JsonNode payload =
          asset.getPayload() != null ? objectMapper.readTree(asset.getPayload()) : null;
      if (payload == null) {
        return;
      }
      String storedFile = payload.path("stored_file_name").asText(null);
      boolean storedInBucket =
          "CLOUDFLARE_R2".equalsIgnoreCase(payload.path("storage_medium").asText());
      if (storedFile != null) {
