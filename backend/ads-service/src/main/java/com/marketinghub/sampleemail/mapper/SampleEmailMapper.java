...
public interface SampleEmailMapper {
  ...
  @Mapping(
      target = "selected",
      expression =
          "java(email.getExperiment() != null && email.getExperiment().getSelectedSampleEmail() != null && email.getExperiment().getSelectedSampleEmail().getId() != null && email.getExperiment().getSelectedSampleEmail().getId().equals(email.getId()))")
