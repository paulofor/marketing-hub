package com.marketinghub.settings;

/** Centraliza os nomes canônicos das configurações gerais permitidas no Marketing Hub. */
public final class GeneralSettingKeys {
  public static final String PRIVACY_POLICY_URL = "privacy_policy_url";
  public static final String EMAIL_SERVICE_SMTP = "email_service_smtp";
  public static final String LEAD_PORTAL_EMAIL_TEMPLATE_HTML = "lead_portal_email_template_html";
  public static final String LEAD_PORTAL_EMAIL_TEMPLATE_SUBJECT =
      "lead_portal_email_template_subject";

  /** Impede instanciação da classe utilitária. */
  private GeneralSettingKeys() {}
}
