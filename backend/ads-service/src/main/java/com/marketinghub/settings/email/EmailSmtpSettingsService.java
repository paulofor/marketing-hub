...
public class EmailSmtpSettingsService {
  ...
  public EmailSmtpSettingsService(
      GeneralSettingService generalSettingService, ObjectMapper objectMapper) {
    this.generalSettingService = generalSettingService;
    this.objectMapper = objectMapper;
  }

  public EmailSmtpSettingsResponse getSettings() {
    Optional<GeneralSettingDto> dto =
        generalSettingService.findByName(GeneralSettingKeys.EMAIL_SERVICE_SMTP);
  ...
  public EmailSmtpSettingsResponse update(UpdateEmailSmtpSettingsRequest request) {
    EmailSmtpSettingsData current =
        generalSettingService
            .findByName(GeneralSettingKeys.EMAIL_SERVICE_SMTP)
    ...
    GeneralSettingDto saved =
        generalSettingService.upsert(GeneralSettingKeys.EMAIL_SERVICE_SMTP, serialized);
  ...
  private EmailSmtpSettingsData merge(
      EmailSmtpSettingsData current, UpdateEmailSmtpSettingsRequest request) {
    boolean authEnabled =
        request.authEnabled() == null
    ...
    boolean useStartTls =
        request.useStartTls() != null
            ? request.useStartTls()
            : current != null && Boolean.TRUE.equals(current.useStartTls());
    boolean useSsl =
        request.useSsl() != null
            ? request.useSsl()
            : current != null && Boolean.TRUE.equals(current.useSsl());
    int connectionTimeout =
        request.connectionTimeoutMs() != null
            ? request.connectionTimeoutMs()
            : current != null && current.connectionTimeoutMs() != null
                ? current.connectionTimeoutMs()
                : DEFAULT_TIMEOUT_MS;
    int readTimeout =
        request.readTimeoutMs() != null
            ? request.readTimeoutMs()
            : current != null && current.readTimeoutMs() != null
                ? current.readTimeoutMs()
                : DEFAULT_TIMEOUT_MS;
    int writeTimeout =
        request.writeTimeoutMs() != null
            ? request.writeTimeoutMs()
            : current != null && current.writeTimeoutMs() != null
                ? current.writeTimeoutMs()
                : DEFAULT_TIMEOUT_MS;
    boolean dryRun =
        request.dryRun() != null
            ? request.dryRun()
            : current != null && Boolean.TRUE.equals(current.dryRun());
  ...
  private void validate(EmailSmtpSettingsData data) {
    ...
    if (data.port() == null || data.port() < 1 || data.port() > 65535) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "Informe uma porta válida entre 1 e 65535.");
    }
    ...
    if (Boolean.TRUE.equals(data.authEnabled())) {
      if (!StringUtils.hasText(data.username())) {
        throw new ResponseStatusException(
            HttpStatus.BAD_REQUEST, "Usuário SMTP é obrigatório quando a autenticação está ativa.");
      }
      if (!StringUtils.hasText(data.password())) {
        throw new ResponseStatusException(
            HttpStatus.BAD_REQUEST, "Defina a senha SMTP para autenticar no provedor.");
      }
    }
    if (data.connectionTimeoutMs() == null
        || data.connectionTimeoutMs() <= 0
        || data.readTimeoutMs() == null
        || data.readTimeoutMs() <= 0
        || data.writeTimeoutMs() == null
        || data.writeTimeoutMs() <= 0) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "Timeouts devem ser maiores que zero.");
    }
  }
  ...
  private String serialize(EmailSmtpSettingsData data) {
    try {
      ...
      throw new ResponseStatusException(
          HttpStatus.INTERNAL_SERVER_ERROR, "Não foi possível salvar a configuração de e-mail", ex);
    }
  }
}
