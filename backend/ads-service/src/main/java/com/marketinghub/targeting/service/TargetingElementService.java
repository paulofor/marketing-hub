...
public class TargetingElementService {
  ...
  public TargetingElementService(
      TargetingElementRepository repository,
  ...
  public TargetingElement requestMetaAdsReprocessing(Long id) {
    ...
    if (element.getType() != TargetingElementType.INTEREST
      ...
      throw new IllegalArgumentException(
          "Tipo de elemento não suportado para reprocessamento Meta Ads");
    }
  ...
  public List<TargetingElement> listByNiche(
      Long nicheId, TargetingElementType type, TargetingElementStatus status) {
    return repository.findByFilters(nicheId, type, status);
  }

  public List<TargetingElement> findApprovedForExperiment(
  ...
  public boolean existsApprovedForExperiment(
      Long nicheId, UUID hypothesisId, TargetingElementType type) {
  ...
  @Transactional
  public void saveGeneratedElements(
      Long nicheId, TargetingElementType type, TargetingElementGenerationResultRequest request) {
    if (nicheId == null || type == null) {
      throw new IllegalArgumentException("nicheId e type são obrigatórios");
    }
    List<CreateTargetingElementRequest> items =
    ...
    log.info(
        "AI Worker reportou {} públicos gerados para nicho {} e tipo {}",
        items.size(),
        nicheId,
        type);
  }
  ...
  @Transactional
  public void markGenerationFailure(
      Long nicheId, TargetingElementType type, TargetingElementGenerationFailureRequest request) {
    if (nicheId == null || type == null) {
      throw new IllegalArgumentException("nicheId e type são obrigatórios");
    }
    var niche = nicheRepository.findById(nicheId).orElseThrow();
    ...
    nicheRepository.save(niche);
    log.warn(
        "AI Worker reportou falha ao gerar públicos para nicho {} e tipo {}: {}",
        nicheId,
        type,
  ...
  private void validateReadyState(TargetingElement element) {
    ...
    if (element.getStatus() == TargetingElementStatus.APPROVED
      ...
      throw new IllegalArgumentException(
          "Não é possível aprovar um público sem ID oficial da Meta");
    }
  }
  ...
  private void addPending(
      List<TargetingElementGenerationPendingDto> pending,
    ...
    pending.add(
        new TargetingElementGenerationPendingDto(
            niche.getId(),
            niche.getName(),
  ...
  private void resetPendingCounter(
      com.marketinghub.niche.MarketNiche niche, TargetingElementType type) {
    switch (type) {
