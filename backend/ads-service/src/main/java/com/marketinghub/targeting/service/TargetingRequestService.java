...
/**
 * Coordena solicitações de público, candidatos gerados por IA e materialização de opções Meta
 * aprovadas.
 */
@Service
public class TargetingRequestService {
  ...
  private static final Pattern LOCATION_SUFFIX_PATTERN =
      Pattern.compile("(?i)\\s+(em|no|na)\\s+[\\p{L}\\s]{2,}$");
  private static final Pattern STATE_SUFFIX_PATTERN =
      Pattern.compile("(?i)\\s+(em|no|na)\\s+[A-Z]{2}$");
  ...
  /**
   * Inicializa o serviço com repositórios de solicitações, candidatos, elementos aprovados e jobs
   * de resolução.
   */
  public TargetingRequestService(
      TargetingRequestRepository requestRepository,
  ...
  private MarketNiche resolveNiche(Long nicheId) {
    ...
    return nicheRepository
        .findById(nicheId)
        .orElseThrow(
            () -> new EntityNotFoundException("Market niche %s not found".formatted(nicheId)));
  }
  ...
  private Hypothesis resolveHypothesis(UUID hypothesisId) {
    ...
    return hypothesisRepository
        ...
        .orElseThrow(
            () -> new EntityNotFoundException("Hypothesis %s not found".formatted(hypothesisId)));
  }

  @Transactional(readOnly = true)
  public List<TargetingRequest> listRequests(
      TargetingRequestStatus status, int limit, Long nicheId, UUID hypothesisId) {
  ...
  public List<TargetingRecentRequestDto> listRecentRequests(int limit) {
    ...
    Map<UUID, TargetingResolutionSummary> summaries =
        resolutionJobService.summarizeByRequestIds(
            requests.stream().map(TargetingRequest::getId).toList());
  ...
  public TargetingRequest getWithCandidates(UUID requestId) {
    return requestRepository
        ...
        .orElseThrow(
            () ->
                new EntityNotFoundException("Targeting request %s not found".formatted(requestId)));
  }

  public List<TargetingRequest> listPendingForAi(int limit) {
    return requestRepository.findByStatus(TargetingRequestStatus.PENDING_AI).stream()
  ...
  public void saveCandidates(UUID requestId, TargetingCandidateIngestionRequest payload) {
    ...
    for (TargetingCandidateIngestionRequest.CandidatePayload candidatePayload :
        payload.getCandidates()) {
  ...
  private TargetingCandidate buildCandidate(
      TargetingCandidateIngestionRequest.CandidatePayload payload, TargetingRequest request) {
    if (payload == null) {
      return null;
    }
    ...
    String locale =
        firstNonBlank(payload.getIdiomaHint(), payload.getIdioma(), request.getLocale());
    String country =
        firstNonBlank(
            payload.getPais(),
  ...
  @Transactional
  public TargetingCandidate reprocessCandidate(
      Long candidateId, TargetingCandidateReprocessRequest payload) {
    ...
    if (payload != null) {
      ...
      if (StringUtils.hasText(seedInput)) {
        ...
        candidate.setSeedVariants(
            normalizeVariants(payload.getSeedVariants(), candidate.getSeed()));
      }
  ...
  /**
   * Aplica o retorno do Facebook Ads Worker e transforma opções validadas em públicos aprovados
   * para experimentos.
   */
  @Transactional
  ...
  /**
   * Materializa cada opção validada pela Meta como elemento aprovado disponível na aba Público do
   * experimento.
   */
  private void materializeApprovedTargetingElements(
      TargetingCandidate candidate, List<OptionPayload> optionPayloads) {
    if (candidate == null
  ...
  /**
   * Converte uma opção Meta validada em elemento de segmentação aprovado e idempotente por
   * nicho/tipo/metaId.
   */
  private TargetingElement toApprovedTargetingElement(
      TargetingCandidate candidate, OptionPayload payload) {
    if (payload == null
        || !StringUtils.hasText(payload.getFacebookId())
        || !StringUtils.hasText(payload.getName())) {
      return null;
    }
    TargetingElementType elementType =
        toElementType(payload.getType() != null ? payload.getType() : candidate.getType());
    if (elementType == null) {
      return null;
    }
    ...
    TargetingElement element =
        targetingElementRepository
            .findFirstByNicheIdAndTypeAndMetaId(
                candidate.getRequest().getNiche().getId(), elementType, metaId)
            .orElseGet(
                () ->
    ...
    element.setDescription(
        firstNonBlank(candidate.getRationale(), payload.getSearchTerm(), candidate.getSeed()));
    ...
    element.setConfidence(
        payload.getFinalScore() != null ? payload.getFinalScore() : candidate.getScore());
  ...
  private TargetingRecentRequestDto toRecentDto(
      TargetingRequest request, TargetingResolutionSummary summary) {
  ...
  private List<String> collectSeeds(TargetingRequest request) {
    ...
    for (TargetingCandidate candidate : request.getCandidates()) {
      ...
      if (!CollectionUtils.isEmpty(candidate.getSeedVariants())) {
        candidate.getSeedVariants().stream().filter(StringUtils::hasText).forEach(seeds::add);
      }
    }
  ...
  private TargetingCandidate loadCandidate(Long candidateId) {
    return candidateRepository
        .findDetailedById(candidateId)
        .orElseThrow(
            () ->
                new EntityNotFoundException(
                    "Targeting candidate %s not found".formatted(candidateId)));
  }
