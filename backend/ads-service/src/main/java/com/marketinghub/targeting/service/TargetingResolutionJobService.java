...
public class TargetingResolutionJobService {
   ...
   */
  public TargetingResolutionJobService(
      TargetingResolutionJobRepository repository,
  ...
  public void enqueueAfterCommit(
      TargetingRequest request, Collection<TargetingCandidate> candidates) {
    if (request == null || request.getId() == null || CollectionUtils.isEmpty(candidates)) {
      return;
    }
    UUID requestId = request.getId();
    List<Long> candidateIds =
        candidates.stream().map(TargetingCandidate::getId).filter(Objects::nonNull).toList();
  ...
  /**
   * Executa o enfileiramento em transação própria para não depender da transação que acabou de
   * commitar.
   */
  private void runEnqueueInNewTransaction(UUID requestId, Collection<Long> candidateIds) {
    try {
      ...
      log.error(
          "Failed to enqueue targeting candidates for Meta resolution: requestId={}, candidateIds={}",
          requestId,
          candidateIds,
          ex);
      throw ex;
  ...
  public Map<UUID, TargetingResolutionSummary> summarizeByRequestIds(List<UUID> requestIds) {
    ...
    Map<UUID, List<TargetingResolutionJob>> grouped =
        jobs.stream()
  ...
  private TargetingResolutionSummary summarize(List<TargetingResolutionJob> jobs) {
    ...
    for (TargetingResolutionJob job : jobs) {
      ...
      if (job.getStartedAt() != null
          && (lastAttempt == null || job.getStartedAt().isAfter(lastAttempt))) {
        lastAttempt = job.getStartedAt();
      }
      if (job.getFinishedAt() != null
          && (lastCompleted == null || job.getFinishedAt().isAfter(lastCompleted))) {
    ...
    return new TargetingResolutionSummary(
        pending, processing, succeeded, failed, lastAttempt, lastCompleted, lastError);
  }
