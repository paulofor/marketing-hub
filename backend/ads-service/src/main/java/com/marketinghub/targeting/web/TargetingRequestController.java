...
public class TargetingRequestController {
  ...
  public TargetingRequestController(
      TargetingRequestService service, TargetingRequestMapper mapper) {
  ...
  @GetMapping("/requests")
  public List<TargetingRequestDto> list(
      @RequestParam(value = "status", required = false) TargetingRequestStatus status,
    ...
    List<TargetingRequest> requests =
        service.listRequests(status, limit != null ? limit : 10, nicheId, hypothesisId);
    return requests.stream()
        .map(
            r ->
                includeCandidates
                    ? mapper.toDetailedDto(r, service.etaSeconds())
                    : mapper.toDto(r, service.etaSeconds()))
        .toList();
  }

  @GetMapping("/requests/recent")
  public List<TargetingRecentRequestDto> listRecent(
      @RequestParam(value = "limit", defaultValue = "10") Integer limit) {
    return service.listRecentRequests(limit != null ? limit : 10);
  }

  @GetMapping("/requests/{requestId}")
  public TargetingRequestDto get(
      @PathVariable UUID requestId,
      @RequestParam(value = "includeCandidates", defaultValue = "true") boolean includeCandidates) {
    TargetingRequest request = service.getWithCandidates(requestId);
    return includeCandidates
        ? mapper.toDetailedDto(request, service.etaSeconds())
        : mapper.toDto(request, service.etaSeconds());
  }
}
