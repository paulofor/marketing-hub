...
public class WhatsAppMessageController {
  ...
  public WhatsAppMessageDto sendMessage(@RequestBody WhatsAppSendMessageRequest request) {
    ...
    WhatsAppMessageType type =
        request.getType() != null ? request.getType() : WhatsAppMessageType.TEXT;
        ...
        case IMAGE -> {
          if (!StringUtils.hasText(request.getImageUrl())) {
            throw new ResponseStatusException(
                HttpStatus.BAD_REQUEST, "Image URL is required for image messages");
          }
          message =
              messagingService.sendImageMessage(
                  request.getTo(), request.getImageUrl(), request.getCaption(), metadata);
        }
        case TEXT -> {
          if (!StringUtils.hasText(request.getTextBody())) {
            throw new ResponseStatusException(
                HttpStatus.BAD_REQUEST, "Body is required for text messages");
          }
          message =
              messagingService.sendTextMessage(request.getTo(), request.getTextBody(), metadata);
        }
        case TEMPLATE ->
            throw new ResponseStatusException(
                HttpStatus.BAD_REQUEST, "Template messages are not supported via console");
        default ->
            throw new ResponseStatusException(
                HttpStatus.BAD_REQUEST, "Unsupported message type: " + type);
      }
