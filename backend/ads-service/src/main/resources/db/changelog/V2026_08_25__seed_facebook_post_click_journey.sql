...
--changeset repo:2026-08-25-seed-facebook-post-click-journey dbms:mysql
...
--changeset repo:2026-08-25-seed-facebook-post-click-journey-instance dbms:mysql
--preconditions onFail:MARK_RAN
--precondition-sql-check expectedResult:0 SELECT COUNT(*) FROM journey WHERE name = 'Lifecycle Pós-Clique Lead Ads 14d - Exemplo';
SET @template_ref = (SELECT id FROM journey_template WHERE name = 'Lifecycle Pós-Clique Lead Ads 14d' LIMIT 1);
INSERT INTO journey(template_id, name, description, status, segment_reference, segment_filter, start_at)
VALUES (
    @template_ref,
    'Lifecycle Pós-Clique Lead Ads 14d - Exemplo',
    'Instância exemplo conectada ao plano pós-clique para leads de campanhas Meta com CAPI/Enhanced Conversions e mix e-mail/push/WhatsApp controlado.',
    'ACTIVE',
    'leads_meta_click_to_wa',
    '{"source":"META_LEAD_AD","consent":["EMAIL","WHATSAPP"],"stage":"novo"}',
    NOW()
);
SET @journey_id = LAST_INSERT_ID();

INSERT INTO journey_metadata(journey_id, meta_key, meta_value) VALUES
(@journey_id, 'owner', 'Marketing Operations'),
(@journey_id, 'automation.cdp_stream', 'segment:fb_click_id + consent logs'),
(@journey_id, 'remarketing.window_days', '7'),
(@journey_id, 'whatsapp.template', 'boa_vindas_valor_utilitario'),
(@journey_id, 'kpi.primary', 'LTV/lead aos 14 dias e taxa de conversão pós-clique.');
