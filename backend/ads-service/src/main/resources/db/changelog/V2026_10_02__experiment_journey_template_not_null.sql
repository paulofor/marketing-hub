...
--precondition-sql-check expectedResult:0 SELECT CASE
    WHEN EXISTS(SELECT 1 FROM experiment WHERE journey_template_id IS NULL)
         AND NOT EXISTS(SELECT 1 FROM journey_template)
    THEN 1
    ELSE 0
END;
