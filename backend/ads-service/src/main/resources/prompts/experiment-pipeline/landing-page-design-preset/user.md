...
- Incorporar no preset os padrões da seção "Padrões de CSS e componentes por elemento" de `docs/pesquisa-profunda/pesquisa-profunda-html-estilos.md`, cobrindo explicitamente os elementos: `<p>`, `<h1>`, `<h2>`, `<h3>`, `<ul>/<li>`, `<button>`/CTA, `<form>`, `<label>`, `<input>`, `<img>` e a mensagem inline de sucesso/erro do formulário (`#form-feedback`, `[data-runtime-feedback='true']`).
...
- Dar acabamento premium obrigatório para a mensagem de sucesso do formulário: ela deve parecer um banner/card de confirmação visível, confiável e integrado ao tema, com fundo de sucesso suave, borda sutil, raio coerente, espaçamento confortável, hierarquia clara entre título e descrição, contraste AA, estado de erro equivalente e posicionamento natural após o CTA sem competir com o botão.
