...
/** Valida a administração de contas Facebook pelo backend. */
@SpringBootTest
@AutoConfigureMockMvc
@TestPropertySource(
    properties = {
      "spring.datasource.url=jdbc:h2:mem:com.marketinghub.ads.FacebookAccountControllerTest-${random.uuid};MODE=MySQL;DB_CLOSE_DELAY=0;DB_CLOSE_ON_EXIT=FALSE",
...
public class FacebookAccountControllerTest {
  ...
  @Autowired FacebookAccountRepository repository;
  ...
  void shouldReturnAccountsWithTokenStatus() throws Exception {
    ...
    repository.save(
        FacebookAccount.builder().name("Account missing token").currency("USD").build());

    mockMvc
        .perform(get("/api/accounts/facebook"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.length()").value(3))
        .andExpect(jsonPath("$[?(@.name=='Account valid')].requiresTokenRenewal", contains(false)))
        .andExpect(
            jsonPath("$[?(@.name=='Account expiring')].requiresTokenRenewal", contains(true)))
        .andExpect(
            jsonPath("$[?(@.name=='Account missing token')].requiresTokenRenewal", contains(true)))
  ...
  void shouldReturnAccountsEligibleForRenewal() throws Exception {
    ...
    mockMvc
        .perform(get("/api/accounts/facebook/renewal/eligible"))
        .andExpect(status().isOk())
  ...
  void shouldRegisterSuccessfulRenewal() throws Exception {
    ...
    mockMvc
        .perform(
            post("/api/accounts/facebook/" + account.getId() + "/token/renewal")
    ...
    assertThat(updated.getTokenRenewalStatus())
        .isEqualTo(FacebookTokenRenewalStatus.SUCCESS.name());
  ...
  void shouldRejectRevalidationWhenTokenMissing() throws Exception {
    ...
    mockMvc
        .perform(post("/api/accounts/facebook/" + account.getId() + "/token/revalidation"))
  ...
  void shouldReturnSafeDiagnosticsWhenTokenMissing() throws Exception {
    ...
    mockMvc
        .perform(post("/api/accounts/facebook/" + account.getId() + "/token/diagnostics"))
        .andExpect(status().isOk())
  ...
  void shouldSkipVideoUploadDiagnosticWhenAdAccountMissing() throws Exception {
    ...
    mockMvc
        .perform(post("/api/accounts/facebook/" + account.getId() + "/token/video-upload-test"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.accountId").value(account.getId()))
  ...
  void shouldPreserveTokenAndHiddenFieldsWhenUpdatingAccountWithoutExplicitValues()
    ...
    mockMvc
        .perform(
            put("/api/accounts/facebook/" + account.getId())
                .contentType(MediaType.APPLICATION_JSON)
                .content(payload))
        .andExpect(status().isOk());
    ...
    assertThat(updated.getTokenLastRefreshedAt())
        .isEqualTo(LocalDateTime.parse("2024-01-01T08:00:00"));
    assertThat(updated.getTokenRenewalStatus())
        .isEqualTo(FacebookTokenRenewalStatus.SUCCESS.name());
    assertThat(updated.getTokenRenewalLastAttemptAt())
        .isEqualTo(LocalDateTime.parse("2023-12-31T12:00:00"));
  ...
  void shouldExposeWorkerConfigurationWhenAccountEnabled() throws Exception {
    ...
    mockMvc
        .perform(get("/api/accounts/facebook/worker-config"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.accountId").value(account.getId()))
