...
class FacebookTokenRevalidationServiceTest {
  ...
  void shouldGenerateNewTokenWithSixtyDayExpiration() {
    ...
    when(repository.save(any(FacebookAccount.class)))
        .thenAnswer(invocation -> invocation.getArgument(0));

    FacebookTokenRevalidationService service =
        new FacebookTokenRevalidationService(
            builder, repository, new ObjectMapper(), "https://graph.facebook.com", "v18.0");
    ...
    server
        .expect(
            requestTo(
                "https://graph.facebook.com/v18.0/oauth/access_token?grant_type=fb_exchange_token&client_id=app-id&client_secret=secret&fb_exchange_token=old-token"))
