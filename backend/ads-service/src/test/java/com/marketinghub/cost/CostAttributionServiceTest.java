...
class CostAttributionServiceTest {
  ...
  /**
   * Confirma que entidade gerenciada recebe apenas alteração em memória para o flush do JPA
   * persistir.
   */
  @Test
  void addCostToExperimentHierarchyDoesNotSqlIncrementManagedExperiment() {
    Experiment experiment = Experiment.builder().id(59L).totalCost(new BigDecimal("10.00")).build();
