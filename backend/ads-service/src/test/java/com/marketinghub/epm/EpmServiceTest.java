...
class EpmServiceTest {
  ...
  void shouldCalculatePlannedTotalAndRemainingBudget() {
    FinancialPlanHypothesisRepository hypothesisRepository =
        mock(FinancialPlanHypothesisRepository.class);
    ExperimentBudgetRepository budgetRepository = mock(ExperimentBudgetRepository.class);
    EpmService service =
        service(null, null, hypothesisRepository, budgetRepository, null, null, null);
    ...
    var response =
        service.createExperimentBudget(
            3L,
            new CreateExperimentBudgetRequest(
                42L,
                "Experimento",
                2_000L,
                3,
                ...
                null,
                ExperimentBudgetStatus.RUNNING,
                null));
  ...
  void shouldCalculateManualMetricProfitAndRatios() {
    ExperimentBudgetRepository budgetRepository = mock(ExperimentBudgetRepository.class);
    ExperimentFinancialMetricRepository metricRepository =
        mock(ExperimentFinancialMetricRepository.class);
    ...
    ExperimentBudget budget =
        ExperimentBudget.builder()
            .id(5L)
            .spendLimitCents(6_000L)
            .actualSpendCents(0L)
            .remainingBudgetCents(6_000L)
            .build();
    when(budgetRepository.findById(5L)).thenReturn(Optional.of(budget));
    when(metricRepository.save(any()))
        .thenAnswer(
            invocation -> {
    ...
    var response =
        service.createExperimentMetric(
            5L,
  ...
  void shouldCalculateBreakEvenSalesForPriceScenario() {
    ...
    var response =
        service.createProductPriceScenario(
            2L, new CreateProductPriceScenarioRequest("R$97", 9_700L, 800L, 0L, 600L, null, null));
  ...
  private EpmService service(
      FinancialPlanRepository planRepository,
      FinancialPlanNicheRepository nicheRepository,
      FinancialPlanHypothesisRepository hypothesisRepository,
      ExperimentBudgetRepository budgetRepository,
      ExperimentFinancialMetricRepository metricRepository,
      ExperimentFinancialDecisionRepository decisionRepository,
      ProductPriceScenarioRepository scenarioRepository) {
    return new EpmService(
        ...
        decisionRepository == null
            ? mock(ExperimentFinancialDecisionRepository.class)
            : decisionRepository,
        scenarioRepository == null
            ? mock(ProductPriceScenarioRepository.class)
            : scenarioRepository);
  }
}
