...
class FrameworkImageGenerationControllerTest {
  ...
  void generateCreatesJobs() throws Exception {
    when(service.enqueueJobsForExperiment(41L))
        .thenReturn(
            List.of(
    ...
    mockMvc
        .perform(MockMvcRequestBuilders.post("/api/experiments/41/framework-images/generate"))
  ...
  void listReturnsProgressByPlanningItem() throws Exception {
    when(service.listJobsByExperiment(8L))
        .thenReturn(
            List.of(
    ...
    mockMvc
        .perform(MockMvcRequestBuilders.get("/api/experiments/8/framework-images"))
  ...
  void summaryReturnsAggregatedCounters() throws Exception {
    when(service.summarizeJobsByExperiment(20L))
        .thenReturn(
            new FrameworkImageGenerationSummaryDto(
                8, 0, 8, 8, 0, 0, Instant.parse("2026-05-13T20:09:00Z")));

    mockMvc
        .perform(MockMvcRequestBuilders.get("/api/experiments/20/framework-images/summary"))
        .andExpect(status().isOk())
