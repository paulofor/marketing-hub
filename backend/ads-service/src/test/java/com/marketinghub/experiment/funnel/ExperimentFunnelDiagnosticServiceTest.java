...
class ExperimentFunnelDiagnosticServiceTest {
  ...
  void setUp() {
    service =
        new ExperimentFunnelDiagnosticService(
            funnelService, new ExperimentFunnelDiagnosticConfig(), experimentRepository);
    lenient()
        .when(experimentRepository.findById(10L))
        .thenReturn(Optional.of(Experiment.builder().id(10L).build()));
    lenient()
        .when(experimentRepository.findById(20L))
        .thenReturn(Optional.of(Experiment.builder().id(20L).build()));
    lenient()
        .when(experimentRepository.findById(30L))
        .thenReturn(Optional.of(Experiment.builder().id(30L).build()));
    lenient()
        .when(experimentRepository.findById(40L))
        .thenReturn(Optional.of(Experiment.builder().id(40L).build()));
  }

  @Test
  void marksStageAsStatisticallyFailedWhenRuleOfThreeDropsBelowMinimumRate() {
    when(funnelService.summarize(10L))
        .thenReturn(
            stageList(
  ...
  void marksTechnicalIssueWhenSuccessesAreHigherThanAttempts() {
    when(funnelService.summarize(20L))
        .thenReturn(
            stageList(
            ...
            item -> {
              if (item.stageKey() == ExperimentFunnelStage.ABERTURA_EMAIL_AMOSTRA) {
                assertThat(item.status())
                    .isEqualTo(FunnelDiagnosticStatus.TECHNICAL_ISSUE_SUSPECTED);
  ...
  void marksInsufficientDataWhenStillBelowMinimumAttemptsForRuleOfThree() {
    when(funnelService.summarize(30L))
        .thenReturn(
            stageList(
  ...
  /**
   * Garante reprovação estatística quando a taxa de acesso ao formulário fica abaixo do mínimo com
   * confiança.
   */
  @Test
  void marksAdInterestAsStatisticallyFailedWhenAccessRateIsBelowMinimumWithConfidence() {
    when(funnelService.summarize(40L))
        .thenReturn(
            stageList(
  ...
  void usesDirectSalesRulesForLowTicketProduct() {
    when(experimentRepository.findById(50L))
        .thenReturn(
            Optional.of(
                Experiment.builder()
                    .id(50L)
                    ...
                    .build()));
    when(funnelService.summarize(50L))
        .thenReturn(
            stageList(
  ...
  void usesMembershipSubscriptionRulesForPdeProduct() {
    when(experimentRepository.findById(66L))
        .thenReturn(
            Optional.of(
                Experiment.builder()
                    .id(66L)
                    ...
                    .build()));
    when(funnelService.summarize(66L))
        .thenReturn(
            stageList(
                stage(ExperimentFunnelStage.VISUALIZACAO_ANUNCIO, 1000),
