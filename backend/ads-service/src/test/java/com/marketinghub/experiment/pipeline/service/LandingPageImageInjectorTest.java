...
class LandingPageImageInjectorTest {
  ...
  void injectsImagesIntoJsonPayloadWhenHtmlDocumentIsPresent() throws Exception {
    ...
    when(jobRepository.findByExperimentIdOrderByCreatedAtDesc(10L))
        .thenReturn(
            List.of(
  ...
  void injectsImagesWhenPayloadIsPureHtml() {
    String html =
        "<!doctype html><html><body><section id=\"s0-hero\"><img src=\"about:blank\""
            + " /></section></body></html>";
  ...
  void injectsByDataPlanningItemKeyWhenSectionIdDoesNotMatch() {
    ...
    when(jobRepository.findByExperimentIdOrderByCreatedAtDesc(anyLong()))
        .thenReturn(
            List.of(completedJob("s0-hero", "https://cdn.example.com/hero-final.jpg", null)));
  ...
  void replacesPlaceholderWithNextGeneratedImageWhenNoDirectMatchExists() {
    ...
    when(jobRepository.findByExperimentIdOrderByCreatedAtDesc(anyLong()))
        .thenReturn(
            List.of(completedJob("s9-proof", "https://cdn.example.com/timeline-final.jpg", null)));
  ...
  void injectsImageByImageSectionAttributes() {
    ...
    when(jobRepository.findByExperimentIdOrderByCreatedAtDesc(anyLong()))
        .thenReturn(
            List.of(
                completedJob(
                    "s0-hero-pt-variant10-v1", "https://cdn.example.com/hero-runtime.jpg", null)));
  ...
  private FrameworkImageGenerationJob completedJob(
      String sectionId, String webUrl, String sourceUrl) {
