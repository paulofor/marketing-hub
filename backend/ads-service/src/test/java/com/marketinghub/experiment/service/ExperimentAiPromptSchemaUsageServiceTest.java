...
class ExperimentAiPromptSchemaUsageServiceTest {
  ...
  @Mock private ExperimentAiPromptSchemaUsageRepository usageRepository;
  @InjectMocks private ExperimentAiPromptSchemaUsageService service;
  ...
  void linkHypothesisTemplatesCreatesUsageFromCompletedExecutions() {
    ...
    Experiment experiment = Experiment.builder().id(52L).hypothesisRef(hypothesis).build();
    ...
    when(hypothesisExecutionRepository.findByHypothesisIdOrderByExecutionRequestedAtAsc(
            hypothesisId))
    ...
    assertThat(captor.getValue().getTemplateKey())
        .isEqualTo("hypothesis-pipeline:hypothesis-pain:v1");
