...
class ExperimentDiagnosticsServiceTest {
  ...
  @Mock private ExperimentFacebookApiLogService facebookApiLogService;
  @Mock private FacebookCampaignPublicationJobStepRepository publicationJobStepRepository;

  @InjectMocks private ExperimentDiagnosticsService service;
  ...
  void shouldNotFlagPendingWhenLegacyMetaIdIsStoredInIdField() {
    ...
    when(publicationJobStepRepository
  ...
  /**
   * Impede que uma falha antiga continue aparecendo quando tentativas mais recentes já tiveram
   * sucesso.
   */
  @Test
  void shouldHideHistoricalFailureWhenMostRecentMetaCallSucceeded() {
    ...
    when(publicationJobStepRepository
            .findTopByExperimentIdAndErrorMessageIsNotNullOrderByOccurredAtDescIdDesc(experimentId))
        .thenReturn(Optional.empty());
    when(facebookApiLogService.findLogs(experimentId, 200))
        .thenReturn(
            List.of(
  ...
  void shouldShowFailureDetailsWhenMostRecentMetaCallFailed() {
    ...
    assertThat(diagnostics.failureDetails().occurredAt())
        .isEqualTo(Instant.parse("2026-06-09T03:24:53Z"));
  }

  /**
   * Mostra a falha operacional do job quando a Meta respondeu 200, mas o worker bloqueou por regra
   * de alcance.
   */
  @Test
  void shouldShowPublicationJobFailureWhenReachValidationBlockedAfterSuccessfulMetaCall() {
    ...
    step.setErrorMessage(
        "Público pequeno demais para publicar: a Meta estimou 1.000 a 1.000 pessoas.");
    ...
    assertThat(diagnostics.failureDetails().occurredAt())
        .isEqualTo(Instant.parse("2026-06-17T00:54:48Z"));
  }

  /** Cria um DTO mínimo de log da API Meta para exercitar a ordenação do diagnóstico. */
  private static ExperimentFacebookApiLogDto apiLog(
      Long id, Integer statusCode, String errorMessage, Instant respondedAt) {
