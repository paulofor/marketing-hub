...
class FacebookCampaignRecommendationServiceTest {
  ...
  @Mock private FacebookAdsRecommendationRepository recommendationRepository;
  ...
  void setUp() {
    service =
        new FacebookCampaignRecommendationService(
            campaignRepository, recommendationRepository, objectMapper);
  }
  ...
  void shouldListOnlyActiveCampaignTargets() {
    ...
    when(campaignRepository.findAllByExperimentStatusAndStatus(
            ExperimentStatus.RUNNING, FacebookAdStatus.ACTIVE))
  ...
  void shouldReplaceRecommendationSnapshotAndClearPreviousError() throws Exception {
    ...
    var payload =
        objectMapper.readTree(
            "{\"data\":[{\"code\":\"1\",\"title\":\"Ajuste orçamento\",\"message\":\"Teste\",\"importance\":\"HIGH\",\"confidence\":\"MEDIUM\",\"blame_field\":\"daily_budget\",\"recommendation_data\":{\"value\":10}}]}");
    Instant collectedAt = Instant.parse("2026-06-10T11:00:00Z");

    service.ingest(
        "camp-1", new FacebookCampaignRecommendationIngestionRequest(collectedAt, payload));

    verify(recommendationRepository).deleteByCampaignId("camp-1");
    ArgumentCaptor<FacebookAdsRecommendation> captor =
        ArgumentCaptor.forClass(FacebookAdsRecommendation.class);
