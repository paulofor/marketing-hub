...
@TestPropertySource(
    properties = {
      "spring.datasource.url=jdbc:h2:mem:com.marketinghub.facebookads.controller.FacebookAdsCampaignControllerTest-${random.uuid};MODE=MySQL;DB_CLOSE_DELAY=0;DB_CLOSE_ON_EXIT=FALSE",
...
class FacebookAdsCampaignControllerTest {
  ...
  @MockBean ExperimentService experimentService;

  @MockBean
  com.marketinghub.repository.jpa.facebookads.FacebookAdsCampaignRepository campaignRepository;

  @MockBean FacebookAccountRepository facebookAccountRepository;
  @MockBean FacebookAdsAdSetRepository adSetRepository;
  @MockBean FacebookAdsAdCreativeRepository adCreativeRepository;
  @MockBean FacebookAdsAdRepository adRepository;
  @MockBean AdSetRepository experimentAdSetRepository;
  @MockBean ExperimentTargetingSelectionRepository targetingSelectionRepository;
  @MockBean CreativeRepository creativeRepository;
  @MockBean GeraSalesPageStageExecutionRepository geraSalesPageStageExecutionRepository;
  @MockBean GeraSalesPagePublicationAuditRepository geraSalesPagePublicationAuditRepository;
  @MockBean FacebookCampaignPublicationJobStepService publicationJobStepService;
  @MockBean CampaignStrategyService campaignStrategyService;
  @MockBean ExperimentSalesPageAbTestService salesPageAbTestService;

  @MockBean LeadPortalMetricsService leadPortalMetricsService;
  ...
  void listsReadyCreativesForFacebookConsumption() throws Exception {
    var experiment = Experiment.builder().id(1L).name("Exp").build();
    var readyCreative =
        Creative.builder()
            .id(101L)
    ...
    var readyVideoCreative =
        Creative.builder()
            ...
            .status(CreativeStatus.READY)
            .build();
    when(creativeRepository.findByExperimentId(1L))
        .thenReturn(List.of(draftCreative, readyVideoCreative, readyCreative));

    mockMvc
        .perform(get("/api/facebook-campaigns/experiments/1/creatives-ready"))
  ...
  void listExperimentsByStatus() throws Exception {
    ...
    var hypothesis =
        Hypothesis.builder().id(java.util.UUID.randomUUID()).title("Hipótese do Nicho").build();
    var journeyTemplate = JourneyTemplate.builder().id(20L).name("Lifecycle Pós-Clique").build();
    var account = FacebookAccount.builder().id(55L).name("Conta Worker").build();
    var instagramAccount =
        InstagramAccount.builder()
            ...
            .name("Estúdio")
            .build();
    var page = FacebookPage.builder().id(9L).account(account).pageId("84").name("Estúdio").build();
    var leadPortalFlow =
    ...
    var exp =
        Experiment.builder()
            ...
            .build();
    when(creativeRepository.existsByExperimentIdAndStatusAndUsableImage(1L, CreativeStatus.READY))
        .thenReturn(true);
    when(targetingSelectionRepository.findByExperimentIdWithTargetingElement(1L))
        .thenReturn(
            List.of(
    ...
    when(leadPortalMetricsService.listExperimentMetrics())
        .thenReturn(
            List.of(
                new LeadPortalExperimentMetricsDto(
                    ...
                    5L,
                    List.of(
                        new LeadPortalExperimentUserDto("Lead 1", "lead@example.com", null, false)),
                    0L,
                    null,
                    null,
                    null,
    ...
    mockMvc
        .perform(get("/api/facebook-campaigns/experiments").param("status", "PLANNED"))
        .andExpect(status().isOk())
  ...
  void experimentsReadyExcludesExperimentsThatAlreadyHaveCampaign() throws Exception {
    ...
    mockMvc
        .perform(get("/api/facebook-campaigns/experiments-ready"))
        .andExpect(status().isOk())
  ...
  void experimentsReadyIncludesExperimentTypeForLowTicketSalesCampaigns() throws Exception {
    ...
    when(creativeRepository.existsByExperimentIdAndStatusAndUsableImage(53L, CreativeStatus.READY))
        .thenReturn(true);
    when(targetingSelectionRepository.findByExperimentIdWithTargetingElement(53L))
        .thenReturn(
            List.of(
                ExperimentTargetingSelection.builder()
                    .candidateType(TargetingCandidateType.INTEREST)
        ...
        .thenReturn(
            Optional.of(
                GeraSalesPagePublicationAudit.builder()
                    ...
                    .html(
                        "data-track-section data-mh-sales-page-analytics page_view page_load_metric section_view_time checkout_click")
  ...
  void createCampaignPersistsHierarchy() throws Exception {
    ...
    var experimentAdSet =
        AdSet.builder().id(101L).experiment(experiment).targetingJson("{}").build();
    ...
    ArgumentCaptor<FacebookAdsCampaign> campaignCaptor =
        ArgumentCaptor.forClass(FacebookAdsCampaign.class);
    ...
    assertThat(targetingJson.path("geo_locations").path("countries").get(0).asText())
        .isEqualTo("BR");
    JsonNode promotedObject =
        savedAdSet.getPromotedObjectJson() != null
            ? objectMapper.readTree(savedAdSet.getPromotedObjectJson())
            : objectMapper.nullNode();
    assertThat(promotedObject.path("page_id").asText()).isEqualTo("12345");

    ArgumentCaptor<FacebookAdsAdCreative> creativeCaptor =
        ArgumentCaptor.forClass(FacebookAdsAdCreative.class);
  ...
  void createCampaignAcceptsRetryForSameCampaignIdWithoutDuplicatingHierarchy() throws Exception {
    var experiment = Experiment.builder().id(37L).name("Experimento 37").build();
  ...
  void syncStatusUpdatesCampaignAdSetAndAdsFromMetaSnapshot() throws Exception {
    ...
    mockMvc
        .perform(
            post("/api/facebook-campaigns/cmp-sync/status-sync")
                .contentType(MediaType.APPLICATION_JSON)
                .content(payload))
  ...
  void syncStatusStopsRunningExperimentWhenMetaCampaignIsPaused() throws Exception {
    ...
    mockMvc
        .perform(
            post("/api/facebook-campaigns/cmp-paused/status-sync")
                .contentType(MediaType.APPLICATION_JSON)
                .content(payload))
        .andExpect(status().isAccepted());
  ...
  @Test
  // Garante que a sincronização de métricas inclui campanhas em execução e encerradas sem
  // fechamento final.
  void listsRunningAndUnfinalizedSettlementCampaignsAsMetricsSyncTargets() throws Exception {
    var runningExperiment = Experiment.builder().id(59L).status(ExperimentStatus.RUNNING).build();
    var stoppedExperiment =
        Experiment.builder().id(58L).status(ExperimentStatus.USER_STOPPED).build();
    ...
    when(campaignRepository.findMetricsSyncTargets(
            ...
            argThat(
                statuses ->
                    statuses.contains(ExperimentStatus.USER_STOPPED)
    ...
    mockMvc
        .perform(get("/api/facebook-campaigns/metrics/sync-targets"))
        .andExpect(status().isOk())
  ...
  void createCampaignPersistsMultipleAds() throws Exception {
    var experiment = Experiment.builder().id(55L).name("Exp Multi").build();
    when(experimentService.get(55L)).thenReturn(experiment);

    var experimentAdSet =
        AdSet.builder().id(202L).experiment(experiment).targetingJson("{}").build();
