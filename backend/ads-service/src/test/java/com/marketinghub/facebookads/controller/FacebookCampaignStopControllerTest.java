...
class FacebookCampaignStopControllerTest {
  ...
  @MockBean FacebookCampaignStopService stopService;
  ...
  void listsStopRequests() throws Exception {
    ...
    mockMvc
        .perform(get("/api/facebook-campaigns/stop-requests"))
            ...
            content()
                .json(
                    "[{"
                        ...
                        + "\"externalId\":\"123\","
                        + "\"experimentId\":10,"
                        + "\"stopReason\":\"FORM_ZERO_CONVERSION_RULE_OF_THREE\"}]",
                    false));
  }
  ...
  void forwardsStopResult() throws Exception {
    mockMvc
        .perform(
            post("/api/facebook-campaigns/camp-1/stop-results")
    ...
    verify(stopService)
        .registerStopResult(eq("camp-1"), successCaptor.capture(), messageCaptor.capture());
