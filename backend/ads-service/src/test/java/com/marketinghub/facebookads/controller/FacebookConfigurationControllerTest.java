...
/** Valida a administração das configurações Facebook. */
@SpringBootTest(classes = AdsServiceApplication.class)
@AutoConfigureMockMvc
@TestPropertySource(
    properties = {
      "spring.datasource.url=jdbc:h2:mem:com.marketinghub.facebookads.controller.FacebookConfigurationControllerTest-${random.uuid};MODE=MySQL;DB_CLOSE_DELAY=0;DB_CLOSE_ON_EXIT=FALSE",
...
class FacebookConfigurationControllerTest {
  ...
  void shouldReportConfigurationStatus() throws Exception {
    mockMvc
        .perform(get("/api/facebook/configuration-status").accept(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.hasConfiguredPages").value(false));

    FacebookAccount account =
        accountRepository.save(FacebookAccount.builder().name("Account").currency("USD").build());

    pageRepository.save(
        FacebookPage.builder().account(account).pageId("123").name("Main Page").build());
  ...
  void shouldExposeRecordedWorkerValidationError() throws Exception {
    ...
    mockMvc
        ...
        .andExpect(
            jsonPath("$.worker.messages[0].message", containsString("ID da conta de anúncios")))
