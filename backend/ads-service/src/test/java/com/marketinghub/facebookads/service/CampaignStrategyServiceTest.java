...
class CampaignStrategyServiceTest {
  @Mock private CampaignStrategyRepository strategyRepository;
  @Mock private CampaignStrategyEvaluationRepository evaluationRepository;
  ...
  void setUp() {
    service =
        new CampaignStrategyService(strategyRepository, evaluationRepository, diagnosticService);
  }
  ...
  void evaluateAfterMetricsKeepsCampaignDecisionAsAuditOnly() {
    ...
    when(diagnosticService.diagnose(58L))
        .thenReturn(
  ...
  private ExperimentFunnelStageDiagnosticDto stage(
      ExperimentFunnelStage stage, long attempts, long successes, double rate) {
