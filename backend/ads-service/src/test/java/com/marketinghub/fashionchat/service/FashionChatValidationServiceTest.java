...
class FashionChatValidationServiceTest {
    ...
    private final FashionChatValidationService service = new FashionChatValidationService(
            restTemplate,
            objectMapper,
            "http://fashion-chat.test");

    /** Deve informar login necessário quando o health ready está degradado e a conta não está autenticada. */
    @Test
    void statusReturnsNotAuthenticatedWhenCodexSessionIsMissing() {
        ...
        server.expect(requestTo("http://fashion-chat.test/codex-app-server/account/read"))
                .andExpect(method(HttpMethod.GET))
                .andRespond(withSuccess("{\"code\":\"CODEX_NOT_AUTHENTICATED\",\"authenticated\":false}",
    ...
    void startLoginReturnsDeviceCodePayload() {
        server.expect(requestTo("http://fashion-chat.test/codex-app-server/account/login/start"))
