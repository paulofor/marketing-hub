...
class FeoFabricacaoV1ServiceTest {
  ...
  void shouldStartFabricationFromExperimentPackage() {
    ...
    when(executionRepository.existsByExperimentIdAndStageCodeAndStatusIn(
            eq(66L), eq("planejamento-entregaveis"), any()))
        .thenReturn(false);
  ...
  void shouldExposePendingExecutionForWorker() {
    ...
    when(executionRepository.findPendingOrStaleRunning(
            eq("planejamento-entregaveis"), any(), any(Pageable.class)))
  ...
  void shouldEnqueueVisualAssetsAfterContentWritingCompletion() {
    FeoFabricacaoV1StageExecution execution =
        FeoFabricacaoV1StageExecution.builder()
            ...
            .status(FeoFabricacaoV1StageStatus.RUNNING)
            .inputPayload(
                "{\"context\":{\"requestId\":\"experiment-66\"},\"plan\":{\"packageTitle\":\"Pacote\"}}")
            .build();
    ...
    service.complete(
        ...
        new FeoFabricacaoV1CompleteRequest(
            ...
            Map.of(
                ...
                "contentPackage",
                    Map.of("qualityScore", 92, "visualAssets", List.of(Map.of("code", "VIS-01")))),
            List.of(),
  ...
  void shouldEnqueuePackageAssemblyAfterVisualAssetGenerationCompletion() {
    FeoFabricacaoV1StageExecution execution =
        FeoFabricacaoV1StageExecution.builder()
            ...
            .status(FeoFabricacaoV1StageStatus.RUNNING)
            .inputPayload(
                "{\"context\":{\"requestId\":\"experiment-66\"},\"contentPackage\":{\"qualityScore\":92}}")
            .build();
  ...
  void shouldMaterializeDeliverablePackageAfterAssemblyCompletion() {
    FeoFabricacaoV1StageExecution execution =
        FeoFabricacaoV1StageExecution.builder()
            ...
            .inputPayload("{\"requestId\":\"experiment-66\",\"experimentId\":\"66\"}")
            .build();
    when(executionRepository.findByIdAndStageCode(8L, "montagem-pacote"))
        .thenReturn(Optional.of(execution));
    when(executionRepository.save(any())).thenAnswer(invocation -> invocation.getArgument(0));
    when(deliverableRepository.save(any())).thenAnswer(invocation -> invocation.getArgument(0));
    when(deliverablePackageRepository.save(any()))
        ...
        new FeoFabricacaoV1CompleteRequest(
            ...
            Map.of(
                ...
                Map.of(
                    ...
                    "items",
                    List.of(
                        Map.of("fileName", "guia.pdf", "role", "Guia principal", "sha256", "abc"))),
                "report",
    ...
    ArgumentCaptor<DeliverablePackage> packageCaptor =
        ArgumentCaptor.forClass(DeliverablePackage.class);
