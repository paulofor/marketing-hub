...
/**
 * Valida a montagem do HTML provisório da etapa de design preset sem inserir metadados técnicos no
 * resultado.
 */
class DesignPresetProvisionalHtmlAssemblerTest {

  /**
   * Garante que a saída do assembler preserve o HTML sem scripts de tracking e sem comentário
   * técnico de jobId.
   */
  @Test
  void shouldAssembleHtmlWithoutInjectingBehaviorTrackingScript() {
    DesignPresetProvisionalHtmlProcessor processor =
        mock(DesignPresetProvisionalHtmlProcessor.class);
    when(processor.process(anyString(), anyString(), anyString(), anyString()))
        .thenReturn(
            "<html><head><title>x</title></head><body><section data-section-id='hero'></section></body></html>");
    DesignPresetProvisionalHtmlAssembler assembler =
        new DesignPresetProvisionalHtmlAssembler(processor, new ObjectMapper());
  ...
  /**
   * Garante que o manifesto consolidado de imagens seja aplicado ao planejamento antes do
   * processador montar o HTML.
   */
  @Test
