...
class GeraLandingImagePlanningControllerTest {
  ...
  void startShouldDelegateToImagePlanningService() {
    ...
    GeraLandingImagePlanningController controller =
        new GeraLandingImagePlanningController(executionService);
    when(executionService.start(44L))
        .thenReturn(new GeraLandingImagePlanningStartResponse("job-1", "INICIADO"));
  ...
  /**
   * Deve receber prompt e delegar persistência do request cru para o serviço da etapa image
   * planning.
   */
  @Test
  ...
  void receiveDispatchShouldDelegateToImagePlanningService() {
    ...
    GeraLandingImagePlanningController controller =
        new GeraLandingImagePlanningController(executionService);
    RecebeDispatchRequest payload =
        new RecebeDispatchRequest(44L, "landing-page-image-planning", "openai-job-1");
