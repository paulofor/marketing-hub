...
/** Valida a persistência e evolução das jornadas comerciais. */
@SpringBootTest(classes = AdsServiceApplication.class)
@org.springframework.test.context.TestPropertySource(
    properties = {
      "spring.datasource.url=jdbc:h2:mem:com.marketinghub.journey.JourneyServiceTest-${random.uuid};MODE=MySQL;DB_CLOSE_DELAY=0;DB_CLOSE_ON_EXIT=FALSE",
  ...
  void metricsAggregatesCountsPerStatus() {
    JourneyTemplate template =
        journeyTemplateRepository.save(
            JourneyTemplate.builder().name("Onboarding Experience").build());

    JourneyMetricsResponse initialMetrics = journeyService.metrics();

    journeyRepository.save(
        Journey.builder().template(template).name("Warm-up").status(JourneyStatus.DRAFT).build());
    ...
    assertThat(metrics.statusBreakdown())
        .containsEntry(
            ...
            initialMetrics.statusBreakdown().getOrDefault(JourneyStatus.ACTIVE, 0L) + 1)
        .containsEntry(
            JourneyStatus.DRAFT,
            initialMetrics.statusBreakdown().getOrDefault(JourneyStatus.DRAFT, 0L) + 1)
        .containsEntry(
            JourneyStatus.PAUSED,
            initialMetrics.statusBreakdown().getOrDefault(JourneyStatus.PAUSED, 0L) + 1);
