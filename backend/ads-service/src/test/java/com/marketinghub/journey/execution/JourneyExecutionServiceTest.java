...
class JourneyExecutionServiceTest {
  @Mock private JourneyAssignmentRepository assignmentRepository;
  @Mock private JourneyStepRepository stepRepository;
  @Mock private EventLogRepository eventLogRepository;
  @Mock private FrequencyCapService frequencyCapService;
  @Mock private RetryBackoffCalculator retryBackoffCalculator;
  @Mock private TelemetryService telemetryService;
  @Mock private JourneyChannelHandler emailHandler;
  ...
  void processDueAssignmentsDispatchesStep() {
    ...
    when(frequencyCapService.evaluate(eq(assignment), any()))
        .thenReturn(FrequencyCapResult.allow());
    when(stepRepository.findFirstByTemplateAndPositionGreaterThanOrderByPositionAsc(
            any(), anyInt()))
    ...
    verify(telemetryService)
        .emitStepDispatched(
            eq(assignment),
            eq(assignment.getCurrentStep()),
            anyMap(),
            argThat(m -> m.containsKey("providerMessageId")));
    ...
    assertThat(eventCaptor.getAllValues())
        ...
        .contains(
            JourneyEventType.STIMULUS_DISPATCHED.getCode(),
            JourneyEventType.JOURNEY_COMPLETED.getCode());
  }
    ...
    verify(eventLogRepository)
        .save(
            argThat(
                log ->
                    log.getEventType()
                        .equals(JourneyEventType.STIMULUS_FREQUENCY_CAPPED.getCode())));
  }
  ...
  void processDueAssignmentsStopsAfterPermanentFailure() {
    ...
    verify(eventLogRepository)
        .save(
            argThat(log -> log.getEventType().equals(JourneyEventType.STIMULUS_FAILED.getCode())));
  }

  private JourneyAssignment buildAssignment(JourneyStimulusType stimulusType) {
    JourneyTemplate template = JourneyTemplate.builder().id(10L).name("Template").build();
    Journey journey =
        Journey.builder()
            .id(22L)
            .template(template)
