...
class MetaAdsChannelHandlerTest {
  ...
  void setup() {
    ...
    RestTemplate restTemplate =
        (RestTemplate)
            Objects.requireNonNull(ReflectionTestUtils.getField(handler, "restTemplate"));
  ...
  void returnsTransientFailureOnRateLimit() {
    ...
    server
        .expect(once(), requestTo("https://graph.facebook.com/v18.0/act_123/ads"))
  ...
  void returnsPermanentFailureWhenDisabled() {
    ...
    MetaAdsChannelHandler disabledHandler =
        new MetaAdsChannelHandler(new RestTemplateBuilder(), properties);
    JourneyStep step =
        JourneyStep.builder()
            .id(1L)
            .template(JourneyTemplate.builder().id(2L).name("template").build())
