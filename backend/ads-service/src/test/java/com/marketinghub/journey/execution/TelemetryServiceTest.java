...
class TelemetryServiceTest {
  ...
  void sendsPixelEventWhenEnabled() {
    ...
    TelemetryService telemetryService =
        new TelemetryService(builder, metaProperties, ga4Properties, new ObjectMapper());
    ...
    metaServer
        .expect(once(), requestTo("https://graph.facebook.com/v18.0/pixel123/events"))
    ...
    JourneyTemplate template = JourneyTemplate.builder().id(9L).name("Template").build();
    Journey journey = Journey.builder().id(1L).template(template).name("Journey").build();
    JourneyStep step =
        JourneyStep.builder()
            .id(2L)
            .template(template)
    ...
    telemetryService.emitStepDispatched(
        assignment, step, Map.of("source", "landing"), Map.of("providerMessageId", "abc123"));
  ...
  void sendsGa4EventWhenEnabled() {
    ...
    gaServer
        .expect(
            once(),
            requestTo(
                "https://www.google-analytics.com/mp/collect?measurement_id=G-XYZ&api_secret=secret"))
        .andExpect(method(HttpMethod.POST))
    ...
    Map<String, Object> dispatchMetadata =
        Map.of("providerMessageId", "abc123", "value", BigDecimal.ONE);

    telemetryService.emitStepDispatched(
        assignment, step, Map.of("source", "landing"), dispatchMetadata);
