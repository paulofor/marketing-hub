...
class WhatsAppChannelHandlerTest {
  @Mock private WhatsAppMessagingService messagingService;
  @Mock private WhatsAppAccountService accountService;
  ...
  void setup() {
    ...
    handler =
        new WhatsAppChannelHandler(messagingService, accountService, properties, objectMapper);
  ...
  void dispatchesTextMessage() {
    ...
    WhatsAppMessage message =
        WhatsAppMessage.builder()
            ...
            .build();
    when(messagingService.sendTextMessage(
            eq(account), eq("+551100000000"), eq("Hello from MarketingHub"), anyMap()))
        .thenReturn(message);

    ChannelDispatchResult result =
        handler.dispatch(assignment, step, Map.of("phone", "+551100000000"));
  ...
  void returnsTransientFailureOnRateLimit() {
    ...
    when(messagingService.sendTextMessage(eq(account), anyString(), anyString(), anyMap()))
        .thenThrow(tooManyRequests);

    ChannelDispatchResult result =
        handler.dispatch(assignment, step, Map.of("phone", "+551100000001"));
