...
class FrequencyCapServiceTest {
  ...
  void blocksWhenDailyLimitReached() {
    ...
    when(eventLogRepository.countByActorIdAndEventTypeAndOccurredAtAfter(
            eq(leadId), anyString(), any()))
        .thenReturn(1L);
  ...
  private JourneyAssignment assignmentWithLead(UUID leadId) {
    return JourneyAssignment.builder().lead(Lead.builder().id(leadId).build()).build();
  }
}
