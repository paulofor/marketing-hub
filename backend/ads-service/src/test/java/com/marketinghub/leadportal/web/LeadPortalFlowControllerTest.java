...
/** Valida os contratos HTTP dos fluxos do Lead Portal. */
@SpringBootTest(classes = AdsServiceApplication.class)
@AutoConfigureMockMvc
@TestPropertySource(
    properties = {
      "spring.datasource.url=jdbc:h2:mem:com.marketinghub.leadportal.web.LeadPortalFlowControllerTest-${random.uuid};MODE=MySQL;DB_CLOSE_DELAY=0;DB_CLOSE_ON_EXIT=FALSE",
...
class LeadPortalFlowControllerTest {
  ...
  @MockBean LeadPortalFlowPublisher flowPublisher;
  ...
  void createFlowSupportsLongTextFields() throws Exception {
    ...
    request.setQuestions(
        List.of(
            buildQuestion(
                longText, longText, LeadPortalQuestionType.TEXTAREA, true, List.of(longText))));
  ...
  void duplicateSlugReturnsConflict() throws Exception {
    ...
    request.setQuestions(
        List.of(
            buildQuestion(
                "Qual o seu e-mail?", "email", LeadPortalQuestionType.EMAIL, true, List.of())));
  ...
  void approvalEndpointUpdatesStatus() throws Exception {
    ...
    mockMvc
        .perform(
            patch("/api/lead-portal-flows/" + flow.getId() + "/approval")
                .contentType(MediaType.APPLICATION_JSON)
                .content("{\"approved\":true}"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.approved").value(true))
        .andExpect(
            jsonPath("$.publicUrl").value("https://portal.example.com/flows/fluxo-aprovacao"));
  ...
  void updateApprovedFlowPublishesChanges() throws Exception {
    ...
    mockMvc
        .perform(
            put("/api/lead-portal-flows/" + flow.getId())
                .contentType(MediaType.APPLICATION_JSON)
                .content(mapper.writeValueAsString(request)))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.name").value("Fluxo Atualizado"));

    verify(flowPublisher)
        .publish(
  ...
  void updateApprovedFlowWithSlugChangeRemovesOldSlugAndPublishesNewOne() throws Exception {
    ...
    request.setExperimentId(experiment.getId());
    request.setQuestions(
        List.of(buildQuestion("Nome", "nome", LeadPortalQuestionType.TEXT, true, List.of())));
  ...
  void listByExperimentReturnsOnlyFlowsFromRequestedExperiment() throws Exception {
    ...
    Experiment experimentB =
        createExperimentInNiche(experimentA.getNiche(), "Experimento Secundário");
  ...
  private Experiment createExperiment() {
    MarketNiche niche =
        marketNicheRepository.save(MarketNiche.builder().name("Nicho Teste").build());
  ...
  private Experiment createExperimentInNiche(MarketNiche niche, String experimentName) {
    Hypothesis hypothesis =
        hypothesisRepository.save(
            Hypothesis.builder().title("Hipótese Teste").marketNiche(niche).build());
    JourneyTemplate template =
        journeyTemplateRepository.save(JourneyTemplate.builder().name("Template Jornada").build());
    Experiment experiment =
        Experiment.builder()
            .niche(niche)
            .name(experimentName)
  ...
  private LeadPortalFlowQuestionRequest buildQuestion(
      String title,
      String dataKey,
