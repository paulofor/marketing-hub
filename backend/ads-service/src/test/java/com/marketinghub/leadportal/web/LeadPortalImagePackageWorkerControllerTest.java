...
class LeadPortalImagePackageWorkerControllerTest {
  ...
  @MockBean private LeadPortalImagePackageWorkerService workerService;
  ...
  void setup() {
    ...
    doNothing()
        .when(workerService)
        .submitResults(any(Long.class), any(LeadPortalWorkerImageResultRequest.class));
  }

  @Test
  void listRecentReturnsPackages() throws Exception {
    LeadPortalWorkerImagePackageDto dto =
    ...
    mockMvc
        .perform(MockMvcRequestBuilders.get("/api/worker/image-packages/recent"))
  ...
  void startProcessingCallsService() throws Exception {
    mockMvc
        .perform(MockMvcRequestBuilders.post("/api/worker/image-packages/{id}/start", 42L))
  ...
  void markFailedForwardsPayload() throws Exception {
    String body =
        objectMapper.writeValueAsString(new LeadPortalWorkerImageFailureRequest("Erro qualquer"));

    mockMvc
        .perform(
            MockMvcRequestBuilders.post("/api/worker/image-packages/{id}/fail", 100L)
  ...
  void retryDelegatesToService() throws Exception {
    String body =
        objectMapper.writeValueAsString(new LeadPortalWorkerImageRetryRequest("Erro temporário"));

    mockMvc
        .perform(
            MockMvcRequestBuilders.post("/api/worker/image-packages/{id}/retry", 55L)
                .contentType(MediaType.APPLICATION_JSON)
                .content(body))
        .andExpect(status().isNoContent());
  ...
  void markFailedWithBlankReasonReturnsBadRequest() throws Exception {
    ...
    mockMvc
        .perform(
            MockMvcRequestBuilders.post("/api/worker/image-packages/{id}/fail", 11L)
                .contentType(MediaType.APPLICATION_JSON)
                .content(body))
  ...
  void submitResultsDelegatesToService() throws Exception {
    ...
    mockMvc
        .perform(
            MockMvcRequestBuilders.post("/api/worker/image-packages/{id}/results", 5L)
                .contentType(MediaType.APPLICATION_JSON)
                .content(body))
        .andExpect(status().isNoContent());

    ArgumentCaptor<LeadPortalWorkerImageResultRequest> captor =
        ArgumentCaptor.forClass(LeadPortalWorkerImageResultRequest.class);
