...
class MdsAdminControllerContractTest {
  ...
  @MockBean private MdsAdminService adminService;

  @MockBean private MdsArtifactService artifactService;

  @MockBean private MdsAdminAuthorizationService authorizationService;
  ...
  void shouldListRequestsWithExpectedContract() throws Exception {
    ...
    when(adminService.listRequests(any(), any(), any(), any(), eq(0), eq(20)))
        .thenReturn(
            new MdsAdminRequestListResponse(
                List.of(
                    new MdsAdminRequestListItemResponse(
                        12L,
                        "fitness",
                        "plateau",
                        "perder gordura",
    ...
    mockMvc
        .perform(get("/api/mds/requests").header("X-User-Role", "ADMIN"))
  ...
  void shouldGetRequestDetail() throws Exception {
    doNothing().when(authorizationService).assertAllowed("ADMIN");
    when(adminService.getRequestDetail(9L))
        .thenReturn(
            new MdsAdminRequestDetailResponse(
                9L,
                ...
                List.of(
                    new MdsAdminProcessingEventResponse(
                        71L,
                        "pipeline",
    ...
    mockMvc
        .perform(get("/api/mds/requests/9").header("X-User-Role", "ADMIN"))
        .andExpect(status().isOk())
  ...
  void shouldRetryRequest() throws Exception {
    doNothing().when(authorizationService).assertAllowed("ADMIN");
    when(adminService.retryRequest(22L))
        .thenReturn(
            new MdsAdminRetryResponse(
                22L, MdsRequestStatus.FAILED, MdsRequestStatus.PENDING, "retry accepted"));

    mockMvc
        .perform(
            post("/api/mds/requests/22/retry")
  ...
  void shouldListArtifactsWithCanonicalEnvelopeAndLineage() throws Exception {
    doNothing().when(authorizationService).assertAllowed("ADMIN");
    when(adminService.listArtifactsWithLineage(12L))
        .thenReturn(
            new MdsAdminArtifactsResponse(
                12L,
                List.of(
                ...
                List.of(
                    new MdsAdminArtifactLineageEdgeResponse(9001L, 301L, 401L, "DERIVED_FROM"))));

    mockMvc
        .perform(get("/api/mds/requests/12/artifacts").header("X-User-Role", "ADMIN"))
        .andExpect(status().isOk())
  ...
  void shouldGetReport() throws Exception {
    ...
    mockMvc
        .perform(get("/api/mds/reports/12").header("X-User-Role", "ADMIN"))
        .andExpect(status().isOk())
