...
class MdsInternalControllerContractTest {
  ...
  @MockBean private MdsRequestService requestService;

  @MockBean private MdsArtifactService artifactService;

  @MockBean private MdsSourceAccessService sourceAccessService;
  ...
  void shouldClaimRequestWithExpectedContract() throws Exception {
    ...
    mockMvc
        .perform(
            post("/api/internal/mds/requests/7/claim")
  ...
  void shouldRejectLineageWhenPathIdDoesNotMatchBody() throws Exception {
    mockMvc
        .perform(
            post("/api/internal/mds/artifacts/31/lineage")
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    """
                                {
  ...
  void shouldCreateLineageWhenPathIdMatchesBody() throws Exception {
    ...
    mockMvc
        .perform(
            post("/api/internal/mds/artifacts/30/lineage")
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    """
                                {
  ...
  void shouldPublishSourceAccessRecordsWithExpectedContract() throws Exception {
    ...
    mockMvc
        .perform(
            post("/api/internal/mds/source-access/publish-batch")
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    """
                                {
  ...
  void shouldGetRecommendedMechanismByRequest() throws Exception {
    when(artifactService.getRecommendedMechanismByRequest(12L))
        .thenReturn(
            new com.marketinghub.mds.dto.MdsRecommendedMechanismResponse(
                12L,
                401L,
                "mechanismSpec",
                "v1",
    ...
    mockMvc
        .perform(get("/api/internal/mds/requests/12/recommended-mechanism"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.requestId").value(12))
  ...
  void shouldGetDiscoveryReportByRequest() throws Exception {
    when(artifactService.getReportByRequest(12L))
        .thenReturn(
            new com.marketinghub.mds.dto.MdsReportResponse(
                12L,
                501L,
    ...
    mockMvc
        .perform(get("/api/internal/mds/reports/12"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.requestId").value(12))
  ...
  void shouldListArtifactsByRequest() throws Exception {
    when(artifactService.listArtifactsByRequest(12L))
        .thenReturn(
            java.util.List.of(
                new com.marketinghub.mds.dto.MdsArtifactSummaryResponse(
                    401L, "mechanismSpec", "v1", "v1", "DRAFT"),
                new com.marketinghub.mds.dto.MdsArtifactSummaryResponse(
                    402L, "practicalKnowledgePack", "v1", "v1", "DRAFT")));

    mockMvc
        .perform(get("/api/internal/mds/requests/12/artifacts"))
        .andExpect(status().isOk())
