...
class MetaAudienceArchitectureTest {
  ...
  @ArchTest
  static final ArchRule metaAudienceBackendMustRemainReadWriteOnly =
      classes()
          .that()
          .resideInAPackage(META_AUDIENCE_PACKAGE + "..")
          .should(notAssumeOperationalExecutionResponsibilityForMetaAudience())
          .because(
              "[ARQUITETURA] [BACKEND][MetaAudience] o backend deve registrar planos CNAE, expor pending e receber "
  ...
  private static ArchCondition<JavaClass>
      notAssumeOperationalExecutionResponsibilityForMetaAudience() {
                ...
                dependency ->
                    events.add(
                        SimpleConditionEvent.violated(
                            item,
                            "[ARQUITETURA] [BACKEND][MetaAudience] "
                                + item.getName()
                                + " depende de "
