...
class MicroserviceExceptionServiceTest {
  ...
  @Autowired private MicroserviceExceptionRepository exceptionRepository;
  ...
  void setUp() {
    service =
        new MicroserviceExceptionService(
            exceptionRepository, microserviceRepository, new ObjectMapper());
  }

  @Test
  void shouldLogAndListExceptions() {
    Microservice microservice =
        microserviceRepository.save(
            Microservice.builder().name("Background Worker").status("ACTIVE").build());

    MicroserviceExceptionRequest request =
        new MicroserviceExceptionRequest(
  ...
  void shouldSummarizeLatestException() {
    Microservice microservice =
        microserviceRepository.save(
            Microservice.builder().name("AI Worker").status("ACTIVE").build());
    ...
    Map<Long, MicroserviceExceptionSummary> summaries =
        service.summarizeByMicroservices(List.of(microservice));
