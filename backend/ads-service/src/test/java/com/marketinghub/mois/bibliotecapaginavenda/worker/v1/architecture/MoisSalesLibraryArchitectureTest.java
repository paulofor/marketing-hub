...
class MoisSalesLibraryArchitectureTest {
  ...
  @ArchTest
  static final ArchRule backend_mois_sales_library_nao_deve_chamar_openai_diretamente =
      classes()
          .that()
          .resideInAPackage("..mois.bibliotecapaginavenda.worker.v1..")
          .should(notDependOnOpenAiRuntimeClients())
          .because(
              "[ARQUITETURA] backend MOIS deve persistir contratos/custos; OpenAI deve rodar no mois-sales-library-worker");
        ...
        for (Dependency dependency : javaClass.getDirectDependenciesFromSelf()) {
          ...
          events.add(
              new SimpleConditionEvent(
                  ...
                  "[ARQUITETURA] "
                      + javaClass.getName()
                      + " não pode depender de client runtime OpenAI "
                      + target.getName()));
        }
      }
    };
