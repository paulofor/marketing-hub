...
class MoisSalesLibraryServiceTest {
  ...
  @Mock private MoisSalesLibraryPricingService pricingService;

  @InjectMocks private MoisSalesLibraryService service;
  ...
  void shouldCreatePendingJobWhenUrlIsNew() {
    MoisSalesLibraryDtos.SalesLibraryIngestRequest request =
        new MoisSalesLibraryDtos.SalesLibraryIngestRequest(
            "10",
            "hotmart",
            List.of(
                new MoisSalesLibraryDtos.SalesLibraryUrlItem(
                    "https://example.com/pagina", "Title", Instant.parse("2026-05-19T00:00:00Z"))));
    ...
    verify(jdbcTemplate)
        .update(
            contains("INSERT INTO mois_sales_page_job_execution"), eq(99L), any(), any(), any());
  }
  ...
  void shouldNotCreatePendingJobWhenUrlAlreadyExists() {
    ...
    verify(jdbcTemplate, never())
        .update(contains("INSERT INTO mois_sales_page_job_execution"), any(), any(), any(), any());
  }
  ...
  void shouldIngestLatestHotmartCollectedProductsWithLimitOf400() throws Exception {
    MoisSalesLibraryDtos.SalesLibraryHotmartCollectedIngestRequest request =
        new MoisSalesLibraryDtos.SalesLibraryHotmartCollectedIngestRequest(
            "workspace-001", null, null);

    given(
            jdbcTemplate.query(
                contains("GROUP BY job_id"), isA(RowMapper.class), eq("workspace-001")))
        .willReturn(List.of("hotmart-job-400"));
    given(
            jdbcTemplate.query(
                contains("SELECT reference_id"),
                isA(RowMapper.class),
                eq("workspace-001"),
                eq("hotmart-job-400"),
                eq(400)))
        .willAnswer(
            invocation -> {
              ...
              ResultSet secondRow =
                  collectedReferenceRow(
                      "ref-2",
                      "Produto com fallback",
                      null,
                      "https://produto.example/pagina",
                      null);
  ...
  /**
   * Garante que o resumo operacional considera captura correta somente quando html_bytes é maior
   * que zero.
   */
  @Test
  void shouldSummarizeCapturedPagesByUsefulHtmlBytes() {
    ...
    MoisSalesLibraryDtos.SalesLibraryPageSummaryResponse response =
        service.summarizePages("workspace-001");
    ...
    org.assertj.core.api.Assertions.assertThat(response.averageCapturesPerHour())
    ...
    org.assertj.core.api.Assertions.assertThat(response.totalModelCostUsd())
        .isEqualByComparingTo("1.75");
  }

  /**
   * Garante que horários DATETIME do MySQL são lidos como UTC antes de serem exibidos no fuso de
   * São Paulo.
   */
  @Test
  ...
  void shouldListPendingAnalysisWithoutClaimingJobs() {
    ...
    verify(jdbcTemplate)
        .query(
            sqlCaptor.capture(), isA(RowMapper.class), eq("workspace-001"), eq("HOTMART"), eq(25));
    org.assertj.core.api.Assertions.assertThat(sqlCaptor.getValue())
        .contains("COALESCE(sp.html_bytes, 0) > 0")
        .contains(
            "COALESCE(sp.analysis_status, sp.current_status) NOT IN ('DONE', 'ANALYZED', 'ANULADO', 'FETCHING')")
  ...
  void shouldListPagesOrderedByMostRecentAnalysisDate() {
    given(
            jdbcTemplate.queryForObject(
                contains("SELECT COUNT(*)"), eq(Long.class), eq("workspace-001")))
        .willReturn(42L);
    given(
            jdbcTemplate.query(
                any(String.class), isA(RowMapper.class), eq("workspace-001"), eq(20), eq(0)))
        .willReturn(List.of());

    MoisSalesLibraryDtos.SalesLibraryPageListResponse response =
        service.listPages("workspace-001", 1, 20);

    org.assertj.core.api.Assertions.assertThat(response.pageSize()).isEqualTo(20);
    ArgumentCaptor<String> sqlCaptor = ArgumentCaptor.forClass(String.class);
    verify(jdbcTemplate)
        .query(sqlCaptor.capture(), isA(RowMapper.class), eq("workspace-001"), eq(20), eq(0));
    org.assertj.core.api.Assertions.assertThat(sqlCaptor.getValue())
        .contains("LEFT JOIN mois_sales_page_market_warmup_summary mws")
        .contains(
            "cr_latest.reference_url = CONVERT(p.url_canonical USING utf8mb4) COLLATE utf8mb4_unicode_ci")
        .contains(
  ...
  /**
   * Garante que o ranking de oportunidades combina página, aquecimento, saturação e recência no
   * backend.
   */
  @Test
  void shouldRankMarketWarmupOpportunitiesByCombinedCommercialScore() throws Exception {
    given(
            jdbcTemplate.query(
                contains("combined_commercial_score"),
                isA(RowMapper.class),
                eq("workspace-001"),
                eq(5)))
        .willAnswer(
            invocation -> {
              ...
              given(row.getBigDecimal("combined_commercial_score"))
                  .willReturn(BigDecimal.valueOf(79.5));
              ...
              given(row.getString("next_experiment_suggestion"))
                  .willReturn("Criar experimento com promessa direta.");
    ...
    org.assertj.core.api.Assertions.assertThat(response.items().get(0).combinedCommercialScore())
        .isEqualByComparingTo("79.5");
    org.assertj.core.api.Assertions.assertThat(response.items().get(0).suggestedNextAction())
        .isEqualTo("Criar experimento com promessa direta.");
    ArgumentCaptor<String> sqlCaptor = ArgumentCaptor.forClass(String.class);
    verify(jdbcTemplate)
        .query(sqlCaptor.capture(), isA(RowMapper.class), eq("workspace-001"), eq(5));
    org.assertj.core.api.Assertions.assertThat(sqlCaptor.getValue())
  ...
  void shouldClaimAnalysisJobWithCapturedRawHtml() throws Exception {
    ...
    given(
            jdbcTemplate.query(
                contains("SELECT e.id AS job_id"),
                isA(RowMapper.class),
                eq("workspace-001"),
                eq("HOTMART")))
        .willAnswer(
            invocation -> {
              ...
              return List.of(
                  mapper.mapRow(
                      analysisClaimRow(77L, 99L, "<html><body>Oferta capturada</body></html>"), 0));
            });
    given(jdbcTemplate.update(contains("SET status = 'FETCHING'"), any(), eq(77L))).willReturn(1);
    given(jdbcTemplate.update(contains("SET current_stage = 'ANALYSIS'"), eq(77L), eq(99L)))
        .willReturn(1);
    ...
    org.assertj.core.api.Assertions.assertThat(response.job().rawHtml())
        .contains("Oferta capturada");
  }

  /**
   * Garante que a etapa 2 cria fila real para páginas capturadas que ainda não possuem análise
   * ativa.
   */
  @Test
  void shouldCreatePendingAnalysisForCapturedPageWhenClaimQueueIsEmpty() throws Exception {
    ...
    given(
        ...
        .willAnswer(
            invocation -> {
              ...
              return List.of(
                  mapper.mapRow(
                      analysisClaimRow(88L, 100L, "<html><body>Oferta refileirada</body></html>"),
                      0));
            });
    given(
            jdbcTemplate.query(
                contains("COALESCE(MAX(all_analysis.attempt), 0) + 1"),
                isA(RowMapper.class),
                eq("workspace-001"),
                eq("HOTMART")))
        .willAnswer(
    ...
    given(
            jdbcTemplate.update(
                contains("INSERT INTO mois_sales_page_job_execution"),
                eq(100L),
                eq("workspace-001"),
                eq(2),
                eq("https://example.com/oferta"),
                eq("AUTO_STAGE_2_CAPTURED_HTML")))
        .willReturn(1);
    given(jdbcTemplate.queryForObject(contains("SELECT LAST_INSERT_ID()"), eq(Long.class)))
        .willReturn(88L);
    given(jdbcTemplate.update(contains("SET current_stage = 'ANALYSIS'"), eq(88L), eq(100L)))
        .willReturn(1);
    ...
    org.assertj.core.api.Assertions.assertThat(response.job().rawHtml())
        .contains("Oferta refileirada");
  }

  /**
   * Garante que o claim de HTML bruto lê a tabela de referências coletadas e retorna a URL
   * reservada.
   */
  @Test
  void shouldClaimCollectedReferenceHtmlFromCollectedReferenceTable() throws Exception {
    ...
    given(
            jdbcTemplate.query(
                contains("FROM mois_collected_reference r"),
                isA(RowMapper.class),
                eq("workspace-001"),
                eq("HOTMART"),
                eq(2000)))
    ...
    given(
            jdbcTemplate.query(
                ...
                eq("workspace-001")))
        .willReturn(List.of());
    given(jdbcTemplate.update(contains("INSERT INTO mois_sales_page"), any(), any(), eq(20L)))
        .willReturn(1);
    given(
            jdbcTemplate.query(
                contains("SELECT workspace_id FROM mois_collected_reference"),
                isA(RowMapper.class),
                eq(20L)))
        .willReturn(List.of("workspace-001"));
    given(
            jdbcTemplate.query(
                contains("WHERE workspace_id = ? AND url_canonical = ?"),
                isA(RowMapper.class),
                eq("workspace-001"),
                eq("https://go.hotmart.com/A1")))
        .willReturn(List.of(99L));
    given(
            jdbcTemplate.update(
                contains("INSERT INTO mois_sales_page_job_execution"), any(), eq(99L)))
        .willReturn(1);
    given(jdbcTemplate.queryForObject(contains("SELECT LAST_INSERT_ID()"), eq(Long.class)))
        .willReturn(10L);
    given(
            jdbcTemplate.update(
                contains("UPDATE mois_sales_page SET last_job_execution_id"), eq(10L), eq(99L)))
        .willReturn(1);

    MoisSalesLibraryDtos.CollectedReferenceHtmlClaimResponse response =
        service.claimCollectedReferenceHtml(request);

    org.assertj.core.api.Assertions.assertThat(response.claimed()).isTrue();
    org.assertj.core.api.Assertions.assertThat(response.job().url())
        .isEqualTo("https://go.hotmart.com/A1");
    org.assertj.core.api.Assertions.assertThat(response.job().urlSource())
        .isEqualTo("SALES_PAGE_URL");
  ...
  /**
   * Garante que o claim pula referências brutas cuja URL canônica já está consolidada na
   * biblioteca.
   */
  @Test
  void shouldSkipAlreadyConsolidatedCollectedReferenceHtmlCandidate() throws Exception {
    ...
    given(
        ...
        .willAnswer(
            invocation -> {
              RowMapper<?> mapper = invocation.getArgument(1);
              ResultSet alreadyConsolidated =
                  htmlCaptureRow(20L, "ref-1", "https://go.hotmart.com/A1");
    ...
    given(jdbcTemplate.update(contains("INSERT INTO mois_sales_page"), any(), any(), eq(21L)))
        .willReturn(1);
    given(
            jdbcTemplate.query(
                contains("SELECT workspace_id FROM mois_collected_reference"),
    ...
    given(
            jdbcTemplate.query(
                ...
                isA(RowMapper.class),
                eq("workspace-001"),
                eq("https://go.hotmart.com/B2")))
        .willReturn(List.of(100L));
    given(
            jdbcTemplate.update(
                contains("INSERT INTO mois_sales_page_job_execution"), any(), eq(100L)))
        .willReturn(1);
    given(jdbcTemplate.queryForObject(contains("SELECT LAST_INSERT_ID()"), eq(Long.class)))
        .willReturn(11L);
    given(
            jdbcTemplate.update(
                contains("UPDATE mois_sales_page SET last_job_execution_id"), eq(11L), eq(100L)))
        .willReturn(1);
    ...
    org.assertj.core.api.Assertions.assertThat(response.claimed()).isTrue();
    org.assertj.core.api.Assertions.assertThat(response.job().collectedReferenceId())
        .isEqualTo(21L);
    org.assertj.core.api.Assertions.assertThat(response.job().url())
        .isEqualTo("https://go.hotmart.com/B2");
  }
  ...
  void shouldCompleteCollectedReferenceHtmlCapture() {
    ...
    given(
            jdbcTemplate.query(
                contains("FROM mois_sales_page_job_execution"), isA(RowMapper.class), eq(10L)))
        .willAnswer(
            invocation -> {
              RowMapper<?> mapper = invocation.getArgument(1);
    ...
    MoisSalesLibraryDtos.CollectedReferenceHtmlPersistResponse response =
        service.completeCollectedReferenceHtml(10L, request);

    verify(jdbcTemplate)
        .update(
            contains("UPDATE mois_sales_page_job_execution"),
            eq("https://final.example/oferta"),
            eq(200),
            eq("text/html"),
            eq("<html><body>Oferta</body></html>"),
            any(),
            eq(32),
            any(),
            eq(10L));
  ...
  void shouldSummarizeUniqueCollectedReferenceUrlsWithoutRawLineCounts() throws Exception {
    given(
            jdbcTemplate.query(
                contains("SELECT DISTINCT source, url_source, effective_url"),
                isA(RowMapper.class),
                eq("workspace-001")))
        .willAnswer(
            invocation -> {
              ...
              return List.of(
                  mapper.mapRow(
                      collectedReferenceUrlSummaryRow(
                          "HOTMART", "SALES_PAGE_URL", "https://offer.example/a?utm=1"),
                      0),
                  mapper.mapRow(
                      collectedReferenceUrlSummaryRow(
                          "HOTMART", "PRODUCT_URL", "https://product.example/p?ref=1"),
                      1),
                  mapper.mapRow(
                      collectedReferenceUrlSummaryRow(
                          "CLICKBANK",
                          "SALES_PAGE_URL",
                          "https://hop.clickbank.net/?affiliate=abc&vendor=x"),
                      2));
            });
    given(
            jdbcTemplate.query(
                contains("SELECT url_canonical FROM mois_sales_page"),
                isA(RowMapper.class),
                eq("workspace-001")))
        .willAnswer(
            invocation -> {
    ...
    MoisSalesLibraryDtos.CollectedReferenceUrlSummaryResponse response =
        service.summarizeCollectedReferenceUrls("workspace-001");
    ...
    org.assertj.core.api.Assertions.assertThat(response.missingFromOperationalLibrary())
        .isEqualTo(2);
  ...
  /**
   * Garante que a listagem de entradas usa a tabela operacional nova, sem ler a tabela legada de
   * URL.
   */
  @Test
  void shouldListEntriesFromOperationalSalesPageTable() throws Exception {
    given(
            jdbcTemplate.queryForObject(
                contains("SELECT COUNT(*) FROM mois_sales_page"),
                eq(Long.class),
                eq("workspace-001")))
        .willReturn(1L);
    given(
            jdbcTemplate.query(
                contains("FROM mois_sales_page"),
                isA(RowMapper.class),
                eq("workspace-001"),
                eq(20),
                eq(0)))
    ...
    MoisSalesLibraryDtos.SalesLibraryEntryPageResponse response =
        service.listEntries("workspace-001", 1, 20);

    org.assertj.core.api.Assertions.assertThat(response.total()).isEqualTo(1);
    org.assertj.core.api.Assertions.assertThat(response.items()).hasSize(1);
    org.assertj.core.api.Assertions.assertThat(response.items().get(0).urlCanonical())
        .isEqualTo("https://example.com/pagina");
    verify(jdbcTemplate, never())
        .queryForObject(contains("mois_sales_library_url_ingest"), eq(Long.class), any());
    verify(jdbcTemplate, never())
        .query(
            contains("FROM mois_sales_library_url_ingest"),
            isA(RowMapper.class),
            any(),
            any(),
            any());
  ...
  void shouldListHotProductDossierCandidates() throws Exception {
    given(
            jdbcTemplate.query(
                contains("pipeline_dossieproduto pd"),
                isA(RowMapper.class),
                eq("workspace-001"),
                eq(BigDecimal.valueOf(80)),
                eq("warmupecosystem.v1"),
                eq(10)))
        .willAnswer(
            invocation -> {
              RowMapper<?> mapper = invocation.getArgument(1);
    ...
    org.assertj.core.api.Assertions.assertThat(response.pipelineCode())
        .isEqualTo("warmupecosystem.v1");
    org.assertj.core.api.Assertions.assertThat(response.items().get(0).pageId()).isEqualTo(401L);
    org.assertj.core.api.Assertions.assertThat(response.items().get(0).hotmartTemperature())
        .isEqualByComparingTo("127.07");
  }
  ...
  void shouldNormalizeCollationWhenListingHotProductDossierCandidates() {
    ...
    org.assertj.core.api.Assertions.assertThat(sqlCaptor.getValue())
        .contains(
            "CONVERT(COALESCE(sales_page_url, product_url) USING utf8mb4) COLLATE utf8mb4_unicode_ci")
        .contains(
            "cr_latest.reference_url = CONVERT(p.url_canonical USING utf8mb4) COLLATE utf8mb4_unicode_ci")
        .contains(
            "pd.id_externo COLLATE utf8mb4_unicode_ci = CONVERT(CAST(p.id AS CHAR) USING utf8mb4) COLLATE utf8mb4_unicode_ci")
        .contains(
            "pd.pipeline_code COLLATE utf8mb4_unicode_ci = CONVERT(? USING utf8mb4) COLLATE utf8mb4_unicode_ci");
  }
  ...
  void shouldEnqueueHotProductDossierCandidates() {
    given(
        ...
        .willReturn(
            List.of(
                new MoisSalesLibraryDtos.HotProductDossierCandidateItem(
                    401L,
                    "workspace-001",
                    "HOTMART",
    ...
    given(jdbcTemplate.update(contains("UPDATE mois_sales_page"), any(), any(), any()))
        .willReturn(1);
    given(
            jdbcTemplate.update(
                contains("INSERT INTO pipeline_dossieproduto"),
                any(),
                any(),
                any(),
                any(),
    ...
    MoisSalesLibraryDtos.HotProductDossierEnqueueResponse response =
        service.enqueueHotProductDossierCandidates(
            new MoisSalesLibraryDtos.HotProductDossierEnqueueRequest(
                "workspace-001", BigDecimal.valueOf(80), 5));

    org.assertj.core.api.Assertions.assertThat(response.enqueued()).isEqualTo(1);
    org.assertj.core.api.Assertions.assertThat(response.pipelineCode())
        .isEqualTo("warmupecosystem.v1");
    org.assertj.core.api.Assertions.assertThat(response.items().get(0).stageCode())
        .isEqualTo("intake");
    verify(jdbcTemplate)
        .update(
            contains("INSERT INTO pipeline_dossieproduto"),
            ...
            eq("warmupecosystem.v1"));
    verify(jdbcTemplate)
        .update(contains("status_pipeline_dossieproduto"), eq("INICIADO"), eq("intake"), eq(401L));
  }
  ...
  void shouldEnqueueSalesPagePatternsWithoutTouchingWarmupLegacyQueue() {
    ...
    MoisSalesLibraryDtos.HotProductDossierEnqueueResponse response =
        service.enqueueHotProductSalesPagePatternCandidates(
            new MoisSalesLibraryDtos.HotProductDossierEnqueueRequest(
                "workspace-001", BigDecimal.valueOf(80), 5));

    org.assertj.core.api.Assertions.assertThat(response.pipelineCode())
        .isEqualTo("salespagepatterns.v1");
    org.assertj.core.api.Assertions.assertThat(response.items().get(0).stageCode())
        .isEqualTo("page-pattern-extraction");
    verify(jdbcTemplate)
        .update(
            contains("INSERT INTO pipeline_dossieproduto"),
            eq("401"),
            eq("page-pattern-extraction"),
            eq("INICIADO"),
            any(),
            eq("v1"),
            eq("salespagepatterns.v1"));
    verify(jdbcTemplate, never())
        .update(contains("status_pipeline_dossieproduto"), any(), any(), any());
  }
  ...
  private void stubOperationalIngest(int... upsertResults) {
    lenient()
        .when(
            jdbcTemplate.update(
                contains("INSERT INTO mois_sales_page"),
                any(),
                any(),
                any(),
                any(),
                ...
                any()))
        .thenReturn(
            upsertResults.length == 0 ? 1 : upsertResults[0],
            java.util.Arrays.stream(upsertResults).skip(1).boxed().toArray(Integer[]::new));
    lenient()
        .when(
            jdbcTemplate.query(
                contains("WHERE workspace_id = ? AND url_canonical = ?"),
    ...
    lenient()
        .when(
            jdbcTemplate.query(
                contains("SELECT p.id, p.workspace_id, p.source"), isA(RowMapper.class), any()))
        .thenReturn(
            List.of(
                new MoisSalesLibraryDtos.SalesLibraryPageResponse(
                    99L,
                    "10",
                    "HOTMART",
    ...
    lenient()
        .when(
            jdbcTemplate.update(
                contains("INSERT INTO mois_sales_page_job_execution"), any(), any(), any()))
        .thenReturn(1);
    lenient()
        .when(jdbcTemplate.queryForObject(contains("SELECT LAST_INSERT_ID()"), eq(Long.class)))
        .thenReturn(123L);
    lenient()
        .when(jdbcTemplate.update(contains("UPDATE mois_sales_page"), any(), any()))
        .thenReturn(1);
  }

  /** Monta uma linha simulada de URL efetiva única da origem bruta coletada. */
  private ResultSet collectedReferenceUrlSummaryRow(
      String source, String urlSource, String effectiveUrl) throws Exception {
  ...
  private ResultSet hotProductDossierCandidateRow() throws Exception {
    ...
    given(resultSet.getTimestamp(eq("dossie_produto_updated_at"), any(Calendar.class)))
        .willReturn(null);
  ...
  private ResultSet htmlCaptureRow(long collectedReferenceId, String referenceId, String url)
      throws Exception {
    ResultSet resultSet = org.mockito.Mockito.mock(ResultSet.class);
  ...
  private ResultSet collectedReferenceRow(
      String referenceId, String productName, String salesPageUrl, String productUrl, String url)
      throws Exception {
    ResultSet resultSet = org.mockito.Mockito.mock(ResultSet.class);
    given(resultSet.getString("reference_id")).willReturn(referenceId);
    ...
    given(resultSet.getTimestamp("collected_at"))
        .willReturn(Timestamp.from(Instant.parse("2026-06-01T21:00:24Z")));
    return resultSet;
  }
}
