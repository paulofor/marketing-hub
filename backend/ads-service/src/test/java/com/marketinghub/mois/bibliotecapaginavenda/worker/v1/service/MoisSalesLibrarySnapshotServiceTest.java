...
public class MoisSalesLibrarySnapshotServiceTest {
  ...
  void setUp() throws IOException {
    ...
    jdbcTemplate.execute(
        "CREATE ALIAS IF NOT EXISTS UTC_TIMESTAMP FOR \"com.marketinghub.mois.bibliotecapaginavenda.worker.v1.service.MoisSalesLibrarySnapshotServiceTest.utcTimestamp\"");
    createSchema();
  ...
  /**
   * Garante que o pipeline registre o destino redirecionado e capture a raiz quando o caminho final
   * falha.
   */
  @Test
  void shouldTryRedirectRootWhenRedirectDestinationIsNotCapturable() {
    ...
    assertThat(response.failed()).isZero();
    assertThat(response.items().getFirst().status()).isEqualTo("CAPTURED");
    assertThat(response.items().getFirst().redirectDestinationUrl())
        .isEqualTo(baseUrl + "/missing");
  ...
  /**
   * Garante que páginas com html_bytes > 0 não sejam recapturadas nem em acionamento forçado da
   * etapa 1.
   */
  @Test
  ...
  /**
   * Garante que a etapa 1 reprocesse páginas sem HTML útil mesmo quando o status anterior dizia
   * CAPTURED.
   */
  @Test
  void shouldProcessCapturedStatusWhenHtmlBytesIsZero() {
    ...
    assertThat(response.items().getFirst().rawHtmlBytes()).isPositive();
    Long persistedHtmlBytes =
        jdbcTemplate.queryForObject(
            "SELECT html_bytes FROM mois_sales_page WHERE id = 10", Long.class);
  ...
  private void insertPageWithState(
      long pageId, String url, String currentStage, String currentStatus, long htmlBytes) {
    jdbcTemplate.update(
        """
