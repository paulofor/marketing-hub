...
class MoisSalesLibraryControllerTest {
  ...
  @MockBean private MoisSalesLibraryService service;

  @MockBean private MoisSalesLibrarySnapshotService snapshotService;

  @MockBean private MoisSalesPageMarketWarmupService marketWarmupService;

  @Test
  void shouldExposeSnapshotCaptureEndpoint() throws Exception {
    when(snapshotService.captureSnapshots(
            any(MoisSalesLibraryDtos.SalesLibrarySnapshotCaptureRequest.class)))
        .thenReturn(
            new MoisSalesLibraryDtos.SalesLibrarySnapshotCaptureResponse(
                "workspace-001",
                3,
                false,
                1,
                ...
                0,
                List.of(
                    new MoisSalesLibraryDtos.SalesLibrarySnapshotCaptureItem(
                        10L,
                        20L,
    ...
    mockMvc
        ...
        .andExpect(
            jsonPath("$.items[0].redirectDestinationUrl").value("https://example.test/sales"))
  ...
  void shouldExposePageSnapshotsEndpoint() throws Exception {
    ...
    mockMvc
        .perform(get("/api/mois/sales-library/pages/10/snapshots"))
  ...
  void shouldExposeHotProductDossierCandidatesEndpoint() throws Exception {
    when(service.listHotProductDossierCandidates("workspace-001", BigDecimal.valueOf(80), 10))
        .thenReturn(
            new MoisSalesLibraryDtos.HotProductDossierCandidateResponse(
                "workspace-001",
                "warmupecosystem.v1",
    ...
    mockMvc
        .perform(
            get("/api/mois/sales-library/hot-products/dossier-candidates")
                .param("workspaceId", "workspace-001")
  ...
  void shouldExposeHotProductDossierEnqueueEndpoint() throws Exception {
    when(service.enqueueHotProductDossierCandidates(
            any(MoisSalesLibraryDtos.HotProductDossierEnqueueRequest.class)))
        .thenReturn(
            new MoisSalesLibraryDtos.HotProductDossierEnqueueResponse(
                "workspace-001",
                "warmupecosystem.v1",
                "Aquecimento e Ecossistema",
                BigDecimal.valueOf(80),
    ...
    mockMvc
        .perform(
            post("/api/mois/sales-library/hot-products/dossier-candidates:enqueue")
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    """
                                {
  ...
  void shouldExposeSalesPagePatternsCandidatesEndpoint() throws Exception {
    when(service.listHotProductSalesPagePatternCandidates(
            "workspace-001", BigDecimal.valueOf(80), 10))
        .thenReturn(
            new MoisSalesLibraryDtos.HotProductDossierCandidateResponse(
                "workspace-001",
  ...
  void shouldExposeWarmupEcosystemCandidatesEndpoint() throws Exception {
    when(service.listHotProductWarmupEcosystemCandidates(
            "workspace-001", BigDecimal.valueOf(80), 10))
        .thenReturn(
            new MoisSalesLibraryDtos.HotProductDossierCandidateResponse(
                "workspace-001",
  ...
  void shouldExposeConsolidatedPagesSummaryEndpoint() throws Exception {
    ...
    mockMvc
        .perform(get("/api/mois/sales-library/pages/summary").param("workspaceId", "workspace-001"))
        .andExpect(status().isOk())
  ...
  /**
   * Garante que o endpoint pending expõe páginas elegíveis para diagnóstico da etapa 2 sem reservar
   * job.
   */
  @Test
  void shouldExposePendingAnalysisEndpoint() throws Exception {
    when(service.listPendingAnalysis("workspace-001", "HOTMART", 25))
        .thenReturn(
            new MoisSalesLibraryDtos.SalesLibraryPendingAnalysisResponse(
                "workspace-001",
                "HOTMART",
                25,
                1,
    ...
    mockMvc
        .perform(
            get("/api/mois/sales-library/pending")
                .param("workspaceId", "workspace-001")
  ...
  void shouldExposeConsolidatedPageExecutionsEndpoint() throws Exception {
    ...
    mockMvc
        .perform(get("/api/mois/sales-library/pages/10/executions"))
        .andExpect(status().isOk())
  ...
  void shouldExposeMarketWarmupRequestEndpoint() throws Exception {
    when(marketWarmupService.requestResearch(10L))
        .thenReturn(
            new MoisSalesLibraryDtos.MarketWarmupRequestResponse(
                10L,
                99L,
    ...
    mockMvc
        .perform(post("/api/mois/sales-library/pages/10/market-warmup:request"))
        .andExpect(status().isAccepted())
  ...
  void shouldMapMarketWarmupRequestBeforeAnalysisToHttp409() throws Exception {
    doThrow(
            new IllegalStateException(
                "Dossiê MOIS só pode ser iniciado depois da análise comercial da página."))
        .when(marketWarmupService)
        .requestResearch(10L);
  ...
  void shouldExposeMarketWarmupSummaryEndpoint() throws Exception {
    when(marketWarmupService.getSummary(10L))
        .thenReturn(
            new MoisSalesLibraryDtos.MarketWarmupSummaryResponse(
                99L,
                10L,
    ...
    mockMvc
        .perform(get("/api/mois/sales-library/pages/10/market-warmup"))
        .andExpect(status().isOk())
  ...
  void shouldExposeMarketWarmupSourcesEndpoint() throws Exception {
    when(marketWarmupService.listSources(10L))
        .thenReturn(
            new MoisSalesLibraryDtos.MarketWarmupSourceListResponse(
                10L,
                99L,
                List.of(
                    new MoisSalesLibraryDtos.MarketWarmupSourceResponse(
                        501L,
                        99L,
                        10L,
    ...
    mockMvc
        .perform(get("/api/mois/sales-library/pages/10/market-warmup/sources"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.pageId").value(10))
  ...
  void shouldExposeMarketWarmupSignalsEndpoint() throws Exception {
    when(marketWarmupService.listSignals(10L))
        .thenReturn(
            new MoisSalesLibraryDtos.MarketWarmupSignalListResponse(
                10L,
                99L,
                List.of(
                    new MoisSalesLibraryDtos.MarketWarmupSignalResponse(
                        701L,
                        99L,
                        501L,
                        10L,
    ...
    mockMvc
        .perform(get("/api/mois/sales-library/pages/10/market-warmup/signals"))
        .andExpect(status().isOk())
  ...
  void shouldExposeMarketWarmupClaimEndpoint() throws Exception {
    when(marketWarmupService.claimJob(any(MoisSalesLibraryDtos.MarketWarmupClaimRequest.class)))
        .thenReturn(
            new MoisSalesLibraryDtos.MarketWarmupClaimResponse(
                true,
  ...
  void shouldExposeMarketWarmupCompleteEndpoint() throws Exception {
    mockMvc
        .perform(
            post("/api/mois/sales-library/market-warmup/jobs/99:complete")
                .contentType(MediaType.APPLICATION_JSON)
                .content(marketWarmupCompletePayload()))
        .andExpect(status().isNoContent());

    verify(marketWarmupService)
        .completeJob(anyLong(), any(MoisSalesLibraryDtos.MarketWarmupCompleteRequest.class));
  }
  ...
  void shouldExposeMarketWarmupReprocessStaleEndpoint() throws Exception {
    when(marketWarmupService.reprocessStaleJobs(
            any(MoisSalesLibraryDtos.MarketWarmupReprocessStaleRequest.class)))
        .thenReturn(
            new MoisSalesLibraryDtos.MarketWarmupReprocessStaleResponse(
                "workspace-001", 120, 2L, Instant.parse("2026-06-10T14:00:00Z")));

    mockMvc
        .perform(
            post("/api/mois/sales-library/market-warmup/jobs:reprocess-stale")
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    """
                                {
  ...
  void shouldExposeMarketWarmupFailEndpoint() throws Exception {
    mockMvc
        .perform(
            post("/api/mois/sales-library/market-warmup/jobs/99:fail")
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    """
                                {
    ...
    verify(marketWarmupService)
        .failJob(anyLong(), any(MoisSalesLibraryDtos.MarketWarmupFailRequest.class));
  }
