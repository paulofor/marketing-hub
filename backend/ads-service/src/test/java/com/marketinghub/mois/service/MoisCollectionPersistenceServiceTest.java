...
class MoisCollectionPersistenceServiceTest {
  ...
  void setUp() {
    service =
        new MoisCollectionPersistenceService(
  ...
  void upsertJobStatePersistsPayloadInDatabase() {
    MoisCollectionPersistenceDtos.CollectionJobStateResponse state =
        sampleState("workspace-001", "COMPLETED");

    MoisCollectionPersistenceDtos.CollectionJobStateResponse response =
        service.upsertJobState("job-1", state);

    assertThat(response).isEqualTo(state);
    verify(jdbcTemplate)
        .update(anyString(), eq("job-1"), eq("workspace-001"), eq("COMPLETED"), anyString(), any());
    verify(jdbcTemplate).update("DELETE FROM mois_collected_reference WHERE job_id = ?", "job-1");
    verify(jdbcTemplate, never())
        .batchUpdate(
            anyString(), eq(List.of()), eq(0), any(ParameterizedPreparedStatementSetter.class));
  }
  ...
  void getJobStateReadsSerializedPayloadFromDatabase() throws Exception {
    MoisCollectionPersistenceDtos.CollectionJobStateResponse state =
        sampleState("workspace-001", "QUEUED");
    String payload = new ObjectMapper().findAndRegisterModules().writeValueAsString(state);

    when(jdbcTemplate.query(anyString(), any(RowMapper.class), eq("job-1")))
        .thenAnswer(
            invocation -> {
              RowMapper<MoisCollectionPersistenceDtos.CollectionJobStateResponse> mapper =
  ...
  void summarizeBySourceAggregatesReferences() throws Exception {
    MoisWorkspaceDtos.CollectedReferenceResponse hotmartRef1 =
        new MoisWorkspaceDtos.CollectedReferenceResponse(
            ...
            java.util.Map.of());
    MoisWorkspaceDtos.CollectedReferenceResponse hotmartRef2 =
        new MoisWorkspaceDtos.CollectedReferenceResponse(
            "ref-2",
            "job-1",
            "HOTMART",
            ...
            0.5,
            0.4,
            Instant.parse("2026-05-04T01:10:00Z"),
            java.util.Map.of());
    MoisWorkspaceDtos.CollectedReferenceResponse metaRef =
        new MoisWorkspaceDtos.CollectedReferenceResponse(
            "ref-3",
            "job-2",
            "META",
            ...
            0.3,
            0.2,
            Instant.parse("2026-05-04T01:20:00Z"),
            java.util.Map.of());
    MoisCollectionPersistenceDtos.CollectionJobStateResponse state =
    ...
    MoisCollectionPersistenceDtos.SourceHighlightListResponse response =
        service.summarizeBySource("workspace-001", "COMPLETED");
  ...
  private MoisCollectionPersistenceDtos.CollectionJobStateResponse sampleState(
    ...
    MoisWorkspaceDtos.CollectionJobResponse job =
        new MoisWorkspaceDtos.CollectionJobResponse(
            ...
            null);
    return new MoisCollectionPersistenceDtos.CollectionJobStateResponse(
        job, List.of(), java.util.Map.of(), null, List.of());
  }
}
