...
class MoisHotmartProductServiceTest {
  ...
  void shouldReturnEmptyListWhenThereIsNoJobForWorkspace() {
    ...
    when(jdbcTemplate.query(
            anyString(), any(org.springframework.jdbc.core.RowMapper.class), eq("workspace-001")))
        .thenReturn(List.of());
        ...
        .thenAnswer(
            invocation -> {
              org.springframework.jdbc.core.RowMapper<
                      MoisHotmartProductDtos.HotmartCollectedProductResponse>
                  mapper = invocation.getArgument(1);
              ...
              when(rs.getObject("hotmart_temperature"))
                  .thenReturn(java.math.BigDecimal.valueOf(88.5));
              when(rs.getDouble("hotmart_temperature")).thenReturn(88.5);
              when(rs.getTimestamp("collected_at"))
                  .thenReturn(Timestamp.from(Instant.parse("2026-06-13T12:00:00Z")));
              return List.of(mapper.mapRow(rs, 0));
            });

    MoisHotmartProductService service = new MoisHotmartProductService(jdbcTemplate);
    MoisHotmartProductDtos.HotmartCollectedProductListResponse response =
        service.listLatestByWorkspace("workspace-001", 1);
  ...
  void shouldListOnlyCycleTwoCandidatesNotAlreadyProcessed() throws Exception {
    JdbcTemplate jdbcTemplate = mock(JdbcTemplate.class);
    when(jdbcTemplate.query(
            contains("Coleta executada via API da Hotmart"),
            any(org.springframework.jdbc.core.RowMapper.class),
            eq("workspace-001")))
        .thenReturn(List.of("cycle-1-job"));
    when(jdbcTemplate.query(
            contains("processed.id IS NULL"),
            any(org.springframework.jdbc.core.RowMapper.class),
            eq("workspace-001"),
            eq("workspace-001"),
            eq("cycle-1-job"),
        ...
        .thenAnswer(
            invocation -> {
              ...
              when(rs.getObject("hotmart_temperature"))
                  .thenReturn(java.math.BigDecimal.valueOf(91.0));
              when(rs.getDouble("hotmart_temperature")).thenReturn(91.0);
              when(rs.getTimestamp("collected_at"))
                  .thenReturn(Timestamp.from(Instant.parse("2026-06-15T12:00:00Z")));
              return List.of(mapper.mapRow(rs, 0));
            });

    MoisHotmartProductService service = new MoisHotmartProductService(jdbcTemplate);
    MoisHotmartProductDtos.HotmartCollectedProductListResponse response =
        service.listCycleTwoCandidatesByWorkspace("workspace-001", 400);
