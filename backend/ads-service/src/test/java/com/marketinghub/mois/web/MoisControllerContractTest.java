...
class MoisControllerContractTest {
  ...
  @MockBean private MoisModuleGateway gateway;

  @MockBean private MoisCollectionPersistenceService collectionPersistenceService;

  @MockBean private MoisHotmartProductService moisHotmartProductService;

  @MockBean private MoisClickbaseProductService moisClickbaseProductService;
  ...
  void shouldAcceptDiscoveryRequest() throws Exception {
    when(gateway.createDiscoveryRequest(any(MoisDiscoveryDtos.CreateDiscoveryRequest.class)))
        .thenReturn(
            new MoisDiscoveryDtos.DiscoveryRequestAcceptedResponse("mois-req-123", "ACCEPTED"));

    mockMvc
        .perform(
            post("/api/v1/mois/discovery-requests")
  ...
  void shouldReturnCollectionOpsSummaryContract() throws Exception {
    when(gateway.getCollectionOpsSummary("workspace-001"))
        .thenReturn(
            new MoisWorkspaceDtos.CollectionOpsSummaryResponse(
                "workspace-001",
                true,
                2,
                0,
                ...
                1,
                List.of(
                    new MoisWorkspaceDtos.CollectionSourceOpsSummaryResponse(
                        "CLICKBANK",
                        2,
                        2,
                        0,
    ...
    mockMvc
        .perform(get("/api/v1/mois/workspaces/workspace-001/collection-ops/summary"))
  ...
  void shouldReturnDiscoveryRequestDetail() throws Exception {
    when(gateway.getDiscoveryRequest(eq("mois-req-001")))
        .thenReturn(
            Optional.of(
                new MoisDiscoveryDtos.DiscoveryRequestDetailResponse(
                    "mois-req-001",
                    "personal trainer",
                    "retencao de alunos",
                    ...
                    List.of(
                        new MoisDiscoveryDtos.ArtifactRefResponse(
                            "mois-art-001", "mois.marketOfferDiscoveryRequest.v1", "v1")))));

    mockMvc
        .perform(get("/api/v1/mois/discovery-requests/mois-req-001"))
        .andExpect(status().isOk())
  ...
  void shouldReturn404WhenOfferDoesNotExist() throws Exception {
    ...
    mockMvc.perform(get("/api/v1/mois/offers/unknown-offer")).andExpect(status().isNotFound());
  }
  ...
  void shouldReturnOfferListContract() throws Exception {
    when(gateway.listOffers(any(), any(), any()))
        .thenReturn(
            new MoisOfferDtos.OfferCardListResponse(
                List.of(
    ...
    mockMvc
        .perform(get("/api/v1/mois/offers"))
        .andExpect(status().isOk())
  ...
  void shouldPropagateCategoryFilterToInsightReportsEndpoint() throws Exception {
    ...
    mockMvc
        .perform(
            get("/api/v1/mois/insight-reports")
  ...
  void shouldExposeExecutiveSummaryEndpointContract() throws Exception {
    when(gateway.getInsightExecutiveSummary("mois-report-001"))
        .thenReturn(
            Optional.of(
                new MoisInsightDtos.InsightExecutiveSummaryResponse(
                    "mois-report-001",
                    "mois-req-001",
                    "nutricao",
                    "perda de peso",
                    new MoisInsightDtos.FrameworkRecommendationResponse(
                        "dor dominante", "resultado", "mecanismo", "prova", List.of("angulo")),
                    List.of(),
                    List.of(),
                    List.of("acao"))));

    mockMvc
        .perform(get("/api/v1/mois/insight-reports/mois-report-001/executive-summary"))
        .andExpect(status().isOk())
  ...
  void shouldExposeHotmartCycleTwoCandidatesEndpointContract() throws Exception {
    when(moisHotmartProductService.listCycleTwoCandidatesByWorkspace(eq("workspace-001"), eq(400)))
        .thenReturn(
            new MoisHotmartProductDtos.HotmartCollectedProductListResponse(
                "workspace-001",
                List.of(
    ...
    mockMvc
        .perform(
            get("/api/v1/mois/hotmart/products/cycle-2-candidates")
  ...
  void shouldReturnWorkspaceDashboardContract() throws Exception {
    when(gateway.getDashboard(eq("workspace-001")))
        .thenReturn(
            new MoisWorkspaceDtos.WorkspaceDashboardResponse(
                "workspace-001",
                new MoisWorkspaceDtos.WorkspaceKpisResponse(12, 4, 2, 1),
                "COLETA",
                List.of(
                    new MoisWorkspaceDtos.RecentAnalysisResponse(
                        "ref-1",
                        "nutricao",
                        "COLETA_CONCLUIDA",
    ...
    mockMvc
        .perform(get("/api/v1/mois/workspaces/workspace-001/dashboard"))
        .andExpect(status().isOk())
  ...
  void shouldValidateReferencePayload() throws Exception {
    mockMvc
        .perform(
            post("/api/v1/mois/references").contentType(MediaType.APPLICATION_JSON).content("{}"))
        .andExpect(status().isBadRequest());
  }
  ...
  void shouldListReferencesByWorkspace() throws Exception {
    when(gateway.listReferences(eq("workspace-001")))
        .thenReturn(
            new MoisWorkspaceDtos.ReferenceListResponse(
                List.of(
                    new MoisWorkspaceDtos.ReferenceResponse(
                        "ref-123",
                        "workspace-001",
    ...
    mockMvc
        .perform(get("/api/v1/mois/references").param("workspaceId", "workspace-001"))
        .andExpect(status().isOk())
  ...
  void shouldUpsertExtractionDraft() throws Exception {
    when(gateway.upsertExtractionDraft(
            eq("ref-123"), any(MoisWorkspaceDtos.UpsertExtractionDraftRequest.class)))
        .thenReturn(
            new MoisWorkspaceDtos.ExtractionDraftResponse(
                "ext-123", "ref-123", "DRAFT", Instant.parse("2026-04-25T12:00:00Z")));

    mockMvc
        .perform(
            post("/api/v1/mois/references/ref-123/extractions")
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    """
                                {
  ...
  void shouldReturnLibraryBlocksContract() throws Exception {
    when(gateway.listLibraryBlocks(eq("workspace-001"), eq("nutricao"), eq("CURSO")))
        .thenReturn(
            new MoisWorkspaceDtos.LibraryBlockListResponse(
                List.of(
  ...
  void shouldCreateComparisonContract() throws Exception {
    when(gateway.createComparison(any(MoisWorkspaceDtos.CreateComparisonRequest.class)))
        .thenReturn(
            new MoisWorkspaceDtos.ComparisonResponse(
                "comparison-1",
                "workspace-001",
                List.of(
                    new MoisWorkspaceDtos.ComparisonDimensionResponse(
                        "PROMESSA", "Mercado", "Atual", "Ajustar headline")),
                List.of(
                    new MoisWorkspaceDtos.ComparisonScorecardResponse(
                        "clareza", 70, "Boa direção com oportunidade de refinamento.")),
                List.of(
                    new MoisWorkspaceDtos.ComparisonImprovementResponse(
                        "imp-1", "HIGH", "Incluir prova mensurável"))));
  ...
  void shouldBuildOfferContract() throws Exception {
    when(gateway.buildOffer(any(MoisWorkspaceDtos.BuildOfferRequest.class)))
        .thenReturn(
            new MoisWorkspaceDtos.BuildOfferResponse(
                "offer-1",
                "workspace-001",
                "READY_TO_EXPORT",
                "Conteúdo proposto",
  ...
  void shouldCreateCollectionJobContract() throws Exception {
    ...
    mockMvc
        .perform(
            post("/api/v1/mois/collection-jobs")
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    """
                                {
  ...
  void shouldListCollectionJobsContract() throws Exception {
    when(collectionPersistenceService.listJobStates(eq("workspace-001"), eq("QUEUED")))
        .thenReturn(
            new MoisCollectionPersistenceDtos.CollectionJobStateListResponse(
                List.of(
    ...
    mockMvc
        .perform(
            get("/api/v1/mois/collection-jobs")
                .param("workspaceId", "workspace-001")
                .param("status", "QUEUED"))
        .andExpect(status().isOk())
  ...
  void shouldListCollectedReferencesByJobContract() throws Exception {
    when(gateway.listCollectedReferencesByJob(
            eq("mois-collect-001"), eq("CLICKBANK"), eq("nutricao"), eq(70), eq("HIGH")))
        .thenReturn(
            Optional.of(
                new MoisWorkspaceDtos.CollectedReferenceListResponse(
                    "mois-collect-001",
                    List.of(
                        new MoisWorkspaceDtos.CollectedReferenceResponse(
                            "ref-auto-001",
                            "mois-collect-001",
                            "CLICKBANK",
    ...
    mockMvc
        .perform(
            get("/api/v1/mois/collection-jobs/mois-collect-001/references")
  ...
  void shouldReturn404WhenCollectionJobReferencesNotFound() throws Exception {
    when(gateway.listCollectedReferencesByJob(
            eq("mois-collect-missing"), eq(null), eq(null), eq(null), eq(null)))
        .thenReturn(Optional.empty());

    mockMvc
        .perform(get("/api/v1/mois/collection-jobs/mois-collect-missing/references"))
  ...
  void shouldFavoriteCollectedReference() throws Exception {
    when(gateway.favoriteCollectedReference("mois-collect-001", "ref-auto-001"))
        .thenReturn(
            Optional.of(
                new MoisWorkspaceDtos.CollectedReferenceActionResponse(
                    "mois-collect-001",
                    "ref-auto-001",
                    "FAVORITE",
                    "ACTIVE",
    ...
    mockMvc
        .perform(
            post("/api/v1/mois/collection-jobs/mois-collect-001/references/ref-auto-001/favorite"))
        .andExpect(status().isOk())
  ...
  void shouldImportCollectedReference() throws Exception {
    ...
    mockMvc
        .perform(
            post("/api/v1/mois/collection-jobs/mois-collect-001/references/ref-auto-001/import"))
        .andExpect(status().isOk())
  ...
  void shouldImportAndStartExtractionCollectedReference() throws Exception {
    ...
    mockMvc
        .perform(
            post(
                "/api/v1/mois/collection-jobs/mois-collect-001/references/ref-auto-001/import-and-start-extraction"))
        .andExpect(status().isOk())
  ...
  void shouldReturnCollectedReferenceLineage() throws Exception {
    ...
    mockMvc
        .perform(
            get("/api/v1/mois/collection-jobs/mois-collect-001/references/ref-auto-001/lineage"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.importedReferenceId").value("ref-imported-001"))
