...
class DossierPendingInputSupportTest {
  ...
  void inputForIncluiRespostasAnterioresDoFluxoAtual() {
    ...
    PipelineDossieProduto entendimento =
        registro(
            "262",
    ...
    assertThat(input.get("previousStages").toString())
        .contains("product-understanding", "restaurar saúde");
    verify(repository)
        .findByIdExternoAndVersaoPipelineAndDataHoraGreaterThanEqualOrderByDataHoraAscIdAsc(
            "262", "v1", inicioAtual);
  }
  ...
  private PipelineDossieProduto registro(
      String idExterno, String etapa, String status, Instant dataHora, String response) {
