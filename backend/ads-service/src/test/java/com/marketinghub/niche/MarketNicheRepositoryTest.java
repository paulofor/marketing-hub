...
class MarketNicheRepositoryTest {
  ...
  @Autowired DifferentiatedTechnologyRepository technologyRepository;
  ...
  void findListItemsOrdersByCreationAndReturnsCost() throws Exception {
    MarketNiche older =
        MarketNiche.builder().name("Nicho antigo").totalCost(new BigDecimal("10.00")).build();
    repository.saveAndFlush(older);
    Thread.sleep(5);
    MarketNiche newer =
        MarketNiche.builder().name("Nicho recente").totalCost(new BigDecimal("20.00")).build();
  ...
  void findAllToGenerateHypothesesReturnsOnlyConfiguredNiches() {
    MarketNiche withHyps = MarketNiche.builder().name("Saúde").hypothesesToGenerate(2).build();
    repository.save(withHyps);

    MarketNiche withoutHyps = MarketNiche.builder().name("Ignored").hypothesesToGenerate(0).build();
    repository.save(withoutHyps);

    var result = repository.findAllToGenerateHypotheses();
    assertThat(result).extracting(MarketNiche::getName).containsExactly("Saúde");
  }
  ...
  void findAllToGenerateHypothesesFetchesDifferentiatedTechnology() {
    DifferentiatedTechnology technology =
        technologyRepository.save(
            DifferentiatedTechnology.builder()
                .name("IA de imagens")
