...
class OpenAiModelPricingSyncServiceTest {
  ...
  @Mock private OpenAiPricingPageClient pricingPageClient;

  @Mock private OpenAiModelRepository repository;

  /**
   * Garante que variantes datadas recebem preços do código-base oficial mais específico sem perder
   * identidade.
   */
  @Test
  void shouldRefreshDatedModelVariantWithBestOfficialBasePricing() {
    OpenAiModelPricing basePrice =
    ...
    OpenAiModelPricingSyncService service =
        new OpenAiModelPricingSyncService(pricingPageClient, repository);
    ...
    assertThat(datedVariant.getPriceInputCachedStandard())
    ...
    assertThat(datedVariant.getPriceInputCachedBatch())
    ...
    assertThat(datedVariant.getPricingSource()).isEqualTo(OpenAiPricingPageClient.PRICING_PAGE_URL);
    assertThat(datedVariant.getLastPricingSyncAt()).isNotNull();
    verify(repository).save(datedVariant);
  }

  /**
   * Garante que modelos-base oficiais continuam sendo criados com identidade publicada na tabela de
   * preços.
   */
  @Test
