...
class OprmGeneralAudienceControllerTest {
  ...
  void shouldListSeedsThroughApi() throws Exception {
    ...
    mockMvc
        .perform(get("/api/oprm/general-audiences/seeds"))
  ...
  void shouldCreateSeedThroughApi() throws Exception {
    when(service.createSeed(any()))
        .thenReturn(
            new GeneralAudienceSeedResponse(
                2L,
                "Beleza",
                ...
                Instant.parse("2026-06-10T10:00:00Z"),
                Instant.parse("2026-06-10T10:00:00Z")));
    CreateGeneralAudienceSeedRequest request =
  ...
  void shouldListSubnichesThroughApi() throws Exception {
    ...
    mockMvc
        .perform(get("/api/oprm/general-audiences/seeds/2/subniches"))
        .andExpect(status().isOk())
  ...
  void shouldCreateSubnicheThroughApi() throws Exception {
    when(service.createSubniche(any(), any()))
        .thenReturn(subnicheResponse(OprmGeneralAudienceSubnicheStatus.DISCOVERED));
    ...
    mockMvc
        .perform(
            post("/api/oprm/general-audiences/seeds/2/subniches")
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(request)))
        .andExpect(status().isCreated())
  ...
  void shouldUpdateSubnicheThroughApi() throws Exception {
    when(service.updateSubniche(any(), any()))
        .thenReturn(subnicheResponse(OprmGeneralAudienceSubnicheStatus.NEEDS_REVIEW));

    mockMvc
        .perform(
            patch("/api/oprm/general-audiences/subniches/5")
                .contentType(MediaType.APPLICATION_JSON)
  ...
  void shouldApproveSubnicheThroughApi() throws Exception {
    when(service.approveSubniche(5L))
        .thenReturn(subnicheResponse(OprmGeneralAudienceSubnicheStatus.APPROVED_FOR_EXPERIMENT));

    mockMvc
        .perform(post("/api/oprm/general-audiences/subniches/5/approve"))
        .andExpect(status().isOk())
  ...
  void shouldRejectSubnicheThroughApi() throws Exception {
    when(service.rejectSubniche(5L))
        .thenReturn(subnicheResponse(OprmGeneralAudienceSubnicheStatus.REJECTED));

    mockMvc
        .perform(post("/api/oprm/general-audiences/subniches/5/reject"))
        .andExpect(status().isOk())
  ...
  void shouldConvertSubnicheToMarketNicheThroughApi() throws Exception {
    when(service.convertSubnicheToMarketNiche(any(), any()))
        .thenReturn(
    ...
    mockMvc
        .perform(
            post("/api/oprm/general-audiences/subniches/5/convert-to-market-niche")
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(request)))
        .andExpect(status().isOk())
  ...
  private GeneralAudienceSubnicheResponse subnicheResponse(
