...
class OprmEstabelecimentoCnaeRaizServiceTest {
  ...
  void setUp() {
    ...
    dataSource.setUrl(
        "jdbc:h2:mem:oprm_estabelecimento_test;MODE=MySQL;DB_CLOSE_DELAY=-1;DB_CLOSE_ON_EXIT=FALSE");
  ...
  void shouldUpsertBatchAndPreserveExistingFlags() {
    service.upsertBatch(
        new OprmEstabelecimentoCnaeRaizBatchRequestDto(
            List.of(
                new OprmEstabelecimentoCnaeRaizUpsertDto(
                    "12.345.678", "6201501", "CONTATO@EXEMPLO.COM"))));
    jdbcTemplate.update(
        "UPDATE oprm_estabelecimento_cnae_raiz SET is_mei = 1, is_simples = 1 WHERE cnpj_raiz = ? AND cnae_code = ?",
        "12345678",
        "6201501");
    ...
    var response =
        service.upsertBatch(
            new OprmEstabelecimentoCnaeRaizBatchRequestDto(
                List.of(
                    new OprmEstabelecimentoCnaeRaizUpsertDto(
                        "12345678", "6201501", "novo@exemplo.com"))));

    assertThat(response.received()).isEqualTo(1);
    assertThat(response.persisted()).isEqualTo(1);
    assertThat(
            jdbcTemplate.queryForObject(
                "SELECT email FROM oprm_estabelecimento_cnae_raiz WHERE cnpj_raiz = ? AND cnae_code = ?",
                String.class,
                "12345678",
                "6201501"))
    ...
    assertThat(
            jdbcTemplate.queryForObject(
                "SELECT is_mei FROM oprm_estabelecimento_cnae_raiz WHERE cnpj_raiz = ? AND cnae_code = ?",
                Integer.class,
                "12345678",
                "6201501"))
        .isEqualTo(1);
    assertThat(
            jdbcTemplate.queryForObject(
                "SELECT is_simples FROM oprm_estabelecimento_cnae_raiz WHERE cnpj_raiz = ? AND cnae_code = ?",
                Integer.class,
                "12345678",
                "6201501"))
        .isEqualTo(1);
        ...
        service.upsertBatch(
            new OprmEstabelecimentoCnaeRaizBatchRequestDto(
                List.of(
                    ...
                    new OprmEstabelecimentoCnaeRaizUpsertDto(
                        "87654321", "9602501", "contato@exemplo.com"))));

    assertThat(response.received()).isEqualTo(2);
    assertThat(response.persisted()).isEqualTo(1);
    assertThat(
            jdbcTemplate.queryForObject(
                "SELECT COUNT(*) FROM oprm_estabelecimento_cnae_raiz", Integer.class))
        .isEqualTo(1);
    assertThat(
            jdbcTemplate.queryForObject(
                "SELECT email FROM oprm_estabelecimento_cnae_raiz WHERE cnae_code = ?",
                String.class,
                "9602501"))
        .isEqualTo("contato@exemplo.com");
  }
}
