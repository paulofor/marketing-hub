...
class BackendNicheResearchSeedBuilderServiceTest {
  ...
  /**
   * Deve expor aprendizado do gate anterior para a etapa de seed não repetir a reprovação
   * dominante.
   */
  @Test
  void listPendingIncludesPreviousQualityGateLearning() {
    ...
    when(routineResearchCycleRepository.findSeedBuilderPendingOrRetryable(
            ...
            any(Pageable.class)))
        .thenReturn(List.of(cycle));
    when(routineCardRepository.findLatestCheckedCardForLearning(
            anyLong(), eq(1001L), any(Pageable.class)))
    ...
    assertThat(result.getFirst().previousNextMove())
        .isEqualTo("Reexecutar busca removendo fontes de solucao");
    assertThat(result.getFirst().previousLearningNotes()).contains("riscoLinguagemSolucao=70");
  }

  /**
   * Deve reaproveitar aprendizado preservado no próprio job reaberto quando os artefatos antigos
   * foram limpos.
   */
  @Test
  void listPendingIncludesSameJobRerunLearningFromCycleNote() {
    ...
    cycle.setErrorMessage(
        "Reexecução de etapas do mesmo job solicitada por AUTO_QUALITY_REPROCESS. "
  ...
  /**
   * Deve enviar subnichos já materializados no CNAE para a IA não repetir o mesmo recorte de
   * mercado.
   */
  @Test
  ...
  void completePersistsSeedQueriesAndCycleTotals() {
    ...
    assertThat(cycleCaptor.getValue().getNicheName())
        .isEqualTo("Manicures autônomas com agenda instável pelo WhatsApp");
    assertThat(cycleCaptor.getValue().getNeutralNicheName())
        .isEqualTo("Manicures autônomas com agenda instável pelo WhatsApp");
    assertThat(cycleCaptor.getValue().getOriginalNicheName())
        .isEqualTo("Cabeleireiros, manicures e pedicures");
  }

  /**
   * Deve persistir o nome retornado pelo executor sem usar o backend para bloquear decisão de
   * fluxo.
   */
  @Test
  void completeAcceptsExecutorReturnedNicheNameWithoutBackendFlowGate() {
    ...
    CompleteNicheResearchSeedBuilderRequest request =
        new CompleteNicheResearchSeedBuilderRequest(
  ...
  /**
   * Deve aceitar seed fraco para o executor e o gate de qualidade decidirem o próximo movimento do
   * fluxo.
   */
  @Test
        ...
        new CompleteNicheResearchSeedBuilderRequest(
            ...
            List.of(
                new NicheResearchQueryRequest(
                    "rotina manicure Brasil", "ROUTINE_DISCOVERY", "web", 1)));
  ...
  /**
   * Deve reabrir automaticamente ciclo falho retryável quando a conclusão da etapa dois passa a
   * funcionar.
   */
  @Test
    ...
    CompleteNicheResearchSeedBuilderRequest request =
        new CompleteNicheResearchSeedBuilderRequest(
            ...
            validQueryRequestsWithFirst(
                "manicure MEI serviços mais procurados Brasil", "PRODUCT_SERVICE_DISCOVERY"));
  ...
  /**
   * Deve aceitar linguagem de solução retornada pelo modelo sem bloquear a criação das queries
   * iniciais.
   */
  @Test
  void completeAcceptsSolutionLanguageQuery() {
    ...
    CompleteNicheResearchSeedBuilderRequest request =
        new CompleteNicheResearchSeedBuilderRequest(
            ...
            "resp_seed",
            validQueryRequestsWithFirst(
                "IA para crescimento de manicure MEI Brasil", "MEI_ROUTINE_DISCOVERY"));
  ...
  /**
   * Deve aceitar query sem marcador literal de MEI/autônomo porque a etapa passa a confiar no
   * modelo.
   */
  @Test
  void completeAcceptsQueryWithoutAudienceMarker() {
    ...
    CompleteNicheResearchSeedBuilderRequest request =
        new CompleteNicheResearchSeedBuilderRequest(
            ...
            "resp_seed",
            validQueryRequestsWithFirst(
                "manicure responsabilidades rotina Brasil", "MEI_ROUTINE_DISCOVERY"));
  ...
  /**
   * Deve aceitar query sem marcador literal brasileiro porque a localização será tratada pelas
   * próximas etapas.
   */
  @Test
  void completeAcceptsQueryWithoutBrazilMarker() {
    ...
    CompleteNicheResearchSeedBuilderRequest request =
        new CompleteNicheResearchSeedBuilderRequest(
            ...
            "resp_seed",
            validQueryRequestsWithFirst(
                "manicure MEI responsabilidades rotina", "MEI_ROUTINE_DISCOVERY"));
  ...
  void completeAcceptsRepeatedCommonWordsInQueryText() {
    ...
    CompleteNicheResearchSeedBuilderRequest request =
        new CompleteNicheResearchSeedBuilderRequest(
            ...
            "resp_seed",
            validQueryRequestsWithFirst(
                "rotina de manicure MEI e atendimento e organização diária Brasil",
  ...
  /**
   * Deve aceitar mais de quinze queries quando o modelo decidir ampliar a cobertura da pesquisa
   * inicial.
   */
  @Test
  void completeAcceptsMoreThanFifteenQueries() {
    ...
    when(routineResearchCycleRepository.findById(1001L)).thenReturn(Optional.of(cycle));
    stubSuccessfulPersistence();
    List<NicheResearchQueryRequest> manyQueries =
        java.util.stream.IntStream.rangeClosed(1, 16)
            .mapToObj(
                index ->
                    new NicheResearchQueryRequest(
                        "manicure MEI rotina Brasil " + index,
                        "MEI_ROUTINE_DISCOVERY",
                        "web",
                        index))
            .toList();
  ...
  void completeCreatesDefaultQueryWhenModelReturnsEmptyQueries() {
    ...
    assertThat(response.totalQueries()).isEqualTo(1);
    assertThat(response.queries().getFirst().queryGoal())
        .isEqualTo("COMMERCIAL_OPERATION_DISCOVERY");
    assertThat(response.queries().getFirst().queryText())
        .contains(
            "WhatsApp Instagram indicação agenda faltas preço cobrança materiais retrabalho Brasil");
  }
  ...
  void failMarksCycleAsFailed() {
    ...
    service.fail(
        1001L, new FailNicheResearchSeedBuilderRequest("IA retornou queries genéricas.", null));
    ...
    assertThat(cycleCaptor.getValue().getErrorMessage())
        .isEqualTo("IA retornou queries genéricas.");
  ...
  /**
   * Monta um payload válido de etapa dois com doze objetivos comerciais-operacionais para
   * MEI/autônomo.
   */
  ...
  private List<NicheResearchQueryRequest> validQueryRequests() {
    return List.of(
        new NicheResearchQueryRequest(
            "manicure autônoma clientes WhatsApp Instagram indicação Brasil",
            "CUSTOMER_ACQUISITION",
            "web",
            1),
        new NicheResearchQueryRequest(
            "profissional autônomo manicure agenda faltas remarcações clientes somem",
            "SCHEDULE_NO_SHOWS",
            "web",
            2),
        new NicheResearchQueryRequest(
            "manicure preço cobrança sinal pacotes recorrência Brasil",
            "PRICING_BILLING",
            "web",
            3),
        new NicheResearchQueryRequest(
        ...
        new NicheResearchQueryRequest(
            "relatos reais manicure autônoma fórum comentários perguntas frequentes",
            "REAL_REPORTS",
            "web",
            5),
        new NicheResearchQueryRequest(
            "cabeleireiro autônomo clientes indicação Instagram WhatsApp",
            "CUSTOMER_ACQUISITION",
            "web",
            6),
        new NicheResearchQueryRequest(
            "cabeleireiro agenda vazia cliente falta remarcar horário",
            "SCHEDULE_NO_SHOWS",
            "web",
            7),
        new NicheResearchQueryRequest(
            "cabeleireiro preço pacote cobrança recorrência salão pequeno",
            "PRICING_BILLING",
            "web",
            8),
        new NicheResearchQueryRequest(
            "pedicure material esterilização tempo atendimento retrabalho",
            "MATERIALS_REWORK",
            "web",
            9),
        new NicheResearchQueryRequest(
            "vídeos comentários manicure autônoma dúvidas clientes cobrança",
            "REAL_REPORTS",
            "web",
            10),
        new NicheResearchQueryRequest(
            "profissional beleza cliente sumiu depois orçamento WhatsApp",
            "SCHEDULE_NO_SHOWS",
            "web",
            11),
        new NicheResearchQueryRequest(
            "profissional autônomo beleza pacotes mensalidade fidelização clientes",
            "PRICING_BILLING",
            "web",
            12));
  }
  ...
  /**
   * Substitui a primeira query válida para testar validações específicas sem quebrar o contrato
   * mínimo.
   */
  private List<NicheResearchQueryRequest> validQueryRequestsWithFirst(
      String queryText, String queryGoal) {
