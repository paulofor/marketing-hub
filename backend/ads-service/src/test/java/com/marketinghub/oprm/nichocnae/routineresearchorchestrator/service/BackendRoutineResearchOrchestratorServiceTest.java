...
class BackendRoutineResearchOrchestratorServiceTest {
  ...
  void listPendingUsesPreviewQueryWithoutPessimisticLock() {
    ...
    when(nicheCandidateRepository.findNextPendingRoutineResearchCandidatePreview(
            any(Pageable.class)))
    ...
    verify(nicheCandidateRepository)
        .findNextPendingRoutineResearchCandidatePreview(pageableCaptor.capture());
  ...
  void listRecentProcessedReturnsLatestCyclesWithProcessedAt() {
    OprmRoutineResearchCycle firstCycle =
        cycle(321L, 55L, "Cabeleireiros e manicures", "2026-06-03T01:00:00Z");
    OprmRoutineResearchCycle secondCycle =
        cycle(320L, 54L, "Lojas de roupas", "2026-06-02T22:00:00Z");
  ...
  /**
   * Deve listar jobs filtrados por status para o executor decidir nova tentativa com aprendizado
   * anterior.
   */
  @Test
  ...
  /**
   * Deve indicar o nicho existente pelo perfil enriquecido quando o candidato não tem vínculo
   * direto.
   */
  @Test
  ...
  void reprocessFailedCycleCreatesNewCycleImmediately() {
    OprmRoutineResearchCycle failedCycle =
        cycle(321L, 55L, "Cabeleireiros e manicures", "2026-06-03T01:00:00Z");
    ...
    assertThat(cycleCaptor.getValue().getReprocessPreservedCostUsd())
        .isEqualByComparingTo("0.05610000");
    assertThat(cycleCaptor.getValue().getErrorMessage())
        .contains("Reexecução de etapas do mesmo job");
  ...
  /**
   * Deve gravar origem automática e preservar o aprendizado do ciclo reprovado para a próxima
   * busca.
   */
  @Test
  void reprocessCycleUsesExternalExecutorTriggerSourceWhenRequested() {
    OprmRoutineResearchCycle failedCycle =
        cycle(321L, 55L, "Escovista em domicílio", "2026-06-03T01:00:00Z");
    ...
    previousCard.setQualityNotes(
        "proximoMovimentoCodigo=REFAZER_BUSCA_SEM_SOLUCAO; riscoLinguagemSolucao=70");
    when(routineResearchCycleRepository.findById(321L)).thenReturn(Optional.of(failedCycle));
    when(nicheCandidateRepository.findById(55L)).thenReturn(Optional.of(candidate));
    when(routineCardRepository.findFirstByResearchCycleIdOrderByIdDesc(321L))
        .thenReturn(Optional.of(previousCard));
    ...
    assertThat(learningCycle.getOriginalNicheName())
        .isEqualTo("Cabeleireiros, manicure e pedicure");
    assertThat(learningCycle.getNeutralNicheName())
        .isEqualTo("Escovista em domicílio com recorrência instável");
    ...
    assertThat(learningCycle.getErrorMessage())
        .contains("O próximo seed deve usar o aprendizado do gate anterior");
    assertThat(learningCycle.getErrorMessage())
  ...
  /**
   * Deve preservar fontes e claims aceitos no ciclo 72 e voltar ao query planner quando faltar
   * evidência de rotina.
   */
  @Test
  void reprocessCycle72PreservesAcceptedKnowledgeAndRewindsToQueryPlanner() {
    OprmRoutineResearchCycle weakCycle =
        cycle(72L, 55L, "Escovista em domicílio", "2026-06-03T01:00:00Z");
    ...
    when(nicheCandidateRepository.findById(55L)).thenReturn(Optional.of(candidate));
    when(routineCardRepository.findFirstByResearchCycleIdOrderByIdDesc(72L))
        .thenReturn(Optional.of(previousCard));
    ...
    var result =
        service.reprocessCycle(
            72L, new RecordRoutineResearchOrchestratorReprocessRequest("AUTO_QUALITY_REPROCESS"));
    ...
    assertThat(reopenedCycle.getErrorMessage())
        .contains("knowledgeVersion=cycle-72-snap-702-sig-802");
    ...
    assertThat(reopenedCycle.getErrorMessage())
        .contains("evidenceGaps=[EXECUTOR_ROUTINE, PUBLIC_EVIDENCE]");
  ...
  void reprocessStalledCycleCreatesNewCycleImmediately() {
    OprmRoutineResearchCycle stalledCycle =
        cycle(321L, 55L, "Cabeleireiros e manicures", "2026-06-03T01:00:00Z");
  ...
  void reprocessEnrichedNicheFailedCycleCreatesNewCycleImmediately() {
    OprmRoutineResearchCycle failedMaterializationCycle =
        cycle(321L, 55L, "Cabeleireiros e manicures", "2026-06-03T01:00:00Z");
    ...
    candidate.setRoutineResearchStatus("RESEARCH_RUNNING");
    candidate.setLastRoutineResearchCycleId(321L);
    when(routineResearchCycleRepository.findById(321L))
        .thenReturn(Optional.of(failedMaterializationCycle));
    when(nicheCandidateRepository.findById(55L)).thenReturn(Optional.of(candidate));
    when(routineResearchCycleRepository.save(any(OprmRoutineResearchCycle.class)))
        .thenAnswer(invocation -> invocation.getArgument(0));
    when(nicheCandidateRepository.save(any(OprmNicheCandidate.class)))
  ...
  void runNextNeutralizesSolutionLanguageAndKeepsOriginalName() {
    ...
    assertThat(result.originalNicheName())
        .isEqualTo("IA para crescimento de Cabeleireiros, manicure e pedicure");
    ...
    assertThat(savedCycle.getOriginalNicheName())
        .isEqualTo("IA para crescimento de Cabeleireiros, manicure e pedicure");
  ...
  void runForCnaeFinishesOpenCyclesBeforeCreatingFreshManualCycle() {
    ...
    candidate.setRoutineResearchStatus("RESEARCH_RUNNING");
    candidate.setLastRoutineResearchCycleId(321L);
    OprmRoutineResearchCycle openCycle =
        cycle(321L, 55L, "Cabeleireiros e manicures", "2026-06-03T01:00:00Z");
    ...
    ArgumentCaptor<Iterable<OprmRoutineResearchCycle>> openCyclesCaptor =
    ...
    assertThat(cancelledCycle.getErrorMessage())
        .contains("reinício manual completo do CNAE 9602501");
  ...
  private OprmRoutineResearchCycle cycle(
      Long id, Long sourceNicheId, String nicheName, String startedAt) {
   ...
   */
  private OprmExtractedSignal signal(
      Long id, Long sourceSnapshotId, String signalType, String evidenceExcerpt) {
