...
class BackendAdaptiveQueryPlannerServiceTest {
  ...
  /**
   * Deve persistir a próxima etapa recebida do executor sem calcular plano ou regra de negócio no
   * backend.
   */
  @Test
  ...
  /**
   * Deve gravar pendência da etapa 3 quando o executor solicitar reprocessamento cognitivo para
   * query planner.
   */
  @Test
  ...
  /**
   * Deve preservar retry técnico sem mudar tentativa cognitiva quando a falha da etapa 3 for
   * infraestrutura.
   */
  @Test
  ...
  private BackendAdaptiveQueryPlannerService service(boolean v2Enabled) {
    return new BackendAdaptiveQueryPlannerService(
