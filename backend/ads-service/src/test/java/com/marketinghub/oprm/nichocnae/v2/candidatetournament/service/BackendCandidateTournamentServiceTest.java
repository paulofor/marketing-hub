...
class BackendCandidateTournamentServiceTest {
  ...
  /**
   * Deve persistir a próxima etapa recebida do executor sem calcular torneio ou regra de negócio no
   * backend.
   */
  @Test
  ...
  /**
   * Deve gravar pendência da etapa 4 quando o executor solicitar reprocessamento cognitivo para
   * query planner.
   */
  @Test
  ...
  /**
   * Deve preservar retry técnico sem mudar tentativa cognitiva quando a falha da etapa 4 for
   * infraestrutura.
   */
  @Test
  ...
  private BackendCandidateTournamentService service(boolean v2Enabled) {
    return new BackendCandidateTournamentService(
