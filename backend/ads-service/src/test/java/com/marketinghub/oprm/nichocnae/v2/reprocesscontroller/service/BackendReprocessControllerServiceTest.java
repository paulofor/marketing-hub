...
/**
 * Valida que o backend da etapa reprocess-controller atua somente como leitura e escrita para o
 * executor.
 */
...
class BackendReprocessControllerServiceTest {
  ...
  void pendingReturnsEmptyWhenFeatureFlagIsDisabled() {
    ...
    verify(stageExecutionRepository, never())
        .findByStageCodeAndStatusOrderByCreatedAtAsc(any(), any(), any());
  }
  ...
  /**
   * Deve persistir exatamente a decisão enviada pelo executor, sem aplicar regra de reprocessamento
   * no backend.
   */
  @Test
  ...
  /**
   * Deve preservar retry técnico sem mudar tentativa cognitiva quando a falha da etapa 9 for
   * infraestrutura.
   */
  @Test
    ...
    var response =
        service.fail(
            ...
            new ReprocessControllerFailureRequest(
                ...
                "TIMEOUT no callback do executor",
                "{}"));
  ...
  private BackendReprocessControllerService service(boolean v2Enabled) {
    return new BackendReprocessControllerService(
        stageExecutionRepository, nicheCandidateRepository, v2Enabled);
  }
