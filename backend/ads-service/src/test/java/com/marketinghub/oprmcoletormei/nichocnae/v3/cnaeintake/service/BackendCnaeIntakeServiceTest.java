...
class BackendCnaeIntakeServiceTest {
  ...
  void setUp() {
    service =
        new BackendCnaeIntakeService(
            repository, cnaeRepository, pipelineNichoCnaeRepository, openAiPricingService);
  }
  ...
  void completeUpdatesCnaePipelineToNextStage() {
    ...
    when(repository.existsByJobIdAndStageCode("job-4781400", "persona-candidate-generator"))
        .thenReturn(false);
    ...
    ArgumentCaptor<OprmNichoCnaeV3StageExecution> executionCaptor =
        ArgumentCaptor.forClass(OprmNichoCnaeV3StageExecution.class);
  ...
  void startCreatesQualifiedCnaeInputPayload() {
    ...
    assertThat(executionCaptor.getValue().getInputPayload())
        .contains("\"cnaeDescription\":\"Comércio varejista de artigos do vestuário\"");
  }
  ...
  void recebeResponseStoresCleanFinalResponse() {
    OprmCnpjCnaeDim cnae = cnae();
    when(cnaeRepository.findById("4781400")).thenReturn(Optional.of(cnae));
    when(pipelineNichoCnaeRepository.save(any(PipelineNichoCnae.class)))
        .thenAnswer(invocation -> invocation.getArgument(0));
    when(openAiPricingService.estimateFlexCost("gpt-5.2", 10, 20))
        .thenReturn(new BigDecimal("0.0003"));
    String rawResponse =
        """
                {
                  "output": [
    ...
    service.recebeResponse(
        "4781400",
        "job-4781400",
        new OprmNichoCnaeV3RecebeResponseRequest(
            rawResponse, null, 10L, 20L, new BigDecimal("9.9999"), "gpt-5.2"));

    ArgumentCaptor<PipelineNichoCnae> pipelineCaptor =
        ArgumentCaptor.forClass(PipelineNichoCnae.class);
