...
class BackendPersonaRoutineMaterializerServiceTest {
  @Mock private OprmNichoCnaeV3StageExecutionRepository repository;

  @Mock private OprmCnpjCnaeDimRepository cnaeRepository;

  @Mock private PersonaRoutineMaterializerNicheGateway nicheGateway;

  @Mock private PipelineNichoCnaeRepository pipelineNichoCnaeRepository;
  ...
  void setUp() {
    service =
        new BackendPersonaRoutineMaterializerService(
            repository,
            cnaeRepository,
            pipelineNichoCnaeRepository,
  ...
  void completeMaterializesFinalPayloadIntoNicheData() {
    ...
    when(repository.save(any(OprmNichoCnaeV3StageExecution.class)))
    ...
    when(nicheGateway.materialize(
            any(PersonaRoutineMaterializerNicheGateway.MarketNicheDraft.class),
            any(PersonaRoutineMaterializerNicheGateway.EnrichedNicheProfileDraft.class)))
        .thenReturn(
            new PersonaRoutineMaterializerNicheGateway.NicheMaterializationResult(
                300L, 400L, Instant.now()));
    ...
    ArgumentCaptor<PersonaRoutineMaterializerNicheGateway.MarketNicheDraft> nicheCaptor =
        ArgumentCaptor.forClass(PersonaRoutineMaterializerNicheGateway.MarketNicheDraft.class);
    ArgumentCaptor<PersonaRoutineMaterializerNicheGateway.EnrichedNicheProfileDraft> profileCaptor =
        ArgumentCaptor.forClass(
            PersonaRoutineMaterializerNicheGateway.EnrichedNicheProfileDraft.class);
    verify(nicheGateway).materialize(nicheCaptor.capture(), profileCaptor.capture());
    assertThat(nicheCaptor.getValue().name())
        .isEqualTo("Lojistas de roupas infantis com rotina de reposição");
    assertThat(nicheCaptor.getValue().sourceCnaeCode()).isEqualTo("4781400");
    assertThat(nicheCaptor.getValue().sourceCnaeDescription())
        .isEqualTo("Comércio varejista de artigos do vestuário");
  ...
  void completeUsesNestedPersonaNameInsteadOfGenericCnaeName() {
    ...
    when(nicheGateway.findPersonaRoutineMaterializedNiche(
            eq("4781400"),
            eq(
                "dona de loja pequena de roupas que vende por whatsapp e controla estoque manualmente")))
        .thenReturn(Optional.empty());
    when(nicheGateway.materialize(
            any(PersonaRoutineMaterializerNicheGateway.MarketNicheDraft.class),
            any(PersonaRoutineMaterializerNicheGateway.EnrichedNicheProfileDraft.class)))
    ...
    assertThat(profileCaptor.getValue().neutralNicheName())
        .isEqualTo(
            "Dona de loja pequena de roupas que vende por WhatsApp e controla estoque manualmente");
  ...
  void pendingListsStartedCnaesFromCurrentStage() {
    ...
    when(repository.findTop1ByCnaeCodeAndStageCodeOrderByCreatedAtDesc(
            "4781400", "persona-routine-materializer"))
