...
class BackendPersonaTournamentServiceTest {
  ...
  void setUp() {
    service =
        new BackendPersonaTournamentService(
  ...
  void completeAllowsFastLaneToPersonaRoutineMaterializer() {
    ...
    when(repository.existsByJobIdAndStageCode("job-8219999", "persona-routine-materializer"))
        .thenReturn(false);
    when(cnaeRepository.findById("8219999")).thenReturn(Optional.of(cnae));

    service.complete(
        73L,
        "{\"winnerPersona\":{\"name\":\"Prestador de apoio administrativo MEI\"}}",
    ...
    assertThat(executionCaptor.getAllValues().get(1).getStageCode())
        .isEqualTo("persona-routine-materializer");
    assertThat(executionCaptor.getAllValues().get(1).getInputPayload())
        .contains("Prestador de apoio administrativo MEI");
