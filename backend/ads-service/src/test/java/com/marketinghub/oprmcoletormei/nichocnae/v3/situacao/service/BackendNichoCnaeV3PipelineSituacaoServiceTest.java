...
class BackendNichoCnaeV3PipelineSituacaoServiceTest {
  @Mock private PipelineNichoCnaeRepository repository;

  @InjectMocks private BackendNichoCnaeV3PipelineSituacaoService service;
  ...
  void searchShouldFilterByStageExternalIdAndStatuses() {
    ...
    var response =
        service.search(
            "source-searcher", "4781400", List.of(" CONCLUIDO ", "FALHA", "CONCLUIDO", ""));
  ...
  /**
   * Deve retornar request e response como eventos separados para a tela montar a auditoria real da
   * OpenAI.
   */
  @Test
  void searchShouldPreserveSeparateRequestAndResponseAuditEvents() {
    ...
    assertThat(response.get(1).request())
        .isEqualTo("{\"model\":\"gpt-5.2\",\"service_tier\":\"flex\"}");
