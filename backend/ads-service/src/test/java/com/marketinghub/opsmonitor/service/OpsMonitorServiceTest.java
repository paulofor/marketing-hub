...
class OpsMonitorServiceTest {
  @Mock private OpsMonitoredModuleRepository moduleRepository;

  @Mock private OpsModuleHealthCheckRepository healthCheckRepository;

  @Mock private OpsModuleIncidentRepository incidentRepository;

  @Mock private OpsModuleAvailabilityDailyRepository availabilityDailyRepository;

  @Mock private GeraLandingStageExecutionRepository geraLandingStageExecutionRepository;

  @Mock private OprmNichoCnaeV3StageExecutionRepository nichoCnaeV3StageExecutionRepository;
  ...
  void listAvailabilityFiltersByCriticalityAndType() {
    ...
    assertThat(filtered.getFirst().attemptedUrl())
        .isEqualTo("http://191.252.120.96:4567/worker-observability/health");
  }
  ...
  /**
   * Garante que o payload bruto de healthcheck suporta respostas Actuator maiores que 255
   * caracteres.
   */
  @Test
  void rawPayloadColumnUsesLongText() throws Exception {
    Column column =
        OpsModuleHealthCheck.class.getDeclaredField("rawPayload").getAnnotation(Column.class);
