...
class ProductDiscoveryControllerTest {
  ...
  void pendingRequiresExplicitPublicPolicyCapability() throws Exception {
    ...
    mockMvc
        .perform(
            get(
                "/api/internal/product-discovery/productdiscovery/v1/candidate-gap-deepening/stage-executions/pending"))
        .andExpect(status().isOk());
  ...
  void adoptsPublicResearchThroughCanonicalEndpoint() throws Exception {
    when(customerInterviewService.adoptPublicEvidence(901L))
        .thenReturn(
            new ProductDiscoveryGapDeepeningResponse(
                ...
                "Pesquisa pública sem entrevistas obrigatórias.",
                List.of(),
                "PUBLIC_SOURCES_V1",
                false,
                false));
    mockMvc
        .perform(post("/api/product-discovery/v1/cycles/901/gap-deepening/public-research"))
        .andExpect(status().isOk())
  ...
  /** Encaminha a retomada ao domínio sem criar outra execução independente. */
  @Test
  void resumesPublicResearchThroughCanonicalEndpoint() throws Exception {
    mockMvc
        .perform(post("/api/product-discovery/v1/cycles/902/gap-deepening/public-research/resume"))
        .andExpect(status().isOk());
    org.mockito.Mockito.verify(customerInterviewService).resumePublicResearch(902L);
  }
