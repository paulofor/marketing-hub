...
/** Valida a persistência de atributos dos prompts operacionais. */
@SpringBootTest(classes = com.marketinghub.ads.AdsServiceApplication.class)
@TestPropertySource(
    properties = {
      "spring.datasource.url=jdbc:h2:mem:com.marketinghub.prompt.PromptAttributeServiceTest-${random.uuid};MODE=MySQL;DB_CLOSE_DELAY=0;DB_CLOSE_ON_EXIT=FALSE",
...
class PromptAttributeServiceTest {
  @Autowired PromptAttributeService service;
  ...
  void createNewAttribute() {
    ...
    PromptAttribute attr =
        attributeRepository.findByEntity_NameAndName(entity.getName(), "entrega").orElse(null);
