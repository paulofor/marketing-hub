...
class MoisSalesLibraryPricingRepositoryTest {
  ...
  @InjectMocks private MoisSalesLibraryPricingRepository repository;

  /**
   * Garante que o repositório consulta a tabela canônica openai_model com código normalizado e
   * original.
   */
  @Test
  void shouldReadBatchPricesFromOpenAiModelTable() throws Exception {
    ...
    Optional<MoisSalesLibraryPricingGateway.ModelPricing> pricing =
        repository.findPricingByModelCode(" GPT-5.2 ");
  ...
  void shouldReturnEmptyWhenModelCodeIsBlank() {
    Optional<MoisSalesLibraryPricingGateway.ModelPricing> pricing =
        repository.findPricingByModelCode(" ");
