...
class TenantContextFilterTest {
  ...
  void shouldSkipTenantValidationForOptionsPreflightRequests() throws Exception {
    MockHttpServletRequest request =
        new MockHttpServletRequest("OPTIONS", "/api/products/1/sales-videos/profiles");
  ...
  void shouldRejectTenantProtectedEndpointsWhenHeaderIsMissing() throws Exception {
    MockHttpServletRequest request =
        new MockHttpServletRequest("GET", "/api/products/1/sales-videos/profiles");
    MockHttpServletResponse response = new MockHttpServletResponse();
    FilterChain chain = Mockito.mock(FilterChain.class);
  ...
  /**
   * Deve proteger criação de vídeo de experimento com o mesmo contexto de tenant do módulo de
   * vídeo.
   */
  @Test
  void shouldApplyTenantContextToExperimentVideoAssetsEndpoints() throws Exception {
    MockHttpServletRequest request =
        new MockHttpServletRequest("POST", "/api/experiments/63/video-assets/veo-render-requests");
  ...
  void shouldAllowReadOnlyProfileCompatibilityWithoutTenantHeader() throws Exception {
    MockHttpServletRequest request =
        new MockHttpServletRequest("GET", "/api/sales-videos/profiles/3");
    MockHttpServletResponse response = new MockHttpServletResponse();
    FilterChain chain = Mockito.mock(FilterChain.class);
