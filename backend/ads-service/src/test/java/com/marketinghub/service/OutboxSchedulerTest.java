...
class OutboxSchedulerTest {
  ...
  @Mock GraphApiClient graphApiClient;
  ...
  void setUp() {
    scheduler =
        new OutboxScheduler(
            outboxRepository, leadRepository, graphApiClient, new WelcomeSequenceFactory(), 10);
  }
  ...
  void processPendingMarksMissingLeadsAsProcessedWithoutSending() {
    UUID leadId = UUID.randomUUID();
    OutboxEvent event = OutboxEvent.builder().id(456L).aggregateId(leadId).build();
