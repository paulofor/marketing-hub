...
/** Valida os contratos HTTP das configurações gerais do backend. */
@SpringBootTest
@AutoConfigureMockMvc
@TestPropertySource(
    properties = {
      "spring.datasource.url=jdbc:h2:mem:com.marketinghub.settings.GeneralSettingControllerTest-${random.uuid};MODE=MySQL;DB_CLOSE_DELAY=0;DB_CLOSE_ON_EXIT=FALSE",
...
class GeneralSettingControllerTest {
  ...
  @Autowired GeneralSettingRepository repository;
  ...
  void shouldReturnSettingByName() throws Exception {
    mockMvc
        .perform(get("/api/settings/" + GeneralSettingKeys.PRIVACY_POLICY_URL))
  ...
  void shouldUpsertSetting() throws Exception {
    mockMvc
        .perform(
            put("/api/settings/" + GeneralSettingKeys.PRIVACY_POLICY_URL)
    ...
    assertThat(repository.findByName(GeneralSettingKeys.PRIVACY_POLICY_URL))
        ...
        .satisfies(
            setting ->
                assertThat(setting.getValue()).isEqualTo("https://privacy.example.com/updated"));
  }
}
