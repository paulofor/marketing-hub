...
class TargetingRequestServiceTest {
  ...
  @Mock private TargetingRequestRepository requestRepository;
  @Mock private TargetingCandidateRepository candidateRepository;
  @Mock private TargetingElementRepository targetingElementRepository;
  @Mock private TargetingResolutionJobRepository resolutionJobRepository;
  @Mock private TargetingResolutionJobService resolutionJobService;
  @Mock private TargetingResolutionSummaryMapper resolutionSummaryMapper;
  ...
  void applyResolutionShouldUpdateJobAsSucceededAndClearLastError() {
    ...
    TargetingRequest request =
        TargetingRequest.builder().id(UUID.randomUUID()).descricao("desc").build();
    TargetingCandidate candidate =
        TargetingCandidate.builder()
            .id(candidateId)
            .request(request)
            .seed("marketing")
    ...
    TargetingCandidateResolutionUpdateRequest payload =
        new TargetingCandidateResolutionUpdateRequest();
    ...
    when(candidateRepository.save(any(TargetingCandidate.class)))
    ...
    ArgumentCaptor<TargetingResolutionJob> jobCaptor =
        ArgumentCaptor.forClass(TargetingResolutionJob.class);
  ...
  void saveCandidatesShouldEnqueuePersistedCandidatesForMetaResolution() {
    ...
    TargetingRequest request =
        TargetingRequest.builder()
            ...
            .build();
    TargetingCandidate savedCandidate =
        TargetingCandidate.builder()
            .id(105L)
            .request(request)
    ...
    assertThat(request.getStatus())
        .isEqualTo(com.marketinghub.targeting.TargetingRequestStatus.COMPLETED);
  ...
  void applyResolutionShouldMaterializeValidatedMetaOptionsAsApprovedTargetingElements() {
    ...
    TargetingRequest request =
        TargetingRequest.builder().id(UUID.randomUUID()).descricao("desc").niche(niche).build();
    TargetingCandidate candidate =
        TargetingCandidate.builder()
            .id(candidateId)
            .request(request)
    ...
    when(candidateRepository.save(any(TargetingCandidate.class)))
        .thenAnswer(invocation -> invocation.getArgument(0));
    when(targetingElementRepository.findFirstByNicheIdAndTypeAndMetaId(
            22L, TargetingElementType.JOB_TITLE, "meta-1"))
        .thenReturn(Optional.empty());
    when(resolutionJobRepository.findByCandidateId(candidateId)).thenReturn(Optional.of(job));

    service.applyResolution(candidateId, payload);

    ArgumentCaptor<TargetingElement> elementCaptor =
        ArgumentCaptor.forClass(TargetingElement.class);
  ...
  private TargetingCandidateResolutionUpdateRequest.OptionPayload option(
      String facebookId, String name) {
    TargetingCandidateResolutionUpdateRequest.OptionPayload payload =
        new TargetingCandidateResolutionUpdateRequest.OptionPayload();
