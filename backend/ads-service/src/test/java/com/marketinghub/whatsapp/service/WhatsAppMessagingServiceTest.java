...
class WhatsAppMessagingServiceTest {
  ...
  void setup() {
    ...
    service =
        new WhatsAppMessagingService(
            restTemplate, accountService, messageRepository, objectMapper, properties);
    account =
  ...
  void sendTextMessagePersistsAndReturnsMessage() {
    server
        .expect(once(), requestTo("https://graph.facebook.com/v18.0/1234/messages"))
        .andExpect(method(HttpMethod.POST))
        .andExpect(header("Authorization", "Bearer token"))
        .andRespond(
            withSuccess("{\"messages\":[{\"id\":\"wamid.555\"}]}", MediaType.APPLICATION_JSON));

    WhatsAppMessage message =
        service.sendTextMessage(account, "+551100000000", "Body", Map.of("source", "test"));
