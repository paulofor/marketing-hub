...
class CatalogPromptInputTest {
  ...
  static Map<String, Object> fixture(String activity, String agent, String schema)
    ...
    result.put("executorModule", "customer-agent-worker");
  ...
  private Map<String, Object> task() throws Exception {
    ...
    result.put("processVersion", 1);
    result.put("agentKey", "customer-agent");
    result.put("activityId", "humanExperienceReview");
    result.put("catalogPrompt", fixture("humanExperienceReview", "customer-agent", SCHEMA));
    return result;
  }
  ...
  void rejectsContractDrift() throws Exception {
    for (String key :
        List.of(
            ...
            "executorModule")) {
      var task = task();
      var prompt = new HashMap<>(fixture("humanExperienceReview", "customer-agent", SCHEMA));
  ...
  void composesActualConsumerFromPinnedCatalogText() throws Exception {
    ...
    var loader = org.mockito.Mockito.mock(PdeExperienceEvidenceLoader.class);
    org.springframework.test.util.ReflectionTestUtils.setField(
        consumer, "pdeExperienceEvidenceLoader", loader);
    var composed =
        org.springframework.test.util.ReflectionTestUtils.invokeMethod(
            consumer, "promptComposition", task(), java.util.List.of());
    String prompt =
        org.springframework.test.util.ReflectionTestUtils.invokeMethod(
            composed, "activityPromptPart");
