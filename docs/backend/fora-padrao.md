...
| `/api/experiments/{id}/reactivate` | `com.marketinghub.experiment.web` | Comando legado de status não confirma reativação financeira na Meta. | Para Facebook, usar `/api/facebook-campaign-resumptions/experiments/{id}`; manter o legado para canais diretos. |
