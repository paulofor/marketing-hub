...
2. O backend persiste o ciclo como `PENDING_FINANCIAL_REVIEW` e abre uma tarefa na caixa de entrada de Plutus, com Apolo como solicitante.
3. Nenhum job de provider existe antes da decisão financeira.
4. Somente a identidade técnica `financial-agent` pode registrar `APPROVED` ou `REJECTED`, sempre com motivo auditável.
5. Uma aprovação cria em modo `TEST` o job do executor oficial de vídeo, vinculado ao ciclo e ao teto em USD. Uma rejeição termina em `FINANCIAL_BLOCKED`.
6. Apolo planeja, gera, inspeciona e devolve o candidato. O provider não decide próxima etapa.
7. O custo conhecido deve ser conciliado no ledger. Novo consumo é bloqueado ao atingir o teto, quando o custo estiver desconhecido ou quando a cobertura financeira estiver incompleta.
8. QA independente decide qualidade. Apolo não aprova o próprio trabalho.
