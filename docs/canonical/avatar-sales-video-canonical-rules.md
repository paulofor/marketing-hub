...
- Para o Método MUSA, Luma está proibida por decisão comercial de 2026-08-13 após score insuficiente e falha operacional. O provider recomendado é `RUNWAY_SEEDANCE_2_5`; Runway Gen-4 é a alternativa explícita, e vídeos acima do limite direto devem usar cenas auditáveis e montagem. Planos legados que mencionem Luma não prevalecem sobre esta regra.
...
- O catálogo de vídeo é persistido e administrável, mas adaptadores técnicos permanecem versionados no executor. Cada modelo passa por `DRAFT`, `HOMOLOGATION`, `ACTIVE` ou `BLOCKED`; ativação exige adaptador, preço, licença comercial e QA verificados. A curadoria Runway usa os identificadores `gen4.5`, `seedance2`, `seedance2_5`, `hailuo3`, `grok_imagine_1_5`, `gen4_turbo`, `veo3.1_fast` e `veo3.1`, sempre sujeitos à confirmação no contrato vigente do endpoint. Cadastro não cria integração nem autoriza produção. Seedance 2.5 e Hailuo 3 usam obrigatoriamente o adapter Runway e o segredo `RUNWAY_API_KEY`; é proibido criar ou usar credencial Pippit, ByteDance ou MiniMax para esses modelos. O job deve preservar o identificador próprio do modelo, e Seedance 2.5 nunca pode ser convertido silenciosamente em `seedance2`. Ambos permanecem em homologação até o endpoint Runway aceitar o identificador real e render, custo, licença e QA comercial serem comprovados.
- Wan3.0 deve entrar somente como candidato `DRAFT` e exige adapter próprio do Alibaba Cloud Model Studio e homologação do acesso internacional. Seedance 2.5 integra o catálogo pelo provider Runway, não por adapter Pippit. A alegação de exportação até 4K deve ser separada de resolução nativa e comprovada no asset final antes de orientar decisão comercial.
