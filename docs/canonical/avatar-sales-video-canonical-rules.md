...
- A curadoria operacional da Runway para geração comercial deve oferecer `gen4.5` como opção geral, `seedance2_5` para maior flexibilidade e duração, `gen4_turbo` para variações econômicas obrigatoriamente orientadas por imagem, `veo3.1_fast` para equilíbrio entre velocidade/custo/qualidade e `veo3.1` para hero premium. Modelos especializados em edição, avatar ou vídeo-como-entrada só entram após contrato próprio ponta a ponta; disponibilidade documental isolada não autoriza exibição na tela.
