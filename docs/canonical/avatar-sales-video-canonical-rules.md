...
- O catálogo de vídeo é persistido e administrável, mas adaptadores técnicos permanecem versionados no executor. Cada modelo passa por `DRAFT`, `HOMOLOGATION`, `ACTIVE` ou `BLOCKED`; ativação exige adaptador, preço, licença comercial e QA verificados. A curadoria Runway usa os identificadores oficiais `gen4.5`, `seedance2`, `gen4_turbo`, `veo3.1_fast` e `veo3.1`. Cadastro não cria integração nem autoriza produção. Hailuo pertence à MiniMax, não à Runway; na data desta decisão a API oficial expõe `MiniMax-Hailuo-2.3`, enquanto “Hailuo 3” não é identificador operacional verificável.
