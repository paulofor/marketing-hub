...
- O catálogo de vídeo é persistido e administrável, mas adaptadores técnicos permanecem versionados no executor. Cada modelo passa por `DRAFT`, `HOMOLOGATION`, `ACTIVE` ou `BLOCKED`; ativação exige adaptador, preço, licença comercial e QA verificados. A curadoria Runway usa os identificadores `gen4.5`, `seedance2`, `hailuo3`, `grok_imagine_1_5`, `gen4_turbo`, `veo3.1_fast` e `veo3.1`, sempre sujeitos à confirmação no contrato vigente do endpoint. Cadastro não cria integração nem autoriza produção. Seedance 2.5 anunciado no Pippit não pode reutilizar o identificador do Seedance 2.0 nem ser ativado antes de API oficial, preço, licença e QA comprovados. Hailuo 3 e Grok Imagine 1.5 usam o adaptador e o token Runway, mas permanecem em homologação até render real, custo, licença e QA comercial comprovados.
- Wan3.0 e Seedance 2.5/Pippit devem entrar somente como candidatos `DRAFT`. O Wan3.0 exige adapter próprio do Alibaba Cloud Model Studio e homologação do acesso internacional; o Seedance 2.5/Pippit exige contrato de API próprio. A alegação de exportação até 4K deve ser separada de resolução nativa e comprovada no asset final antes de orientar decisão comercial.
