...
- O catálogo de vídeo é persistido e administrável, mas adaptadores técnicos permanecem versionados no executor. Cada modelo passa por `DRAFT`, `HOMOLOGATION`, `ACTIVE` ou `BLOCKED`; ativação exige adaptador, preço, licença comercial e QA verificados. A curadoria Runway usa os identificadores oficiais `gen4.5`, `seedance2_5`, `hailuo3`, `gen4_turbo`, `veo3.1_fast` e `veo3.1`. Cadastro não cria integração nem autoriza produção. Hailuo 3 usa o adaptador e o token Runway, mas permanece em homologação até render real e QA comercial.
