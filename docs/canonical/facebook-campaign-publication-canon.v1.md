...
11. **Agendamento sem gasto antecipado** – `/api/facebook-campaign-resumptions/pending` só entrega a autorização na data inicial em `America/Sao_Paulo`. O worker relê gasto, moeda, destino, campanha, conjunto e `min_campaign_group_spend_cap`. Campanha com orçamento diário mantém `daily_budget` no ad set e recebe `spend_cap` no nível da campanha quando o teto autorizado atende ao mínimo da conta. Se o mínimo de campanha exceder o teto autorizado, somente um conjunto diário pode preservar `daily_budget` e receber `lifetime_spend_cap` exatamente igual ao teto total; é proibido elevar o teto ou usar esse fallback com múltiplos conjuntos. O modo vitalício legado continua aceito sem conversão silenciosa. Estado `RUNNING` só é confirmado depois do readback nativo de orçamento diário, teto escolhido, prazo e estado ativo; divergência ou resposta HTTP inválida mantém a campanha pausada e o corpo de erro auditável.
...
`lifetime_budget` no modo vitalício; no modo diário, confirma `daily_budget` e o
teto na campanha ou em `lifetime_spend_cap` quando o mínimo da conta exceder a
autorização. Prazo, identidade, gasto e atribuição anteriores são preservados.
