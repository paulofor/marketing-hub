...
> - formaliza que a publicação inicial usa orçamento diário no ad set (`budgetMode=ADSET`); na retomada abaixo do mínimo de `spend_cap`, permite migração auditada para diário na campanha e teto no único ad set sem orçamento próprio
...
11. **Agendamento sem gasto antecipado** – `/api/facebook-campaign-resumptions/pending` só entrega a autorização na data inicial em `America/Sao_Paulo`. O worker relê gasto, moeda, destino, campanha, conjunto e `min_campaign_group_spend_cap`. Campanha com orçamento diário mantém `daily_budget` no ad set e recebe `spend_cap` no nível da campanha quando o teto autorizado atende ao mínimo da conta. Se o mínimo de campanha exceder o teto, a Meta não aceita `daily_budget` e `lifetime_spend_cap` simultaneamente no mesmo conjunto: com a campanha pausada e exatamente um ad set, o worker migra o `daily_budget` autorizado para a campanha, omite `spend_cap` quando ele já está ausente, relê o conjunto e exige que a Meta tenha removido seu orçamento próprio antes de aplicar `lifetime_spend_cap` exatamente igual ao teto total. Nunca enviar `spend_cap=0`, rejeitado pela Meta com `100/1885099`, nem `daily_budget=0`, rejeitado com `100/1885272`; se houver teto positivo anterior ou o orçamento do conjunto permanecer após a migração, falhar antes de aplicar o teto. É proibido elevar o teto, usar esse fallback com múltiplos conjuntos ou ativar uma migração parcial. Retry pode completar a mesma migração após conferir valores e identidades, sem duplicar objetos. O modo vitalício legado continua aceito sem conversão silenciosa. Estado `RUNNING` só é confirmado depois do readback nativo dos dois níveis, teto, prazo e estado ativo; divergência ou resposta HTTP inválida mantém a campanha pausada e o corpo de erro auditável.
...
confirma `daily_budget` na campanha e `lifetime_spend_cap` no único ad set sem
orçamento próprio. Campanha sem teto anterior omite `spend_cap`; valor zero nunca
é enviado e teto positivo anterior bloqueia a migração antes de qualquer escrita.
