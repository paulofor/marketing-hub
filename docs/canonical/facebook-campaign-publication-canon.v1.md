...
5. **Fallback manual de targeting** – quando não houver playbook de ad set válido nem ad set persistido para o experimento, o `facebook-ads-worker` deve buscar o pacote manual aprovado no backend por `/api/facebook-adsets/experiments-ready`, selecionar o item do experimento e montar localmente o `targeting` da Meta.
