...
4. **Fonte de verdade de gasto e trava emergencial** – a Meta é a fonte de verdade para gasto de mídia. O endpoint `POST /api/facebook-campaigns/{campaignId}/metrics` deve persistir a métrica sincronizada mesmo quando regras derivadas de parada, estratégia ou diagnóstico falharem; falhas secundárias não podem devolver `500` e congelar o gasto no Hub. Como proteção financeira, se o Facebook Ads Worker ler na Meta gasto acumulado de pelo menos **R$ 25,00** com **zero leads**, ele deve pausar a campanha diretamente na Meta antes de depender do aceite do backend.
