...
11. **Agendamento sem gasto antecipado** – `/api/facebook-campaign-resumptions/pending` só entrega a autorização na data inicial em `America/Sao_Paulo`. O worker relê gasto, moeda, destino, campanha e conjunto; campanha com orçamento diário mantém `daily_budget` no ad set e recebe `spend_cap` no nível da campanha. O modo vitalício legado continua aceito sem converter silenciosamente o modo da campanha. Estado `RUNNING` só é confirmado depois do readback nativo de orçamento, teto, prazo e estado ativo.
