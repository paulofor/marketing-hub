...
> - formaliza que a publicação inicial usa orçamento diário no ad set (`budgetMode=ADSET`); na retomada abaixo do mínimo de `spend_cap`, permite migração auditada para orçamento vitalício na campanha, sem ampliar teto ou média diária restante
...
11. **Agendamento sem gasto antecipado** – `/api/facebook-campaign-resumptions/pending` só entrega a autorização na data inicial em `America/Sao_Paulo`. O worker relê gasto, moeda, destino, campanha, conjunto e `min_campaign_group_spend_cap`. Campanha com orçamento diário mantém `daily_budget` no ad set e recebe `spend_cap` no nível da campanha quando o teto autorizado atende ao mínimo da conta. Se o mínimo de campanha exceder o teto, a Meta não aceita orçamento diário de campanha e `lifetime_spend_cap` no conjunto (`100/1885652`). Com a campanha pausada e exatamente um ad set, o worker migra para `lifetime_budget` na campanha, definido como o menor valor entre o teto total autorizado e o gasto confirmado somado ao orçamento diário vezes os dias restantes. A média restante, arredondada para cima, não pode superar o diário autorizado. A campanha recebe `stop_time`; o conjunto recebe o mesmo `end_time` e deve ficar sem orçamento ou limite próprio. Nunca enviar `spend_cap=0`, rejeitado com `100/1885099`, nem `daily_budget=0`, rejeitado com `100/1885272`; `daily_budget` com `lifetime_spend_cap` no conjunto também permanece proibido (`100/1885624`). Se houver teto positivo anterior ou restar orçamento/limite no conjunto, falhar antes de ativar. É proibido elevar o teto, usar esse fallback com múltiplos conjuntos ou ativar uma migração parcial. Retry pode completar a mesma migração após conferir valores e identidades, sem duplicar objetos. O modo vitalício legado continua aceito sem conversão silenciosa. Estado `RUNNING` só é confirmado depois do readback nativo dos dois níveis, teto, prazo e estado ativo; divergência ou resposta HTTP inválida mantém a campanha pausada e o corpo de erro auditável.
...
confirma `lifetime_budget` na campanha, limitado ao teto e à capacidade diária restante,
com o único ad set sem orçamento ou limite próprio. Campanha sem teto anterior omite
`spend_cap`; valor zero nunca é enviado e teto positivo anterior bloqueia a migração
antes de qualquer escrita.
Prazo, identidade, gasto e atribuição anteriores são preservados.
