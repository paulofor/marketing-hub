...
6. **Intervalo acumulado explícito** – a coleta de Insights deve consultar `time_range` desde `start_time` da campanha até o dia atual e agregar com `time_increment=all_days`. É proibido interpretar a resposta vazia de `date_preset=maximum` como zero comercial, pois a Graph API pode omitir a entrega do dia nesse preset. Quando `start_time` estiver indisponível, o fallback deve respeitar a janela de retenção suportada pela Meta e preservar a auditoria da decisão.
