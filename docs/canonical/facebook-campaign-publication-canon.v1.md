...
11. **Agendamento sem gasto antecipado** – `/api/facebook-campaign-resumptions/pending` só entrega a autorização na data inicial em `America/Sao_Paulo`. O worker relê gasto, moeda, destino, campanha, conjunto e `min_campaign_group_spend_cap`. Campanha com orçamento diário mantém `daily_budget` no ad set e recebe `spend_cap` no nível da campanha quando o teto autorizado atende ao mínimo da conta. Se o mínimo de campanha exceder o teto, a Meta não aceita `daily_budget` e `lifetime_spend_cap` simultaneamente no mesmo conjunto: com a campanha pausada e exatamente um ad set, o worker migra o `daily_budget` autorizado para a campanha, omite `spend_cap` quando ele já está ausente, zera o orçamento próprio do conjunto e aplica nele `lifetime_spend_cap` exatamente igual ao teto total. Nunca enviar `spend_cap=0`, rejeitado pela Meta com `100/1885099`; se houver teto positivo anterior, falhar antes da primeira mutação até que sua remoção tenha contrato oficial e teste preventivo. É proibido elevar o teto, usar esse fallback com múltiplos conjuntos ou ativar uma migração parcial. Retry pode completar a mesma migração após conferir valores e identidades, sem duplicar objetos. O modo vitalício legado continua aceito sem conversão silenciosa. Estado `RUNNING` só é confirmado depois do readback nativo dos dois níveis, teto, prazo e estado ativo; divergência ou resposta HTTP inválida mantém a campanha pausada e o corpo de erro auditável.
