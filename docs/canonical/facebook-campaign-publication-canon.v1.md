...
> - formaliza que a publicação inicial usa orçamento diário no ad set (`budgetMode=ADSET`); na retomada abaixo do mínimo de `spend_cap`, preserva a hierarquia diária pausada e cria uma substituta auditada com orçamento vitalício no único ad set, sem ampliar teto ou média diária restante
> - exige intervalo temporal explícito desde o início da campanha na coleta acumulada de Insights, sem depender de `date_preset=maximum`
...
> - torna obrigatório um teto total de mídia por experimento Facebook, separado do orçamento diário do ad set, e a pausa no primeiro sync que alcançar esse teto
...
3. `creative-consumption` — consumir somente criativos aprovados pelo contrato do módulo Facebook.
   A reutilização de anúncio da biblioteca do produto deve criar uma nova revisão vinculada ao criativo de origem, em `DRAFT` e com parecer do agente `PENDING`; reutilização nunca herda aprovação para o novo experimento.
...
| `experiment.daily_budget`, `experiment.media_spend_limit`, `experiment.zero_result_spend_limit`, `experiment.zero_purchase_spend_limit`, `experiment.purchase_stop_count`, `facebook_release_requested_at`, `funnel_reset_at`, `market_niche.facebook_pixel_id`, `market_niche.facebook_pixel_requested_at`, `market_niche.facebook_pixel_request_status`, `status` | `marketinghubdb.experiment` | Controlam orçamento diário do ad set, teto absoluto, paradas financeiras, liberação, resets e sincronismo de pixel. |
...
Decisão canônica de 2026-08-14: o detalhe do experimento deve exibir um gate consolidado, vindo integralmente de `/api/experiments/{experimentId}/readiness`, para a transição segura de `PLANNED` para `RUNNING`. O gate deve informar separadamente landing aprovada, ao menos um criativo publicável, checkout ou destino de conversão configurado, instrumentação obrigatória, público publicável e ausência de etapas comerciais bloqueantes. A UI não pode recomputar esses estados. `eligibleForRunning=true` somente quando todos os requisitos retornados em `runningGateRequirements` estiverem prontos; mudar apenas o status não substitui nenhum requisito nem autoriza publicação ou gasto.
...
1. **Criativos aprovados**
   ...
   - O botão **Gerar anúncios do pipeline** pode produzir até 3 anúncios (texto + prompt) via Worker AI (`gpt-image-2.5-sunburst`). Eles entram como `DRAFT` e precisam ser aprovados antes da liberação.
...
- **Orçamento diário** – `experiment.daily_budget` deve estar preenchido para refletir a automação de mídia. Na publicação inicial, esse valor é orçamento controlado por ad set: o `facebook-ads-worker` deve reportar `budgetMode=ADSET`, enviar `daily_budget` no conjunto de anúncios e criar a campanha sem orçamento próprio com `is_adset_budget_sharing_enabled=false`. Orçamento no nível da campanha não deve ser escolhido para uma validação inicial; a única exceção é a migração auditada de uma campanha existente descrita no item 11, quando a própria Meta impede aplicar o teto autorizado na camada original.
...
3. **Invariante de unicidade de campanha por experimento** – **é proibido** publicar duas campanhas ativas para o mesmo `experiment_id` na mesma plataforma. Se já existir campanha vinculada ao experimento, uma nova liberação deve operar em modo de atualização/reuso da campanha existente (ad sets/anúncios) e nunca criar uma segunda campanha paralela. A única exceção física é a substituição auditada de retomada: a anterior permanece pausada, recebe `superseded_by_campaign_id`, a nova recebe `replaces_campaign_id` e apenas uma delas é tratada como vigente e sincronizável. Para novas publicações, o backend deve materializar a proteção em `facebook_ads_campaign.publication_key`, com chave única por experimento e plataforma.
...
10. **Retomada de campanha existente** – uma campanha já vinculada nunca volta pela fila de primeira publicação. A interface registra em `facebook_campaign_resumption` orçamento diário, teto acumulado incluindo gasto anterior, início, fim, limite sem resultado primário, limite sem compra e quantidade de compras que encerra a coleta. O gate humano do processo só pode aceitar essa campanha quando a autorização estruturada mais recente coincide com o plano vigente e ainda não expirou.
11. **Agendamento sem gasto antecipado** – `/api/facebook-campaign-resumptions/pending` só entrega a autorização na data inicial em `America/Sao_Paulo`. O worker relê gasto, moeda, destino, campanha, conjunto e `min_campaign_group_spend_cap`. Campanha com orçamento diário mantém `daily_budget` no ad set e recebe `spend_cap` no nível da campanha quando o saldo autorizado atende ao mínimo da conta. Quando o mínimo excede o teto, não se converte a campanha existente: a Meta rejeita a troca de diário para vitalício com `100/1885630`. Com a origem e seu único ad set pausados, o worker cria ou recupera por nomes determinísticos uma campanha substituta sem orçamento de campanha, um único ad set com `lifetime_budget` igual ao menor valor entre o saldo acumulado restante e o orçamento diário multiplicado pelos dias disponíveis, e anúncios que reutilizam exatamente os criativos aprovados. A origem permanece pausada e auditável; o backend liga as duas campanhas, congela as métricas anteriores e sincroniza somente a substituta, somando o retrato anterior ao novo insight. A média restante, arredondada para cima, não pode superar o diário autorizado. Nunca enviar `spend_cap=0`, rejeitado com `100/1885099`, nem `daily_budget=0`, rejeitado com `100/1885272`; `daily_budget` com `lifetime_spend_cap` no conjunto também permanece proibido (`100/1885624`), assim como orçamento diário de campanha com limite vitalício no conjunto (`100/1885652`). É proibido elevar o teto, substituir campanhas com múltiplos conjuntos, mudar criativos, ativar duas campanhas ou aceitar hierarquia parcial. Retry recupera objetos determinísticos e não os duplica. Se o callback perder a resposta, o worker consulta o pedido; só preserva a ativação quando o backend já registrou `COMPLETED`. Qualquer outra divergência pausa origem e substituta. O modo vitalício legado continua aceito sem conversão silenciosa. Estado `RUNNING` só é confirmado depois do readback nativo dos dois níveis, saldo, prazo e estado ativo.
12. **Teto prevalece sobre ritmo diário** – `daily_budget × dias` é capacidade máxima de entrega, não gasto obrigatório. É válido o teto acumulado ser menor que esse produto: a Meta e o backend interrompem no primeiro limite alcançado. Orçamento diário nunca pode superar o próprio teto.
13. **Condições de parada auditáveis** – `zero_result_spend_limit` governa ausência de qualquer resultado primário; `zero_purchase_spend_limit` governa especificamente zero compras, mesmo quando já houve formulário ou outro sinal intermediário; `purchase_stop_count` encerra a coleta como sucesso comercial. Teto e ausência de compra encerram como parada comercial, sem fabricar venda. Falha comprovada de checkout ou entrega exige pausa operacional e nova homologação antes de continuar.
14. **Upload de imagem por bytes com reuso canônico (Meta `image_hash`)** – para criativos de anúncio, o worker **não** deve fazer upload cego da mesma imagem em toda execução e **não pode depender de URL externa como caminho de publicação**. O fluxo obrigatório é:
...
6. **Mínimo operacional de público** – no fallback manual, ao menos 1 item aprovado e identificável pela Meta em `INTEREST`, `JOB_TITLE` ou `BEHAVIOR` é obrigatório. Nenhuma dessas categorias é requisito isolado: qualquer item escolhido e aprovado pelo usuário já pode liberar a campanha.
   A tela e o backend devem oferecer somente elementos globais do nicho ou vinculados à mesma hipótese do experimento; elementos de outra hipótese não podem ser exibidos como selecionáveis nem aceitos na gravação.
...
4. **Fonte de verdade de gasto e travas emergenciais** – a Meta é a fonte de verdade para gasto de mídia. O endpoint `POST /api/facebook-campaigns/{campaignId}/metrics` deve persistir a métrica sincronizada mesmo quando regras derivadas de parada, estratégia ou diagnóstico falharem; falhas secundárias não podem devolver `500` e congelar o gasto no Hub. Como proteção financeira, o Facebook Ads Worker deve pausar a campanha diretamente na Meta antes de depender do aceite do backend quando ler gasto acumulado de pelo menos **R$ 25,00** com **zero leads** ou quando o gasto alcançar o `media_spend_limit` do experimento, prevalecendo o primeiro limite atingido.
5. **Reconciliação de pausa manual** – quando o retrato de status da Meta indicar campanha `PAUSED` e o experimento ainda estiver `RUNNING`, o backend deve reconciliar o experimento para `USER_STOPPED`. Campanha pausada na Meta não pode continuar operacionalmente em execução no Hub.
6. **Intervalo acumulado explícito** – a coleta de Insights deve consultar `time_range` desde `start_time` da campanha até o dia atual e agregar com `time_increment=all_days`. O JSON de `time_range` deve chegar à Graph API com exatamente uma codificação de URL; dupla codificação transforma o intervalo em texto inválido, bloqueia a coleta e não pode ser tratada como zero comercial. É proibido interpretar a resposta vazia de `date_preset=maximum` como zero comercial, pois a Graph API pode omitir a entrega do dia nesse preset. Quando `start_time` estiver indisponível, o fallback deve respeitar a janela de retenção suportada pela Meta e preservar a auditoria da decisão.
...
`media_spend_limit`. O executor conserva o modo do único conjunto: confirma
`lifetime_budget` no modo vitalício; no modo diário, confirma `daily_budget` no ad
set e teto na campanha quando compatíveis. Se o mínimo da conta exceder a autorização,
a campanha diária original não muda de tipo: permanece pausada e é substituída por
outra campanha sem orçamento CBO, com um único ad set de `lifetime_budget` limitado ao
saldo acumulado e à capacidade diária restante. Campanha sem teto anterior omite
`spend_cap`; valor zero nunca é enviado e teto positivo anterior bloqueia a substituição
antes de qualquer escrita. A resposta real `100/1885630` comprova que a Meta não permite
converter o tipo de orçamento da campanha existente. Prazo, criativos, gasto, métricas e
atribuição anteriores são preservados por vínculos explícitos entre origem e substituta.
