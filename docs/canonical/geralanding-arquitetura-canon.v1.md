...
- Quando um Quality Review aprovado estiver correlacionado a uma tarefa BPM por `agent-task:<id>`, o callback do backend deve criar ou concluir a ocorrência da atividade `technical` na mesma versão e referência operacional da tarefa. A ocorrência usa início, término, custo, recomendação e referência da execução real de `gera_landing_stage_execution`; somente depois dessa consolidação o backend conclui a tarefa composta de Dédalo e libera as sucessoras. Replays da mesma execução são idempotentes, e aprovações históricas correlacionadas devem ser retroagidas por regra genérica, nunca por identificador de produto, experimento ou tarefa.
