...
- Quando o GeraLanding usar insumos comerciais gerados pelo MOIS, o fluxo canônico é **MOIS grava no banco → GeraLanding lê do banco pelo backend/repository permitido**. Services do GeraLanding não devem depender diretamente de gateways, services, DTOs ou packages funcionais do MOIS para montar contratos `pending`; a integração entre módulos deve acontecer por dados persistidos e auditáveis.
...
- A etapa `landing-page-copy` deve usar configuração dedicada `copy.worker.model`, com padrão `gpt-5.4`, porque a copy final é responsável por clareza comercial, promessa, CTA e redução de esforço percebido pelo lead.
- A etapa `landing-page-image-planning` deve usar configuração dedicada `imageplanning.worker.model`, com padrão `gpt-5.4`, porque o planejamento visual traduz a oferta em prompts de imagem coerentes com a copy e com a prova percebida.
...
- A aprovação do Quality Review usa gate híbrido calibrado: `score >= 85`, todas as dimensões essenciais com nota mínima 8/10, especificidade de público ao menos `medium`, prontidão comercial `strong` ou `excellent`, nenhum `blockingIssue` e nenhuma etapa em `recommendedRegeneration`. Refinamentos que não impedem tráfego seguro devem ficar em `improvementOpportunities` e não bloquear a publicação. Checkout/formulário/link inoperante, promessa incoerente, ausência de prova suficiente, baixa legibilidade, falha responsiva, artefato técnico ou dimensão essencial abaixo de 8 continuam eliminatórios independentemente do score total.
...
- Feedbacks recorrentes do Quality Review devem retroalimentar os prompts upstream: wireframe deve estruturar hero, CTAs, prova visual e formulário para permitir execução premium; copy deve manter coerência entre promessa, CTA e dados realmente coletados; design preset deve materializar botões, formulário, navegação, responsividade e título publicável sem aparência de wireframe ou marcador técnico. Ao reexecutar `landing-page-design-preset`, o prompt deve receber o JSON anterior de `experiment.landing_page_design_preset` e o JSON mais recente de `experiment.landing_page_quality_review`, quando existirem, para comparar o preset gerado anteriormente com as falhas de qualidade e corrigir causas-raiz indicadas em `blockingIssues`/`recommendedRegeneration` sem copiar metadados técnicos para o artefato final. O assembler de HTML final deve bloquear título/metadado técnico provisório mesmo que um artefato anterior tente carregá-lo.
- Quando `recommendedRegeneration` contiver `LANDING_PAGE_HTML`, o design preset pode reconstruir a composição visual do HTML anterior dentro do schema canônico, inclusive reorganizando hierarquia, seções e wrappers para corrigir a evidência renderizada. Essa exceção não permite alterar ids funcionais, contratos de campo, checkout, CTA principal, preço, oferta, tracking, scripts, assets aprovados, promessa, copy ou imagem; o resultado deve voltar ao Quality Review e obedecer idempotência por hash, custo e limite de convergência.
...
- O backend deve registrar o hash SHA-256 do HTML antes de enfileirar o Quality Review. No mesmo ciclo autônomo, HTML sem mudança reutiliza a execução existente e não pode consumir nova revisão. Cada ciclo admite no máximo quatro versões distintas revisadas; ao atingir o limite, o fluxo bloqueia para decisão humana em vez de continuar regenerando e revisando.
- Quando um Quality Review aprovado estiver correlacionado a uma tarefa BPM por `agent-task:<id>`, o callback do backend deve criar ou concluir a ocorrência da atividade `technical` na mesma versão e referência operacional da tarefa. A ocorrência usa início, término, custo, recomendação e referência da execução real de `gera_landing_stage_execution`; somente depois dessa consolidação o backend conclui a tarefa de HTML de Íris — ou a tarefa composta histórica de Dédalo — e libera as sucessoras. Replays da mesma execução são idempotentes, e aprovações históricas correlacionadas devem ser retroagidas por regra genérica, nunca por identificador de produto, experimento ou tarefa.
- `gera_landing_stage_execution.prompt_content` deve ser `LONGTEXT NOT NULL`. O backend deve preservar integralmente briefings e prompts de convergência; truncamento ou redução para caber no banco é proibido porque elimina contexto e auditoria do aprendizado entre versões.
