...
| `landingPageImagePlanning` | Geração dos prompts finais por imagem para o modelo de imagem | `images[]` (`sectionId`, `slotId`, `imageBindingKey`, `imagePrompt`) | Rejeita payload sem `images[]` com prompts por item; quantidade e bindings devem espelhar o wireframe; `slotId` deve existir em `copySlots` da `sectionId` no wireframe. |
