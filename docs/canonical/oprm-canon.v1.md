...
- No caminho operacional rápido do NichoCNAE v3, a etapa `persona-candidate-generator` pode usar OpenAI com ferramenta `web_search` habilitada para pesquisar fontes públicas abertas e montar personas candidatas com evidência de rotina; essa pesquisa deve continuar neutra, sem criar dor/oferta/produto/mecanismo, e deve registrar fontes/limitações em `webEvidence` para auditoria e validação posterior.
- No NichoCNAE v3, a etapa `routine-query-planner` deve transformar persona, tarefas e sinais em queries com núcleo operacional de domínio, removendo termos genéricos de validação ou controle interno como "validar", "confirmar", "rotina" e "controle" quando eles atraírem dicionários, serviços utilitários ou resultados fora do CNAE.
- No NichoCNAE v3, a etapa `source-searcher` deve manter filtro rígido contra fonte fraca, comercial, solução prematura ou fora do domínio, mas deve executar fallback determinístico com termos do domínio quando as queries planejadas retornarem apenas ruído. A correção esperada é melhorar a busca, não relaxar o gate de fonte.
...
- No NichoCNAE v3, o caminho operacional padrão deve priorizar materialização rápida de um perfil de nicho enriquecido assim que houver persona vencedora, rotina e tarefas suficientes para orientar hipótese/oferta/produto. A etapa `persona-tournament` pode avançar diretamente para `persona-routine-materializer` com `materializationMode=FAST_PERSONA_ROUTINE_PROFILE`; as etapas de busca pública, coleta, síntese e gate permanecem como aprofundamento posterior, mas não podem bloquear a entrega mínima de nicho enriquecido quando o objetivo do usuário for apenas obter insumo comercial inicial.
...
- Toda chamada OpenAI do OPRM NichoCNAE deve usar Responses API e modelo operacional `gpt-5.2`, seguindo a política canônica de `service_tier`: primeira tentativa em Flex, segunda tentativa em Flex e terceira tentativa em Standard/default quando a chamada suportar fallback sem quebrar o contrato. A configuração persistida de etapa no backend e o fallback local do executor devem preservar essa sequência.
