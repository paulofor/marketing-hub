...
- Antes de gerar o seed final, a etapa `oprmNicheResearchSeedBuilder` deve quebrar CNAEs amplos em 3 a 7 subnichos operacionais focados em executor MEI/autônomo, usando CNAE, descrição, volume MEI e score OPRM. Cada subnicho deve ser comparado por recorrência, urgência da dor, capacidade de pagar, clareza do resultado e compatibilidade com produto digital, e as etapas profundas seguintes devem pesquisar apenas o subnicho mais promissor.
...
- Decisão operacional vigente: o CNAE é apenas fonte inicial de descoberta e auditoria; o pipeline não deve criar nem materializar o nicho amplo quando houver pesquisa NichoCNAE. A etapa `oprmNicheResearchSeedBuilder` deve escolher um subnicho específico vencedor, com público executor, contexto operacional e dor/resultado observável, e esse subnicho deve substituir o nome neutro do ciclo para todas as etapas profundas posteriores.
- O subnicho vencedor deve ser escolhido por critérios comerciais explícitos: recorrência, urgência da dor, capacidade de pagar, clareza do resultado e compatibilidade com produto digital. Se a IA retornar apenas o CNAE amplo, uma variação genérica do CNAE ou um nome curto demais para representar público/contexto/dor, o backend deve bloquear a conclusão da etapa em vez de permitir materialização ampla.
- Antes de persistir queries para busca, coleta e extração profundas, o backend deve executar um pré-gate comercial determinístico sobre o seed retornado. Esse pré-gate deve exigir cobertura mínima dos critérios de recorrência, urgência da dor, capacidade de pagar, clareza do resultado, compatibilidade com produto digital e famílias de query ligadas a dor, pagamento/cobrança/preço, resultado, aquisição/fidelização e evidência pública; quando a cobertura mínima não existir, a etapa deve falhar cedo para evitar gastar execução completa em subnicho fraco.
...
- O gate de qualidade do NichoCNAE deve medir `dor vendável`, não apenas rotina existente. Uma dor vendável precisa ter evidência mínima de urgência, recorrência, impacto em dinheiro/tempo, tentativa operacional de resolver e resultado desejado; rotina rica sem dor comprável deve voltar para mais pesquisa antes de hipótese, oferta ou materialização.
...
- Testes unitários devem cobrir que reprovações por solução, fonte antiga, desvio corporativo, aquisição/canais fracos e dor vendável fraca recebem movimentos diferentes e coerentes com a causa-raiz.
