...
Decisão canônica de 2026-08-08, complementada em 2026-09-25: correções comerciais de uma hipótese devem criar uma nova versão pela tela, nunca sobrescrever a hipótese histórica. A nova versão deve registrar origem, raiz e número sequencial da linhagem; preservar produto e nicho; recriar seu próprio snapshot de framework a partir de problema, promessa, mecanismo, prova/entrega, regra de sucesso, tipo e preço informados na versão; iniciar o checklist novamente não aprovado; e começar em `BACKLOG`. O framework da origem permanece íntegro apenas em sua versão histórica e nunca pode ser copiado como verdade da nova versão quando os campos comerciais mudaram. Pacote de oferta e entregáveis derivados também não são herdados: precisam ser preparados novamente para a versão, evitando que escopo ou preço antigos liberem um experimento novo. Versionar hipótese não cria experimento, não publica ativos, não ativa mídia e não autoriza gasto.
...
- em PDE de abordagem individual consentida, um run `PRODUCTION` em `READY_TO_PUBLISH` substitui a
  exigência de criativo de mídia paga quando sua homologação cobriu o material da abordagem e a
  microexperiência. Landing, checkout/entrega, distribuição consentida e qualidade dos dados só
  podem ser reconhecidos pelo mesmo run quando cada dimensão possuir, respectivamente, os gates
  `LANDING_QUALITY_REVIEW_APPROVED`, `CHECKOUT_AND_DELIVERY_CAN_BE_COMPLETED`,
  `DIRECT_CHANNEL_READINESS_CONFIRMED` e `DATA_FRESHNESS_VALID` em `PASS`, com referência de
  evidência persistida. O status do run isolado nunca substitui esses gates nem permite inferir
  conclusão de um processo histórico;
...
- quando o pipeline for executado novamente para um CNAE que já possui nicho, a execução não deve ser bloqueada por esse histórico; ao final, o nicho/perfil enriquecido existente deve ser atualizado com as novas informações e `updated_at` renovado, em vez de criar duplicidade silenciosa;
