...
## 2026-04-16 — contrato consolidado de endpoints requeridos do backend para OPRM

**Status:** concluído

**Resumo:**
Foi criado um Swagger consolidado em `docs/novos-modulos/OPRM` com todos os endpoints que o OPRM precisa que o backend ofereça, cobrindo operações de worker (jobs, status, artifacts, feedback e heartbeat) e endpoints de leitura do workspace para as telas do módulo.

**O que foi implementado:**
- criação do arquivo `oprm-backend-required-endpoints.swagger.yaml` com contrato OpenAPI 3.0.3 consolidado
- inclusão de endpoints de orquestração de jobs (`create`, `claim`, `detail`, `status`) com payloads e respostas tipadas
- inclusão de endpoints para publicação e listagem de artefatos, com envelope canônico e status de artefato
- inclusão de endpoints de workspace (`/workspace/routine` e `/workspace/insights`) e suporte à home de ocupações (`/jobs/workspace/occupations`)
- inclusão de endpoints de feedback (`publish` e `history`) e heartbeat operacional
