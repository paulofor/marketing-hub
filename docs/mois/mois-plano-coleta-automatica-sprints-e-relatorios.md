...
- **Período:** 25/04/2026
- **Escopo concluído:**
  - normalização de score de sucesso (0–100) com fórmula ponderada;
  - classificação de confiança (`LOW`/`MEDIUM`/`HIGH`) derivada do score;
  - ordenação por score e data de coleta;
  - filtros por `source`, `niche`, `timeWindow`, `minScore` e `confidenceLevel` na listagem de referências coletadas;
  - fechamento do gap arquitetural: regras de negócio de workspace (dashboard, references, extraction, library, comparison e build-offer) movidas para o módulo MOIS, com backend operando como gateway/contrato.
- **Evidências (PRs, commits, testes):**
  - módulo MOIS: `MoisDomainService` (normalização, ranking e filtros) e `MoisDomainController` (query params de filtro);
  - módulo MOIS: `MoisDomainService` e `MoisDomainController` também responsáveis pelos fluxos de workspace Sprint 1;
  - backend: `MoisModuleGateway` e `MoisController` atuando como delegadores para todo o domínio MOIS;
  - testes: `MoisDomainServiceTest`, `MoisDomainControllerTest` e `MoisControllerContractTest`.
- **Riscos/pendências:**
  - score ainda depende de sinais seedados no domínio, faltando conectores reais para fontes externas;
  - calibração dos pesos deve ser revisada com dados de produção.
- **Próximos passos:** iniciar Sprint 3 para UX dedicada da coleta automática com filtros e ações operacionais.
