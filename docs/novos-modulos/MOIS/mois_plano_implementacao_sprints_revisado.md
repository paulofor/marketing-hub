...
- manter o **MOIS como módulo/serviço separado**, com diretório, container, host e ciclo operacional próprios, deixando o backend como integrador institucional, consumidor de contratos e responsável apenas pelo que for canonicamente seu;
...
3. **MOIS como serviço separado**  
   O MOIS deve existir e evoluir no módulo `mois/`, com diretório, projeto, container, host e operação próprios. O backend não deve absorver a implementação interna do domínio do MOIS.

4. **Backend como integrador institucional**  
   O backend atua como consumidor institucional de contratos, ponto de integração entre módulos e responsável apenas pelos domínios e persistências que lhe pertencem canonicamente.

5. **Reuso com baixo acoplamento**  
...
6. **Registro obrigatório de pendências**  
...
Além disso, a partir desta versão revisada do plano, o Codex deve assumir como invariável arquitetural que **o MOIS é implementado no módulo `mois/`**. O backend pode ter clients, gateways, contratos administrativos e integrações, mas não deve voltar a concentrar a lógica central do domínio MOIS.
...
### Nota de correção arquitetural pós-Sprint 4

Os fechamentos das Sprints 1 a 4 foram preservados neste documento como **registro histórico da execução original**. Entretanto, durante a evolução do módulo foi identificado um desvio arquitetural: parte da implementação inicial foi empurrada para o backend por ambiguidade do plano.

Esse desvio foi corrigido em sprints de ajuste executadas fora do fluxo principal. A partir deste ponto, este plano passa a considerar como estado canônico que:

- o **MOIS vive no módulo `mois/`**, como serviço Spring Boot separado;
- o **MOIS possui diretório, container, host, configuração e ciclo operacional próprios**;
- o backend atua como **integrador institucional**, client/gateway e consumidor de contratos, sem reabsorver a lógica central do domínio;
- toda Sprint a partir da Sprint 5 deve ser executada levando em conta essa arquitetura corrigida.

Ao interpretar o histórico anterior, o Codex deve preservar o valor do que já foi implementado, mas **não deve reproduzir a alocação indevida do domínio no `backend/ads-service`**.
...
### Escopo
- consolidar, no módulo `mois/`, os artefatos já gerados em relatórios ou cards de inteligência;
- implementar agregações úteis para leitura estratégica dentro do próprio serviço MOIS;
- sintetizar padrões recorrentes de mercado;
- começar a apontar saturação, repetição, lacunas e diferenciação aparente;
- expor endpoint(s) de consulta do resultado consolidado no próprio MOIS;
- manter no backend apenas integrações, contratos institucionais e consumo controlado do resultado.

### Entregáveis esperados
- `marketOfferInsightReport` implementado no módulo `mois/`;
- serviço de consolidação do domínio do MOIS no próprio serviço separado;
- endpoint de leitura de relatório consolidado no MOIS;
- filtros básicos por nicho, categoria ou request;
- documentação dos resultados expostos e, se necessário, do consumo pelo backend.
...
### Critério de pronto
A sprint termina quando o MOIS, como serviço separado, conseguir devolver uma leitura consolidada e útil sobre o mercado analisado, em vez de apenas artefatos fragmentados, sem recolocar a lógica central do domínio dentro do `backend/ads-service`.
...
- manter o backend como ponto de integração institucional, sem deslocar a execução do domínio para fora do `mois/`.
...
- clients, gateways ou contratos de consumo no backend apenas quando realmente necessários.
...
### Critério de pronto
A sprint termina quando o MOIS puder ser consumido por pelo menos um fluxo real do Marketing Hub com contrato explícito e previsível, preservando a separação arquitetural entre `mois/` e `backend/ads-service`.
...
- endurecer também os contratos entre `mois/` e backend para evitar regressão da separação arquitetural.
...
### Critério de pronto
A sprint termina quando o MOIS estiver operacionalmente mais previsível, com contratos testados e instrumentação mínima suficiente para manutenção, incluindo os limites claros entre o serviço `mois/` e seus consumidores.
...
- consolidar também a documentação da arquitetura corrigida do MOIS como módulo separado.
...
- registro explícito da arquitetura final do MOIS no ecossistema.

### Fora do escopo
- construir a UI completa nesta sprint, salvo decisão explícita posterior.

### Critério de pronto
A sprint termina quando houver clareza suficiente para operar o MOIS, evoluí-lo e eventualmente conectá-lo a telas próprias sem reabrir a definição básica do módulo, preservando o entendimento de que o domínio vive em `mois/`.
...
**Orientação obrigatória para a fase seguinte:**
- 
- 
- 
...
Entre a Sprint 4 e a Sprint 5 houve sprints de ajuste arquitetural executadas fora do fluxo principal para corrigir a alocação inicial indevida do domínio MOIS no backend. Esta versão do plano já assume a arquitetura corrigida como base para a continuidade.
...
- MOIS como módulo/serviço separado, com diretório, container, host e operação próprios;
- backend como integrador institucional e consumidor de contratos do MOIS, sem reabsorver o domínio;
