...
| PDE Platform AI Worker / Clube MUSA | `191.252.102.54` | `pde-platform/docker-compose.deploy.yml` | `OPENAI_API_KEY_FILE` ou `OPENAI_API_KEY`; `OPENAI_API_KEY_HOST_FILE` quando usar arquivo montado | orientacoes da Consultora MUSA nos 7 dias | worker processa pendencia de IA e registra resultado no backend | cliente recebe PDE sem apoio personalizado da IA, reduzindo valor percebido e ativacao |
