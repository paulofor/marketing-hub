...
|---|---:|---|---|---|---|---|
| Reconciliador automático de publicadores | `191.252.181.168` | `.github/workflows/reconcile-publishers.yml`; registro em `/var/lib/marketinghub/deploy-coordination` | `VPS_SSH_CHAVE` já usada pelo publicador central; `github.token` com `contents: read` e `actions: write`; chave pública do host fixada em `scripts/deploy-control-known-hosts` | retomada automática de intervenção homologada | SSH estrito, lock persistente, revisão integrada e recibos GitHub auditados; nenhuma chave privada nova | retomada permanece pendente com erro explícito; não libera pausa nem troca imagens por SSH |
| Nove publicadores do VPS de agentes | `163.245.202.80` | workflows da fila `deploy-vps-163-245-202-80` | `GROWTH_OPERATOR_VPS_SSH_KEY` prioritária; `VPS_SSH_CHAVE`, `VPS_SSH_KEY` e `SSH_PRIVATE_KEY` como fallbacks de autenticação | qualquer publicação no host | preflight valida arquivo, identidade do servidor e autenticação real não interativa; SSH/SCP/rsync reutilizam a configuração; recusa total bloqueia antes de qualquer mutação | agentes permanecem na versão anterior; fallback não substitui restaurar autorização no host quando todas as chaves forem recusadas |
...
| PDE Platform backend / Mira privado | `163.245.200.7` | `pde-platform/docker-compose.deploy.yml` | `PDE_MIRA_PRIVATE_ACCESS_1`, `PDE_MIRA_PRIVATE_ACCESS_2`, `PDE_MIRA_PRIVATE_QA_TOKEN` | prototipo privado de Mira ativo | deploy `v7` executa Playwright com o token exclusivo de QA, registra cinco eventos `QA_INTERNAL` e preserva os dois acessos humanos sem uso | gate pode ser aprovado sem jornada funcional ou QA pode contaminar as leituras humanas |
| Customer Agent Worker / homologação multiagente PDE | `163.245.202.80` | `customer-agent-worker/docker-compose.yml` e workflow próprio | `PDE_INTERNAL_API_TOKEN` já compartilhado com o PDE Platform pelo GitHub Environment `production` | harness v1 ou cenários sintéticos de Psique ativos | preflight bloqueia valor vazio; o worker abre sessão `AGENT_VALIDATION`, lê a evidência sanitizada e nunca registra o token em entrada, saída, screenshot ou log | homologação técnica e os três pareceres de Psique ficam bloqueados; nunca usar convite humano ou token de QA como fallback |
...
| MarketingHub backend principal / Biblioteca do Harness | `191.252.181.168` | `deploy/docker-compose.yml` | `HARNESS_LIBRARY_INTERNAL_SIGNING_KEY` no GitHub Environment `production`, provisionada em `/root/infra/harness-library/secrets/internal_signing_key` e montada como `HARNESS_LIBRARY_INTERNAL_SIGNING_KEY_FILE=/run/secrets/harness_library_internal_signing_key` | endpoints internos de cadastro e curadoria habilitados | deploy confirma arquivo com ao menos 32 caracteres e chamada assinada do gateway alcança a listagem | gateway não consegue cadastrar nem consultar cartões persistidos |
| Harness Library API | `163.245.200.7` / `https://mkthub.api.br` | `harness-library-api/docker-compose.deploy.yml` | `HARNESS_LIBRARY_API_KEY`, `HARNESS_LIBRARY_INTERNAL_SIGNING_KEY`, token GHCR e chave SSH do host no GitHub Environment `production`; arquivos internos `/run/secrets/harness_library_api_key` e `/run/secrets/harness_library_internal_signing_key`, pertencentes a `10001:10001` e modo `0400` | sempre que o gateway estiver publicado | container não root fica saudável, lê os dois secrets sem expô-los, porta permanece em `127.0.0.1:8103`, listagem autenticada completa o handshake com o backend e o proxy expõe somente HTTPS | cadastro externo fica indisponível ou, se a porta for exposta sem TLS, o segredo pode ser interceptado |
...
| Product Discovery Worker / Argos | `163.245.202.80` | `product-discovery-worker/docker-compose.yml` + `docker-compose.deploy.yml` | origem `BRAVE_SEARCH_API_KEY_HOST_FILE=/root/infra/brave-token/brave_api_key`; copia protegida `/root/infra/argos/secrets/brave_search_api_key`; arquivo interno `BRAVE_SEARCH_API_KEY_FILE=/run/secrets/brave_search_api_key`; sessao individual `ARGOS_CODEX_HOME=/root/infra/argos/codex-home` montada em `/home/node/.codex` | provider Brave ou executor Codex ativos nas pesquisas factuais | preflights nao interativos dentro da imagem confirmam `credential=CONFIGURED`, diretório Codex gravável e `codex login status`; `/healthz` informa `UP` e uma consulta controlada retorna evidências sem revelar credencial ou sessão | descoberta de mercado fica sem pesquisa factual ou Argos bloqueia antes do planejamento, interrompendo a selecao de oportunidades comerciais |
...
| Experiment Strategist Worker / Atena | `163.245.202.80` | `experiment-strategist-worker/docker-compose.yml` e workflow próprio | `EXPERIMENT_STRATEGIST_CODEX_HOME` individual; `CLARITY_API_TOKEN_FILE` montado por `EXPERIMENT_STRATEGIST_CLARITY_TOKEN_FILE_HOST` quando houver consulta Clarity; transporte usa as chaves da fila de agentes | sessão obrigatória para inferência; Clarity somente para pesquisa agregada que use essa integração | sessão gravável e `codex login status`, health e callback real com prompt/resultado; estado de entrega em volume próprio sobrevive à recriação | atividade fica bloqueada ou parecer não chega ao ciclo; não compartilhar sessão, não recriar aprovação nem expor tokens |
