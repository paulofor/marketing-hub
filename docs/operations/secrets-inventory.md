...
| Product Discovery Worker / Argos | `191.252.120.96` | `product-discovery-worker/docker-compose.yml` + `docker-compose.deploy.yml` | origem `BRAVE_SEARCH_API_KEY_HOST_FILE=/root/infra/brave-token/brave_api_key`; copia protegida `/root/infra/argos/secrets/brave_search_api_key`; arquivo interno `BRAVE_SEARCH_API_KEY_FILE=/run/secrets/brave_search_api_key`; sessao individual `ARGOS_CODEX_HOME=/root/infra/argos/codex-home` montada em `/home/node/.codex` | provider Brave ou executor Codex ativos nas pesquisas factuais | preflights nao interativos dentro da imagem confirmam `credential=CONFIGURED`, `config.toml` legivel, diretorio gravavel e `codex login status`; `/healthz` informa `UP` e uma consulta controlada retorna evidencias sem revelar credencial ou sessao | descoberta de mercado fica sem pesquisa factual ou Argos bloqueia antes do planejamento, interrompendo a selecao de oportunidades comerciais |
