...
| Video Management | `177.153.62.107` ou host configurado | `deploy/docker-compose.yml` | `GEMINI_API_KEY_HOST_FILE=/root/infra/gemini-token/gemini_api_key`, `LUMA_API_KEY_HOST_FILE=/root/infra/luma-token/luma_api_key`, `KLING_API_KEY_HOST_FILE=/root/infra/kling-token/kling_api_key`, arquivos internos `GEMINI_API_KEY_FILE`, `LUMA_API_KEY_FILE`, `KLING_API_KEY_FILE` | providers VEO, Luma Ray ou Kling habilitados | job do provider avanca sem erro de autenticacao e `/api/status` nao revela credenciais | videos de venda nao renderizam ou fallback de criativo fica indisponivel |
