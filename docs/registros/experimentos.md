## 2026-06-20 — Etapa 3 da opção Meta Instant Form nos relatórios de experimento

- solicitação: executar a etapa 3 da nova opção para usar Meta Instant Form no lugar da landing em experimentos.
- foi feito: o material de relatório passou a carregar `captureDestinationType` e detalhes operacionais do Instant Form (`facebookFormId`, aprovação e publicação).
- interface e markdown: relatórios agora mostram o destino de captura; quando o destino é `META_INSTANT_FORM`, a UI e o relatório completo deixam claro que analytics de landing não se aplica como fonte primária e que a leitura deve usar métricas/leads da Meta e o snapshot do formulário.

## 2026-06-20 — Etapa 2 da opção Meta Instant Form nos experimentos

- solicitação: executar a etapa 2 da nova opção para usar Meta Instant Form no lugar da landing em experimentos.
- foi feito: a prontidão do experimento passou a trocar o bloqueio de destino conforme `captureDestinationType`; `LANDING_PAGE` continua exigindo landing/URL e `META_INSTANT_FORM` passa a exigir formulário aprovado e endereçável pela Meta.
- publicação: o Facebook Ads Worker passou a respeitar explicitamente `captureDestinationType`; campanhas de Instant Form usam modo on-ad com `lead_gen_form_id` e sem link externo no criativo, enquanto campanhas de landing ignoram formulários vinculados apenas como metadado.
- interface: o checklist da tela do experimento passou a mostrar o bloqueio correto para Landing ou Meta Instant Form usando a resposta de prontidão do backend.

## 2026-06-20 — Etapa 1 da opção de destino Instant Form no experimento

- solicitação: executar a etapa 1 da nova opção para usar Meta Instant Form no lugar da landing em experimentos.
- causa-raiz: o experimento já podia referenciar `leadPortalFlow` e `facebookInstantForm`, mas não havia um campo explícito dizendo qual destino era a escolha oficial da campanha, gerando risco de inferência ambígua.
- foi feito: adicionado `captureDestinationType` ao experimento com valores `LANDING_PAGE` e `META_INSTANT_FORM`, default `LANDING_PAGE`, exposto em DTOs/API e exibido como escolha simples nas telas de detalhe, criação e edição do experimento.
- documentação: o cânone de procedimento de experimento passou a declarar `captureDestinationType` como fonte de verdade da escolha operacional de captura.
- próximos passos: adaptar prontidão/liberação do Facebook Ads Worker e relatórios por destino em etapas futuras.
- arquivos alterados:
  - backend/ads-service/src/main/java/com/marketinghub/experiment/Experiment.java
  - backend/ads-service/src/main/java/com/marketinghub/experiment/ExperimentCaptureDestinationType.java
  ...
  - backend/ads-service/src/main/resources/db/changelog/V2026_06_20__add_capture_destination_to_experiment.sql
...
## 2026-06-20 — Validação de criação de Meta Instant Form por token operacional

- solicitação: criar um Instant Form fake para testar se o token operacional da Meta possui permissão real de criação/gerenciamento e registrar o procedimento em documento canônico.
- causa-raiz: a validação anterior confirmava leitura/listagem e chegava à validação de payload de criação, mas ainda não havia uma criação real registrada como evidência operacional.
- foi feito: criado na Meta o formulário descartável `TESTE_API_MARKETING_HUB_INSTANT_FORM_FAKE_20260620` na página `485863027935937`; a Graph API retornou o identificador `2487034981799094` e a listagem posterior confirmou `status=ACTIVE` e `leads_count=0`.
- cuidado operacional: nenhum token foi registrado; respostas com URLs de paginação da Graph API devem ser sanitizadas antes de entrar em documentação, PR ou resposta ao usuário.
- documentação: o procedimento canônico de teste foi registrado em `docs/canonical/facebook-campaign-publication-canon.v1.md`.
- arquivos alterados:
