...
## 2026-07-11 — Link visível no nome do experimento no planejamento
...
- causa-raiz operacional: a tool `pde_db_health` confirmava apenas o schema `marketinghubdb`, mas não expunha host/porta/URL sanitizada do datasource PDE; isso permitia interpretar métricas do MCP sem provar que a consulta usava a mesma base do summary público da v6.
...
- Em 2026-08-07, a homologação final do experimento fake 84 comprovou que a oferta de `1 post + 1 story` estava configurada com apenas uma saída e nenhuma imagem gratuita. A tela administrativa foi usada para alinhar o experimento e o fluxo publicado a duas saídas. No Lead Portal, o modelo canônico `AI_PERSONALIZED_SAMPLE_FUNNEL` passou a liberar gratuitamente todo o lote planejado, com teste que impede regressão sem afetar formulários pagos.
...
- reconciliação adicional: os Insights oficiais da campanha continuavam vazios e os acessos anteriores possuíam origem de validação da Meta ou automação, todos sem `visitorId`; portanto, permanecem como evidência técnica e não como tráfego ou venda;
- correção local de mensuração: cada evento passa a registrar qualidade `HUMAN`, `AUTOMATED` ou `UNKNOWN`; apenas identidade first-party sem sinal de automação alimenta o placar comercial, sem excluir registros históricos;
- fechamento da auditoria histórica: URLs oficiais com `mh_test=1` ou `mh_audit=*` são classificadas como automação tanto na ingestão quanto no backfill, impedindo que emulações mobile antigas sejam promovidas a visitas comerciais;
- correção local de desempenho: as oito imagens aprovadas da landing somavam aproximadamente 19 MB; o Lead Portal mantém a URL canônica no `src`, publica derivados JPEG de até 960 px e orienta o navegador a carregar primeiro somente a imagem prioritária;
- governança do subprocesso: a Mesa do agente passa a permitir contexto direto `experiment:88`, vinculando as tarefas de Operação e otimização de experimento v3 ao experimento em vez de atribuí-las artificialmente a um plano comercial;
- causa-raiz de execução: Hermes consumia somente a fila especializada de Plano Comercial e, por isso, uma atividade BPM criada para `experiment:88` permaneceria sem executor; o `growth-operator-worker` passa a consumir exclusivamente as atividades de sua responsabilidade em `operacao-otimizacao-experimento` v3, consultar o experimento por MCP somente leitura e reportar resultado, evidências e tokens na tarefa canônica;
- prevenção: o contrato versionado do worker impede avanço com métricas indefinidas, tráfego automatizado, menos de cem visitas humanas válidas, p95 acima de quatro segundos ou erros de recurso, e o custo continua calculado pelo backend a partir do uso real do modelo;
- homologação após publicação: confirmar zero tráfego comercial enquanto a Meta estiver sem impressões, gerar evento humano identificado, validar p95 inferior a quatro segundos e zero erro de recurso em desktop e mobile antes de acumular a amostra de cem visitas.
- evidência posterior no experimento #88: a Meta acumulou 205 impressões, quatro cliques e R$ 4,68, mas o worker persistia erro HTTP 400 porque o JSON de `time_range` chegava duplamente codificado; o banco permaneceu corretamente com o último agregado válido em zero, sem inventar métrica.
...
- **Correção local:** o deploy passa a reconciliar a sessão pelo UID/GID real, validar Codex e Brave
  dentro da imagem, preservar o restante do script remoto e exigir recriação e saúde. O histórico
  mostra a causa no próprio cartão, cada solicitação ganha uma rota de detalhe e um comando
  idempotente de nova tentativa com a mesma entrada.
