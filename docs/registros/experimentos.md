...
- foi feito: a tela de novo teste passou a salvar localmente o rascunho em andamento, incluindo `promiseRequestId` e opções retornadas pela IA; a lista de Testes de Nicho agora exibe o botão **Continuar teste em criação** quando existe rascunho local; a tela de criação também ganhou o botão **Voltar para Testes de Nicho**.
