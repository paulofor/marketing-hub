...
- **O que resolveu efetivamente no histórico**:
  ...
  - em 2026-09-26, o sucessor Facebook #94 do Capella foi criado pela tela, mas o checkout auditado do #88 não podia ser reutilizado: o fluxo de sucessor só aceitava troca do canal direto para Facebook, enquanto a criação manual não persistia a linhagem. A tela agora oferece adoção explícita da origem para um experimento `PLANNED`; o backend exige igualdade de produto, nicho, hipótese, território de desejo, tipo, mecanismo, objetivo, preço e identidades Meta, copia somente página e checkout e bloqueia destino com campanha, métrica ou liberação. Testes de serviço, HTTP e interface preservam a segregação de campanha, métricas, público e criativos.
...
- **Regra preventiva**:
  ...
  - em 2026-08-29, a tarefa #263 de Psique capturou e persistiu seis evidências da Rigel, mas a
    Responses API rejeitou o schema antes do parecer porque `visualAudit.mobileFirst` e
    `foldAnalyses.deviceProfile` usavam `const` sem `type`. Os três schemas BPM v2 equivalentes
    passam a declarar os tipos `boolean` e `string`; o teste sensorial percorre recursivamente cada
    schema e impede qualquer nova constante sem tipo explícito.
...
- **Fechamento estrutural em 2026-09-03:** todo novo ciclo passa por saldo/quota oficiais e Model Router `dryRun`; o backend reserva atomicamente a soma dos tetos duros por cena, exige modelo ativo com adapter, preço, licença e QA homologados, congela o payload e bloqueia drift na resposta paga. Falha terminal nunca cria substituição paga automática. O parecer de Plutus é auditado antes do callback e reutilizado após falha de rede, impedindo novo consumo de IA. Testes cobrem concorrência, saldo insuficiente, teto inseguro, rota não homologada, hash, expiração, idempotência e drift.
...
## LOOP-DESTINO-CONDICIONAL-LANDING-OBRIGATORIA — produção duplicada no ciclo privado

- **Data:** 2026-09-12.
- **Evidência:** Vega, ciclo 2, experimento 92; tarefas 409/410 bloqueadas em `select`. O contrato 402 aponta para o PDE privado v12 já homologado em 395–399; o criativo 406 e seus pareceres 407/408 preservam esse destino. A definição do pai condiciona a landing ao destino aprovado, mas o executor abria o filho sempre. A integração também exigia plano/slot comerciais ausentes no sucessor.
- **Causa-raiz:** a escolha contextual do destino não era um contrato backend; a presença de `subprocessCode` era interpretada como obrigatoriedade. A mensagem sobre prova visual faltante descrevia a consequência de ter escolhido o fluxo errado.
- **Correção:** backend resolve destino privado aprovado ou subprocesso comercial; a integração privada usa provas vigentes do próprio ciclo. Versionamento, hashes, revisões e decisão de uso são revalidados. O filho indevido recebe `CLOSED` com motivo e mantém objetivos pendentes, falhas, custos e retorno ao pai. Trabalho em curso é aguardado.
...
## LOOP-ESTUDIO-ROTEIRO-REESCRITO-AO-SALVAR — correção em 13/09/2026

- Os projetos históricos #1 e #3 do Vega contêm prefixos repetidos ou “Valor antigo
  nao padronizado” no gancho. A leitura do formulário usa o gancho inteiro como dor
  e o canal como público; a gravação recompõe gancho e roteiro em cada salvamento.
- A regressão local reproduziu a perda ao alterar apenas o título. A correção usa
  os campos canônicos já persistidos, permite edição explícita e torna campanhas
  livres com sugestões, para não obrigar um sucessor a selecionar versão antiga.
- Contratos verificam dois salvamentos sucessivos, texto final explícito e campanha
  própria. A matriz cobre preflight sem consumo, orçamento e desktop/mobile.
- Evidências: [recuperação dos vídeos de Vega](../homologacao/vega-videos-briefing-estudio-v1.md).

### Recorrência audiovisual do LOOP-BPM-DECISAO-HUMANA-COMO-EXECUCAO — 13/09/2026

Após concluir VIDEO_BRIEF no evento12, Vega/#92 abriu projetos4/5 e preflights12/13.
O fornecedor respondeu `PROVIDER_ROUTER_CONFIG_MISSING`; o pai ainda anunciava
WAITING_ACTIVITY, porque a projeção só cobria VIDEO_BRIEF. A orientação agora lê o
último preflight da peça/versão, expõe responsável, projeto e evidência e permite a
pausa sem inventar execução. Tentativa nova ativa oculta o bloqueio antigo. Testes
ProcessRunVideoGuidanceTest, ProcessRunGuidanceTest, VideoProductionCycleGuidanceQueryTest,
...
## LOOP-OPALA-GATE-FINAL-COMPARA-JSON-LITERAL — 19/09/2026
