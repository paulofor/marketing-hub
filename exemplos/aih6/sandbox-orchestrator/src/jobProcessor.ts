...
export class SandboxJobProcessor implements JobProcessor {
  ...
  private buildCodexAppServerInput(job: SandboxJob): Array<Record<string, string>> {
    ...
    const marketingFinalJsonInstruction = 'Na resposta final do modo MKT, responda somente com JSON válido no formato {"titulo":"<título muito curto, uma frase simples>","comentario":"<resposta principal em Markdown>","impactoAumentoVendas":"medio","alterouCodigoRepositorio":false,"resumoCodigoPr":"","sugestaoMelhoriaAmbiente":"<sugestão de melhoria do ambiente, ou string vazia>"}. O campo impactoAumentoVendas deve ser exclusivamente "baixo", "medio" ou "alto". Use alterouCodigoRepositorio=true somente quando arquivos versionados tiverem sido criados, removidos ou alterados. Não escreva texto fora do JSON.';
    const emailTestingInstruction = this.buildSandboxEmailInstruction();
    const taskDescription = this.isChatgptCodexMarketing(job)
      ? `Modo Codex ChatGPT MKT ativo: baixe e analise o repositório como fonte de relatórios de marketing, principalmente arquivos Markdown. Priorize campanhas, estratégias, funis, canais, criativos, métricas, resultados, aprendizados e oportunidades de marketing digital. Gere orientações acionáveis de melhoria em português e só prepare mudanças para PR quando o usuário solicitar explicitamente. ${marketingObjectiveInstruction} ${bestAnswerInstruction} ${localDevelopmentInstruction} ${marketingDecisionInstruction} ${marketingFinalJsonInstruction} ${emailTestingInstruction}
