...
- Em retomada de um único ad set diário, consultar `min_campaign_group_spend_cap`; quando o saldo total autorizado for menor que esse mínimo, nunca combine orçamento diário de campanha com `lifetime_spend_cap`, pois a Meta rejeita tipos diferentes com `100/1885652`, e nunca tente converter a campanha existente para vitalício, rejeitado com `100/1885630`. Com tudo pausado, preserve a origem e crie ou recupere uma hierarquia substituta determinística: campanha sem orçamento CBO, um único ad set com `lifetime_budget` limitado ao menor valor entre o saldo acumulado restante e o orçamento diário vezes os dias restantes, e anúncios que reutilizam os criativos aprovados. A média restante arredondada para cima não pode superar o diário autorizado. Exija releitura exata, origem pausada, término e estado antes de ativar; retries não duplicam objetos, falha não confirmada pausa ambas e o backend agrega o histórico sem elevar o teto.
...
- Na substituição de uma campanha CBO por hierarquia ABO, preserve a estratégia de lance confirmada. A Graph API pode expor `LOWEST_COST_WITHOUT_CAP` apenas na campanha de origem e omiti-la no ad set; nesse caso copie o valor da campanha para o novo ad set, pois omiti-lo causa `100/2490487`. Para estratégia com limite, exija também `bid_amount` positivo confirmado no ad set de origem e falhe antes da criação se ele não existir. Releia estratégia e valor na hierarquia substituta antes de ativar.
...
- Em estimativas de alcance (`/reachestimate`), bloqueie quando a Meta retornar público abaixo do mínimo canônico; se o público vier acima de 20.000.000 ou se os limites não vierem na resposta, registre aviso e siga como teste controlado, salvo erro explícito de segmentação ou outro bloqueio canônico. Público amplo deve ser avaliado por criativo, objetivo, CPM, CTR, leads, vendas e qualidade do tráfego.
...
- Na fila de resolução de targeting (`targeting_resolution_job`), o worker deve recriar automaticamente jobs ausentes para candidatos ainda em `PENDING_FACEBOOK_MATCH` antes de reivindicar a fila, evitando que sugestões geradas fiquem sem validação Meta.
...
- Retomadas financeiras devem consumir `/api/facebook-campaign-resumptions/pending`, reservar por
  ...
  `spend_cap` da campanha quando o saldo atende ao mínimo da conta; abaixo desse mínimo, a retomada
  mantém a hierarquia diária pausada e cria uma substituta sem orçamento CBO, cujo único conjunto
  usa `lifetime_budget` sobre o saldo restante e reutiliza os criativos aprovados. O valor vitalício
  nunca supera o teto acumulado nem a capacidade diária remanescente autorizada. A origem permanece
  auditável, apenas a substituta vira vigente e as métricas são agregadas. Conjunto vitalício já
  existente mantém seu modo.
...
- O fallback de segmentação manual para publicação de campanha pertence ao `facebook-ads-worker`: quando não houver playbook pronto, buscar o pacote aprovado no backend pelo contrato enxuto (`/api/facebook-adsets/experiments/{experimentId}/targeting-package`) e montar o targeting da Meta localmente, sem carregar o DTO completo do experimento, sem delegar ao `ai-worker` e sem chamar OpenAI. Qualquer item aprovado e identificável pela Meta (`INTEREST`, `JOB_TITLE` ou `BEHAVIOR`) já atende o mínimo operacional de público; não exigir cargo isoladamente.
...
- Na publicação de campanhas, preserve o `publicationJobId` recebido em `/api/facebook-campaigns/experiments-ready` e registre cada interação com a Graph API da Meta em `POST /api/facebook-campaigns/publication-job-steps`, incluindo payload enviado, payload recebido, endpoint, método, status e etapa.
