...
export async function readCodexAccount(client: CodexAppServerClient): Promise<CodexAccountState> {
  if (!client.isReady()) {
    return unavailableAccount('CODEX_APP_SERVER_UNAVAILABLE');
  }
  ...
  try {
    ...
    const message = err instanceof CodexAppServerError || err instanceof Error ? err.message : String(err);
    return unavailableAccount(message || 'CODEX_APP_SERVER_UNAVAILABLE');
  }
}
...
function unavailableAccount(blockReason: string): CodexAccountState {
  return {
    connected: false,
    status: 'unavailable',
    requiresOpenaiAuth: true,
