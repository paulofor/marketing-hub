...
describe("videoProviderCatalog", () => {
  ...
  it("inclui Seedance 2.0 sob a Runway com duracao direta de ate 15 segundos", () => {
    const provider = findSalesVideoProviderOption("RUNWAY_SEEDANCE_2");

    expect(provider).toBeDefined();
    expect(provider?.label).toBe("Seedance 2.0 via Runway");
  ...
  it("declara limites por solicitacao para os providers integrados", () => {
    ...
    expect(
      findSalesVideoProviderOption("RUNWAY_SEEDANCE_2")
        ?.maxDirectDurationSeconds,
    ).toBe(15);
    expect(
