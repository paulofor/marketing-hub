...
export default function EditAgentPage() {
  ...
  const initialValue = useMemo<AgentPayload | null>(() => {
    ...
    return {
      ...
      businessObjective: agent.businessObjective ?? "",
      successMetrics: agent.successMetrics ?? "",
      modelName: agent.modelName ?? "",
      triggerPolicy: agent.triggerPolicy ?? "",
      authorityPolicy: agent.authorityPolicy ?? "",
      responsibilityContract: agent.responsibilityContract ?? "",
      orchestratorPolicy: agent.orchestratorPolicy ?? "",
      analysisPolicy: agent.analysisPolicy ?? "",
      offeringPolicy: agent.offeringPolicy ?? "",
      promptContractPath: agent.promptContractPath ?? "",
      schemaContractPath: agent.schemaContractPath ?? "",
