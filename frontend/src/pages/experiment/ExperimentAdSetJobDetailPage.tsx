  ...
  if (!data || !data.job) {
    return (
      <div className="container-fluid">
        ...
        <Link to={`/experiments/${experimentId}/adset-workflow`} className="btn btn-link">
  ...
  return (
    <div className="container-fluid">
          ...
          <div className="row gy-2">
            ...
            {job.errorMessage ? (
              <div className="col-12">
                <strong className="text-danger">Último erro:</strong> {job.errorMessage}
              </div>
            ) : null}
          </div>
        </div>
      ...
      <div className="card">
        ...
        <div className="card-body">
          {!apiLogs?.length ? (
            <p className="text-muted mb-0">Nenhuma chamada registrada para este job.</p>
          ) : (
            apiLogs.map((log) => {
              const durationMs = log.requestedAt && log.respondedAt
                ? Math.max(0, new Date(log.respondedAt).getTime() - new Date(log.requestedAt).getTime())
                : null;
              return (
                <div key={log.id} className="mb-4 border rounded">
                  <div className="p-3 border-bottom bg-light d-flex flex-column flex-md-row justify-content-between gap-2">
                    <div>
                      <strong>{log.provider}</strong> · {log.httpMethod ?? "—"} · {log.endpoint ?? "—"}
                    </div>
                    <div className="text-muted small">
                      Início: {formatDateTime(log.requestedAt)}
                      {log.respondedAt ? ` · Fim: ${formatDateTime(log.respondedAt)}` : null}
                    ...
                    <div className="row g-3">
                      <div className="col-12 col-lg-6">
                        <details open>
                          ...
                          <pre className="bg-dark text-white p-2 mt-2 rounded" style={{ whiteSpace: "pre-wrap" }}>
