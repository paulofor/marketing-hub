...
function tryParseJsonBlock(raw: string): unknown | undefined {
...
function buildPromptLineItems(promptUsed: string): PromptLineItem[] {
  return promptUsed.split("\n").map((line) => {
    ...
    if (inlineJsonMatch) {
      ...
      const parsedJson = tryParseJsonBlock(jsonRaw);
    ...
    const parsedJson = tryParseJsonBlock(trimmed);
    if (parsedJson !== undefined) {
      return { kind: "json", jsonValue: parsedJson };
    }
...
function PromptJsonTreeNode({
  ...
  if (!isArray && !isObject) {
    return (
      <div className="small">
        ...
        <code>{formatJsonPrimitive(value)}</code>
      </div>
    );
  }
  ...
  const nodeLabel = isArray
    ? `${label ? `${label}: ` : ""}Array(${entries.length})`
    : `${label ? `${label}: ` : ""}Objeto(${entries.length})`;

  return (
    <details open={depth < 1}>
      <summary className="small">{nodeLabel}</summary>
            ...
            if (item.kind === "json") {
              return (
                ...
                >
                  {item.jsonTitle ? (
                    <p className="mb-1">
                      <strong>{item.jsonTitle}:</strong>
                    </p>
                  ) : null}
                  <PromptJsonTreeNode value={item.jsonValue} />
                </div>
              );
            }
