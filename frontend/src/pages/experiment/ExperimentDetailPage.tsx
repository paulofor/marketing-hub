...
export default function ExperimentDetailPage() {
  ...
  const { data: deliverables } = useDeliverablesByNiche(nicheIdParam);
  const { data: deliverablePackages } = useDeliverablePackagesByExperiment(expId);
  ...
  const createPackage = useCreateDeliverablePackage(expId);
  useBreadcrumbs([
    {
      label: niche?.name || "...",
  ...
  const deliverableOptions = Array.isArray(deliverables) ? deliverables : [];
  const packages = Array.isArray(deliverablePackages)
    ? [...deliverablePackages].sort((a, b) => {
  ...
  const onCreatePackage = handleSubmitPackage(
    async (values) => {
      try {
        ...
        alert("Não foi possível salvar o pacote de entregáveis.");
      }
    },
    (errors) => {
      console.log("Package form errors", errors);
    },
  );
  return (
    <div>
            ...
            <div className="row g-3">
              <div className="col-md-4">
                ...
                <input
                  ...
                  {...registerPackage("name", { required: true })}
                />
              </div>
              <div className="col-md-4">
              ...
              <div className="col-md-4">
                ...
                <select
                  ...
                  disabled={createPackage.isPending || deliverableOptions.length === 0}
                  {...registerPackage("deliverableIds")}
                >
                ...
                <small className="text-muted">
                  Use Ctrl/Cmd + clique para selecionar múltiplos.
                </small>
              </div>
              <div className="col-12">
                ...
                <textarea
                  ...
                  disabled={createPackage.isPending}
                  {...registerPackage("prompt", { required: true })}
                />
              </div>
              <div className="col-12">
                ...
                <div key={pkg.id} className="border rounded-3 p-3 bg-body-tertiary">
                  ...
                  {pkg.deliverables.length > 0 ? (
                    ...
                    <p className="text-muted small mb-2">
                      Nenhum entregável associado ainda.
                    </p>
                  )}
                  <details className="mt-2">
                    ...
                    <pre className="bg-body-secondary rounded-3 p-2 mt-2 small text-muted">
                      {pkg.prompt}
                    </pre>
                  </details>
                  <div className="text-muted small mt-2">
                    {`Atualizado em ${
                      pkg.updatedAt
                        ? new Date(pkg.updatedAt).toLocaleString("pt-BR")
                        : "-"
                    }`}
                  </div>
                </div>
