...
export default function DirectContactSamplePanel({
  ...
  return (
    <section className="card border-0 bg-light mb-4" aria-label="Amostra direta consentida">
      <div className="card-body">
        <div className="d-flex flex-wrap justify-content-between align-items-start gap-3">
          ...
          >
            {sample.data.recordedContacts}/{sample.data.targetContacts} contatos
          </span>
        </div>
        ...
        {sample.data.readyForHermesReview ? (
          <div className="alert alert-success mt-3 mb-0 d-flex gap-2" role="status">
            <CheckCircle2 size={20} aria-hidden="true" />
            A amostra atingiu a meta. Reavalie esta atividade para Hermes
            consolidar o resultado real.
          </div>
        ) : (
        ...
        <form className="row g-3 mt-1" onSubmit={handleSubmit}>
          ...
          <div className="col-12 col-md-6 col-lg-4">
            <label className="form-label" htmlFor="direct-contact-contacted-at">
          ...
          <div className="col-12 col-lg-4">
            <label className="form-label" htmlFor="direct-contact-recorded-by">
              Registrado por
            </label>
            <input
              className="form-control"
          ...
          <div className="col-12">
            <button
              ...
              type="submit"
              disabled={register.isPending || sample.data.readyForHermesReview}
            >
              <UserPlus size={17} aria-hidden="true" />
              {register.isPending ? "Registrando..." : "Registrar contato realizado"}
            </button>
          </div>
        </form>
              ...
              <tbody>
                {sample.data.contacts.map((contact, index) => (
                  <tr key={contact.id}>
                    ...
                    <td>{new Date(contact.consentRecordedAt).toLocaleString("pt-BR")}</td>
                    <td>{new Date(contact.contactedAt).toLocaleString("pt-BR")}</td>
