...
describe("ProductProcessActivityExecutionsPage", () => {
  ...
  it("uses the backend position of an adopted Opala preparation instead of the product's live version", async () => {
    ...
    renderPage(
      "/products/4/value-chain-history/processes/77/activities?learningCycleId=2&chainId=14#activity-entry",
    );

    expect(
      await screen.findByRole("heading", {
        name: "Vega · Processo 5.2 — Preparar operação comercial Opala",
      }),
    ).toBeInTheDocument();
