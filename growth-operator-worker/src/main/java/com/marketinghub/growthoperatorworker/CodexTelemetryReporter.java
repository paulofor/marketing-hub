...
/** Responsabilidade: enviar ao backend a atividade auditável do Codex do Operador. */
@Component
public class CodexTelemetryReporter {
  ...
  public CodexTelemetryReporter(WorkerProperties properties) {
    ...
    private void send(String action, boolean terminal) {
      try {
        ...
        backend
            .post()
            .uri(
                "/api/codex-agent-telemetry/v1/internal/GROWTH_OPERATOR/executions/{id}/{action}",
                id,
                action)
            .body(body)
            .retrieve()
        ...
        log.warn("Falha na telemetria Codex do Operador; executionId={}", id, ex);
      }
    }
  }
}
