...
[[ "$round" =~ ^[a-zA-Z0-9_-]+$ ]]
output="$PWD/artifacts/product-next-activity/$round"
mkdir -p "$output"
run() {
  local label=$1
  shift
...
run tests npm --prefix frontend test -- --run src/components/ProductNextActivitySummary.test.tsx src/components/ProductValueChainCycleSummary.test.tsx src/components/ProductValueChainPosition.test.tsx src/pages/HomePage.test.tsx src/pages/product/ProductListPage.test.tsx src/pages/product/ProductProcessActivityExecutionsPage.test.tsx src/pages/product/ProductValueChainHistoryPage.test.tsx src/pages/learningCycle/LearningCyclesPage.test.tsx
run typecheck npm --prefix frontend run typecheck
run build npm --prefix frontend run build
run browser env "NEXT_ACTIVITY_OUTPUT=$output/browser" node infra/testing/product-next-activity/browser.cjs
run format npm exec --yes --package=prettier@3.6.2 -- prettier --check frontend/src/components/ProductNextActivity* frontend/src/components/ProductValueChainCycleSummary.tsx frontend/src/components/ProductValueChainCycleSummary.test.tsx frontend/src/components/ProductValueChainPosition.tsx frontend/src/components/ProductValueChainPosition.test.tsx frontend/src/api/businessProcess/useProductProcessActivityExecutions.ts frontend/src/api/learningCycle/useCycleProcessContext.ts infra/testing/product-next-activity/browser.cjs infra/testing/product-next-activity/rigel-activities.json
