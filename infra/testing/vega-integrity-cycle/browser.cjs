...
(async () => {
  const output = path.resolve(process.env.VEGA_GATE_BROWSER_OUTPUT || 'artifacts/vega-one-action/browser');
  const contract = path.resolve(process.env.VEGA_GATE_BROWSER_SCREEN || 'artifacts/vega-one-action/prep/screen.json.recovery.json');
  ...
  try {
    for (const [name, profile] of [['desktop', { viewport: { width: 1440, height: 1080 } }], ['iphone', devices['iPhone 15 Pro']], ['pixel', devices['Pixel 7']]]) {
      ...
      let release, historyFailureResponses = 0, cycleFailureResponses = 0, auditReads = 0;
      ...
      await page.route('**/*', async route => {
        ...
        if (url.pathname.startsWith('/api/')) {
          ...
          if (url.pathname.endsWith('/prompt-audit')) {
            auditReads++;
            assert.equal(url.searchParams.get('sourceReference'), 'experiment:92');
            const taskId = Number(url.pathname.split('/').at(-2));
            if (auditReads === 1) return route.fulfill({ status: 503, json: { message: 'Falha simulada da auditoria' } });
            return route.fulfill({ json: { taskId, sourceReference: 'experiment:92',
              promptSent: 'Prompt integral da tarefa selecionada, com aprendizado preservado.',
              agentPromptPart: 'Dédalo', activityPromptPart: 'Correção do mesmo ciclo.' } });
          }
          ...
          if (url.pathname.endsWith('/activity-executions')) {
            ...
            assert.equal(url.searchParams.get('includePromptAudit'), 'false');
            const compact = structuredClone(snapshots[state]);
            for (const activity of compact.activities) for (const task of activity.tasks) {
              task.promptSent = null; task.activityPromptPart = null; task.agentPromptPart = null;
            }
      ...
      const dependent = page.locator('#activity-technicalHomologation');
      await expect(dependent.getByRole('button', { name: 'Criar tarefa de correção' })).toHaveCount(0);
      const correctionLink = dependent.getByRole('link', { name: /Ir para .*Corrigir o protótipo/ });
      await expect(correctionLink).toHaveAttribute('href', /learningCycleId=2&chainId=14#activity-prototypeCorrection$/);
      ...
      await card.getByRole('button', { name: 'Criar tarefa de correção' }).click();
      const pendingButton = card.getByRole('button', { name: 'Executando...' });
      ...
      await expect(card.locator('.product-process-activity-control').getByRole('button')).toHaveCount(0);
      await expect(dependent.getByLabel('Acompanhamento da tarefa')).not.toContainText('900378');
      ...
      await expect(card.locator('.product-process-activity-control').getByRole('button')).toHaveCount(0);
      await expect(card).toContainText('O objetivo da atividade já foi atingido neste ciclo.');
      await expect(tracking.getByText('A atividade ainda não foi concluída.')).toHaveCount(0);
      assert.equal(auditReads, 0, 'O acompanhamento não transfere prompts de tarefas antigas.');
      const taskAudit = card.locator('details.business-process-document').filter({ hasText: 'Tarefa #900378' });
      await taskAudit.locator(':scope > summary').click();
      await taskAudit.getByRole('button', { name: 'Ver prompts desta tarefa' }).click();
      await expect(taskAudit.getByRole('alert')).toContainText('Não foi possível carregar os prompts');
      await taskAudit.getByRole('button', { name: 'Tentar novamente' }).click();
      await expect(taskAudit).toContainText('Prompt integral da tarefa selecionada, com aprendizado preservado.');
      assert.equal(auditReads, 2);
      await taskAudit.getByRole('button', { name: 'Ocultar prompts desta tarefa' }).click();
      await taskAudit.locator(':scope > summary').click();
      ...
      results.push({ device: name, passed: true, posts, historyReads, progressReads, historyFailureResponses, cycleFailureResponses, auditReads });
      console.log(`PASS ${name}: tarefa exclusiva da correção, link da dependência, retorno na viewport, fila, execução, reload, erro de conexão, bloqueio, conclusão e isolamento`);
      await session.close();
    }
