...
def main():
    ...
    if not args.image.startswith('marketing-hub/') or not args.image.endswith((':vega393-gate-v12', ':vega393-gate-v12-ui', ':vega393-gate-v12-ui2', ':vega393-gate-v12-ui3')):
    ...
    if service not in ['backend', 'frontend', 'customer-agent-worker', 'meta-ad-approver-worker', 'vega-private-frontend']:
        raise ValueError('Serviço fora do escopo da validação multiagente do Vega.')
    expected_images = {
        'backend': ['marketing-hub/backend:vega393-gate-v12', 'marketing-hub/backend:vega393-gate-v12-ui', 'marketing-hub/backend:vega393-gate-v12-ui2', 'marketing-hub/backend:vega393-gate-v12-ui3'],
        'frontend': ['marketing-hub/frontend:vega393-gate-v12-ui'],
        'customer-agent-worker': 'marketing-hub/customer-agent-worker:vega393-gate-v12',
        'meta-ad-approver-worker': 'marketing-hub/meta-ad-approver-worker:vega393-gate-v12',
        'vega-private-frontend': 'marketing-hub/pde-platform-frontend-vega:vega393-gate-v12',
    }
    expected = expected_images[service]
    if args.image not in (expected if isinstance(expected, list) else [expected]):
        raise ValueError('Imagem incompatível com o serviço selecionado.')
    ...
    elif service == 'meta-ad-approver-worker':
        environment_values.update(META_AD_APPROVER_IMAGE=args.image,
            META_AD_APPROVER_CODEX_HOME=mounts['/home/approver/.codex'],
            MARKETING_HUB_REPOSITORY_HOST=mounts['/workspace/marketing-hub'])
    elif service == 'frontend':
        # O Compose compartilhado valida também as variáveis obrigatórias do backend.
        # Seus valores servem apenas à interpolação e nunca entram no serviço frontend.
        backend = json.loads(subprocess.check_output(['docker', 'inspect', 'marketinghub-backend']))[0]
        if backend['Config']['Labels'].get('com.docker.compose.project') != project:
            raise ValueError('Backend de outro projeto não pode fornecer contexto ao Compose.')
        backend_env = dict(v.split('=', 1) for v in backend['Config']['Env'] if '=' in v)
        environment_values = dict(os.environ) | backend_env | running_env
    ...
    if service == 'backend':
        environment['PDE_VEGA_PROTOTYPE_VERSION'] = 'musa-pde-entry-v12-primeiro-ajuste-aplicavel'
    elif service in ['customer-agent-worker', 'meta-ad-approver-worker']:
        environment['AGENT_BUILD_REFERENCE'] = 'vega393-gate-v12'
