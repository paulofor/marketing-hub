...
- O proxy (`lead-portal-payments-service/docker-compose*.yml`) está na mesma network e possui um upstream dedicado (`institutional-site`). Assim, `https://vitrineproduto.online` é entregue pelo container estático enquanto `https://pagamentopalf.online` continua apontando para o backend de pagamentos.
