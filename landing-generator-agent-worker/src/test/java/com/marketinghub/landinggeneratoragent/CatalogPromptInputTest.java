...
class CatalogPromptInputTest {
  private static final String SCHEMA = "prompts/opala-commercial/v1/preparation-schema.json";
  ...
  static Map<String, Object> fixture(String activity, String agent, String schema)
    ...
    result.put("executorModule", "landing-generator-agent-worker");
  ...
  private Map<String, Object> task() throws Exception {
    ...
    result.put("processVersion", 1);
    result.put("agentKey", "landing-generator");
    result.put("activityId", "entry");
    result.put("catalogPrompt", fixture("entry", "landing-generator", SCHEMA));
    return result;
  }
  ...
  void rejectsContractDrift() throws Exception {
    for (String key :
        List.of(
            ...
            "executorModule")) {
      var task = task();
      var prompt = new HashMap<>(fixture("entry", "landing-generator", SCHEMA));
  ...
  void composesActualConsumerFromPinnedCatalogText() throws Exception {
    ...
    properties.setBackendUrl("http://127.0.0.1:9");
    var consumer =
