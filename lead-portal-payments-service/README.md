...
- O mesmo proxy publica o Clube MUSA em `https://clubemusa.com.br` e nos subdomínios versionados. Cada versão produtiva ativa deve apontar para a porta pública local do container próprio da PDE Platform: `v5.clubemusa.com.br` para `5176`, `v6.clubemusa.com.br` para `5177` e `v7.clubemusa.com.br` para `5178`. Enquanto `v1` e `v2` estiverem como slots legados, eles direcionam para a experiência estável da v5; enquanto `v8`, `v9` e `v10` estiverem apenas reservados no DNS/certificado, o proxy direciona esses hosts para a experiência estável da v7, evitando página morta até haver contrato próprio. Isso evita 502 por falha de resolução DNS entre projetos Docker distintos. O certificado de `clubemusa.com.br` precisa conter esses subdomínios como SANs antes de recriar o proxy.
