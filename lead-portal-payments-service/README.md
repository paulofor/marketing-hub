...
- O compose agora inclui um **proxy Nginx** (porta 80/443) que termina o TLS com os certificados do Let’s Encrypt montados em `/etc/letsencrypt` e persiste tudo em `/etc/nginx/certs/live/<domínio>`. A configuração padrão está em `nginx.conf` e expõe `pagamentopalf.site` com redirecionamento automático para HTTPS. Em produção, montamos `/etc/nginx/certs` do host (que já possui o certificado válido de `vitrineproduto.online` em `/etc/nginx/certs/live/vitrineproduto.online/`) e mantemos o bind de `/etc/letsencrypt`; em ambiente local continuamos com os certificados autoassinados em `docker/proxy/certs/dev` para não quebrar o container. Como as portas públicas são infraestrutura comercial compartilhada, somente o proxy usa `restart: always` e healthcheck do Nginx: ele deve voltar após reboot mesmo que tenha sido parado antes do reinício do daemon.
> **Nota:** o proxy executa `docker/proxy/ensure-certs.sh` antes de iniciar o Nginx. O script itera sobre os domínios definidos (por padrão `pagamentopalf.site`, `vitrineproduto.online`, `digicomdigital.com.br` e `clubemusa.com.br`), reaproveita os arquivos já disponíveis em `/etc/nginx/certs/live/<domínio>` quando existirem e, caso contrário, copia automaticamente de `/etc/letsencrypt` ou utiliza o fallback autoassinado. Assim que o certificado real for emitido com o profile `certbot`, basta recriar o serviço para que o arquivo atualizado seja detectado.
- O mesmo proxy também publica `https://vitrineproduto.online` e o site institucional em `https://www.digicomdigital.com.br`, roteando ambos para o container `institutional-site` ligado à network compartilhada (`public-net`). O domínio raiz `digicomdigital.com.br` redireciona para `www`. Certifique-se de que o compose do site esteja conectado a essa network (variáveis `INSTITUTIONAL_SITE_NETWORK=public-net` e `INSTITUTIONAL_SITE_NETWORK_EXTERNAL=true`) para que o upstream `institutional-site` seja resolvido corretamente.
- O mesmo proxy publica o Clube MUSA em `https://clubemusa.com.br` e nos subdomínios versionados. Cada versão produtiva ativa deve apontar para a porta pública local do container próprio da PDE Platform: `v5.clubemusa.com.br` para `5176`, `v6.clubemusa.com.br` para `5177`, `v7.clubemusa.com.br` para `5178` e `v8.clubemusa.com.br` para `5181`. Enquanto `v1` e `v2` estiverem como slots legados, eles direcionam para a experiência estável da v5; enquanto `v9` e `v10` estiverem apenas reservados no DNS/certificado, o proxy direciona esses hosts para a experiência estável da v7, evitando página morta até haver contrato próprio. Isso evita 502 por falha de resolução DNS entre projetos Docker distintos. O certificado de `clubemusa.com.br` precisa conter esses subdomínios como SANs antes de recriar o proxy.
...
Para renovar o certificado do Clube MUSA com os subdomínios versionados:

```bash
cd lead-portal-payments-service
CERTBOT_PRIMARY_DOMAIN=clubemusa.com.br \
CERTBOT_EXTRA_DOMAINS="v1.clubemusa.com.br v2.clubemusa.com.br v5.clubemusa.com.br v6.clubemusa.com.br v7.clubemusa.com.br v8.clubemusa.com.br v9.clubemusa.com.br v10.clubemusa.com.br" \
docker compose -f docker-compose.deploy.yml --profile certbot run --rm certbot-pagamentos
...
1. **Testes** – valida o contrato de ciclo de vida do proxy e roda `mvn test` com Java 21. A homologação local adicional `scripts/test-public-proxy-lifecycle-e2e.sh` encerra o processo Nginx, comprova reinício automático e revalida a rota HTTPS do Kit com upstream falso.
