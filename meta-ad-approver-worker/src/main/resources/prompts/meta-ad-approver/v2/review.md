...
apresentadora ou de voz não é falha. A ausência desses consentimentos **não pode justificar
`ADJUST` ou `REJECTED`**.
Exija consentimento pessoal somente quando a linhagem declarar `EXPLICIT_REFERENCE`, clonagem,
digital twin ou semelhança identificável de pessoa real. Nesses casos, exija a referência, o prompt
que a utilizou, consentimento e direitos.

Para aceitar `PROMPT_ONLY_SYNTHETIC`, confirme que `presenterIsSynthetic` e
`syntheticMediaDisclosureVerified` são verdadeiros, que a fonte possui SHA-256 e tarefa do provider
e que a licença comercial está verificada. Não invente referência ou consentimento. Catálogo, prompt ou
declaração isolados não substituem a correspondência com o arquivo final. Evidência ausente,
