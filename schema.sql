...
CREATE TABLE lead_portal_flow (
    ...
    simple_form_style_id BIGINT,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    CONSTRAINT fk_lead_portal_flow_simple_form_style FOREIGN KEY (simple_form_style_id) REFERENCES lead_portal_simple_form_style(id)
);
...
CREATE TABLE lead_portal_premium_delivery (
    id BIGINT AUTO_INCREMENT PRIMARY KEY,
...
CREATE TABLE ad_set (
    ...
    targeting_request_id BINARY(16),
    budget DECIMAL(10,2),
    duration_days INT,
    prompt LONGTEXT,
    model VARCHAR(255),
...
CREATE TABLE prompt_domain (
    ...
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    UNIQUE KEY uk_prompt_domain_code (code)
);
...
CREATE TABLE prompt_domain_object (
    ...
    UNIQUE KEY uk_prompt_domain_object (prompt_domain_id, object_type)
);

INSERT INTO prompt_domain (code, name, description) VALUES
...
SELECT id, 'NICHE' FROM prompt_domain WHERE code = 'NICHE_DETAILED_DESCRIPTION';

INSERT INTO prompt_domain_object (prompt_domain_id, object_type)
SELECT id, 'NICHE' FROM prompt_domain WHERE code = 'NICHE_HYPOTHESIS';
INSERT INTO prompt_domain_object (prompt_domain_id, object_type)
SELECT id, 'DETAILED_DESCRIPTION' FROM prompt_domain WHERE code = 'NICHE_HYPOTHESIS';
INSERT INTO prompt_domain_object (prompt_domain_id, object_type)
