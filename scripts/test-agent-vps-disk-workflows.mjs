...
for (const module of modules) {
  ...
  const gate = deploy.indexOf("< scripts/ensure-agent-vps-disk-space.sh");
  assert.ok(gate > 0, `Gate de disco ausente: ${module}`);
...
console.log("Nove deploys protegidos por capacidade de disco antes de qualquer publicação.");
