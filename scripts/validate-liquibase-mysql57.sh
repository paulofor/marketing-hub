...
text = path.read_text(encoding="utf-8")
required_order = [
    "V2031_06_10__create_agent_tables.sql",
    "changesets/2026-08-04-agent-governance-v1.yaml",
    "changesets/2026-08-08-market-radar-agent-v1.yaml",
]
...
if positions != sorted(positions):
    print(
        f"{path}: ordem inválida do Agente Radar; crie as tabelas base e de governança antes da carga do agente",
        file=sys.stderr,
    )
    sys.exit(1)
PY
...
log "Validando dependências do histórico de geração de imagens"
python3 - "${MASTER_CHANGELOG}" <<'PY'
import sys
from pathlib import Path
...
text = path.read_text(encoding="utf-8")
required_order = [
    "changesets/2026-07-16-image-generation-request.yaml",
    "changesets/2026-08-08-image-generation-history.yaml",
]
...
if positions != sorted(positions):
    print(
        f"{path}: ordem inválida do histórico de imagens; crie image_generation_request antes de alterá-la",
        file=sys.stderr,
    )
    sys.exit(1)
PY
