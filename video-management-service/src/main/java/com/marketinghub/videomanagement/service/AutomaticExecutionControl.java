...
/** Responsabilidade: bloquear rotinas automáticas de Apolo quando o backend determinar STOP. */
@Component
public class AutomaticExecutionControl {
    ...
    private static final String AGENT_KEY = "videomaker";
    private final RestClient backend;

    /** Configura uma consulta curta usando o mesmo backend operacional de Apolo. */
    public AutomaticExecutionControl(
    ...
    public boolean allowsAutomaticExecution() {
        try {
            ControlState state = backend.get()
            ...
            log.error("Falha ao comprovar PLAY; Apolo permanecerá parado. agentKey={}", AGENT_KEY, ex);
            return false;
        }
    }
