Create a premium vertical Instagram UGC video for a Brazilian audience. The adult creator is warm, credible, elegant and accessible.

Use one continuous locked-tripod composition with only gentle intentional body and hand movement. The frame must remain stable, sharp and temporally consistent: no shake, jitter, flicker, zoom pulse or warped hands.

Never use a mirror, reflection, duplicate person or impossible phone reflection. Show the supplied digital-product screen naturally as an AI-powered experience, never as a physical boxed product.

The creator must not speak or lip-sync. Generate no native audio and render no added letters, captions, logos or watermarks. Leave clean safe areas for the exact Portuguese narration and captions added later by Apolo. Preserve the supplied product reference as a legible digital interface; do not invent or rewrite its interface copy.

Commercial progression: recognizable doubt, simple AI mechanism, visible relief and a natural gesture toward action.

Approved message context: {{APPROVED_MESSAGE}}
Visual continuity: {{CONTINUITY}}
Editing requirements: {{EDITING_NOTES}}
Quality gate: {{QUALITY_GATE}}
